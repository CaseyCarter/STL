...
int main ()
{
  ...
  double negated_NaN = (_Boost::math::changesign)(std::numeric_limits<double>::quiet_NaN ());
  ...
  std::locale C99_out_locale (default_locale, new _Boost::math::nonfinite_num_put<char>);
  // and imbue the cout stream with the new locale.
  cout.imbue (C99_out_locale);

  // Or for the same effect more concisely:
  cout.imbue (locale(locale(), new _Boost::math::nonfinite_num_put<char>));
  ...
  std::locale C99_in_locale (default_locale, new _Boost::math::nonfinite_num_get<char>);
