...
implementation of `_Boost::math::cbrt`, mainly by using a better computed initial 'guess'
...
// using _Boost::math::make_tuple;
// using _Boost::math::tie;
...
However, it defaults to `_Boost::uintmax_t maxit = (std::numeric_limits<_Boost::uintmax_t>::max)();`
...
{ 
  ...
  { 
    // Return both f(x) and f'(x) and f''(x).
    T fx = _Boost::math::pow<5>(x) - a;    // Difference (estimate x^3 - value).
    T dx = 5 * _Boost::math::pow<4>(x);    // 1st derivative = 5x^4.
    T d2x = 20 * _Boost::math::pow<3>(x);  // 2nd derivative = 20 x^3
    return std::make_tuple(fx, dx, d2x);  // 'return' fx, dx and d2x.
  }
private:
...
{ 
  ...
  const _Boost::uintmax_t maxit = 50;
  _Boost::uintmax_t it = maxit;
...
//[root_finding_main_1
  try
  {
    ...
    std::cout << "cbrt(28) " << _Boost::math::cbrt(28.) << std::endl; // _Boost::math:: version of cbrt.
