...
int main()
{
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<double> spline(v.data(), v.size(), 0 /* start time */, step);

    // Now we can evaluate the spline wherever we please.
    std::mt19937 gen;
    _Boost::random::uniform_real_distribution<double> absissa(0, v.size()*step);
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<double> p(population.data(), population.size(), t0, time_step);
    ...
    // we could use a predefined one such as _Boost::math::tools::eps_tolerance<double>, but that
    ...
    auto result =  _Boost::math::tools::bisect(f, 1910.0, 1920.0, termination, iterations);
    ...
    iterations = 1000;
    time = _Boost::math::tools::newton_raphson_iterate(f_n, 1910.0, 1900.0, 2000.0, 20, iterations);
