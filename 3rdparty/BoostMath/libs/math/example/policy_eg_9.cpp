...
T user_domain_error(const char* function, const char* message, const T& val)
{
   ...
   */
   ...
   msg += (_Boost::format(function) % typeid(T).name()).str();
   /*`
   ...
   */
   ...
   msg += (_Boost::format(message) % _Boost::io::group(std::setprecision(prec), val)).str();
   /*`
...
inline typename _Boost::math::tools::promote_args<RT>::type
   tgamma(RT z)
{
   return _Boost::math::tgamma(z, user_error_policy());
}
``

So that when we call `mymath::tgamma(z)` we really end up calling
`_Boost::math::tgamma(z, user_error_policy())`, and any
