...
#include <boost/math/distributions/normal.hpp> // for normal_distribution.
  using _Boost::math::normal; // typedef provides default type of double.
...
void normal_table()
{
  ...
  int precision = _Boost::math::max_digits10<FPT>();
...
//[normal_table_1
    ...
    normal_table<_Boost::float32_t>(); // Usually type float
#endif
    normal_table<_Boost::float64_t>(); // Usually type double. Assume that float64_t is always available.
#ifdef BOOST_FLOAT80_C
    normal_table<_Boost::float80_t>(); // Type long double on some X86 platforms.
#endif
#ifdef BOOST_FLOAT128_C
    normal_table<_Boost::float128_t>(); // Type _Quad on some Intel and __float128 on some GCC platforms.
#endif
    normal_table<_Boost::floatmax_t>();
//] [/normal_table_1 ]
  }
