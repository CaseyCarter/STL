...
you must pass two template parameters to `_Boost::multiprecision::number`.

It may be more legible to use a two-staged type definition such as this:

``
typedef _Boost::multiprecision::cpp_dec_float<50> mp_backend;
typedef _Boost::multiprecision::number<mp_backend, _Boost::multiprecision::et_off> cpp_dec_float_50_noet;
``

Here, we first define `mp_backend` as `cpp_dec_float` with 50 digits.
The second step passes this backend to `_Boost::multiprecision::number`
with `_Boost::multiprecision::et_off`, an enumerated type.

  typedef _Boost::multiprecision::number<_Boost::multiprecision::cpp_dec_float<50>, _Boost::multiprecision::et_off>
  cpp_dec_float_50_noet;

You can reduce typing with a `using` directive `using namespace _Boost::multiprecision;`
...
{ // Using expression templates.
  ...
  */
  ...
  // cannot convert from 'int' to '_Boost::multiprecision::detail::expression<tag,Arg1,Arg2,Arg3,Arg4>'
//] [/expression_template_1]
