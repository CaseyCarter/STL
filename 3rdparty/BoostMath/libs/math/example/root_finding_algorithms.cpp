...
//using _Boost::math::tools::newton_raphson_iterate;
//using _Boost::math::tools::halley_iterate; //
//using _Boost::math::tools::eps_tolerance; // Binary functor for specified number of bits.
//using _Boost::math::tools::bracket_and_solve_root;
//using _Boost::math::tools::toms748_solve;
//using _Boost::math::tools::schroder_iterate;

#include <boost/math/special_functions/next.hpp> // For float_distance.
#include <tuple> // for tuple and make_tuple.
#include <boost/math/special_functions/cbrt.hpp> // For _Boost::math::cbrt.
...
{ // for a floating-point type, float, double ...
  ...
  // Vector of values for each algorithm, std::cbrt, _Boost::math::cbrt, TOMS748, Newton, Halley.
  //std::vector< _Boost::int_least64_t> times;  converted to int.
  std::vector<int> times;
  //_Boost::int_least64_t min_time = std::numeric_limits<_Boost::int_least64_t>::max(); // Used to normalize times (as int).
  std::vector<double> normed_times;
  _Boost::int_least64_t min_time = (std::numeric_limits<_Boost::int_least64_t>::max)(); // Used to normalize times.
...
{ // return cube root of x using bracket_and_solve (using NO derivatives).
  ...
  { // (_Boost::is_class<T>)
    ...
    guess = _Boost::math::cbrt<T>(dx); // Get guess using double.
  }
  
  T factor = 2; // How big steps to take when searching.

  const _Boost::uintmax_t maxit = 50; // Limit to maximum iterations.
  _Boost::uintmax_t it = maxit; // Initially our chosen max iterations, but updated with actual.
...
{ // return cube root of x using 1st derivative and Newton_Raphson.
  ...
  if(_Boost::is_fundamental<T>::value)
  {
     ...
     guess = _Boost::math::cbrt(static_cast<double>(x));
  ...
  const _Boost::uintmax_t maxit = 20;
  _Boost::uintmax_t it = maxit;
...
int test_root(cpp_bin_float_100 big_value, cpp_bin_float_100 answer, const char* type_name)
{
  ...
  using _Boost::timer::nanosecond_type;
  using _Boost::timer::cpu_times;
  using _Boost::timer::cpu_timer;
  ...
  //  }
  //  ti.stop();
  //  long int distance = static_cast<int>(_Boost::math::float_distance<T>(result, ans));
  ...
  //}
  //{
  //  //algorithm_names.push_back("_Boost::math::cbrt"); // .
  //  cpu_timer ti; // Can start, pause, resume and stop, and read elapsed.
  //  ti.start();
  //  for (long i = 0; i < count; ++i)
  //  {
  //    result = _Boost::math::cbrt(to_root); // 
  //  }
  //  now = ti.elapsed();
  //  int time = static_cast<int>(now.user / count);
...
int main()
{
  using namespace _Boost::multiprecision;
...
1>  _Boost::math::cbrt TOMS748 Newton Halley Shroeder _Boost::math::cbrt TOMS748 Newton Halley Shroeder
1>  2 types compared.
...
1>  _Boost::math::cbrt TOMS748 Newton Halley Shroeder _Boost::math::cbrt TOMS748 Newton Halley Shroeder _Boost::math::cbrt TOMS748 Newton Halley Shroeder
1>  3 types compared.
1>  Precision of full type = 102 decimal digits
...
1>  Roots: 3.0365889718756622 3.0365889718756618 3.0365889718756627 3.0365889718756627 3.0365889718756627
1>  ==================================================================
1>  typeid(T).name()class _Boost::multiprecision::number<class _Boost::multiprecision::backends::cpp_bin_float<50,10,void,int,0,0>,0>, maxdigits10 = 52
...
1>  5 algorithms used:
1>  15 algorithms used:
1>  _Boost::math::cbrt TOMS748 Newton Halley Shroeder
