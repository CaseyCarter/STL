...
//`Ensure that all possibly significant digits (17) including trailing zeros are shown.
 ...
 //] [/ float128_example_3]
  ...
  using _Boost::multiprecision::float128;  // Wraps, for example, __float128 or _Quad.
  // or
  ...
  // float128 pi0 = _Boost::math::constants::pi(); // Compile fails - need to specify a type for the constant!

  float128 pi1 = _Boost::math::constants::pi<float128>();  // Returns a constant of type float128.
  std::cout << sqrt(pi1) << std::endl; // 1.77245385090551602729816748334114514

  float128 pi2 = _Boost::math::constants::pi<__float128>(); // Constant of type __float128 gets converted to float128 on the assignment.
  ...
  std::cout << _Boost::math::constants::root_pi<float128>() << std::endl; // 1.77245385090551602729816748334114514
  ...
  // constexpr float128 root_pi_constexpr = _Boost::math::constants::root_pi(); // Missing type for constant.

  // Best current way to get a constexpr is to use a Boost.Math constant if one is available.
  constexpr float128 root_pi_constexpr = _Boost::math::constants::root_pi<float128>();
  ...
  float128 ee = _Boost::math::constants::e<float128>();
...
Floating-point type _Boost::float128_t is NOT available.
