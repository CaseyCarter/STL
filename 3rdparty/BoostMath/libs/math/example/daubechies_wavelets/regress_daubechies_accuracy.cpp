...
int main(int argc, char** argv)
{
    ...
    _Boost::split(header_strs, header_line, _Boost::is_any_of(","));
    for (auto & s : header_strs) {
        _Boost::algorithm::trim(s);
    }
    ...
    while(std::getline(ifs, line))
    {
        ...
        _Boost::split(strs, line, _Boost::is_any_of(","));
        for (auto & s : strs)
        {
            _Boost::algorithm::trim(s);
        }
    ...
    auto q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, matched_holder);
    assert(std::get<1>(q) < 0);
    std::cout << "Matched Holder    : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, linear);
    assert(std::get<1>(q) < 0);
    std::cout << "Linear            : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, quadratic_b_spline);
    assert(std::get<1>(q) < 0);
    std::cout << "Quadratic B-spline: " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, cubic_b_spline);
    assert(std::get<1>(q) < 0);
    std::cout << "Cubic B-spline    : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, quintic_b_spline);
    assert(std::get<1>(q) < 0);
    std::cout << "Quintic B-spline  : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, cubic_hermite);
    assert(std::get<1>(q) < 0);
    std::cout << "Cubic Hermite     : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, pchip);
    assert(std::get<1>(q) < 0);
    std::cout << "PCHIP             : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, makima);
    assert(std::get<1>(q) < 0);
    std::cout << "Makima            : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, fotaylor);
    assert(std::get<1>(q) < 0);
    std::cout << "First-order Taylor: " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    if (sotaylor.size() > 0)
    {
    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, quintic_hermite);
    assert(std::get<1>(q) < 0);
    std::cout << "Quintic Hermite   : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, sotaylor);
    assert(std::get<1>(q) < 0);
    ...
    if (totaylor.size() > 0)
    {
    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, totaylor);
    assert(std::get<1>(q) < 0);
    std::cout << "3rd order Taylor  : " << std::get<0>(q) << " - " << std::abs(std::get<1>(q)) << "r, R^2 = " << std::get<2>(q) << "\n";

    q  = _Boost::math::statistics::simple_ordinary_least_squares_with_R_squared(r, septic_hermite);
    assert(std::get<1>(q) < 0);
