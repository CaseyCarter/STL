...
void DyadicGrid(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        auto v = _Boost::math::daubechies_scaling_dyadic_grid<Real, 4, 0>(j);
...
void ScalingConstructor(benchmark::State & state)
{
    for (auto _ : state)
    {
        benchmark::DoNotOptimize(_Boost::math::daubechies_scaling<Real, p>());
    }
}
...
{
    using _Boost::math::interpolators::cubic_hermite;
    auto n = state.range(0);
    std::vector<Real> x(n);
    std::vector<Real> y(n);
    std::vector<Real> dydx(n);
    std::random_device rd;
    _Boost::random::uniform_real_distribution<Real> dis(Real(0), Real(1));
    x[0] = dis(rd);
    y[0] = dis(rd);
    dydx[0] = dis(rd);
...
{
    using _Boost::math::interpolators::detail::cardinal_cubic_hermite_detail;
    auto n = state.range(0);
    std::vector<Real> y(n);
    std::vector<Real> dydx(n);
    std::random_device rd;
...
void CardinalCubicHermiteAOS(benchmark::State & state)
{
    ...
    using _Boost::math::interpolators::detail::cardinal_cubic_hermite_detail_aos;
    Real dx = Real(1)/Real(8);
    Real x0 = 0;
...
{
    using _Boost::math::interpolators::detail::cardinal_quintic_hermite_detail;
    auto n = state.range(0);
    std::vector<Real> y(n);
    std::vector<Real> dydx(n);
...
void CardinalQuinticHermiteAOS(benchmark::State & state)
{
    ...
    using _Boost::math::interpolators::detail::cardinal_quintic_hermite_detail_aos;
    Real dx = Real(1)/Real(8);
    Real x0 = 0;
    Real xf = x0 + (dat.size()-1)*dx;
...
{
    using _Boost::math::interpolators::detail::septic_hermite_detail;
    auto n = state.range(0);
    std::vector<Real> x(n);
    std::vector<Real> y(n);
    std::vector<Real> dydx(n);
...
{
    using _Boost::math::interpolators::detail::cardinal_septic_hermite_detail;
    auto n = state.range(0);
    std::vector<Real> y(n);
    std::vector<Real> dydx(n);
    std::vector<Real> d2ydx2(n);
...
{
    using _Boost::math::interpolators::detail::cardinal_septic_hermite_detail_aos;
    auto n = state.range(0);
