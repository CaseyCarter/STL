...
typedef _Boost::multiprecision::number<_Boost::multiprecision::cpp_bin_float<237, _Boost::multiprecision::backends::digit_base_2, std::allocator<char>, _Boost::int32_t, -262142, 262143>,  _Boost::multiprecision::et_off> octuple_type;

#ifdef BOOST_HAS_FLOAT128
typedef _Boost::multiprecision::float128 float128_t;
#else
typedef _Boost::multiprecision::cpp_bin_float_quad float128_t;
#endif
...
std::list<std::vector<Real>> integer_grid()
{
    ...
    auto c = _Boost::math::filters::daubechies_scaling_filter<Real, p>();
    for (auto & x : c)
    {
        x *= _Boost::math::constants::root_two<Real>();
    }
    ...
    for (size_t j = 0; j < eigs.size(); ++j) {
        ...
        Real alpha = pow(-1, j)*_Boost::math::factorial<Real>(j)/sum;
        ...
        std::cout << "Moment sum = " << sum << ", expected = " << pow(-1, j)*_Boost::math::factorial<Real>(j) << "\n";

        assert(abs(sum - pow(-1, j)*_Boost::math::factorial<Real>(j))/abs(pow(-1, j)*_Boost::math::factorial<Real>(j)) < sqrt(std::numeric_limits<Real>::epsilon()));
...
void write_grid(std::ofstream & fs)
{
    ...
    size_t j = 0;
    fs << std::setprecision(std::numeric_limits< _Boost::multiprecision::cpp_bin_float_quad>::max_digits10);
...
int main()
{
    ...
    fs << std::hexfloat << std::setprecision(std::numeric_limits<_Boost::multiprecision::cpp_bin_float_quad>::max_digits10);

    _Boost::hana::for_each(std::make_index_sequence<p_max>(), [&](auto idx){
