...
{
    auto psi = _Boost::math::daubechies_wavelet<Real, p>();
    if (grid_refinements >= 0)
    {
        psi = _Boost::math::daubechies_wavelet<Real, p>(grid_refinements);
    }
...
{
    auto psi1 = _Boost::math::daubechies_wavelet<Real, p>(1);
    ...
    auto psi2 = _Boost::math::daubechies_wavelet<Real, p>(2);
    daub.add_fn(psi2, "red");

    auto psi21 = _Boost::math::daubechies_wavelet<Real, p>(21);
    daub.add_fn(psi21);
...
void plot_condition_number()
{
    ...
    auto phi = _Boost::math::daubechies_wavelet<Real, p>();
...
{
    auto psi_coarse = _Boost::math::daubechies_wavelet<CoarseReal, p>(coarse_refinements);

    std::string title = std::to_string(p) + " vanishing moment ULP plot at " + std::to_string(coarse_refinements) + " refinements and " + _Boost::core::demangle(typeid(CoarseReal).name()) + " precision";
    title = "";

    std::string filename = "daubechies_" + std::to_string(p) + "_wavelet_" + _Boost::core::demangle(typeid(CoarseReal).name()) + "_" + std::to_string(coarse_refinements) + "_refinements.svg";
    int samples = 20000;
    int clip = 20;
...
int main()
{
    _Boost::hana::for_each(std::make_index_sequence<18>(), [&](auto i){ plot_psi<double, i+2>(); });
    _Boost::hana::for_each(std::make_index_sequence<17>(), [&](auto i){ plot_dpsi<double, i+3>(); });
    _Boost::hana::for_each(std::make_index_sequence<17>(), [&](auto i){ plot_condition_number<double, i+3>(); });
    _Boost::hana::for_each(std::make_index_sequence<18>(), [&](auto i){ plot_convergence<double, i+2>(); });

    using PreciseReal = float128;
    using CoarseReal = double;
    int precise_refinements = 22;
    constexpr const int p = 9;
    std::cout << "Computing precise wavelet function in " << _Boost::core::demangle(typeid(PreciseReal).name()) << " precision.\n";
    auto phi_precise = _Boost::math::daubechies_wavelet<PreciseReal, p>(precise_refinements);
    std::cout << "Beginning comparison with functions computed in " << _Boost::core::demangle(typeid(CoarseReal).name()) << " precision.\n";
