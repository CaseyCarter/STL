...
{
    std::cout << "Choosing refinement for " << _Boost::core::demangle(typeid(Real).name()) << " precision Daubechies scaling function with " << p << " vanishing moments.\n";
    using std::abs;
    int rmax = 22;
    auto phi_dense = _Boost::math::daubechies_scaling_dyadic_grid<PreciseReal, p, 0>(rmax);
    ...
    for (int r = 2; r <= 18; ++r)
    {
        ...
        auto phi = _Boost::math::daubechies_scaling<Real, p>(r);
        ...
        for (size_t i = 0; i < phi_dense.size(); ++i)
        {
            ...
            Real flt_distance = abs(_Boost::math::float_distance(computed, expected));
...
void find_best_interpolator()
{
    ...
    auto phi_dense_precise = _Boost::math::daubechies_scaling_dyadic_grid<PreciseReal, p, 0>(rmax);
    ...
    for (int r = 2; r < 13; ++r)
    {
        ...
        auto phi = _Boost::math::daubechies_scaling_dyadic_grid<Real, p, 0>(r);
        auto phi_prime = _Boost::math::daubechies_scaling_dyadic_grid<Real, p, 1>(r);
...

        {
            ...
            auto mh = _Boost::math::detail::matched_holder(std::move(phi_copy), std::move(phi_prime_copy), r, Real(0));
            Real sup = 0;
        ...
        {
            auto qbs = _Boost::math::interpolators::cardinal_quadratic_b_spline(phi.data(), phi.size(), Real(0), dx, phi_prime.front(), phi_prime.back());
            Real qbs_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i)
            {
                Real x = i*dx_dense;
        ...
        {
            auto cbs = _Boost::math::interpolators::cardinal_cubic_b_spline(phi.data(), phi.size(), Real(0), dx, phi_prime.front(), phi_prime.back());
            Real cbs_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i)
            {
                Real x = i*dx_dense;
        ...
        {
            auto qbs = _Boost::math::interpolators::cardinal_quintic_b_spline(phi.data(), phi.size(), Real(0), dx, {0,0}, {0,0});
            Real qbs_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i)
            {
                Real x = i*dx_dense;
...

        {
            auto phi_copy = phi;
            auto phi_prime_copy = phi_prime;
            auto ch = _Boost::math::interpolators::cardinal_cubic_hermite(std::move(phi_copy), std::move(phi_prime_copy), Real(0), dx);
            Real chs_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i)
            {
                Real x = i*dx_dense;
...

        {
            ...
            auto x_copy = x;
            auto phi_prime_copy = phi_prime;
            auto pc = _Boost::math::interpolators::pchip(std::move(x_copy), std::move(phi_copy));
            Real pchip_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i)
            {
                Real x = i*dx_dense;
...

        {
            auto phi_copy = phi;
            auto x_copy = x;
            auto pc = _Boost::math::interpolators::makima(std::move(x_copy), std::move(phi_copy));
            Real makima_sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i) {
              Real x = i*dx_dense;
              Real diff = abs(phi_dense[i] - pc(x));
        ...
        /*{
            auto phi_copy = phi;
            auto ws = _Boost::math::interpolators::whittaker_shannon(std::move(phi_copy), Real(0), dx);
            Real sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i) {
              Real x = i*dx_dense;
              using std::abs;
        ...
        {
            auto trig = _Boost::math::interpolators::cardinal_trigonometric(phi, Real(0), dx);
            Real sup = 0;
            for (size_t i = 0; i < phi_dense.size(); ++i) {
              Real x = i*dx_dense;
              using std::abs;
        ...
        if constexpr (p > 2) {
            auto phi_dbl_prime = _Boost::math::daubechies_scaling_dyadic_grid<Real, p, 2>(r);

            {
                auto phi_copy = phi;
                auto phi_prime_copy = phi_prime;
                auto phi_dbl_prime_copy = phi_dbl_prime;
                auto qh = _Boost::math::interpolators::cardinal_quintic_hermite(std::move(phi_copy), std::move(phi_prime_copy), std::move(phi_dbl_prime_copy), Real(0), dx);
                Real qh_sup = 0;
                for (size_t i = 0; i < phi_dense.size(); ++i)
                {
                    Real x = i*dx_dense;
        ...
        {
            auto phi_dbl_prime = _Boost::math::daubechies_scaling_dyadic_grid<Real, p, 2>(r);
            auto phi_triple_prime = _Boost::math::daubechies_scaling_dyadic_grid<Real, p, 3>(r);
...

            {
                ...
                auto sh = _Boost::math::interpolators::cardinal_septic_hermite(std::move(phi_copy), std::move(phi_prime_copy), std::move(phi_dbl_prime_copy), std::move(phi_triple_prime_copy), Real(0), dx);
                Real septic_sup = 0;
                for (size_t i = 0; i < phi_dense.size(); ++i)
                {
                    Real x = i*dx_dense;
...
int main()
{
    //_Boost::hana::for_each(std::make_index_sequence<4>(), [&](auto i){ choose_refinement<double, float128, i+16>(); });
    _Boost::hana::for_each(std::make_index_sequence<12>(), [&](auto i){ find_best_interpolator<double, float128, i+2>(); });
}
