...
void plot_phi(int grid_refinements = -1)
{
    ...
    {
        phi = _Boost::math::daubechies_scaling<Real, p>(grid_refinements);
    }
    Real a = 0;
...
{
    auto phi0 = _Boost::math::daubechies_scaling<Real, p>(0);
    Real a = 0;
    ...
    auto phi1 = _Boost::math::daubechies_scaling<Real, p>(1);
    daub.add_fn(phi1, "orange");
    auto phi2 = _Boost::math::daubechies_scaling<Real, p>(2);
    daub.add_fn(phi2, "red");

    auto phi21 = _Boost::math::daubechies_scaling<Real, p>(21);
    daub.add_fn(phi21);
...
{
    auto phi_coarse = _Boost::math::daubechies_scaling<CoarseReal, p>(coarse_refinements);
    ...
    std::string filename = "daubechies_" + std::to_string(p) + "_" + _Boost::core::demangle(typeid(CoarseReal).name()) + "_" + std::to_string(coarse_refinements) + "_refinements.svg";
    int samples = 20000;
    int clip = 20;
...
int main()
{
    _Boost::hana::for_each(std::make_index_sequence<18>(), [&](auto i){ plot_phi<double, i+2>(); });
    _Boost::hana::for_each(std::make_index_sequence<17>(), [&](auto i){ plot_dphi<double, i+3>(); });
    ...
    std::cout << "Computing precise scaling function in " << _Boost::core::demangle(typeid(PreciseReal).name()) << " precision.\n";
    auto phi_precise = _Boost::math::daubechies_scaling<PreciseReal, p>(precise_refinements);
