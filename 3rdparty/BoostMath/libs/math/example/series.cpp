...
T log1p(T x)
{
   ...
   return _Boost::math::tools::sum_series(s, std::numeric_limits<T>::epsilon(), max_iter);
}
//]
...
std::complex<T> log1p(std::complex<T> x)
{
   ...
   _Boost::uintmax_t max_iter = 1000;
   // Add it up, with enough precision for full machine precision:
   return _Boost::math::tools::sum_series(s, std::complex<T>(std::numeric_limits<T>::epsilon()), max_iter);
}
//]
