...
{ //! \returns fifth root of x using bracket_and_solve (no derivatives).
  ...
  //_Boost::uintmax_t maxit = (std::numeric_limits<_Boost::uintmax_t>::max)();
  // (std::numeric_limits<_Boost::uintmax_t>::max)() = 18446744073709551615
  ...
  const _Boost::uintmax_t maxit = 50; // Chosen max iterations,
  // but updated on exit with actual iteration count.

  // We could also have used a maximum iterations provided by any policy:
  // _Boost::uintmax_t max_it = policies::get_max_root_iterations<Policy>();

  _Boost::uintmax_t it = maxit; // Initially our chosen max iterations,
...
{ //! \return fifth root of x using 1st derivative and Newton_Raphson.
  ...
  const _Boost::uintmax_t maxit = 20; // Optionally limit the number of iterations.
  _Boost::uintmax_t it = maxit; // limit the number of iterations.
...
//_Boost::uintmax_t maxit = (std::numeric_limits<_Boost::uintmax_t>::max)();
// the default (std::numeric_limits<_Boost::uintmax_t>::max)() = 18446744073709551615
...
{ // return fifth root of x using 1st and 2nd derivatives and Halley.
  using namespace std;  // Help ADL of std functions.
  using namespace _Boost::math; // halley_iterate
