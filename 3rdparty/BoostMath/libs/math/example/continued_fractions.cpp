...
inline std::complex<T> expint_as_fraction(unsigned n, std::complex<T> const& z)
{
   ...
   std::complex<T> result = _Boost::math::tools::continued_fraction_b(
      f,
...
inline std::complex<T> gamma_Q_as_fraction(const std::complex<T>& a, const std::complex<T>& z)
{
   ...
   return pow(z, a) / (exp(z) *(z - a + T(1) + _Boost::math::tools::continued_fraction_a(f, eps)));
}
//]
inline _Boost::multiprecision::cpp_complex_50 gamma_Q_as_fraction(const _Boost::multiprecision::cpp_complex_50& a, const _Boost::multiprecision::cpp_complex_50& z)
{
   upper_incomplete_gamma_fract<_Boost::multiprecision::cpp_complex_50> f(a, z);
   _Boost::multiprecision::cpp_complex_50 eps(std::numeric_limits<_Boost::multiprecision::cpp_complex_50::value_type>::epsilon());
   return pow(z, a) / (exp(z) * (z - a + 1 + _Boost::math::tools::continued_fraction_a(f, eps)));
}
...
int main()
{
   ...
   _Boost::multiprecision::cpp_complex_50 am(3, 3), zm(3, 2);
