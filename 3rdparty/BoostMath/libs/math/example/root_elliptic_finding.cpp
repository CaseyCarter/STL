...
{ //  Nth root of x using only function - no derivatives.
   ...
   T operator()(T const& x)
   {
      ...
      T k = sqrt(1 - b * b / (a * a));
      return 4 * a * _Boost::math::ellint_2(k) - m_arc;
   }
private:
...
{ // Functor also returning 1st derivative.
   ...
   std::pair<T, T> operator()(T const& x)
   {
      ...
      T b2 = b * b;
      T k = sqrt(1 - b2 / a2);
      T Ek = _Boost::math::ellint_2(k);
      T Kk = _Boost::math::ellint_1(k);
      T fx = 4 * a * Ek - m_arc;
...
int test_root(cpp_bin_float_100 big_radius, cpp_bin_float_100 big_arc, cpp_bin_float_100 answer, const char* type_name, std::size_t type_no)
{
  ...
  long eval_count = _Boost::is_floating_point<T>::value ? 1000000 : 10000; // To give a sufficiently stable timing for the fast built-in types,
