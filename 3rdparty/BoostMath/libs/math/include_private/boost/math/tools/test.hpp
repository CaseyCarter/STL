...
{
private:
   _Boost::math::tools::stats<T> stat;   // Statistics for the test.
...
public:
   ...
   _Boost::uintmax_t count()const{ return stat.count(); }
...
{
   return _Boost::math::relative_difference(a, b);
}
...
//
// Function test accepts an matrix of input values (probably a 2D _Boost::array)
...
#endif
      if(!(_Boost::math::isfinite)(point) && (_Boost::math::isfinite)(expected))
      {
...
void test_check_throw(Val val, std::domain_error const* e)
{
   ...
   {
      BOOST_CHECK((_Boost::math::isnan)(val));
   }
}
...
void test_check_throw(Val v, std::overflow_error const* e)
{
   ...
   errno = 0;
   BOOST_CHECK((v >= _Boost::math::tools::max_value<Val>()) || (v <= -_Boost::math::tools::max_value<Val>()));
}

template <class Val>
void test_check_throw(Val v, _Boost::math::rounding_error const* e)
{
   BOOST_CHECK(errno == ERANGE);
   errno = 0;
...
#  define BOOST_MATH_CHECK_THROW(x, ExceptionType) _Boost::math::tools::test_check_throw(x, static_cast<ExceptionType const*>(0));
#else
