...
{
   typedef _Boost::function1<T, T const &> function_type;
public:
...
public:
   typedef _Boost::function1<T, T const &> function_type;
   typedef _Boost::numeric::ublas::vector<T> vector_type;
   typedef _Boost::numeric::ublas::matrix<T> matrix_type;
...
void remez_minimax<T>::init_chebyshev()
{
   ...
   //
   vector_type l_solution = _Boost::math::tools::solve(A, b);
         ...
         int bits)
{
   ...
   if(bits == 0)
   {
      ...
      m_precision = (std::min)(24, (_Boost::math::policies::digits<T, _Boost::math::policies::policy<> >() / 2) - 2);
   }
   else
   {
      // can't be more accurate than half the bits of T:
      m_precision = (std::min)(bits, (_Boost::math::policies::digits<T, _Boost::math::policies::policy<> >() / 2) - 2);
   }
...
T remez_minimax<T>::iterate()
{
   ...
   do{
      ...
      // Now go ahead and solve the expression to get our solution:
      //
      solution = _Boost::math::tools::solve(A, b);
...
{
   _Boost::scoped_array<T> a(new T[orderN + 1]);
   if(pinned)
      a[0] = 0;
   unsigned terms = pinned ? orderN : orderN + 1;
   for(unsigned i = 0; i < terms; ++i)
      a[pinned ? i+1 : i] = solution[i];
   return _Boost::math::tools::polynomial<T>(&a[0], orderN);
}
...
polynomial<T> remez_minimax<T>::denominator()const
{
   ...
   _Boost::scoped_array<T> a(new T[terms]);
   a[0] = 1;
   for(unsigned i = 0; i < orderD; ++i)
      a[i+1] = solution[i + offsetD];
   return _Boost::math::tools::polynomial<T>(&a[0], orderD);
}
