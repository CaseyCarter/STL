...
//
template <class T>
_Boost::numeric::ublas::vector<T> solve(
          const _Boost::numeric::ublas::matrix<T>& A_,
          const _Boost::numeric::ublas::vector<T>& b_)
{
   //BOOST_ASSERT(A_.size() == b_.size());

   _Boost::numeric::ublas::matrix<T> A(A_);
   _Boost::numeric::ublas::vector<T> b(b_);
   _Boost::numeric::ublas::permutation_matrix<> piv(b.size());
   lu_factorize(A, piv);
   lu_substitute(A, piv, b);
   //
   // iterate to reduce error:
   //
   _Boost::numeric::ublas::vector<T> delta(b.size());
