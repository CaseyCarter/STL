...
#elif defined(USE_CPP_FLOAT)
typedef _Boost::multiprecision::number<_Boost::multiprecision::cpp_dec_float<500> > generator_type;
#else
typedef ntl::RR generator_type;
#endif

inline void print_constant(const char* name, generator_type(*f)(const _Boost::integral_constant<int, 0>&))
{
#ifdef USE_MPFR
...
#endif
   generator_type value = f(_Boost::integral_constant<int, 0>());
   std::stringstream os;
...
#define BOOST_CONSTANTS_GENERATE(name) \
   _Boost::math::constants::print_constant(#name, \
   & _Boost::math::constants::detail::BOOST_JOIN(constant_, name)<_Boost::math::constants::generator_type>::get)
