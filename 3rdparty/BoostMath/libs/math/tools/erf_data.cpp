...
{
   _Boost::math::tuple<mp_t, mp_t> operator()(mp_t z)
   {
      ...
      mp_t g1, g2;
      g1 = _Boost::math::tgamma_lower(mp_t(0.5), z * z);
      g1 /= sqrt(_Boost::math::constants::pi<mp_t>());
      g1 *= sign;
      ...
      {
         g2 = _Boost::math::tgamma(mp_t(0.5), z * z);
         g2 /= sqrt(_Boost::math::constants::pi<mp_t>());
      }
      if(sign < 1)
         g2 = 2 - g2;
      return _Boost::math::make_tuple(g1, g2);
   }
};
...
_Boost::math::tuple<mp_t, mp_t> erfc_inv(mp_t r)
{
   mp_t x = exp(-r * r);
   x = x.convert_to<double>();
   std::cout << x << "   ";
   mp_t result = _Boost::math::erfc_inv(x);
   std::cout << result << std::endl;
   return _Boost::math::make_tuple(x, result);
}
...
int main(int argc, char*argv [])
{
   ...
   if(argc >= 2)
   {
      ...
      {
         _Boost::math::tuple<mp_t, mp_t> (*f)(mp_t);
         f = erfc_inv;
         std::cout << "Welcome.\n"
