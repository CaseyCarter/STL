...
template<class T>
_Boost::math::tuple<T, T, T, T> legendre_p_data(T n, T x)
{
   n = floor(n);
   T r1 = legendre_p(_Boost::math::tools::real_cast<int>(n), x);
   T r2 = legendre_q(_Boost::math::tools::real_cast<int>(n), x);
   return _Boost::math::make_tuple(n, x, r1, r2);
}
   
template<class T>
_Boost::math::tuple<T, T, T, T> assoc_legendre_p_data(T n, T x)
{
   static _Boost::mt19937 r;
   int l = real_cast<int>(floor(n));
   _Boost::uniform_int<> ui((std::max)(-l, -40), (std::min)(l, 40));
   int m = ui(r);
   T r1 = legendre_p(l, m, x);
   return _Boost::math::make_tuple(n, m, x, r1);
}
