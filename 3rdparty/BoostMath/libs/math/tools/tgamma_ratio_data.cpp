...
_Boost::math::tuple<mp_t, mp_t> 
   tgamma_ratio(const mp_t& a, const mp_t& delta)
{
   if(delta > a)
      throw std::domain_error("");
   mp_t tg = _Boost::math::tgamma(a);
   mp_t r1 = tg / _Boost::math::tgamma(a + delta);
   mp_t r2 = tg / _Boost::math::tgamma(a - delta);
   if((r1 > (std::numeric_limits<float>::max)()) || (r2 > (std::numeric_limits<float>::max)()))
      throw std::domain_error("");

   return _Boost::math::make_tuple(r1, r2);
}

mp_t tgamma_ratio2(const mp_t& a, const mp_t& b)
{
   return _Boost::math::tgamma(a) / _Boost::math::tgamma(b);
}
