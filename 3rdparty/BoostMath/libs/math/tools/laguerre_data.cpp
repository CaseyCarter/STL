...
template<class T>
_Boost::math::tuple<T, T, T> laguerre2_data(T n, T x)
{
   n = floor(n);
   T r1 = laguerre(_Boost::math::tools::real_cast<unsigned>(n), x);
   return _Boost::math::make_tuple(n, x, r1);
}
   
template<class T>
_Boost::math::tuple<T, T, T, T> laguerre3_data(T n, T m, T x)
{
   n = floor(n);
   m = floor(m);
