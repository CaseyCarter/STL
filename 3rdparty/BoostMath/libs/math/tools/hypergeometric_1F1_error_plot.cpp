...
void print_value(double x, std::ostream& os = std::cout)
{
   ...
   m = std::ldexp(m, 54);
   e -= 54;
   _Boost::int64_t val = (_Boost::int64_t)m;
...
struct error_data
{
   error_data(double a, double b, double z, _Boost::intmax_t e)
      : a(a), b(b), z(z), error(e) {}
   double a, b, z;
   _Boost::intmax_t error;
...
int main()
{
   try {
      ...
      typedef _Boost::iostreams::tee_device<std::ostream, std::ostream> tee_sink;
      typedef _Boost::iostreams::stream<tee_sink> tee_stream;
      ...
      _Boost::random::mt19937 gen(std::time(0));
      _Boost::random::uniform_real_distribution<test_type> a_dist(min_a, max_a);
      _Boost::random::uniform_real_distribution<test_type> b_dist(min_b, max_b);
      _Boost::random::uniform_real_distribution<test_type> z_dist(min_z, max_z);
      ...
      do
      {
         ...
         try {
            mp_expected = _Boost::math::hypergeometric_pFq_precision({ mpfr_float(a) }, { mpfr_float(b) }, mpfr_float(z), 25, 200.0);
         ...
         try
         {
            found = _Boost::math::hypergeometric_1F1(a, b, z);
         }
         catch (const std::exception&)
         {
            ++evaluation_errors;
         ...
         test_type err = _Boost::math::epsilon_difference(found, expected);
         if (err > max_error)
         {
         ...
         try {
            errors.insert(error_data(a, b, z, _Boost::math::lltrunc(err)));
         }
         catch (...)
         {
