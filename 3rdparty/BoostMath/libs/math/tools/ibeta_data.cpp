...
T get_ibeta_fraction1(T a, T b, T x)
{
   ...
   T fract = _Boost::math::tools::continued_fraction_a(f, _Boost::math::policies::digits<T, _Boost::math::policies::policy<> >());
   T denom = (a * (fract + 1));
...
{
   float f = _Boost::math::tools::real_cast<float>(r);
   return force_truncate(&f);
}

_Boost::mt19937 rnd;
_Boost::uniform_real<float> ur_a(1.0F, 5.0F);
_Boost::variate_generator<_Boost::mt19937, _Boost::uniform_real<float> > gen(rnd, ur_a);
_Boost::uniform_real<float> ur_a2(0.0F, 100.0F);
_Boost::variate_generator<_Boost::mt19937, _Boost::uniform_real<float> > gen2(rnd, ur_a2);

struct beta_data_generator
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t, mp_t, mp_t> operator()(mp_t ap, mp_t bp, mp_t x_)
   {
      ...
      return _Boost::math::make_tuple(a, b, x, ib_full.first, ib_full.second, ib_reg.first, ib_reg.second);
   }
};
...
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t, mp_t, mp_t> operator()(mp_t x_)
   {
      mp_t a = gen2();      
      mp_t b = gen2(); 
      mp_t x = x_;
      ...
      x = ConvPrec(x, 22);
      std::cout << a << " " << b << " " << x << std::endl;
      //mp_t exp_beta = _Boost::math::beta(a, b, x);
      std::pair<mp_t, mp_t> ib_full = ibeta_fraction1(mp_t(a), mp_t(b), mp_t(x));
      /*exp_beta = _Boost::math::tools::relative_error(ib_full.first, exp_beta);
      if(exp_beta > 1e-40)
      {
...
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t, mp_t, mp_t> operator()(mp_t a, mp_t b, mp_t x_)
   {
      float x = truncate_to_float(real_cast<float>(x_));
      std::cout << a << " " << b << " " << x << std::endl;
...
int main(int, char* [])
{
   ...
   while(i != j)
   {
      ...
      std::string s = _Boost::lexical_cast<std::string>((*i)[3]);
      mp_t v3 = _Boost::lexical_cast<mp_t>(s);
