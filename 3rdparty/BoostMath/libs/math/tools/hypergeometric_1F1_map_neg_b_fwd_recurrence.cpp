...
using _Boost::multiprecision::mpfr_float;
...
typedef _Boost::multiprecision::cpp_bin_float_quad test_type;
         ...
         for (test_type b = b_start; b < b_end; b_start < 0 ? b /= b_mult : b *= b_mult)
         {
            ...
            try {
               for (test_type z = 1; z < 1e10; z *= z_mult, z_mult *= 2)
               {
                  ...
                  test_type calc = _Boost::math::tools::function_ratio_from_forwards_recurrence(_Boost::math::detail::hypergeometric_1F1_recurrence_a_and_b_coefficients<test_type>(a, b, z), std::numeric_limits<test_type>::epsilon() * 2, max_iter);
                  test_type reference = (test_type)(_Boost::math::hypergeometric_pFq_precision({ mpfr_float(a) }, { mpfr_float(b) }, mpfr_float(z), 50, time_limit) / _Boost::math::hypergeometric_pFq_precision({ mpfr_float(a + 1) }, { mpfr_float(b + 1) }, mpfr_float(z), std::numeric_limits<test_type>::digits10 * 2, time_limit));
                  double err = (double)_Boost::math::epsilon_difference(reference, calc);
            ...
            else
            {
               ...
               //
               z_limit = last_good == 0 ? 0 : _Boost::math::tools::bisect([&a, b, error_limit, time_limit](test_type z)
               {
                  _Boost::uintmax_t max_iter = 1000;
                  test_type calc = _Boost::math::tools::function_ratio_from_forwards_recurrence(_Boost::math::detail::hypergeometric_1F1_recurrence_a_and_b_coefficients<test_type>(a, b, z), std::numeric_limits<test_type>::epsilon() * 2, max_iter);
                  test_type reference = (test_type)(_Boost::math::hypergeometric_pFq_precision({ mpfr_float(a) }, { mpfr_float(b) }, mpfr_float(z), 50, time_limit + 20) / _Boost::math::hypergeometric_pFq_precision({ mpfr_float(a + 1) }, { mpfr_float(b + 1) }, mpfr_float(z), std::numeric_limits<test_type>::digits10 * 2, time_limit + 20));
                  test_type err = _Boost::math::epsilon_difference(reference, calc);
                  return err < error_limit ? 1 : -1;
               }, bad, last_good, _Boost::math::tools::equal_floor()).first;
            ...
            for (test_type z = bad; z > 1; z /= z_mult, z_mult *= 2)
            {
               ...
               try {
                  _Boost::uintmax_t max_iter = 1000;
                  test_type calc = _Boost::math::tools::function_ratio_from_backwards_recurrence(_Boost::math::detail::hypergeometric_1F1_recurrence_a_and_b_coefficients<test_type>(a, b, z), std::numeric_limits<test_type>::epsilon() * 2, max_iter);
                  test_type reference = (test_type)(_Boost::math::hypergeometric_pFq_precision({ mpfr_float(a) }, { mpfr_float(b) }, mpfr_float(z), 50, time_limit) / _Boost::math::hypergeometric_pFq_precision({ mpfr_float(a - 1) }, { mpfr_float(b - 1) }, mpfr_float(z), std::numeric_limits<test_type>::digits10 * 2, time_limit));
                  test_type err = _Boost::math::epsilon_difference(reference, calc);
            ...
            else
            {
               ...
               // At this stage last_good and bad should bracket the edge of the domain, bisect to narrow things down:
               //
               lower_z_limit = last_good == 0 ? 0 : _Boost::math::tools::bisect([&a, b, error_limit, time_limit](test_type z)
               {
                  _Boost::uintmax_t max_iter = 1000;
                  test_type calc = _Boost::math::tools::function_ratio_from_backwards_recurrence(_Boost::math::detail::hypergeometric_1F1_recurrence_a_and_b_coefficients<test_type>(a, b, z), std::numeric_limits<test_type>::epsilon() * 2, max_iter);
                  test_type reference = (test_type)(_Boost::math::hypergeometric_pFq_precision({ mpfr_float(a) }, { mpfr_float(b) }, mpfr_float(z), 50, time_limit + 20) / _Boost::math::hypergeometric_pFq_precision({ mpfr_float(a - 1) }, { mpfr_float(b - 1) }, mpfr_float(z), std::numeric_limits<test_type>::digits10 * 2, time_limit + 20));
                  test_type err = _Boost::math::epsilon_difference(reference, calc);
                  return err < error_limit ? 1 : -1;
               }, last_good, bad, _Boost::math::tools::equal_floor()).first;
