...
{
   return (v / x) * _Boost::math::cyl_bessel_j(v, x) - _Boost::math::cyl_bessel_j(v+1, x);
}

template <class T>
T bessel_y_derivative_bare(T v, T x)
{
   return (v / x) * _Boost::math::cyl_neumann(v, x) - _Boost::math::cyl_neumann(v+1, x);
}

template <class T>
T bessel_i_derivative_bare(T v, T x)
{
   return (v / x) * _Boost::math::cyl_bessel_i(v, x) + _Boost::math::cyl_bessel_i(v+1, x);
}

template <class T>
T bessel_k_derivative_bare(T v, T x)
{
   return (v / x) * _Boost::math::cyl_bessel_k(v, x) - _Boost::math::cyl_bessel_k(v+1, x);
}
...
T sph_bessel_j_derivative_bare(T v, T x)
{
   ...
   if(v == 0)
      return -_Boost::math::sph_bessel(1, x);
   return _Boost::math::sph_bessel(itrunc(v-1), x) - ((v + 1) / x) * _Boost::math::sph_bessel(itrunc(v), x);
}
...
T sph_bessel_y_derivative_bare(T v, T x)
{
   if((v < 0) || (floor(v) != v))
      throw std::domain_error("");
   if(v == 0)
      return -_Boost::math::sph_neumann(1, x);
   return _Boost::math::sph_neumann(itrunc(v-1), x) - ((v + 1) / x) * _Boost::math::sph_neumann(itrunc(v), x);
}

namespace opt = _Boost::program_options;
using FloatType = _Boost::multiprecision::number<_Boost::multiprecision::mpfr_float_backend<200u> >;
...
void updateTestCount(Lines& source_lines, std::size_t mult)
{
   ...
   const auto where = std::find_if(std::begin(source_lines), std::end(source_lines),
      [](const std::string& str){ return str.find("_Boost::array") != std::string::npos; });
   auto& str = *where;
   ...
   const auto new_value = mult * _Boost::lexical_cast<std::size_t>(str.substr(pos, digits_length));
   str.replace(pos, digits_length, _Boost::lexical_cast<std::string>(new_value));
}
