...
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t, mp_t, mp_t> operator()
      (mp_t bp, mp_t x_, mp_t p_)
   {
      ...
      mp_t inv = _Boost::math::ibeta_inva(mp_t(b), mp_t(x), mp_t(p));
      std::cout << " " << inv << std::flush;
      mp_t invc = _Boost::math::ibetac_inva(mp_t(b), mp_t(x), mp_t(p));
      std::cout << " " << invc << std::endl;
      mp_t invb = _Boost::math::ibeta_invb(mp_t(b), mp_t(x), mp_t(p));
      std::cout << " " << invb << std::flush;
      mp_t invbc = _Boost::math::ibetac_invb(mp_t(b), mp_t(x), mp_t(p));
      std::cout << " " << invbc << std::endl;
      return _Boost::math::make_tuple(b, x, p, inv, invc, invb, invbc);
   }
};
