...
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t> operator()(mp_t a, mp_t x)
   {
      ...
      //mp_t g = _Boost::math::tgamma(a);
      mp_t lg = _Boost::math::tgamma_lower(a, z);
      mp_t ug = _Boost::math::tgamma(a, z);
      mp_t rlg = _Boost::math::gamma_p(a, z);
      mp_t rug = _Boost::math::gamma_q(a, z);

      return _Boost::math::make_tuple(z, ug, rug, lg, rlg);
   }
};

struct gamma_inverse_generator
{
   _Boost::math::tuple<mp_t, mp_t> operator()(const mp_t a, const mp_t p)
   {
      mp_t x1 = _Boost::math::gamma_p_inv(a, p);
      mp_t x2 = _Boost::math::gamma_q_inv(a, p);
      std::cout << "Inverse for " << a << " " << p << std::endl;
      return _Boost::math::make_tuple(x1, x2);
   }
};

struct gamma_inverse_generator_a
{
   _Boost::math::tuple<mp_t, mp_t> operator()(const mp_t x, const mp_t p)
   {
      mp_t x1 = _Boost::math::gamma_p_inva(x, p);
      mp_t x2 = _Boost::math::gamma_q_inva(x, p);
...
int main(int argc, char*argv [])
{
   ...
   else
   {
      ...
      arg2.type |= _Boost::math::tools::dummy_param;
