...
template<class T>
_Boost::math::tuple<T, T, T, T, T, T> spherical_harmonic_data(T i)
{
   static _Boost::mt19937 r;

   int n = real_cast<int>(floor(i));
   _Boost::uniform_int<> ui(0, (std::min)(n, 40));
   int m = ui(r);
   
   _Boost::uniform_real<float> ur(-2*constants::pi<float>(), 2*constants::pi<float>());
   float _theta = ur(r);
   float _phi = ur(r);
   ...
   return _Boost::math::make_tuple(n, m, theta, phi, r1, r2);
}
