...
{
   _Boost::math::tuple<mp_t, mp_t, mp_t, mp_t, mp_t> operator()(mp_t ap, mp_t bp, mp_t x_)
   {
      ...
      mp_t inv = _Boost::math::ibeta_inv(mp_t(a), mp_t(b), mp_t(x));
      std::cout << " " << inv << std::flush;
      mp_t invc = _Boost::math::ibetac_inv(mp_t(a), mp_t(b), mp_t(x));
      std::cout << " " << invc << std::endl;
      return _Boost::math::make_tuple(a, b, x, inv, invc);
   }
};
