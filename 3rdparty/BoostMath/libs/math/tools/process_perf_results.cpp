...
int main(int argc, const char* argv[])
{
   ...
   _Boost::regex e("^Testing\\s+(\\S+)\\s+(\\S+)");
   std::string f;
   for(int i = 1; i < argc-1; ++i)
   {
      std::ifstream is(argv[i]);
      load_file(f, is);
      _Boost::sregex_iterator a(f.begin(), f.end(), e), b;
      while(a != b)
      {
         results[(*a).str(1)] = _Boost::lexical_cast<double>((*a).str(2));
         ++a;
      }
   }
   //
   ...
   while(is.good())
   {
      std::getline(is, f);
      os << f << std::endl;
      _Boost::sregex_iterator i(f.begin(), f.end(), e), j;
      ...
      std::string newstr;
      std::string tail;
      i = _Boost::sregex_iterator(f.begin(), f.end(), e);
      while(i != j)
      {
         std::string item = (*i).str(1);
         ...
         if(results.find(item) != results.end())
         {
            ...
            newstr += "][para (";
            newstr += (_Boost::format("%.3e") % results[item]).str();
            newstr += "s)]]";
         }
         else
         {
