...
namespace _Boost
{
  namespace math
  {
    namespace constants
    {
        ...
        template <class T> struct constant_pi
        {
          private:
            ...
            static const T result
               ...
               _Boost::is_convertible<const char*, T>());
              return result;
            }
          ...
          public:
            // Default implementations from string of decimal digits:
            static inline T get(const _Boost::integral_constant<int, construct_from_string>&)
            {
              constant_initializer<T, & constant_pi<T>::get_from_string >::do_nothing();
              return get_from_string();
            }
            // Float, double and long double versions:
            static inline T get(const _Boost::integral_constant<int, construct_from_float>)
            {
              return 3.141592653589793238462643383279502884e+00F;
            }
            static inline  T get(const _Boost::integral_constant<int, construct_from_double>&)
            {
              return 3.141592653589793238462643383279502884e+00;
            }
            static inline  T get(const _Boost::integral_constant<int, construct_from_long_double>&)
            {
            ...
            template <int N> static inline T get(const _Boost::integral_constant<int, N>& n)
            {
            ...
            static inline T get(const _Boost::integral_constant<int, 0>&)
            {
              return tools::digits<T>() > max_string_digits ? compute<0>() : get(_Boost::integral_constant<int, construct_from_string>());
            }
         }; // template <class T> struct constant_pi
      } //  namespace detail
      ...
      {
        return pi<T, _Boost::math::policies::policy<> >()
      }
    } //     namespace constants
