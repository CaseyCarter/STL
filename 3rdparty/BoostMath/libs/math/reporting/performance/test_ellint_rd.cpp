...
#include "ellint_rd_0xy.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::ellint_rd(v[0], v[1], v[2]);  }, [](const std::vector<double>& v){ return v[3];  });
   ...
   std::string function = "ellint_rd[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "ellint_rd";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_rd(v[0], v[1], v[2]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_rd(v[0], v[1], v[2], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH))
