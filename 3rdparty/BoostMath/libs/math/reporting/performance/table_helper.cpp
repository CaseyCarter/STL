...
void add_cell(_Boost::intmax_t val, const std::string& table_name, const std::string& row_name, const std::string& column_heading);
...
{
   static const _Boost::regex e("[^a-zA-Z0-9]+");
   std::string result = _Boost::regex_replace(s, e, "_");
   while(result[0] == '_')
      result.erase(0);
   return result;
}
...
_Boost::filesystem::path path_to_content;
...
struct content_loader
{
   ...
   ~content_loader()
   {
      _Boost::interprocess::named_mutex mu(_Boost::interprocess::open_or_create, "handle_test_result");
      _Boost::interprocess::scoped_lock<_Boost::interprocess::named_mutex> lock(mu);
      _Boost::filesystem::path p(__FILE__);
      p = p.parent_path();
      p /= "doc";
      p /= "performance_tables.qbk";
      path_to_content = p;
      if(_Boost::filesystem::exists(p))
      {
         _Boost::filesystem::ifstream is(p);
         if(is.good())
         {
            do
            {
      ...
      {
         add_cell(static_cast<_Boost::uintmax_t>(std::get<0>(*i) / 1e-9), std::get<1>(*i), std::get<2>(*i), std::get<3>(*i));
      }
      //
      // Write out the results:
      //
      _Boost::filesystem::ofstream os(path_to_content);
      os << content;
   }
...
{
   static const _Boost::regex item_e(
      "\\["
      "([^\\[\\]]*(?0)?)*"
      "\\]"
      );

   _Boost::regex_token_iterator<std::string::const_iterator> i(begin, end, item_e), j;

   while(i != j)
   {
      // Add a row:
      table.push_back(std::vector<std::string>());
      _Boost::regex_token_iterator<std::string::const_iterator> k(i->first + 1, i->second - 1, item_e);
      while(k != j)
      {
         // Add a cell:
...
void add_to_all_sections(const std::string& id, std::string list_name)
{
   ...
   else
   {
      ...
      _Boost::regex_token_iterator<std::string::const_iterator> i(content.begin() + pos + 12 + list_name.size(), content.end(), item_e), j;
...
std::string get_colour(_Boost::uintmax_t val, _Boost::uintmax_t best)
{
   if(val <= best * 1.2)
      return "green";
   if(val > best * 2)
...
_Boost::intmax_t get_value_from_cell(const std::string& cell)
{
   static const _Boost::regex time_e("(\\d+)ns");
   _Boost::smatch what;
   if(regex_search(cell, what, time_e))
   {
      return _Boost::lexical_cast<_Boost::uintmax_t>(what.str(1));
   }
   return -1;
}
...
void add_cell(_Boost::intmax_t val, const std::string& table_name, const std::string& row_name, const std::string& column_heading)
{
   ...
   _Boost::regex table_e("\\[table:" + table_id
      + "\\s[^\\[]++"
      "((\\["
      "([^\\[\\]]*+(?2)?+)*+"
      "\\]\\s*+)*+\\s*+)"
   ...
   if(regex_search(content, table_location, table_e))
   {
      ...
      //
      _Boost::uintmax_t best = (std::numeric_limits<_Boost::uintmax_t>::max)();
      std::vector<_Boost::intmax_t> values;
      for(unsigned i = 1; i < table_data[row_id].size(); ++i)
      {
         ...
         else
         {
            ...
            _Boost::uintmax_t cell_val = get_value_from_cell(table_data[row_id][i]);
      ...
      for(unsigned i = 1; i < table_data[row_id].size(); ++i)
      {
         ...
         else
         {
            ...
            s += "[br](";
            s += _Boost::lexical_cast<std::string>(values[i - 1]) + "ns)]";
         }
      }
      //
   ...
   else
   {
      ...
      new_table += _Boost::lexical_cast<std::string>(val);
      new_table += "ns)]]]\n]\n]\n";
...
void report_execution_time(double t, std::string table, std::string row, std::string heading)
{
   ...
   //add_cell(static_cast<_Boost::uintmax_t>(t / 1e-9), table, row, heading);
}
