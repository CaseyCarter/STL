...
#  include "igamma_int_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::tgamma(v[0], v[1]);  }, [](const std::vector<double>& v){ return v[2];  });
   ...
   std::string function = "tgamma (incomplete)[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "tgamma (incomplete)";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::tgamma(v[0], v[1]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::tgamma(v[0], v[1], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH))
