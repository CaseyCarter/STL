...
#  include "digamma_neg_data.ipp"
    static const _Boost::array<_Boost::array<T, 2>, 5> digamma_bugs = {{
   ...
   static const _Boost::array<_Boost::array<T, 2>, 40> digamma_integers = { {
   ...
   static const _Boost::array<_Boost::array<T, 2>, 41> digamma_half_integers = { {
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::digamma(v[0]);  }, [](const std::vector<double>& v){ return v[1];  });
   ...
   std::string function = "digamma[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "digamma";

   double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::digamma(v[0]);  });
   ...
   {
      double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::digamma(v[0], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH))
