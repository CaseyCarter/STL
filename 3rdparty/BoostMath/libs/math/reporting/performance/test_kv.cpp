...
static const _Boost::array<_Boost::array<T, 3>, 11> kv_data = { {
...
static const _Boost::array<_Boost::array<T, 3>, 5> kv_large_data = { {
...
#include "bessel_k_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::cyl_bessel_k(v[0], v[1]);  }, [](const std::vector<double>& v){ return v[2];  });
   ...
   std::string function = "cyl_bessel_k[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "cyl_bessel_k";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cyl_bessel_k(v[0], v[1]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cyl_bessel_k(v[0], v[1], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_LIBSTDCXX))
