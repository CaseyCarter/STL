...
static const _Boost::array<_Boost::array<T, 2>, 10> data2 = { {
   { { SC_(-1.0), SC_(1.0) } },
...
#include "ellint_e_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::ellint_2(v[0]);  }, [](const std::vector<double>& v){ return v[1];  });
   ...
   std::string function = "ellint_2 (complete)[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "ellint_2 (complete)";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_2(v[0]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_2(v[0], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_LIBSTDCXX))
