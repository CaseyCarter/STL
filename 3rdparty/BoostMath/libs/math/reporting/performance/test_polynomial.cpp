...
struct tester
{
   ...
   double test_add()
   {
      ...
      return _Boost::chrono::duration_cast<_Boost::chrono::duration<double> >(w.elapsed()).count();
   }
...
private:
   ...
   {
      _Boost::uniform_int<> ui(2, issmall ? 5 : 40), ui2(1, 10000);
...
template <class N>
_Boost::random::mt19937 tester<N>::gen;

template <class Number>
void test(const char* type)
{
   std::cout << "Testing type: " << type << std::endl;
   tester<_Boost::math::tools::polynomial<Number> > t;
...
int main()
{
   test<_Boost::uint64_t>("_Boost::uint64_t");
   test<double>("double");
   max_reps = 100;
   test<_Boost::multiprecision::cpp_int>("cpp_int");
   return 0;
}
