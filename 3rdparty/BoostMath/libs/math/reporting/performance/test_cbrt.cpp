...
#  include "../../test/cbrt_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::cbrt(v[1]);  }, [](const std::vector<double>& v){ return v[0];  });
   ...
   std::string function = "cbrt[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "cbrt";

   double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cbrt(v[1]);  });
   std::cout << time << std::endl;
#if defined(COMPILER_COMPARISON_TABLES)
   report_execution_time(time, std::string("Compiler Option Comparison on ") + platform_name(), "_Boost::math::cbrt", get_compiler_options_name());
#else
   ...
   {
      double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cbrt(v[1], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_C99) || defined(TEST_LIBSTDCXX))
