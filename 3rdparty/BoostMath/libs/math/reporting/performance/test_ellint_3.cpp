...
static const _Boost::array<_Boost::array<T, 4>, 65> data1 = { {
   ...
   { { SC_(-1E-170), _Boost::math::constants::pi<T>() / 4, SC_(1E-164), SC_(0.785398163397448309615660845819875721049292349843776455243736) } },
   { { SC_(-1E-170), _Boost::math::constants::pi<T>() / 4, SC_(-1E-164), SC_(0.785398163397448309615660845819875721049292349843776455243736) } },
   { { -ldexp(T(1.0), -52), _Boost::math::constants::pi<T>() / 4, SC_(0.9375), SC_(0.866032844934895872810905364370384153285798081574191920571016) } },
   { { -ldexp(T(1.0), -52), _Boost::math::constants::pi<T>() / 4, SC_(-0.9375), SC_(0.866032844934895872810905364370384153285798081574191920571016) } },
...
#include "ellint_pi3_large_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[2], v[0], v[1]);  }, [](const std::vector<double>& v){ return v[3];  });
   ...
   std::string function = "ellint_3[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "ellint_3";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[2], v[0], v[1]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[2], v[0], v[1], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_LIBSTDCXX))
