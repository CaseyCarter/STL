...
static const _Boost::array<_Boost::array<T, 3>, 11> yv_data = { {
   ...
   { { SC_(8.5), _Boost::math::constants::pi<T>() * 4, SC_(0.257086543428224355151772807588810984369026142375675714560864) } },
   { { SC_(-8.5), _Boost::math::constants::pi<T>() * 4, SC_(0.0436807946352780974532519564114026730332781693877984686758680) } },
   } };
static const _Boost::array<_Boost::array<T, 3>, 7> yv_large_data = { {
...
#include "bessel_yv_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::cyl_neumann(v[0], v[1]);  }, [](const std::vector<double>& v){ return v[2];  });
   ...
   std::string function = "cyl_neumann[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "cyl_neumann";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cyl_neumann(v[0], v[1]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::cyl_neumann(v[0], v[1], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_LIBSTDCXX))
