...
#  include "../../test/log1p_expm1_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::expm1(v[0]);  }, [](const std::vector<double>& v){ return v[2];  });
   ...
   std::string function = "expm1[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "expm1";

   double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::expm1(v[0]);  });
   ...
   {
      double time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::expm1(v[0], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_C99) || defined(TEST_LIBSTDCXX))
