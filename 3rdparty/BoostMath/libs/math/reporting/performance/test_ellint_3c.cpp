...
#include "ellint_pi2_data.ipp"
   ...
   screen_data([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[1], v[0]);  }, [](const std::vector<double>& v){ return v[2];  });
   ...
   std::string function = "ellint_3 (complete)[br](" + _Boost::lexical_cast<std::string>(data_used) + "/" + _Boost::lexical_cast<std::string>(data_total) + " tests selected)";
   std::string function_short = "ellint_3 (complete)";

   double time;

   time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[1], v[0]);  });
   ...
   {
      time = exec_timed_test([](const std::vector<double>& v){  return _Boost::math::ellint_3(v[1], v[0], _Boost::math::policies::make_policy(_Boost::math::policies::promote_double<false>()));  });
      std::cout << time << std::endl;
#if !defined(COMPILER_COMPARISON_TABLES) && (defined(TEST_GSL) || defined(TEST_RMATH) || defined(TEST_LIBSTDCXX))
