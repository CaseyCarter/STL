...
inline std::string sanitize_string(const std::string& s)
{
   ...
   return _Boost::regex_replace(s, e, "_");
}
...
{
   _Boost::interprocess::named_mutex mu;
   _Boost::interprocess::scoped_lock<_Boost::interprocess::named_mutex> lock;
   content_loader() : mu(_Boost::interprocess::open_or_create, "handle_test_result"), lock(mu)
   {
...
{
   loader.instantiate();
   const _Boost::regex e("\\[template\\s+" + cell_name + 
      "\\[\\]([^\\n]*)\\]$");
...
{
   _Boost::regex content_e("\\[template\\s+" + id + 
      "\\[\\]\\s+"
      "("
         "[^\\]\\[]*"
         "(?:"
...
void remove_error_content(const std::string& error_id)
{
   ...
   _Boost::regex content_e("\\[template\\s+" + error_id +
      "\\[\\]\\s+"
      "("
         "[^\\]\\[]*"
         "(?:"
...
void handle_test_result(const _Boost::math::tools::test_result<T>& result,
                       const Seq& worst, int row, 
                       const char* type_name, 
                       const char* test_name, 
                       const char* group_name)
{
   T eps = _Boost::math::tools::epsilon<T>();
   ...
   ss << "Max = ";
   if((_Boost::math::isfinite)(max_error_found))
      ss << max_error_found;
   else
      ss << "+INF";
   ss << "[epsilon] (Mean = ";
   if((_Boost::math::isfinite)(mean_error_found))
      ss << mean_error_found;
   else
      ss << "+INF";
   ss << "[epsilon])";
