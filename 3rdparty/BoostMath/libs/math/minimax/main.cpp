...
#define BOOST_UBLAS_TYPE_CHECK_EPSILON (type_traits<real_type>::type_sqrt (_Boost::math::tools::epsilon <real_type>()))
#define BOOST_UBLAS_TYPE_CHECK_MIN (type_traits<real_type>::type_sqrt ( _Boost::math::tools::min_value<real_type>()))
...
using namespace _Boost::spirit::classic;
...
int orderN(3);
int orderD(1);
int target_precision = _Boost::math::tools::digits<long double>();
...
_Boost::shared_ptr<_Boost::math::tools::remez_minimax<mp_type> > p_remez;
...
void step_some(unsigned count)
{
   try{
      ...
      if(!started)
      {
         ...
         //
         p_remez.reset(new _Boost::math::tools::remez_minimax<mp_type>(
            &the_function, 
            orderN, orderD, 
            a, b, 
            pin, 
            ...
            skew, 
            working_precision));
         std::cout << "Max error in interpolated form: " << std::setprecision(3) << std::scientific << _Boost::math::tools::real_cast<double>(p_remez->max_error()) << std::endl;
         //
      ...
      for(i = 0; i < count; ++i)
      {
         ...
         std::cout 
            << "Maximum Deviation Found:                     " << std::setprecision(3) << std::scientific << _Boost::math::tools::real_cast<double>(p_remez->max_error()) << std::endl
            << "Expected Error Term:                         " << std::setprecision(3) << std::scientific << _Boost::math::tools::real_cast<double>(p_remez->error_term()) << std::endl
            << "Maximum Relative Change in Control Points:   " << std::setprecision(3) << std::scientific << _Boost::math::tools::real_cast<double>(r) << std::endl;
      }
   }
...
void show(const char*, const char*)
{
   ...
   if(started)
   {
      _Boost::math::tools::polynomial<mp_type> n = p_remez->numerator();
      _Boost::math::tools::polynomial<mp_type> d = p_remez->denominator();
      ...
      _Boost::numeric::ublas::vector<mp_type> v = p_remez->zero_points();
...
void do_graph(unsigned points)
{
   ...
   while(points > 1)
   {
      ...
      std::cout << std::setprecision(10) << std::setw(30) << std::left 
         << _Boost::lexical_cast<std::string>(x) << the_function(x) << std::endl;
      --points;
      x += step;
   }
   std::cout << std::setprecision(10) << std::setw(30) << std::left 
      << _Boost::lexical_cast<std::string>(b) << the_function(b) << std::endl;
}
...
template <class Backend, _Boost::multiprecision::expression_template_option ET>
mp_type convert_to_rr(const _Boost::multiprecision::number<Backend, ET>& val)
{
   return _Boost::lexical_cast<mp_type>(val.str());
}
...
void do_test(T, const char* name)
{
   ...
   if(started)
   {
      ...
      // polynomials:
      //
      _Boost::math::tools::polynomial<T> n, d;
      _Boost::math::tools::polynomial<mp_type> nr, dr;
      ...
      {
         cn.push_back(_Boost::math::tools::real_cast<T>(cn1[i]));
      }
      for(unsigned i = 0; i < cd1.size(); ++i)
      {
         cd.push_back(_Boost::math::tools::real_cast<T>(cd1[i]));
      }
      //
      ...
      //
      _Boost::numeric::ublas::vector<mp_type> 
      ...
      for(unsigned i = 0; i < zeros.size(); ++i)
      {
         ...
         T absissa = _Boost::math::tools::real_cast<T>(zeros[i]);
         mp_type test_result = convert_to_rr(n.evaluate(absissa) / d.evaluate(absissa));
         mp_type cheb_result = convert_to_rr(_Boost::math::tools::evaluate_chebyshev(cn, absissa) / _Boost::math::tools::evaluate_chebyshev(cd, absissa));
         mp_type err, cheb_err;
         if(rel_error)
         {
            err = _Boost::math::tools::relative_error(test_result, true_result);
            cheb_err = _Boost::math::tools::relative_error(cheb_result, true_result);
         }
         else
         {
         ...
         std::cout << std::setprecision(6) << std::setw(15) << std::left << absissa
            << std::setw(15) << std::left << _Boost::math::tools::real_cast<T>(err) << _Boost::math::tools::real_cast<T>(cheb_err) << std::endl;
      }
      //
      ...
      for(unsigned i = 0; i < cheb.size(); ++i)
      {
         ...
         T absissa = _Boost::math::tools::real_cast<T>(cheb[i]);
         mp_type test_result = convert_to_rr(n.evaluate(absissa) / d.evaluate(absissa));
         mp_type cheb_result = convert_to_rr(_Boost::math::tools::evaluate_chebyshev(cn, absissa) / _Boost::math::tools::evaluate_chebyshev(cd, absissa));
         mp_type err, cheb_err;
         if(rel_error)
         ...
         if(err > max_error)
            max_error = err;
         std::cout << std::setprecision(6) << std::setw(15) << std::left << absissa
            << std::setw(15) << std::left << _Boost::math::tools::real_cast<T>(err) << 
            _Boost::math::tools::real_cast<T>(cheb_err) << std::endl;
      }
      ...
      std::cout << msg << std::setprecision(6) << "Poly: " << std::setw(20) << std::left
         << _Boost::math::tools::real_cast<T>(max_error) << "Cheb: " << _Boost::math::tools::real_cast<T>(cheb_max_error) << std::endl;
   }
   else
   {
...
{
   do_test((_Boost::multiprecision::cpp_bin_float_double_extended)(0), "float80");
}

void test_float128(const char*, const char*)
{
   do_test((_Boost::multiprecision::cpp_bin_float_quad)(0), "float128");
}
...
void do_test_n(T, const char* name, unsigned count)
{
   ...
   if(started)
   {
      ...
      for(mp_type x = a; x <= b; x += step)
      {
         ...
         T absissa = _Boost::math::tools::real_cast<T>(x);
         mp_type test_result = convert_to_rr(n.evaluate(absissa) / d.evaluate(absissa));
         ...
         std::cout << std::setprecision(6) << std::setw(15) << std::left << _Boost::math::tools::real_cast<double>(absissa)
            << (test_result < true_result ? "-" : "") << std::setw(20) << std::left 
            << _Boost::math::tools::real_cast<double>(err) 
            << _Boost::math::tools::real_cast<double>(cheb_err) << std::endl;
      }
      std::string msg = "Max Error found at ";
      msg += name;
      msg += " precision = ";
      ...
      std::cout << msg << "Poly: " << std::setprecision(6) 
         ...
         << _Boost::math::tools::real_cast<T>(max_error) 
         << " Cheb: " << _Boost::math::tools::real_cast<T>(max_cheb_error) << std::endl;
   }
   else
   {
      std::cout << "Nothing to test: try converging an approximation first!!!" << std::endl;
...
{
   do_test_n((_Boost::multiprecision::cpp_bin_float_double_extended)(0), "float80", n);
}

void test_float128_n(unsigned n)
{
   do_test_n((_Boost::multiprecision::cpp_bin_float_quad)(0), "float128", n);
}
...
void graph_poly(const char*, const char*)
{
   ...
   if(started)
   {
      ...
      // polynomials:
      //
      _Boost::math::tools::polynomial<mp_type> n, d;
      n = p_remez->numerator();
