  ...
  namespace constants
  {
   ...
   public:
      typedef _Boost::integral_constant<int,
         ...
         _Boost::is_convertible<float, Real>::value && (real_precision::value <= float_precision::value)? construct_from_float :
         _Boost::is_convertible<double, Real>::value && (real_precision::value <= double_precision::value)? construct_from_double :
         _Boost::is_convertible<long double, Real>::value && (real_precision::value <= long_double_precision::value)? construct_from_long_double :
#ifdef BOOST_MATH_USE_FLOAT128
         _Boost::is_convertible<BOOST_MATH_FLOAT128_TYPE, Real>::value && (real_precision::value <= 113) ? construct_from_float128 :
#endif
...
#define BOOST_MATH_CONSTANT_THREAD_HELPER(name, prefix) \
      _Boost::once_flag f = BOOST_ONCE_INIT;\
      _Boost::call_once(f, &BOOST_JOIN(BOOST_JOIN(string_, get_), name)<T>);
#else
#define BOOST_MATH_CONSTANT_THREAD_HELPER(name, prefix)
#endif
   ...
   namespace detail{
      ...
      template <class T, int N, const T& (*F)(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((_Boost::integral_constant<int, N>)) BOOST_MATH_APPEND_EXPLICIT_TEMPLATE_TYPE_SPEC(T))>
...
#  define BOOST_MATH_FLOAT128_CONSTANT_OVERLOAD(x) \
   static inline BOOST_CONSTEXPR T get(const _Boost::integral_constant<int, construct_from_float128>&) BOOST_NOEXCEPT\
...
#define BOOST_DEFINE_MATH_CONSTANT(name, x, y)\
   ...
   {\
      static const T result(_Boost::math::tools::convert_from_string<T>(y));\
      return result;\
   }\
   /* This one is for very high precision that is none the less known at compile time: */ \
   template <int N> static T compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((_Boost::integral_constant<int, N>)));\
   template <int N> static inline const T& get_from_compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC((_Boost::integral_constant<int, N>)))\
   {\
   ...
   {\
      ...
      int current_digits = _Boost::math::tools::digits<T>();\
      if(digits != current_digits)\
      {\
         value = current_digits > max_string_digits ? compute<0>() : T(_Boost::math::tools::convert_from_string<T>(y));\
   ...
   public:\
   static inline const T& get(const _Boost::integral_constant<int, construct_from_string>&)\
   {\
      constant_initializer<T, & BOOST_JOIN(constant_, name)<T>::get_from_string >::force_instantiate();\
      return get_from_string();\
   }\
   static inline BOOST_CONSTEXPR T get(const _Boost::integral_constant<int, construct_from_float>) BOOST_NOEXCEPT\
   { return BOOST_JOIN(x, F); }\
   static inline BOOST_CONSTEXPR T get(const _Boost::integral_constant<int, construct_from_double>&) BOOST_NOEXCEPT\
   { return x; }\
   static inline BOOST_CONSTEXPR T get(const _Boost::integral_constant<int, construct_from_long_double>&) BOOST_NOEXCEPT\
   { return BOOST_JOIN(x, L); }\
   BOOST_MATH_FLOAT128_CONSTANT_OVERLOAD(x) \
   template <int N> static inline const T& get(const _Boost::integral_constant<int, N>&)\
   {\
   ...
   { return name<T, _Boost::math::policies::policy<> >(); }\
   \
   \
