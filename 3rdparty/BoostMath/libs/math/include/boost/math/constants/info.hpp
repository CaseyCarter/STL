...
namespace _Boost{ namespace math{ namespace constants{
...
#pragma warning(disable:4127)
   ...
   if(precision_type::value)
   {
      ...
      {
       os <<
       "_Boost::math::policies::precision<" << nameof<T>() << ", " << nameof<Policy>() << " reports that the compile time precision is \n" << precision_type::value << " binary digits.\n";
      }
      else if (std::numeric_limits<T>::radix == 10)
      {
         os <<
   ...
   {
      os <<
         "_Boost::math::policies::precision<" << nameof<T>() << ", Policy> \n"
         "reports that there is no compile type precision available.\n"
         "_Boost::math::tools::digits<" << nameof<T>() << ">() \n"
         "reports that the current runtime precision is \n" <<
         _Boost::math::tools::digits<T>() << " binary digits.\n";
   }
   ...
   case 0:
      ...
      if(_Boost::math::tools::digits<T>() > max_string_digits)
      {
...
{
   print_info_on_type<T, _Boost::math::policies::policy<> >(os);
}
