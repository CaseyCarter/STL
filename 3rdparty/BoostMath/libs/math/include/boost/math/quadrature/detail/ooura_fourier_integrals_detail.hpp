...
namespace _Boost { namespace math { namespace quadrature { namespace detail {
...
std::pair<Real, Real> ooura_sin_node_and_weight(long n, Real h, Real alpha)
{
    ...
    if (n == 0) {
        ...
        Real weight = pi<Real>()*_Boost::math::sin_pi(1/(eta_prime_0*h));
...
private:
    ...
    Real estimate_integral(F const & f, Real omega, size_t i) {
        ...
        //auto cond = _Boost::math::tools::summation_condition_number<Real, true>(0);
        Real I0 = 0;
