...
public:
   exp_sinh(size_t max_refinements = 9)
    ...
    template<class F>
    auto integrate(const F& f, Real a, Real b, Real tol = _Boost::math::tools::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))  const;
    template<class F>
    auto integrate(const F& f, Real tol = _Boost::math::tools::root_epsilon<Real>(), Real* error = nullptr, Real* L1 = nullptr, std::size_t* levels = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))  const;
...
auto exp_sinh<Real, Policy>::integrate(const F& f, Real a, Real b, Real tolerance, Real* error, Real* L1, std::size_t* levels)->decltype(std::declval<F>()(std::declval<Real>()))  const
{
    ...
    using _Boost::math::quadrature::detail::exp_sinh_detail;

    static const char* function = "_Boost::math::quadrature::exp_sinh<%1%>::integrate";

    // Neither limit may be a NaN:
    if((_Boost::math::isnan)(a) || (_Boost::math::isnan)(b))
    {
    ...
    if ((_Boost::math::isfinite)(a) && (b >= _Boost::math::tools::max_value<Real>()))
    {
    ...
    if ((_Boost::math::isfinite)(b) && a <= -_Boost::math::tools::max_value<Real>())
    {
    ...
    // Infinite limits:
    if ((a <= -_Boost::math::tools::max_value<Real>()) && (b >= _Boost::math::tools::max_value<Real>()))
    {
