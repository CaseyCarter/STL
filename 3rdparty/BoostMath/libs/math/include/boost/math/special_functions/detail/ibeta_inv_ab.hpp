...
struct beta_inv_ab_t
{
   ...
   {
      return invert ? 
         p - _Boost::math::ibetac(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) 
         : _Boost::math::ibeta(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) - p;
   }
private:
   T b, z, p;
   bool invert, swap_ab;
...
T ibeta_inv_ab_imp(const T& b, const T& z, const T& p, const T& q, bool swap_ab, const Policy& pol)
{
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      return policies::raise_evaluation_error<T>("_Boost::math::ibeta_invab_imp<%1%>(%1%,%1%,%1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::ibeta_inva<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      ...
      ibetac_inva(RT1 b, RT2 x, RT3 q, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::ibetac_inva<%1%>(%1%,%1%,%1%)";
   if(q == 1)
   {
      return policies::raise_overflow_error<result_type>(function, 0, Policy());
   }
      ...
      ibeta_invb(RT1 a, RT2 x, RT3 p, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::ibeta_invb<%1%>(%1%,%1%,%1%)";
   if(p == 0)
   {
      return tools::min_value<result_type>();
   }
...
{
   return _Boost::math::ibeta_inva(b, x, p, policies::policy<>());
}
...
{
   return _Boost::math::ibetac_inva(b, x, q, policies::policy<>());
}
...
{
   return _Boost::math::ibeta_invb(a, x, p, policies::policy<>());
}
...
{
   return _Boost::math::ibetac_invb(a, x, q, policies::policy<>());
}
