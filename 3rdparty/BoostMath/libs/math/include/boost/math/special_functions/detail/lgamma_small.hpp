...
//
template <class T, class Policy, class Lanczos>
T lgamma_small_imp(T z, T zm1, T zm2, const _Boost::integral_constant<int, 64>&, const Policy& /* l */, const Lanczos&)
{
...
template <class T, class Policy, class Lanczos>
T lgamma_small_imp(T z, T zm1, T zm2, const _Boost::integral_constant<int, 113>&, const Policy& /* l */, const Lanczos&)
{
   //
   // This version uses rational approximations for small
...
template <class T, class Policy, class Lanczos>
T lgamma_small_imp(T z, T zm1, T zm2, const _Boost::integral_constant<int, 0>&, const Policy& pol, const Lanczos&)
{
   //
   ...
   else if(z >= 1.5)
   {
      // special case near 2:
      T dz = zm2;
      result = dz * log((z + Lanczos::g() - T(0.5)) / _Boost::math::constants::e<T>());
      result += _Boost::math::log1p(dz / (Lanczos::g() + T(1.5)), pol) * T(1.5);
      result += _Boost::math::log1p(Lanczos::lanczos_sum_near_2(dz), pol);
   }
   else
   {
      // special case near 1:
      T dz = zm1;
      result = dz * log((z + Lanczos::g() - T(0.5)) / _Boost::math::constants::e<T>());
      result += _Boost::math::log1p(dz / (Lanczos::g() + T(0.5)), pol) / 2;
      result += _Boost::math::log1p(Lanczos::lanczos_sum_near_1(dz), pol);
   }
   return result;
}
