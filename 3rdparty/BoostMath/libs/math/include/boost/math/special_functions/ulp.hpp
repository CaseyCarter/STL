...
T ulp_imp(const T& val, const _Boost::true_type&, const Policy& pol)
{
...
// non-binary version:
template <class T, class Policy>
T ulp_imp(const T& val, const _Boost::false_type&, const Policy& pol)
{
...
inline typename tools::promote_args<T>::type ulp(const T& val, const Policy& pol)
{
   ...
   return detail::ulp_imp(static_cast<result_type>(val), _Boost::integral_constant<bool, !std::numeric_limits<result_type>::is_specialized || (std::numeric_limits<result_type>::radix == 2)>(), pol);
}
