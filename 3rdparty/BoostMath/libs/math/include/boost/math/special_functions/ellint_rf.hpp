...
namespace _Boost { namespace math { namespace detail{
   ...
   T ellint_rf_imp(T x, T y, T z, const Policy& pol)
   {
      ...
      T An = (x + y + z) / 3;
      T A0 = An;
      T Q = pow(3 * _Boost::math::tools::epsilon<T>(), T(-1) / 8) * (std::max)((std::max)(fabs(An - xn), fabs(An - yn)), fabs(An - zn));
      T fn = 1;


      // duplication
      unsigned k = 1;
      for(; k < _Boost::math::policies::get_max_series_iterations<Policy>(); ++k)
      {
         T root_x = sqrt(xn);
         T root_y = sqrt(yn);
         T root_z = sqrt(zn);
   ...
   ellint_rf(T1 x, T2 y, T3 z, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_rf_imp(
         ...
         static_cast<value_type>(z), pol), "_Boost::math::ellint_rf<%1%>(%1%,%1%,%1%)");
}
