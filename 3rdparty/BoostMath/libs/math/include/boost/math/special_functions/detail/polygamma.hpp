...
namespace _Boost { namespace math { namespace detail{
  ...
  T polygamma_atinfinityplus(const int n, const T& x, const Policy& pol, const char* function) // for large values of x such as for x> 400
  {
     ...
     if(n + x == x)
     {
        ...
        if((nlx < tools::log_max_value<T>()) && (n < (int)max_factorial<T>::value))
           return ((n & 1) ? 1 : -1) * _Boost::math::factorial<T>(n - 1) * pow(x, -n);
        else
         return ((n & 1) ? 1 : -1) * exp(_Boost::math::lgamma(T(n), pol) - n * log(x));
     }
     ...
     //
     part_term = ((n > (int)_Boost::math::max_factorial<T>::value) && (T(n) * n > tools::log_max_value<T>())) 
        ? T(0) : static_cast<T>(_Boost::math::factorial<T>(n - 1, pol) * pow(x, -n - 1));
     if(part_term == 0)
     {
        // Either n is very large, or the power term underflows,
        // set the initial values of part_term, term and sum via logs:
        part_term = static_cast<T>(_Boost::math::lgamma(n, pol) - (n + 1) * log(x));
        sum = exp(part_term + log(n + 2 * x) - _Boost::math::constants::ln_two<T>());
        part_term += log(T(n) * (n + 1)) - _Boost::math::constants::ln_two<T>() - log(x);
     ...
     for(unsigned k = 1;;)
     {
        term = part_term * _Boost::math::bernoulli_b2n<T>(k, pol);
        sum += term;
        //
  ...
  T polygamma_attransitionplus(const int n, const T& x, const Policy& pol, const char* function)
  {
    ...
    if(iter > (int)policies::get_max_series_iterations<Policy>())
       return policies::raise_evaluation_error<T>(function, ("Exceeded maximum series evaluations evaluating at n = " + _Boost::lexical_cast<std::string>(n) + " and x = %1%").c_str(), x, pol);
    ...
    if(log(z + iter) * minus_m_minus_one > -tools::log_max_value<T>())
    {
       ...
       sum0 *= _Boost::math::factorial<T>(n);
    }
    else
    {
       for(int k = 1; k <= iter; ++k)
       {
          T log_term = log(z) * minus_m_minus_one + _Boost::math::lgamma(T(n + 1), pol);
          sum0 += exp(log_term);
          z += 1;
       }
    }
  ...
  T polygamma_nearzero(int n, T x, const Policy& pol, const char* function)
  {
     ...
     //
     T scale = _Boost::math::factorial<T>(n, pol);
     //
     ...
     T prefix = pow(x, n + 1);
     if(prefix == 0)
        return _Boost::math::policies::raise_overflow_error<T>(function, 0, pol);
     prefix = 1 / prefix;
     //
     ...
     {
        // Get the k'th term:
        T term = factorial_part * _Boost::math::zeta(T(k + n + 1), pol);
        sum += term;
        // Termination condition:
        if(fabs(term) < fabs(sum * _Boost::math::policies::get_epsilon<T, Policy>()))
           break;
        //
     ...
     //
     if(_Boost::math::tools::max_value<T>() / scale < sum)
        return _Boost::math::policies::raise_overflow_error<T>(function, 0, pol);
     sum *= scale;
  ...
  T poly_cot_pi(int n, T x, T xc, const Policy& pol, const char* function)
  {
     ...
     //
     T s = fabs(x) < fabs(xc) ? _Boost::math::sin_pi(x, pol) : _Boost::math::sin_pi(xc, pol);
     T c = _Boost::math::cos_pi(x, pol);
     switch(n)
     {
     case 1:
     ...
     case 2:
     {
        return 2 * constants::pi<T, Policy>() * constants::pi<T, Policy>() * c / _Boost::math::pow<3>(s, pol);
     }
     case 3:
     {
        int P[] = { -2, -4 };
        return _Boost::math::pow<3>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<4>(s, pol);
     }
     case 4:
     {
        int P[] = { 16, 8 };
        return _Boost::math::pow<4>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<5>(s, pol);
     }
     case 5:
     {
        int P[] = { -16, -88, -16 };
        return _Boost::math::pow<5>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<6>(s, pol);
     }
     case 6:
     {
        int P[] = { 272, 416, 32 };
        return _Boost::math::pow<6>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<7>(s, pol);
     }
     case 7:
     {
        int P[] = { -272, -2880, -1824, -64 };
        return _Boost::math::pow<7>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<8>(s, pol);
     }
     case 8:
     {
        int P[] = { 7936, 24576, 7680, 128 };
        return _Boost::math::pow<8>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<9>(s, pol);
     }
     case 9:
     {
        int P[] = { -7936, -137216, -185856, -31616, -256 };
        return _Boost::math::pow<9>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<10>(s, pol);
     }
     case 10:
     {
        int P[] = { 353792, 1841152, 1304832, 128512, 512 };
        return _Boost::math::pow<10>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<11>(s, pol);
     }
     case 11:
     {
        int P[] = { -353792, -9061376, -21253376, -8728576, -518656, -1024};
        return _Boost::math::pow<11>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<12>(s, pol);
     }
     case 12:
     {
        int P[] = { 22368256, 175627264, 222398464, 56520704, 2084864, 2048 };
        return _Boost::math::pow<12>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<13>(s, pol);
     }
#ifndef BOOST_NO_LONG_LONG
     case 13:
     {
        long long P[] = { -22368256LL, -795300864LL, -2868264960LL, -2174832640LL, -357888000LL, -8361984LL, -4096 };
        return _Boost::math::pow<13>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<14>(s, pol);
     }
     case 14:
     {
        long long P[] = { 1903757312LL, 21016670208LL, 41731645440LL, 20261765120LL, 2230947840LL, 33497088LL, 8192 };
        return _Boost::math::pow<14>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<15>(s, pol);
     }
     case 15:
     {
        long long P[] = { -1903757312LL, -89702612992LL, -460858269696LL, -559148810240LL, -182172651520LL, -13754155008LL, -134094848LL, -16384 };
        return _Boost::math::pow<15>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<16>(s, pol);
     }
     case 16:
     {
        ...
        return _Boost::math::pow<16>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<17>(s, pol);
     }
     case 17:
     {
        ...
        return _Boost::math::pow<17>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<18>(s, pol);
     }
     case 18:
     {
        ...
        return _Boost::math::pow<18>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<19>(s, pol);
     }
     case 19:
     {
        ...
        return _Boost::math::pow<19>(constants::pi<T, Policy>(), pol) * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<20>(s, pol);
     }
     case 20:
     {
        ...
        return _Boost::math::pow<20>(constants::pi<T, Policy>(), pol) * c * tools::evaluate_even_polynomial(P, c) / _Boost::math::pow<21>(s, pol);
     }
#endif
     }
     ...
     static _Boost::detail::lightweight_mutex m;
     _Boost::detail::lightweight_mutex::scoped_lock l(m);
#endif
     ...
     T sum = _Boost::math::tools::evaluate_even_polynomial(&table[index][0], c, table[index].size());
     if(index & 1)
     ...
     //
     T power_terms = n * log(_Boost::math::constants::pi<T>());
     if(s == 0)
        return sum * _Boost::math::policies::raise_overflow_error<T>(function, 0, pol);
     power_terms -= log(fabs(s)) * (n + 1);
     power_terms += _Boost::math::lgamma(T(n));
     power_terms += log(fabs(sum));

     if(power_terms > _Boost::math::tools::log_max_value<T>())
        return sum * _Boost::math::policies::raise_overflow_error<T>(function, 0, pol);

     return exp(power_terms) * ((s < 0) && ((n + 1) & 1) ? -1 : 1) * _Boost::math::sign(sum);
  }
  ...
  struct polygamma_initializer
  {
     struct init
     {
        init()
        {
           ...
           _Boost::math::polygamma(30, T(-2.5f), Policy());
        }
  ...
  {
    BOOST_MATH_STD_USING
    static const char* function = "_Boost::math::polygamma<%1%>(int, %1%)";
    ...
    {
       return (n & 1 ? 1 : -1) * _Boost::math::factorial<T>(n, pol) * _Boost::math::zeta(T(n + 1), pol);
    }
    else if(x == 0.5f)
    {
       T result = (n & 1 ? 1 : -1) * _Boost::math::factorial<T>(n, pol) * _Boost::math::zeta(T(n + 1), pol);
       if(fabs(result) >= ldexp(tools::max_value<T>(), -n - 1))
          return _Boost::math::sign(result) * policies::raise_overflow_error<T>(function, 0, pol);
