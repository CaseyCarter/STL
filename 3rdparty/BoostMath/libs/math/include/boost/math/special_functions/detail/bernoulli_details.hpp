...
T b2n_asymptotic(int n)
{
   ...
   const T approximate_log_of_bernoulli_bn = 
        ((_Boost::math::constants::half<T>() + nx) * log(nx))
        + ((_Boost::math::constants::half<T>() - nx) * log(_Boost::math::constants::pi<T>()))
        + (((T(3) / 2) - nx) * _Boost::math::constants::ln_two<T>())
        + ((nx * (T(2) - (nx2 * 7) * (1 + ((nx2 * 30) * ((nx2 * 12) - 1))))) / (((nx2 * nx2) * nx2) * 2520));
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > tools::log_max_value<T>() 
      ? policies::raise_overflow_error<T>("_Boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, nx, Policy())
...
T t2n_asymptotic(int n)
{
   ...
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("_Boost::math::tangent_t2n<%1%>(std::size_t)", 0, T(n), Policy());
   t2n *= p2;
   p2 -= 1;
   if(tools::max_value<T>() / p2 < t2n)
      return policies::raise_overflow_error<T>("_Boost::math::tangent_t2n<%1%>(std::size_t)", 0, Policy());
   t2n *= p2;
   return t2n;
}
//
...
struct max_bernoulli_root_functor
{
   ...
   double operator()(double n)
   {
      ...
      const double approximate_log_of_bernoulli_bn
         =   ((_Boost::math::constants::half<double>() + n) * log(n))
           + ((_Boost::math::constants::half<double>() - n) * log(_Boost::math::constants::pi<double>()))
           + (((double(3) / 2) - n) * _Boost::math::constants::ln_two<double>())
...
template <class T, class Policy>
inline std::size_t find_bernoulli_overflow_limit(const _Boost::false_type&)
{
   ...
   ulong_long_type t = lltrunc(_Boost::math::tools::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   _Boost::math::tools::equal_floor tol;
   _Boost::uintmax_t max_iter = _Boost::math::policies::get_max_root_iterations<Policy>();
   double result = _Boost::math::tools::toms748_solve(fun, sqrt(double(t)), double(t), tol, max_iter).first / 2;
...
template <class T, class Policy>
inline std::size_t find_bernoulli_overflow_limit(const _Boost::true_type&)
{
...
std::size_t b2n_overflow_limit()
{
   ...
   typedef _Boost::integral_constant<bool, (bernoulli_imp_variant<T>::value >= 1) && (bernoulli_imp_variant<T>::value <= 3)> tag_type;
...
struct bernoulli_initializer
{
   struct init
   {
      init()
      {
         ...
         //
         _Boost::math::bernoulli_b2n<T>(2, Policy());
#ifndef BOOST_NO_EXCEPTIONS
         try{
#endif
            _Boost::math::bernoulli_b2n<T>(max_bernoulli_b2n<T>::value + 1, Policy());
#ifndef BOOST_NO_EXCEPTIONS
         } catch(const std::overflow_error&){}
#endif
         _Boost::math::tangent_t2n<T>(2, Policy());
      }
...
// _Boost::container::static_vector here, but that allocates on the stack, which may well
...
public:
   bernoulli_numbers_cache() : m_overflow_limit((std::numeric_limits<std::size_t>::max)())
      ...
      , m_counter(0)
#endif
      , m_current_precision(_Boost::math::tools::digits<T>())
   {}
   ...
   void tangent(std::size_t m)
   {
      ...
      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         ...
         if(i >= min_overflow_index && (_Boost::math::tools::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), _Boost::math::tools::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (_Boost::math::tools::max_value<T>() / (i - j) < m_intermediates[j])
                  || (_Boost::math::tools::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (_Boost::math::tools::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
                  || ((_Boost::math::isinf)(m_intermediates[j]))
                );
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("_Boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      //
      if(m_current_precision < _Boost::math::tools::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("_Boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[i];
         ++out;
      }
      ...
      //
      _Boost::detail::lightweight_mutex::scoped_lock l(m_mutex);
      if(m_current_precision < _Boost::math::tools::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      if((static_cast<std::size_t>(m_counter.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
         || (static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < _Boost::math::tools::digits<T>()))
      {
         _Boost::detail::lightweight_mutex::scoped_lock l(m_mutex);
         ...
         if((static_cast<std::size_t>(m_counter.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < start + n)
            || (static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < _Boost::math::tools::digits<T>()))
         {
            if(static_cast<int>(m_current_precision.load(BOOST_MATH_ATOMIC_NS::memory_order_consume)) < _Boost::math::tools::digits<T>())
            {
               bn.clear();
               tn.clear();
               m_intermediates.clear();
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("_Boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol) : bn[static_cast<typename container_type::size_type>(i)];
         ++out;
      }
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         if(i >= m_overflow_limit)
            *out = policies::raise_overflow_error<T>("_Boost::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(i), pol);
         else
         {
...
#elif defined(BOOST_MATH_NO_ATOMIC_INT)
   _Boost::detail::lightweight_mutex m_mutex;
   int m_current_precision;
#else
   _Boost::detail::lightweight_mutex m_mutex;
