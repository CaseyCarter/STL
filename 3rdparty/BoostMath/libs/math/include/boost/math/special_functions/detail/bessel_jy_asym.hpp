...
{
   T c = 1;
   T p = (v / _Boost::math::sin_pi(v, pol)) * pow(x / 2, -v) / _Boost::math::tgamma(1 - v, pol);
   T q = (v / _Boost::math::sin_pi(v, pol)) * pow(x / 2, v) / _Boost::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = _Boost::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (_Boost::math::isfinite)(s) ? 
      s : policies::raise_overflow_error<T>("_Boost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
