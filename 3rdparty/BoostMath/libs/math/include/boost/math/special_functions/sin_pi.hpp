...
inline typename tools::promote_args<T>::type sin_pi(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(_Boost::math::detail::sin_pi_imp<value_type>(x, forwarding_policy()), "sin_pi");
}

template <class T>
inline typename tools::promote_args<T>::type sin_pi(T x)
{
   return _Boost::math::sin_pi(x, policies::policy<>());
}
