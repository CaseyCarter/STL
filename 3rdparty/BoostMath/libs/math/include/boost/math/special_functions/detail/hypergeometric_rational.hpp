...
#define BOOST_MATH_HYPERGEOMETRIC_RATIONAL_HPP
  ...
  inline T hypergeometric_1F1_rational(const T& ap, const T& cp, const T& zp, const Policy& )
  {
    ...
    const unsigned max_iterations = _Boost::math::policies::get_max_series_iterations<Policy>();
    ...
    for (unsigned k = 2; k < max_iterations; ++k)
    {
      ...
      if ((fabs(result) * _Boost::math::tools::epsilon<T>()) > fabs(result - prev_result) / fabs(result))
        break;
