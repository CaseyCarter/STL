  ...
  namespace _Boost { namespace math { namespace detail {
     ...
     inline T hypergeometric_1F1_imp(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling);
      ...
      T max_b_for_1F1_small_a_negative_b_by_ratio(const T& z)
      {
         ...
         //
         b += (std::max)(0, _Boost::math::tools::digits<T>() - 53) / 5;
         return b;
      }
      ...
      T hypergeometric_1F1_small_a_negative_b_by_ratio(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
      {
         ...
         T ratio = _Boost::math::tools::function_ratio_from_forwards_recurrence(_Boost::math::detail::hypergeometric_1F1_recurrence_b_coefficients<T>(a, b, z), _Boost::math::tools::epsilon<T>(), max_iter);
         _Boost::math::policies::check_series_iterations<T>("_Boost::math::hypergeometric_1F1_small_a_negative_b_by_ratio<%1%>(%1%,%1%,%1%)", max_iter, pol);
         T first = 1;
         T second = 1 / ratio;
         int scaling1 = 0;
         BOOST_ASSERT(b + iterations != a);
         second = _Boost::math::tools::apply_recurrence_relation_forward(_Boost::math::detail::hypergeometric_1F1_recurrence_b_coefficients<T>(a, b + 1, z), iterations, first, second, &scaling1);
         int scaling2 = 0;
