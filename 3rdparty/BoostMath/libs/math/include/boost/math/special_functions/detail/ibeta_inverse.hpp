...
struct temme_root_finder
{
   ...
   _Boost::math::tuple<T, T> operator()(T x)
   {
      ...
      if(y == 0)
      {
         ...
         return _Boost::math::make_tuple(static_cast<T>(-big), static_cast<T>(-big));
      }
      if(x == 0)
      {
         T big = tools::max_value<T>() / 4;
         return _Boost::math::make_tuple(static_cast<T>(-big), big);
      }
      T f = log(x) + a * log(y) + t;
      T f1 = (1 / x) - (a / (y));
...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   //
   T eta0 = _Boost::math::erfc_inv(2 * z, pol);
   eta0 /= -sqrt(a / 2);
...
T temme_method_3_ibeta_inverse(T a, T b, T p, T q, const Policy& pol)
{
   ...
   T eta0;
   if(p < q)
      eta0 = _Boost::math::gamma_q_inv(b, p, pol);
   else
      eta0 = _Boost::math::gamma_p_inv(b, q, pol);
   eta0 /= a;
   //
...
struct ibeta_roots
{
   ...
   _Boost::math::tuple<T, T, T> operator()(T x)
   {
      BOOST_MATH_STD_USING // ADL of std names
...
T ibeta_inv_imp(T a, T b, T p, T q, const Policy& pol, T* py)
{
   ...
   else if(b == 1)
   {
      if(p < q)
      {
         ...
         {
            x = pow(p, 1 / a);
            y = -_Boost::math::expm1(log(p) / a, pol);
         }
         else
         {
            x = pow(p, 1 / a);
      ...
      {
         x = exp(_Boost::math::log1p(-q, pol) / a);
         y = -_Boost::math::expm1(_Boost::math::log1p(-q, pol) / a, pol);
      }
      if(invert)
         std::swap(x, y);
      if(py)
   ...
   else if(a + b > 5)
   {
      ...
      else
      {
         ...
         if((lambda >= 0.2) && (lambda <= 0.8) && (r >= 10))
         {
            ...
            {
               x = ppa * pow(a * _Boost::math::beta(a, b, pol), 1/a);
            }
            else
   ...
   else if((a < 1) && (b < 1))
   {
      ...
      //
      T fs = _Boost::math::ibeta(a, b, xs, pol) - p;
      ...
      T xg = pow(a * p * _Boost::math::beta(a, b, pol), 1/a);
      x = xg / (1 + xg);
      y = 1 / (1 + xg);
      //
   ...
   else if((a > 1) && (b > 1))
   {
      ...
      T xs = (a - 1) / (a + b - 2);
      T xs2 = (b - 1) / (a + b - 2);
      T ps = _Boost::math::ibeta(a, b, xs, pol) - p;
      ...
      //
      T lx = log(p * a * _Boost::math::beta(a, b, pol)) / a;
      x = exp(lx);
      y = x < 0.9 ? T(1 - x) : (T)(-_Boost::math::expm1(lx, pol));
   ...
   else /*if((a <= 1) != (b <= 1))*/
   {
      ...
      if(pow(p, 1/a) < 0.5)
      {
         x = pow(p * a * _Boost::math::beta(a, b, pol), 1 / a);
         if(x == 0)
            x = _Boost::math::tools::min_value<T>();
         y = 1 - x;
      }
      else /*if(pow(q, 1/b) < 0.1)*/
      {
         // model a distorted quarter circle:
         y = pow(1 - pow(p, b * _Boost::math::beta(a, b, pol)), 1/b);
         if(y == 0)
            y = _Boost::math::tools::min_value<T>();
         x = 1 - y;
      }
   }
   ...
   if(lower == 0)
   {
      if(invert && (py == 0))
      {
         ...
         //
         lower = _Boost::math::tools::epsilon<T>();
         if(x < lower)
            x = lower;
      }
      else
         lower = _Boost::math::tools::min_value<T>();
      if(x < lower)
         x = lower;
   }
   //
   ...
   //
   int digits = _Boost::math::policies::digits<T, Policy>() / 2;
   if((x < 1e-50) && ((a < 1) || (b < 1)))
   {
      //
   ...
   x = _Boost::math::tools::halley_iterate(
      _Boost::math::detail::ibeta_roots<T, Policy>(a, b, (p < q ? p : q), (p < q ? false : true)), x, lower, upper, digits, max_iter);
   policies::check_root_iterations<T>("_Boost::math::ibeta<%1%>(%1%, %1%, %1%)", max_iter, pol);
   //
   ...
   //BOOST_ASSERT((x != lower) || (x == _Boost::math::tools::min_value<T>()) || (x == _Boost::math::tools::epsilon<T>()));
   //
...
{
   static const char* function = "_Boost::math::ibeta_inv<%1%>(%1%,%1%,%1%)";
   BOOST_FPU_EXCEPTION_GUARD
...
{
   static const char* function = "_Boost::math::ibetac_inv<%1%>(%1%,%1%,%1%)";
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename tools::promote_args<T1, T2, T3, T4>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   typedef typename policies::normalise<
