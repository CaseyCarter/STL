...
T bessel_jn(int n, T x, const Policy& pol)
{
    ...
    if (n < abs(x))                         // forward recurrence
    {
        prev = bessel_j0(x);
        current = bessel_j1(x);
        policies::check_series_iterations<T>("_Boost::math::bessel_j_n<%1%>(%1%,%1%)", n, pol);
        for (int k = 1; k < n; k++)
        {
            T fact = 2 * k / x;
            //
    ...
    else                                    // backward recurrence
    {
        ...
        _Boost::math::detail::CF1_jy(static_cast<T>(n), x, &fn, &s, pol);
        prev = fn;
        current = 1;
    ...
    if(tools::max_value<T>() * scale < fabs(value))
       return policies::raise_overflow_error<T>("_Boost::math::bessel_jn<%1%>(%1%,%1%)", 0, pol);
