...
#include <boost/math/tools/precision.hpp> // _Boost::math::tools::max_value().
...
{
  BOOST_MATH_STD_USING
  static const T max_diff = _Boost::math::tools::root_epsilon<T>() * fabs(w_est);
...
// single step (if _Boost::false_type) or iterate (if _Boost::true_type).
// Selected at compile-time using parameter 3.
template <class T>
inline
T lambert_w_maybe_halley_iterate(T z, T w, _Boost::false_type const&)
{
...
template <class T>
inline
T lambert_w_maybe_halley_iterate(T z, T w, _Boost::true_type const&)
{
...
template <class T>
inline
double maybe_reduce_to_double(const T& z, const _Boost::true_type&)
{
...
template <class T>
inline
T maybe_reduce_to_double(const T& z, const _Boost::false_type&)
...
template <class T>
inline
double must_reduce_to_double(const T& z, const _Boost::true_type&)
{
   return static_cast<double>(z); // Reduce to double precision.
}
...
template <class T>
inline
double must_reduce_to_double(const T& z, const _Boost::false_type&)
{ // try a lexical_cast and hope for the best:
   return _Boost::lexical_cast<double>(z);
}
...
T schroeder_update(const T w, const T y)
{
  ...
  using _Boost::math::float_distance;
...
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 0> const&);   //  for float (32-bit) type.

template <class T, class Policy>
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 1> const&);   //  for double (64-bit) type.

template <class T, class Policy>
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 2> const&);   //  for long double (double extended 80-bit) type.

template <class T, class Policy>
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 3> const&);   //  for long double (128-bit) type.

template <class T, class Policy>
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 4> const&);   //  for float128 quadmath Q type.

template <class T, class Policy>
T lambert_w0_small_z(T x, const Policy&, _Boost::integral_constant<int, 5> const&);   //  Generic multiprecision T.
...
template <class T, class Policy>
T lambert_w0_small_z(T z, const Policy&, _Boost::integral_constant<int, 0> const&)
{
  ...
  return result;
} // template <class T>   T lambert_w0_small_z(T x, _Boost::integral_constant<int, 0> const&)
...
template <class T, class Policy>
T lambert_w0_small_z(const T z, const Policy&, _Boost::integral_constant<int, 1> const&)
{
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  std::streamsize prec = std::cout.precision(std::numeric_limits<T>::max_digits10); // Save.
  ...
  return result;
} // T lambert_w0_small_z(const T z, _Boost::integral_constant<int, 1> const&)
...
template <class T, class Policy>
T lambert_w0_small_z(const T z, const Policy&, _Boost::integral_constant<int, 2> const&)
{
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
...
#endif // BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  return result;
}  // long double lambert_w0_small_z(const T z, _Boost::integral_constant<int, 1> const&)
...
// (This is NOT used for 128-bit quad _Boost::multiprecision::float128 type which required a suffix Q
...
template <class T, class Policy>
T lambert_w0_small_z(const T z, const Policy&, _Boost::integral_constant<int, 3> const&)
{
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  std::streamsize precision = std::cout.precision(std::numeric_limits<T>::max_digits10); // Save.
...
#endif // BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  return result;
}  // T lambert_w0_small_z(const T z, _Boost::integral_constant<int, 3> const&)
...
template <class T, class Policy>
T lambert_w0_small_z(const T z, const Policy&, _Boost::integral_constant<int, 4> const&)
{
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  std::streamsize precision = std::cout.precision(std::numeric_limits<T>::max_digits10); // Save.
  ...
  return result;
}  // T lambert_w0_small_z(const T z, _Boost::integral_constant<int, 4> const&) float128

#else

template <class T, class Policy>
inline T lambert_w0_small_z(const T z, const Policy& pol, _Boost::integral_constant<int, 4> const&)
{
   return lambert_w0_small_z(z, pol, _Boost::integral_constant<int, 5>());
}
...
template <class T, class Policy>
inline T lambert_w0_small_z(T z, const Policy& pol, _Boost::integral_constant<int, 5> const&)
{
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  std::streamsize precision = std::cout.precision(std::numeric_limits<T>::max_digits10); // Save.
  ...
  using _Boost::math::policies::get_epsilon; // for type T.
  using _Boost::math::tools::sum_series;
  using _Boost::math::tools::evaluate_polynomial;
  ...
  _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>(); // Max iterations from policy.
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_W_SMALL_Z_SERIES
  ...
  //sum_series(Functor& func, int bits, _Boost::uintmax_t& max_terms, const U& init_value)
  ...
  policies::check_series_iterations<T>("_Boost::math::lambert_w0_small_z<%1%>(%1%)", max_iter, pol);
...
{
   BOOST_MATH_STD_USING
   const T p2 = 2 * (_Boost::math::constants::e<T>() * z + 1);
   const T p = sqrt(p2);
   return p;
}
...
//template <class T, class Policy>
//T lambert_w0_imp(T w, const Policy& pol, const _Boost::integral_constant<int, 0>&); // 32 bit usually float.
//template <class T, class Policy>
//T lambert_w0_imp(T w, const Policy& pol, const _Boost::integral_constant<int, 1>&); //  64 bit usually double.
//template <class T, class Policy>
//T lambert_w0_imp(T w, const Policy& pol, const _Boost::integral_constant<int, 2>&); // 80-bit long double.
...
T lambert_w_positive_rational_float(T z)
{
   ...
   if (z < 2)
   {
      ...
      { // 0.05 < z < 0.5
        ...
        // Maximum Relative Change in Control Points : 7.555e-04 Y offset : -8.196592331e-01
         ...
         return z * (Y + _Boost::math::tools::evaluate_polynomial(P, z) / _Boost::math::tools::evaluate_polynomial(Q, z));
      }
      else
      { // 0.5 < z < 2
        // Max error in interpolated form: 1.018e-08
         ...
         return z * (Y + _Boost::math::tools::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
      ...
      return Y + _Boost::math::tools::evaluate_rational(P, Q, z);
   }
   else if (z < 18)
   {
      // 6 < z < 18
   ...
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + _Boost::math::tools::evaluate_polynomial(P, log_w) / _Boost::math::tools::evaluate_polynomial(Q, log_w);
   }
...
T lambert_w_negative_rational_float(T z, const Policy& pol)
{
   ...
   { // Very close to branch singularity.
     // Max error in interpolated form: 5.269e-08
      ...
      return -d / (Y + _Boost::math::tools::evaluate_polynomial(P, d) / _Boost::math::tools::evaluate_polynomial(Q, d));
   }
   else
   {
...
template <class T, class Policy>
inline T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 1>&)
{
  static const char* function = "_Boost::math::lambert_w0<%1%>"; // For error messages.
  BOOST_MATH_STD_USING // Aid ADL of std functions.

  if ((_Boost::math::isnan)(z))
  {
    return _Boost::math::policies::raise_domain_error<T>(function, "Expected a value > -e^-1 (-0.367879...) but got %1%.", z, pol);
  }
  if ((_Boost::math::isinf)(z))
  {
    return _Boost::math::policies::raise_overflow_error<T>(function, "Expected a finite value but got %1%.", z, pol);
  }
   ...
   else if (z <= -0.3678794411714423215955237701614608674458111310f)
   {
      if (z < -0.3678794411714423215955237701614608674458111310f)
         return _Boost::math::policies::raise_domain_error<T>(function, "Expected z >= -e^-1 (-0.367879...) but got %1%.", z, pol);
      return -1;
   }
   else // z < 0.05
   {
...
} // T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 1>&) for 32-bit usually float.
...
T lambert_w_positive_rational_double(T z)
{
   ...
   if (z < 2)
   {
      if (z < 0.5)
      {
         ...
         return z * (offset + _Boost::math::tools::evaluate_polynomial(P, z) / _Boost::math::tools::evaluate_polynomial(Q, z));
      }
      else
      {
         ...
         return z * (offset + _Boost::math::tools::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
   ...
   else if (z < 18)
   {
      ...
      return offset + _Boost::math::tools::evaluate_rational(P, Q, z);
   }
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + _Boost::math::tools::evaluate_rational(P, Q, log_w);
   }
   else if (z < 7.896296e+13)  // 9.2 < log(z) <= 32
   {
...
template <class T, class Policy>
inline T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 2>&)
{
   static const char* function = "_Boost::math::lambert_w0<%1%>";
   BOOST_MATH_STD_USING // Aid ADL of std functions.
...
} // T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 2>&) 64-bit precision, usually double.
...
template <class T, class Policy>
inline T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 0>&)
{
   static const char* function = "_Boost::math::lambert_w0<%1%>";
   BOOST_MATH_STD_USING // Aid ADL of std functions.
   ...
   { // Very close to branch point so rational polynomials are not usable.
      if (z <= -_Boost::math::constants::exp_minus_one<T>())
      {
         if (z == -_Boost::math::constants::exp_minus_one<T>())
   ...
   typedef _Boost::integral_constant<bool,
   ...
   //   if (_Boost::is_constructible<double, T>() == true).
   T w = lambert_w0_imp(maybe_reduce_to_double(z, _Boost::is_constructible<double, T>()), pol, _Boost::integral_constant<int, 2>());

   return lambert_w_maybe_halley_iterate(w, z, tag_type());

} // T lambert_w0_imp(T z, const Policy& pol, const _Boost::integral_constant<int, 0>&)  all extended precision types.
...
T lambert_wm1_imp(const T z, const Policy&  pol)
{
  ...
  BOOST_STATIC_ASSERT_MSG(!_Boost::is_integral<T>::value,
    "Must be floating-point or fixed type (not integer type), for example: lambert_wm1(1.), not lambert_wm1(1)!");

  BOOST_MATH_STD_USING // Aid argument dependent lookup (ADL) of abs.

  const char* function = "_Boost::math::lambert_wm1<RealType>(<RealType>)"; // Used for error messages.
  ...
  if (z > -_Boost::math::tools::min_value<T>())
  ...
  if (z == -_Boost::math::constants::exp_minus_one<T>()) // == singularity/branch point z = -exp(-1) = -3.6787944.
  ...
  { // Close to singularity/branch point z = -0.3678794411714423215955237701614608727 but on W-1 branch.
    ...
    if (p2 > 0)
    {
      ...
      if (_Boost::math::tools::digits<T>() > 53)
  ...
  { // T is more precise than 64-bit double (or long double, or ?),
    ...
    using _Boost::math::policies::precision;
    using _Boost::math::policies::digits10;
    using _Boost::math::policies::digits2;
    using _Boost::math::policies::policy;
    // Compute a 50-bit precision approximate W0 in a double (no Halley refinement).
    T double_approx(static_cast<T>(lambert_wm1_imp(must_reduce_to_double(z, _Boost::is_constructible<double, T>()), policy<digits2<50> >())));
#ifdef BOOST_MATH_INSTRUMENT_LAMBERT_WM1_NOT_BUILTIN
    std::streamsize saved_precision = std::cout.precision(std::numeric_limits<T>::max_digits10);
  ...
  { // Use a lookup table to find the nearest integer part of Lambert W as starting point for Bisection.
    using namespace _Boost::math::lambert_w_detail::lambert_w_lookup;
  ...
  bisect:
    ...
    typedef typename mpl::if_c<_Boost::is_constructible<lookup_t, T>::value, lookup_t, T>::type calc_type;
...
//! Lambert W0 using User-defined policy.
  template <class T, class Policy>
  inline
    typename _Boost::math::tools::promote_args<T>::type
  ...
  lambert_w0_prime(T z, const Policy& pol)
  {
    ...
    if (z == - _Boost::math::constants::exp_minus_one<result_type>())
    {
        return numeric_limits<result_type>::has_infinity ? numeric_limits<result_type>::infinity() : _Boost::math::tools::max_value<result_type>();
    }
  ...
  lambert_wm1_prime(T z, const Policy& pol)
  {
    ...
    //}
    //if (z == - _Boost::math::constants::exp_minus_one<result_type>())
    if (z == 0 || z == - _Boost::math::constants::exp_minus_one<result_type>())
    {
        return numeric_limits<result_type>::has_infinity ? -numeric_limits<result_type>::infinity() : -_Boost::math::tools::max_value<result_type>();
    }
