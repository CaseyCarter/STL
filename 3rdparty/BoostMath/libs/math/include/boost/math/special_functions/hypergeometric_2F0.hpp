...
namespace _Boost { namespace math { namespace detail {
   ...
   inline T hypergeometric_2F0_imp(T a1, T a2, const T& z, const Policy& pol, bool asymptotic = false)
   {
      ...
      static const char* const function = "_Boost::math::hypergeometric_2F0<%1%,%1%,%1%>(%1%,%1%,%1%)";
      ...
      if ((a1 == a2 - 0.5f) && (z < 0))
      {
         ...
         int n = static_cast<int>(static_cast<_Boost::uintmax_t>(_Boost::math::lltrunc(-2 * a1)));
         T smz = sqrt(-z);
         return pow(2 / smz, -n) * _Boost::math::hermite(n, 1 / smz);
      }
      ...
      if (is_a1_integer && is_a2_integer)
      {
         if ((a1 < 1) && (a2 <= a1))
         {
            const unsigned int n = static_cast<unsigned int>(static_cast<_Boost::uintmax_t>(_Boost::math::lltrunc(-a1)));
            const unsigned int m = static_cast<unsigned int>(static_cast<_Boost::uintmax_t>(_Boost::math::lltrunc(-a2 - n)));

            return (pow(z, T(n)) * _Boost::math::factorial<T>(n, pol)) *
               _Boost::math::laguerre(n, m, -(1 / z), pol);
         }
         else if ((a2 < 1) && (a1 <= a2))
         {
            // function is symmetric for a1 and a2
            const unsigned int n = static_cast<unsigned int>(static_cast<_Boost::uintmax_t>(_Boost::math::lltrunc(-a2)));
            const unsigned int m = static_cast<unsigned int>(static_cast<_Boost::uintmax_t>(_Boost::math::lltrunc(-a1 - n)));
...
inline typename tools::promote_args<T1, T2, T3>::type hypergeometric_2F0(T1 a1, T2 a2, T3 z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::hypergeometric_2F0_imp<value_type>(
         ...
         forwarding_policy()),
      "_Boost::math::hypergeometric_2F0<%1%>(%1%,%1%,%1%)");
}
