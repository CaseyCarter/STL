...
T inverse_students_t_hill(T ndf, T u, const Policy& pol)
{
   ...
   if (ndf > 1e20f)
      return -_Boost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
   ...
   if (y > (0.05f + a))
   {
      ...
      //
      x = -_Boost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
      y = x * x;
...
T inverse_students_t_tail_series(T df, T v, const Policy& pol)
{
   ...
   T w = _Boost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
T inverse_students_t_body_series(T df, T u, const Policy& pol)
{
   ...
   //
   T v = _Boost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
calculate_real:
      if(df > 0x10000000)
      {
         result = -_Boost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
         if((pexact) && (df >= 1e20))
            *pexact = true;
      }
      else if(df < 3)
      {
         ...
         if(u > crossover)
         {
            result = _Boost::math::detail::inverse_students_t_body_series(df, u, pol);
         }
         else
         {
            result = _Boost::math::detail::inverse_students_t_tail_series(df, u, pol);
         }
      }
      else
      {
         ...
         if(u > crossover)
         {
            result = _Boost::math::detail::inverse_students_t_hill(df, u, pol);
         }
         else
         {
            result = _Boost::math::detail::inverse_students_t_tail_series(df, u, pol);
...
inline T find_ibeta_inv_from_t_dist(T a, T p, T /*q*/, T* py, const Policy& pol)
{
   ...
   T v = 1 - u;
   T df = a * 2;
   T t = _Boost::math::detail::inverse_students_t(df, u, v, pol);
   *py = t * t / (df + t * t);
   return df / (df + t * t);
}

template <class T, class Policy>
inline T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const _Boost::false_type*)
{
   BOOST_MATH_STD_USING
   //
   ...
   if(df * y > tools::max_value<T>() * x)
      t = policies::raise_overflow_error<T>("_Boost::math::students_t_quantile<%1%>(%1%,%1%)", 0, pol);
   else
      t = sqrt(df * y / x);
   //
...
template <class T, class Policy>
T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const _Boost::true_type*)
{
   BOOST_MATH_STD_USING
   bool invert = false;
   if((df < 2) && (floor(df) != df))
      return _Boost::math::detail::fast_students_t_quantile_imp(df, p, pol, static_cast<_Boost::false_type*>(0));
   if(p > 0.5)
   {
      p = 1 - p;
      invert = true;
...
inline T fast_students_t_quantile(T df, T p, const Policy& pol)
{
   ...
   > tag_type;
   return policies::checked_narrowing_cast<T, forwarding_policy>(fast_students_t_quantile_imp(static_cast<value_type>(df), static_cast<value_type>(p), pol, static_cast<tag_type*>(0)), "_Boost::math::students_t_quantile<%1%>(%1%,%1%,%1%)");
}
