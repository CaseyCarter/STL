...
inline typename tools::promote_args<T>::type cos_pi(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(_Boost::math::detail::cos_pi_imp<value_type>(x, forwarding_policy()), "cos_pi");
}

template <class T>
inline typename tools::promote_args<T>::type cos_pi(T x)
{
   return _Boost::math::cos_pi(x, policies::policy<>());
}
