...
inline T bessel_j_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T prefix;
   if (v < _Boost::math::max_factorial<T>::value)
   {
      prefix = pow(x / 2, v - 1) / 2 / _Boost::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = (v - 1) * log(x / 2) - constants::ln_two<T>() - _Boost::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if (0 == prefix)
      return prefix;
...
#endif
   _Boost::math::policies::check_series_iterations<T>("_Boost::math::bessel_j_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T p = log(x / 2);
   T scale = 1;
   bool need_logs = (v >= _Boost::math::max_factorial<T>::value) || (_Boost::math::tools::log_max_value<T>() / v < fabs(p));
   if (!need_logs)
   {
      gam = _Boost::math::tgamma(v, pol);
      p = pow(x / 2, v + 1) * 2;
      if (_Boost::math::tools::max_value<T>() * p < gam)
      {
         scale /= gam;
         gam = 1;
         if (_Boost::math::tools::max_value<T>() * p < gam)
      ...
      prefix = -gam / (_Boost::math::constants::pi<T>() * p);
   }
   else
   {
      gam = _Boost::math::lgamma(v, pol);
      p = (v + 1) * p + constants::ln_two<T>();
      prefix = gam - log(_Boost::math::constants::pi<T>()) - p;
      if (_Boost::math::tools::log_max_value<T>() < prefix)
      {
         prefix -= log(_Boost::math::tools::max_value<T>() / 4);
         scale /= (_Boost::math::tools::max_value<T>() / 4);
         if (_Boost::math::tools::log_max_value<T>() < prefix)
         {
            return _Boost::math::policies::raise_overflow_error<T>(function, 0, pol);
         }
   ...
   T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   _Boost::math::policies::check_series_iterations<T>("_Boost::math::bessel_y_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   p = pow(x / 2, v - 1) / 2;
   if (!need_logs)
   {
      prefix = _Boost::math::tgamma(-v, pol) * _Boost::math::cos_pi(v) * p / _Boost::math::constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = _Boost::math::lgamma(-v, &sgn, pol) + (v - 1) * log(x / 2) - constants::ln_two<T>();
      prefix = exp(prefix) * sgn / _Boost::math::constants::pi<T>();
   }
   bessel_y_derivative_small_z_series_term_b<T, Policy> s2(v, x);
   max_iter = _Boost::math::policies::get_max_series_iterations<Policy>();
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T b = _Boost::math::tools::sum_series(s2, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T b = _Boost::math::tools::sum_series(s2, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
