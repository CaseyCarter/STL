...
struct zeta_series_cache_size
{
   ...
   // to zeta for example.
   //
   typedef typename _Boost::math::policies::precision<T,Policy>::type precision_type;
   typedef typename mpl::if_<
      mpl::less_equal<precision_type, _Boost::integral_constant<int, 0> >,
      _Boost::integral_constant<int, 5000>,
      typename mpl::if_<
         mpl::less_equal<precision_type, _Boost::integral_constant<int, 64> >,
         _Boost::integral_constant<int, 70>,
         typename mpl::if_<
            mpl::less_equal<precision_type, _Boost::integral_constant<int, 113> >,
            _Boost::integral_constant<int, 100>,
            _Boost::integral_constant<int, 5000>
         >::type
      >::type
   >::type type;
...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   return sum * 1 / -_Boost::math::powm1(T(2), sc);
}
...
inline T zeta_series2_imp(T s, const Policy& pol)
{
   ...
   T result = tools::sum_series(
      ...
      max_iter);
   policies::check_series_iterations<T>("_Boost::math::zeta_series2<%1%>(%1%)", max_iter, pol);
   return result;
}
#endif
...
T zeta_polynomial_series(T s, T sc, Policy const &)
{
   ...
   //
   BOOST_MATH_STD_USING
   int n = itrunc(T(log(_Boost::math::tools::epsilon<T>()) / -2));
   T sum = 0;
   T two_n = ldexp(T(1), n);
   int ej_sign = 1;
   for(int j = 0; j < n; ++j)
...
template <class T, class Policy>
T zeta_imp_prec(T s, T sc, const Policy& pol, const _Boost::integral_constant<int, 0>&)
{
   BOOST_MATH_STD_USING
   T result;
...
template <class T, class Policy>
inline T zeta_imp_prec(T s, T sc, const Policy&, const _Boost::integral_constant<int, 53>&)
{
   BOOST_MATH_STD_USING
   T result;
   if(s < 1)
...
template <class T, class Policy>
T zeta_imp_prec(T s, T sc, const Policy&, const _Boost::integral_constant<int, 64>&)
{
   BOOST_MATH_STD_USING
   T result;
   if(s < 1)
...
template <class T, class Policy>
T zeta_imp_prec(T s, T sc, const Policy&, const _Boost::integral_constant<int, 113>&)
{
   BOOST_MATH_STD_USING
   T result;
   if(s < 1)
...
{
   BOOST_MATH_STD_USING
   static const char* function = "_Boost::math::zeta<%1%>";
   if(sc == 0)
         ...
         if(v == s)
         {
            if(v < 0)
            {
               if(((-v) & 1) == 0)
                  return 0;
               int n = (-v + 1) / 2;
               if(n <= (int)_Boost::math::max_bernoulli_b2n<T>::value)
                  return T((-v & 1) ? -1 : 1) * _Boost::math::unchecked_bernoulli_b2n<T>(n) / (1 - v);
            }
            else if((v & 1) == 0)
            {
               if(((v / 2) <= (int)_Boost::math::max_bernoulli_b2n<T>::value) && (v <= (int)_Boost::math::max_factorial<T>::value))
                  return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                     _Boost::math::unchecked_bernoulli_b2n<T>(v / 2) / _Boost::math::unchecked_factorial<T>(v);
               return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                  _Boost::math::bernoulli_b2n<T>(v / 2) / _Boost::math::factorial<T>(v);
            }
            else
               return zeta_imp_odd_integer(v, sc, pol, _Boost::integral_constant<bool, (Tag::value <= 113) && Tag::value>());
         }
#ifndef BOOST_NO_EXCEPTIONS
      }
      catch(const _Boost::math::rounding_error&){} // Just fall through, s is too large to round
   ...
   else if(s < 0)
   {
      ...
      else
      {
         ...
         {
            T mult = _Boost::math::sin_pi(0.5f * sc, pol) * 2 * zeta_imp(s, sc, pol, tag);
            result = _Boost::math::lgamma(s, pol);
            ...
            if(tools::max_value<T>() / fabs(mult) < result)
               return _Boost::math::sign(mult) * policies::raise_overflow_error<T>(function, 0, pol);
            result *= mult;
         }
         else
         {
            result = _Boost::math::sin_pi(0.5f * sc, pol)
               * 2 * pow(2 * constants::pi<T>(), -s) 
               * _Boost::math::tgamma(s, pol) 
...
struct zeta_initializer
{
   struct init
   {
      ...
      static void do_init(const _Boost::integral_constant<int, 0>&){ _Boost::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const _Boost::integral_constant<int, 53>&){ _Boost::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const _Boost::integral_constant<int, 64>&)
      {
         _Boost::math::zeta(static_cast<T>(0.5), Policy());
         _Boost::math::zeta(static_cast<T>(1.5), Policy());
         _Boost::math::zeta(static_cast<T>(3.5), Policy());
         _Boost::math::zeta(static_cast<T>(6.5), Policy());
         _Boost::math::zeta(static_cast<T>(14.5), Policy());
         _Boost::math::zeta(static_cast<T>(40.5), Policy());

         _Boost::math::zeta(static_cast<T>(5), Policy());
      }
      static void do_init(const _Boost::integral_constant<int, 113>&)
      {
         ...
         _Boost::math::zeta(static_cast<T>(1.5), Policy());
         _Boost::math::zeta(static_cast<T>(3.5), Policy());
         _Boost::math::zeta(static_cast<T>(5.5), Policy());
         _Boost::math::zeta(static_cast<T>(9.5), Policy());
         _Boost::math::zeta(static_cast<T>(16.5), Policy());
         _Boost::math::zeta(static_cast<T>(25.5), Policy());
         _Boost::math::zeta(static_cast<T>(70.5), Policy());
