...
T erf_inv_imp(const T& p, const T& q, const Policy&, const _Boost::integral_constant<int, 64>*)
{
...
struct erf_roots
{
   _Boost::math::tuple<T,T,T> operator()(const T& guess)
   {
      BOOST_MATH_STD_USING
      T derivative = sign * (2 / sqrt(constants::pi<T>())) * exp(-(guess * guess));
      T derivative2 = -2 * guess * derivative;
      return _Boost::math::make_tuple(((sign > 0) ? static_cast<T>(_Boost::math::erf(guess, Policy()) - target) : static_cast<T>(_Boost::math::erfc(guess, Policy())) - target), derivative, derivative2);
   }
...
template <class T, class Policy>
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const _Boost::integral_constant<int, 0>*)
{
   //
   // Generic version, get a guess that's accurate to 64-bits (10^-19)
   //
   T guess = erf_inv_imp(p, q, pol, static_cast<_Boost::integral_constant<int, 64> const*>(0));
   T result;
   //
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      policies::check_root_iterations<T>("_Boost::math::erf_inv<%1%>", max_iter, pol);
   }
   else
   {
      result = guess;
...
struct erf_inv_initializer
{
   struct init
   {
      ...
      static void do_init()
      {
         ...
         {
            _Boost::math::erf_inv(static_cast<T>(0.25), Policy());
            _Boost::math::erf_inv(static_cast<T>(0.55), Policy());
            _Boost::math::erf_inv(static_cast<T>(0.95), Policy());
            _Boost::math::erfc_inv(static_cast<T>(1e-15), Policy());
            ...
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130))))
               _Boost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130)), Policy());
            ...
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800))))
               _Boost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800)), Policy());
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900))))
               _Boost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900)), Policy());
#else
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800))))
               _Boost::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800)), Policy());
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900))))
               _Boost::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900)), Policy());
#endif
         }
      }
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   static const char* function = "_Boost::math::erfc_inv<%1%>(%1%, %1%)";
   if((z < 0) || (z > 2))
...
typename tools::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   ...
   // Begin by testing for domain errors, and other special cases:
   //
   static const char* function = "_Boost::math::erf_inv<%1%>(%1%, %1%)";
   if((z < -1) || (z > 1))
