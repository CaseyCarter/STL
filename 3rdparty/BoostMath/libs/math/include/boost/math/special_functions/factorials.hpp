...
{
   BOOST_STATIC_ASSERT(!_Boost::is_integral<T>::value);
   ...
   T result = _Boost::math::tgamma(static_cast<T>(i+1), pol);
...
T double_factorial(unsigned i, const Policy& pol)
{
   ...
   if(i & 1)
   {
      ...
      //
      T result = _Boost::math::tgamma(static_cast<T>(i) / 2 + 1, pol) / sqrt(constants::pi<T>());
   ...
   //
   return policies::raise_overflow_error<T>("_Boost::math::double_factorial<%1%>(unsigned)", 0, pol);
}
...
T rising_factorial_imp(T x, int n, const Policy& pol)
{
   ...
   {
      if(n < 0)
         return -_Boost::math::tgamma_delta_ratio(x + 1, static_cast<T>(-n), pol);
      else
         return 0;
   }
   if((x < 1) && (x + n < 0))
   {
      T val = _Boost::math::tgamma_delta_ratio(1 - x, static_cast<T>(-n), pol);
   ...
   // use case:
   //
   return 1 / _Boost::math::tgamma_delta_ratio(x, static_cast<T>(n), pol);
}
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x < 0.5f)
   {
      ...
      if(n > max_factorial<T>::value - 2)
      {
         ...
         T t1 = x * _Boost::math::falling_factorial(x - 1, max_factorial<T>::value - 2);
         T t2 = _Boost::math::falling_factorial(x - max_factorial<T>::value + 1, n - max_factorial<T>::value + 1);
         if(tools::max_value<T>() / fabs(t1) < fabs(t2))
            return _Boost::math::sign(t1) * _Boost::math::sign(t2) * policies::raise_overflow_error<T>("_Boost::math::falling_factorial<%1%>", 0, pol);
         return t1 * t2;
      }
      return x * _Boost::math::falling_factorial(x - 1, n - 1);
   }
   if(x <= n - 1)
   {
      //
      ...
      if(n2 == xp1)
         return 0;
      T result = _Boost::math::tgamma_delta_ratio(xp1, -static_cast<T>(n2), pol);
      x -= n2;
      result *= x;
      ++n2;
      if(n2 < n)
