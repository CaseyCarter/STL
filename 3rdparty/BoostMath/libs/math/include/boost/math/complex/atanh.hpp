...
std::complex<T> atanh(const std::complex<T>& z)
{
   ...
   static const T log_two = _Boost::math::constants::ln_two<T>();
...
#pragma warning(disable:4127)
   ...
   //
   if((_Boost::math::isnan)(x))
   {
      if((_Boost::math::isnan)(y))
         return std::complex<T>(x, x);
      else if((_Boost::math::isinf)(y))
         return std::complex<T>(0, ((_Boost::math::signbit)(z.imag()) ? -half_pi : half_pi));
      else
         return std::complex<T>(x, x);
   }
   else if((_Boost::math::isnan)(y))
   {
      if(x == 0)
   ...
   else if((x > safe_lower) && (x < safe_upper) && (y > safe_lower) && (y < safe_upper))
   {
      ...
      // 
      real = _Boost::math::log1p(four * x / (mxm1*mxm1 + yy));
      real /= four;
      if((_Boost::math::signbit)(z.real()))
         real = (_Boost::math::changesign)(real);

      imag = std::atan2((y * two), (mxm1*(one+x) - yy));
      imag /= two;
      if(z.imag() < 0)
         imag = (_Boost::math::changesign)(imag);
   }
   else
   {
      //
      ...
      //    real = _Boost::math::log1p(4x / ((x-1)^2 + y^2))
      ...
      if(x >= safe_upper)
      {
         ...
         if((_Boost::math::isinf)(x) || (_Boost::math::isinf)(y))
         {
            real = 0;
         }
         else if(y >= safe_upper)
         {
            ...
            real = _Boost::math::log1p((four/y) / (x/y + y/x));
         }
         else if(y > one)
         {
            // Big x: divide through by x:
            real = _Boost::math::log1p(four / (x + y*y/x));
         }
         else
         {
            // Big x small y, as above but neglect y^2/x:
            real = _Boost::math::log1p(four/x);
         }
      }
      else if(y >= safe_upper)
      {
         if(x > one)
         {
            ...
            real = _Boost::math::log1p((four*x/y) / (y + mxm1*mxm1/y));
         }
         else
         {
      ...
      else if (x != one)
      {
         ...
         if(y > safe_lower)
            div += y*y;
         real = _Boost::math::log1p(four*x/div);
      }
      else
         real = _Boost::math::changesign(two * (std::log(y) - log_two));
