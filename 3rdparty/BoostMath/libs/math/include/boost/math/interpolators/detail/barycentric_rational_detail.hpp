...
barycentric_rational_imp<Real>::barycentric_rational_imp(InputIterator1 start_x, InputIterator1 end_x, InputIterator2 start_y, size_t approximation_order)
{
    ...
    for(unsigned i = 0; start_x != end_x; ++start_x, ++start_y, ++i)
    {
        ...
        if(_Boost::math::isnan(*start_x))
        {
            std::string msg = std::string("x[") + _Boost::lexical_cast<std::string>(i) + "] is a NAN";
            throw std::domain_error(msg);
        }

        if(_Boost::math::isnan(*start_y))
        {
           std::string msg = std::string("y[") + _Boost::lexical_cast<std::string>(i) + "] is a NAN";
           throw std::domain_error(msg);
        }
            ...
            for(int64_t j = i; j <= j_max; ++j)
            {
                ...
                if (abs(diff) < (numeric_limits<Real>::min)())
                {
                   std::string msg = std::string("Spacing between  x[")
                      + _Boost::lexical_cast<std::string>(k) + std::string("] and x[")
                      + _Boost::lexical_cast<std::string>(i) + std::string("] is ")
                      + _Boost::lexical_cast<std::string>(diff) + std::string(", which is smaller than the epsilon of ")
                      + _Boost::core::demangle(typeid(Real).name());
