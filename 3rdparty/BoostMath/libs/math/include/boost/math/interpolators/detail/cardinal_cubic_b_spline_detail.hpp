...
namespace _Boost{ namespace math{ namespace interpolators{ namespace detail{
...
Real b3_spline(Real x)
{
    ...
    if (absx < 1)
    {
        Real y = 2 - absx;
        Real z = 1 - absx;
        return _Boost::math::constants::sixth<Real>()*(y*y*y - 4*z*z*z);
    }
    if (absx < 2)
    {
        Real y = 2 - absx;
        return _Boost::math::constants::sixth<Real>()*y*y*y;
    }
    return (Real) 0;
}
...
Real b3_spline_prime(Real x)
{
    ...
    if (x < 1)
    {
        return x*(3*_Boost::math::constants::half<Real>()*x - 2);
    }
    if (x < 2)
    {
        return -_Boost::math::constants::half<Real>()*(2 - x)*(2 - x);
    }
    return (Real) 0;
}
                                             ...
                                             Real left_endpoint_derivative, Real right_endpoint_derivative) : m_a(left_endpoint), m_avg(0)
{
    ...
    if (length < 5)
    {
        if (_Boost::math::isnan(left_endpoint_derivative) || _Boost::math::isnan(right_endpoint_derivative))
        {
    ...
    if (_Boost::math::isnan(left_endpoint))
    {
...
Real cardinal_cubic_b_spline_imp<Real>::operator()(Real x) const
{
    ...
    size_t k_min = (size_t) (max)(static_cast<long>(0), _Boost::math::ltrunc(ceil(t - 2)));
    size_t k_max = (size_t) (max)((min)(static_cast<long>(m_beta.size() - 1), _Boost::math::ltrunc(floor(t + 2))), (long) 0);
...
Real cardinal_cubic_b_spline_imp<Real>::prime(Real x) const
{
    ...
    size_t k_min = (size_t) (max)(static_cast<long>(0), _Boost::math::ltrunc(ceil(t - 2)));
    size_t k_max = (size_t) (min)(static_cast<long>(m_beta.size() - 1), _Boost::math::ltrunc(floor(t + 2)));
