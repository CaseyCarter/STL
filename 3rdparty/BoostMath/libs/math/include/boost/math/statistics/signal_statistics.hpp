...
namespace _Boost::math::statistics {
...
{
    return _Boost::math::statistics::absolute_gini_coefficient(v.begin(), v.end());
}
...
auto sample_absolute_gini_coefficient(ForwardIterator first, ForwardIterator last)
{
    ...
    return n*_Boost::math::statistics::absolute_gini_coefficient(first, last)/(n-1);
}

template<class RandomAccessContainer>
inline auto sample_absolute_gini_coefficient(RandomAccessContainer & v)
{
    return _Boost::math::statistics::sample_absolute_gini_coefficient(v.begin(), v.end());
}
...
{
    return _Boost::math::statistics::hoyer_sparsity(v.cbegin(), v.cend());
}
...
auto oracle_snr(Container const & signal, Container const & noisy_signal)
{
    ...
    else if constexpr (_Boost::math::tools::is_complex_type<Real>::value)
...
auto mean_invariant_oracle_snr(Container const & signal, Container const & noisy_signal)
{
    ...
    Real mu = _Boost::math::statistics::mean(signal);
    Real numerator = 0;
    Real denominator = 0;
    for (size_t i = 0; i < signal.size(); ++i)
    {
...
{
    using std::log10;
    return 10*log10(_Boost::math::statistics::mean_invariant_oracle_snr(signal, noisy_signal));
}
...
{
    using std::log10;
    return 10*log10(_Boost::math::statistics::oracle_snr(signal, noisy_signal));
}
...
auto m2m4_snr_estimator(ForwardIterator first, ForwardIterator last, decltype(*first) estimated_signal_kurtosis=1, decltype(*first) estimated_noise_kurtosis=3)
{
    ...
    if constexpr (std::is_floating_point<Real>::value || std::numeric_limits<Real>::max_exponent)
    {
        ...
        auto [M1, M2, M3, M4] = _Boost::math::statistics::first_four_moments(first, last);
        if (M4 == 0)
        {
        ...
        Real bn = 2*M2*(3-ka);
        Real cn = ka*M2*M2 - M4;
        auto [S0, S1] = _Boost::math::tools::quadratic_roots(a, bs, cs);
        if (S1 > 0)
        {
            auto N = M2 - S1;
            if (N > 0)
        ...
        auto [N0, N1] = _Boost::math::tools::quadratic_roots(a, bn, cn);
        if (N1 > 0)
        {
            auto S = M2 - N1;
            if (S > 0)
