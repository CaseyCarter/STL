...
auto ljung_box(RandomAccessIterator begin, RandomAccessIterator end, int64_t lags = -1, int64_t fit_dof = 0) {
    ...
    auto mu = _Boost::math::statistics::mean(begin, end);
    ...
    auto chi = _Boost::math::chi_squared_distribution<Real, no_promote_policy>(Real(lags - fit_dof));

    Real pvalue = 1 - _Boost::math::cdf(chi, Q);
