...
public:
   ...
   inverse_chi_squared_distribution(RealType df, RealType l_scale) : m_df(df), m_scale (l_scale)
   {
      RealType result;
      detail::check_df(
         "_Boost::math::inverse_chi_squared_distribution<%1%>::inverse_chi_squared_distribution",
         m_df, &result, Policy())
         && detail::check_scale(
"_Boost::math::inverse_chi_squared_distribution<%1%>::inverse_chi_squared_distribution",
...
RealType pdf(const inverse_chi_squared_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "_Boost::math::pdf(const inverse_chi_squared_distribution<%1%>&, %1%)";
...
{
   static const char* function = "_Boost::math::cdf(const inverse_chi_squared_distribution<%1%>&, %1%)";
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   RealType error_result;
   ...
   // RealType shape = df /2; // inv_gamma shape,
   // RealType scale = df * scale/2; // inv_gamma scale,
   // result = _Boost::math::gamma_q(shape, scale / x, Policy()); // inverse_gamma code.
   return _Boost::math::gamma_q(df / 2, (df * (scale / 2)) / x, Policy());
} // cdf
...
inline RealType quantile(const inverse_chi_squared_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   static const char* function = "_Boost::math::quantile(const inverse_chi_squared_distribution<%1%>&, %1%)";
   // Error check:
   RealType error_result;
...
{ // Mean of inverse Chi-Squared distribution.
   ...
   static const char* function = "_Boost::math::mean(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 2)
      return policies::raise_domain_error<RealType>(
         function,
...
{ // Variance of inverse Chi-Squared distribution.
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   static const char* function = "_Boost::math::variance(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 4)
   {
      return policies::raise_domain_error<RealType>(
         function,
...
{ // mode is not defined in Mathematica.
  ...
  // for origin of the formula used below.
   ...
   RealType df = dist.degrees_of_freedom();
   RealType scale = dist.scale();
   static const char* function = "_Boost::math::mode(const inverse_chi_squared_distribution<%1%>&)";
   if(df < 0)
      return policies::raise_domain_error<RealType>(
         function,
...
inline RealType skewness(const inverse_chi_squared_distribution<RealType, Policy>& dist)
{
   ...
   RealType df = dist.degrees_of_freedom();
   static const char* function = "_Boost::math::skewness(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 6)
      return policies::raise_domain_error<RealType>(
         function,
...
{
   RealType df = dist.degrees_of_freedom();
   static const char* function = "_Boost::math::kurtosis(const inverse_chi_squared_distribution<%1%>&)";
   if(df <= 8)
      return policies::raise_domain_error<RealType>(
         function,
