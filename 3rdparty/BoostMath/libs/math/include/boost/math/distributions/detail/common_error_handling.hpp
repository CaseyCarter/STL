...
{
   if((prob < 0) || (prob > 1) || !(_Boost::math::isfinite)(prob))
   {
...
{ //  df > 0 but NOT +infinity allowed.
   if((df <= 0) || !(_Boost::math::isfinite)(df))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_scale(
      ...
      const Policy& pol)
{
   if((scale <= 0) || !(_Boost::math::isfinite)(scale))
...
inline bool check_non_centrality(
      ...
      const Policy& pol)
{
   if((ncp < 0) || !(_Boost::math::isfinite)(ncp))
   { // Assume scale == 0 is NOT valid for any distribution.
      *result = policies::raise_domain_error<RealType>(
         function,
