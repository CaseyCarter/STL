...
namespace _Boost
{
  namespace math
  {
    namespace poisson_detail
    {
      ...
      {
        if(!(_Boost::math::isfinite)(mean) || (mean < 0))
        {
    ...
    public:
      ...
      { // Expected mean number of events that occur during the given interval.
        ...
        poisson_detail::check_dist(
           "_Boost::math::poisson_distribution<%1%>::poisson_distribution",
          m_l,
          &r, Policy());
    ...
    { // Probability Density/Mass Function.
      ...
      if(false == poisson_detail::check_dist_and_k(
        "_Boost::math::pdf(const poisson_distribution<%1%>&, %1%)",
        mean,
        k,
        &result, Policy()))
      {
      ...
      return _Boost::math::gamma_p_derivative(k+1, mean, Policy());
    } // pdf
    ...
    { // Cumulative Distribution Function Poisson.
      ...
      RealType result = 0;
      if(false == poisson_detail::check_dist_and_k(
        "_Boost::math::cdf(const poisson_distribution<%1%>&, %1%)",
        mean,
        k,
        &result, Policy()))
      {
    ...
    { // Complemented Cumulative Distribution Function Poisson
      ...
      { // Avoid repeated checks on k and mean in gamma_p.
         return -_Boost::math::expm1(-mean, Policy());
      }
    ...
    { // Quantile (or Percent Point) Poisson function.
      ...
      static const char* function = "_Boost::math::quantile(const poisson_distribution<%1%>&, %1%)";
      ...
      if(z < 1)
         ...
         guess = _Boost::math::detail::inverse_poisson_cornish_fisher(z, p, RealType(1-p), Policy());
      if(z > 5)
      {
         if(z > 1000)
            factor = 1.01f;
    ...
    { // Quantile (or Percent Point) of Poisson function.
      ...
      //
      // Error checks:
      static const char* function = "_Boost::math::quantile(complement(const poisson_distribution<%1%>&, %1%))";
      RealType q = c.param;
      ...
      if(z < 1)
         ...
         guess = _Boost::math::detail::inverse_poisson_cornish_fisher(z, RealType(1-q), q, Policy());
      if(z > 5)
      {
         if(z > 1000)
            factor = 1.01f;
