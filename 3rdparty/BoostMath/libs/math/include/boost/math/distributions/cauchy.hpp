...
RealType cdf_imp(const cauchy_distribution<RealType, Policy>& dist, const RealType& x, bool complement)
{
   ...
   static const char* function = "_Boost::math::cdf(cauchy<%1%>&, %1%)";
   RealType result = 0;
      ...
      bool complement)
{
   ...
   //
   static const char* function = "_Boost::math::quantile(cauchy<%1%>&, %1%)";
   BOOST_MATH_STD_USING // for ADL of std functions
...
public:
   ...
   {
    static const char* function = "_Boost::math::cauchy_distribution<%1%>::cauchy_distribution";
     RealType result;
...
{  
   ...
   static const char* function = "_Boost::math::pdf(cauchy<%1%>&, %1%)";
   RealType result = 0;
   RealType location = dist.location();
   RealType scale = dist.scale();
   if(false == detail::check_scale("_Boost::math::pdf(cauchy<%1%>&, %1%)", scale, &result, Policy()))
   {
      return result;
   }
   if(false == detail::check_location("_Boost::math::pdf(cauchy<%1%>&, %1%)", location, &result, Policy()))
   {
      return result;
   }
...
inline RealType kurtosis_excess(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "_Boost::math::kurtosis_excess(cauchy<%1%>&)",
      "The Cauchy distribution does not have a kurtosis: "
      "the only possible return value is %1%.",
      std::numeric_limits<RealType>::quiet_NaN(), Policy());
}
