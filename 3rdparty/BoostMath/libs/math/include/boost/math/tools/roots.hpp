...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, _Boost::uintmax_t& max_iter, const Policy& pol) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<Policy>::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   T fmin = f(min);
   T fmax = f(max);
   if (fmin == 0)
   ...
   // Error checking:
   //
   static const char* function = "_Boost::math::tools::bisect<%1%>";
   if (min >= max)
   {
      return _Boost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "Arguments in wrong order in _Boost::math::tools::bisect (first arg=%1%)", min, pol));
   }
   if (fmin * fmax >= 0)
   {
      return _Boost::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "No change of sign in _Boost::math::tools::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol));
   }

   //
   // Three function invocations so far:
   //
   _Boost::uintmax_t count = max_iter;
   if (count < 3)
      count = 0;
   else
      count -= 3;
   ...
   static _Boost::uintmax_t max_count = 0;
...
inline std::pair<T, T> bisect(F f, T min, T max, Tol tol, _Boost::uintmax_t& max_iter)  BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
...
{
   _Boost::uintmax_t m = (std::numeric_limits<_Boost::uintmax_t>::max)();
   return bisect(f, min, max, tol, m, policies::policy<>());
}


template <class F, class T>
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, _Boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   BOOST_MATH_STD_USING

   static const char* function = "_Boost::math::tools::newton_raphson_iterate<%1%>";
   if (min >= max)
   {
      return policies::raise_evaluation_error(function, "Range arguments in wrong order in _Boost::math::tools::newton_raphson_iterate(first arg=%1%)", min, _Boost::math::policies::policy<>());
   }
   ...
   do {
      ...
      {
         return policies::raise_evaluation_error(function, "There appears to be no root to be found in _Boost::math::tools::newton_raphson_iterate, perhaps we have a local minima near current best guess of %1%", guess, _Boost::math::policies::policy<>());
      }
...
namespace detail {
   ...
   template <class F, class T>
   T bracket_root_towards_min(F f, T guess, const T& f0, T& min, T& max, _Boost::uintmax_t& count) BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())));

   template <class F, class T>
   T bracket_root_towards_max(F f, T guess, const T& f0, T& min, T& max, _Boost::uintmax_t& count) BOOST_NOEXCEPT_IF(BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
   {
      using std::fabs;
      //
   ...
   T second_order_root_finder(F f, T guess, T min, T max, int digits, _Boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
   {
      BOOST_MATH_STD_USING
        ...
        << ", digits = " << digits << ", max_iter = " << max_iter << std::endl;
#endif
      static const char* function = "_Boost::math::tools::halley_iterate<%1%>";
      if (min >= max)
      {
         return policies::raise_evaluation_error(function, "Range arguments in wrong order in _Boost::math::tools::halley_iterate(first arg=%1%)", min, _Boost::math::policies::policy<>());
      }
...
template <class F, class T>
T halley_iterate(F f, T guess, T min, T max, int digits, _Boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
...
template <class F, class T>
T schroder_iterate(F f, T guess, T min, T max, int digits, _Boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
...
//
template <class F, class T>
T schroeder_iterate(F f, T guess, T min, T max, int digits, _Boost::uintmax_t& max_iter) BOOST_NOEXCEPT_IF(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   return detail::second_order_root_finder<detail::schroder_stepper>(f, guess, min, max, digits, max_iter);
}
...
std::pair<T, T> quadratic_roots_imp(T const& a, T const& b, T const& c)
{
   ...
   else if constexpr (_Boost::math::tools::is_complex_type<T>::value)
   {
