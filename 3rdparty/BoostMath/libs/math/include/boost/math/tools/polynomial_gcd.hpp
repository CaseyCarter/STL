...
namespace _Boost{
   ...
   namespace integer {
      ...
      namespace gcd_detail {
         ...
         template <class T>
         struct gcd_traits<_Boost::math::tools::polynomial<T> >
         {
            inline static const _Boost::math::tools::polynomial<T>& abs(const _Boost::math::tools::polynomial<T>& val) { return val; }
...
{
    return x ? _Boost::integer::gcd_range(x.data().begin(), x.data().end()).first : T(0);
}
...
gcd(polynomial<T> const &u, polynomial<T> const &v)
{
   return _Boost::integer::gcd_detail::Euclid_gcd(u, v);
}
