...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(_Boost::math::sign(fa) * _Boost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(_Boost::math::sign(A) * _Boost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, _Boost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "_Boost::math::tools::toms748_solve<%1%>";
   ...
   b = bx;
   if(a >= b)
      return _Boost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
   ...
   if(_Boost::math::sign(fa) * _Boost::math::sign(fb) > 0)
      return _Boost::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
...
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, _Boost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, _Boost::uintmax_t& max_iter, const Policy& pol)
{
   if (max_iter <= 2)
      return std::make_pair(ax, bx);
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, _Boost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, _Boost::uintmax_t& max_iter, const Policy& pol)
{
   BOOST_MATH_STD_USING
   static const char* function = "_Boost::math::tools::bracket_and_solve_root<%1%>";
   //
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      {
         if(count == 0)
            return _Boost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", b, pol));
         //
   ...
   else
   {
      ...
      while((_Boost::math::sign)(fb) == (_Boost::math::sign)(fa))
      {
         ...
         if(count == 0)
            return _Boost::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", a, pol));
         //
         // Heuristic: normally it's best not to increase the step sizes as we'll just end up
         // with a really wide range to search for the root.  However, if the initial guess was *really*
         // bad then we need to speed up the search otherwise we'll take forever if we're orders of
...
template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, _Boost::uintmax_t& max_iter)
{
