...
auto sup_norm(ForwardIterator first, ForwardIterator last)
{
    ...
    using T = typename std::iterator_traits<ForwardIterator>::value_type;
    using std::abs;
    if constexpr (_Boost::math::tools::is_complex_type<T>::value)
    {
...
auto lp_norm(ForwardIterator first, ForwardIterator last, unsigned p)
{
    ...
    using std::isfinite;
    using RealOrComplex = typename std::iterator_traits<ForwardIterator>::value_type;
    if constexpr (_Boost::math::tools::is_complex_type<RealOrComplex>::value)
    {
        using std::norm;
        ...
        if (!isfinite(result))
        {
            auto a = _Boost::math::tools::sup_norm(first, last);
            Real lp = 0;
            for (auto it = first; it != last; ++it)
            {
                lp += pow(abs(*it)/a, p);
    ...
    else if constexpr (is_floating_point<RealOrComplex>::value || std::numeric_limits<RealOrComplex>::max_exponent)
    {
        ...
        if (!isfinite(result))
        {
            RealOrComplex a = _Boost::math::tools::sup_norm(first, last);
            lp = 0;
            for (auto it = first; it != last; ++it)
            {
                lp += pow(abs(*it)/a, p);
    ...
    else
    {
        ...
        if (!isfinite(result))
        {
            double a = _Boost::math::tools::sup_norm(first, last);
            lp = 0;
            for (auto it = first; it != last; ++it)
            {
                double tmp = *it;
