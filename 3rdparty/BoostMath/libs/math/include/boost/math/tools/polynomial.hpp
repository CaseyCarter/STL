...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   if(r & 1)
      result = -result;
   result /= n - r;
   result *= _Boost::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
public:
   ...
   template <class U>
   explicit polynomial(const U& point, typename _Boost::enable_if<_Boost::is_convertible<U, T> >::type* = 0)
   {
       if (point != U(0))
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data[i] = _Boost::math::tools::real_cast<T>(p[i]);
      }
   }
#ifdef BOOST_MATH_HAS_IS_CONST_ITERABLE
    template <class Range>
    explicit polynomial(const Range& r, typename _Boost::enable_if<_Boost::math::tools::detail::is_const_iterable<Range> >::type* = 0) 
   ...
   {
      return m_data.size() > 0 ? _Boost::math::tools::evaluate_polynomial(&m_data[0], z, m_data.size()) : T(0);
   }
...
#pragma warning(pop)
   ...
   template <class U>
   typename _Boost::enable_if_c<_Boost::is_constructible<T, U>::value, polynomial&>::type operator +=(const U& value)
   {
       addition(value);
       normalize();
       return *this;
...
template <class T, class U>
inline typename _Boost::enable_if_c<_Boost::is_constructible<T, U>::value, polynomial<T> >::type operator + (polynomial<T> a, const U& b)
{
   a += b;
   return a;
}
...
template <class T, class U>
inline typename _Boost::enable_if_c<_Boost::is_constructible<T, U>::value, polynomial<T> >::type operator % (const polynomial<T>&, const U&)
{
...
inline typename _Boost::enable_if_c<_Boost::is_constructible<T, U>::value, polynomial<T> >::type operator + (const U& a, polynomial<T> b)
{
   b += a;
   return b;
}
