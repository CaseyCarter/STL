...
void do_test_cyl_bessel_j(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::cyl_bessel_j<value_type, value_type>;
#else
   pg funcp = _Boost::math::cyl_bessel_j;
#endif
...
T cyl_bessel_j_int_wrapper(T v, T x)
{
   ...
   return static_cast<T>(BESSEL_JN_FUNCTION_TO_TEST(_Boost::math::itrunc(v), x));
#else
   return static_cast<T>(_Boost::math::cyl_bessel_j(_Boost::math::itrunc(v), x));
#endif
}
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   pg funcp = _Boost::math::sph_bessel<value_type>;
#else
   pg funcp = _Boost::math::sph_bessel;
#endif
...
void test_bessel(T, const char* name)
{
   ...
   // 
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 6> j0_tricky = {{
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 5> j1_tricky = {{
        // Big numbers make the accuracy of std::sin the limiting factor:
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 20> jv_data = {{
...
#include "sph_bessel_data.ipp"
    ...
    // Some special cases:
    //
    BOOST_CHECK_EQUAL(_Boost::math::sph_bessel(0, T(0)), T(1));
    BOOST_CHECK_EQUAL(_Boost::math::sph_bessel(1, T(0)), T(0));
    BOOST_CHECK_EQUAL(_Boost::math::sph_bessel(100000, T(0)), T(0));

    //
    // Special cases that are errors:
    //
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j(T(-2.5), T(0)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j(T(-2.5), T(-2)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j(T(2.5), T(-2)), std::domain_error);
}
