...
using _Boost::multiprecision::float128;
#endif
...
{
    std::cout << "Testing Daubechies filters with " << p << " vanishing moments on type " << _Boost::core::demangle(typeid(Real).name()) << "\n";
    Real tol = 3*std::numeric_limits<Real>::epsilon();
    using _Boost::math::filters::daubechies_scaling_filter;
    using _Boost::math::filters::daubechies_wavelet_filter;
...
void test_agreement_with_ten_lectures()
{
    ...
    auto h2_ = _Boost::math::filters::daubechies_scaling_filter<double, 2>();
    ...
    auto h3_ = _Boost::math::filters::daubechies_scaling_filter<double, 3>();
    ...
    auto h4_ = _Boost::math::filters::daubechies_scaling_filter<double, 4>();
...
void test_filter_ulp_distance()
{
    std::cout << "Testing filters ULP distance between types "
              << _Boost::core::demangle(typeid(Real1).name()) << "and"
              << _Boost::core::demangle(typeid(Real2).name()) << "\n";
    using _Boost::math::filters::daubechies_scaling_filter;
...
{
    std::cout << "Testing integer grid with " << p << " vanishing moments and " << order << " derivative on type " << _Boost::core::demangle(typeid(Real).name()) << "\n";
    using _Boost::math::detail::daubechies_scaling_integer_grid;
    using _Boost::math::tools::summation_condition_number;
...
{
    std::cout << "Testing dyadic grid on type " << _Boost::core::demangle(typeid(Real).name()) << "\n";
    auto f = [&](auto i)
    {
        auto phijk = _Boost::math::daubechies_scaling_dyadic_grid<Real, i+2, 0>(0);
        auto phik = _Boost::math::detail::daubechies_scaling_integer_grid<Real, i+2, 0>();
        ...
        {
            phijk = _Boost::math::daubechies_scaling_dyadic_grid<Real, i+2, 0>(j);
            phik = _Boost::math::detail::daubechies_scaling_integer_grid<Real, i+2, 0>();
            ...
            {
                auto cond = _Boost::math::tools::summation_condition_number<Real>(0);
                for (uint64_t l = 0; l < static_cast<uint64_t>(phik.size()); ++l)
                {
    ...
    _Boost::hana::for_each(std::make_index_sequence<18>(), f);
}
...
{
    auto phi1_3 = _Boost::math::detail::daubechies_scaling_integer_grid<long double, 3, 1>();
    ...
    auto phi1_4 = _Boost::math::detail::daubechies_scaling_integer_grid<long double, 4, 1>();
    ...
    auto phi1_5 = _Boost::math::detail::daubechies_scaling_integer_grid<long double, 5, 1>();
...
{
    std::cout << "Testing " << p << " vanishing moment scaling function quadratures on type " << _Boost::core::demangle(typeid(Real).name()) << "\n";
    ...
    if constexpr (p == 2)
    {
        ...
        auto phi = _Boost::math::daubechies_scaling<Real, p>();
...
int main()
{
    _Boost::hana::for_each(std::make_index_sequence<18>(), [&](auto i){
    ...
    _Boost::hana::for_each(std::make_index_sequence<19>(), [&](auto i){
    ...
    _Boost::hana::for_each(std::make_index_sequence<18>(), [&](auto idx){
    ...
    return _Boost::math::test::report_errors();
}
