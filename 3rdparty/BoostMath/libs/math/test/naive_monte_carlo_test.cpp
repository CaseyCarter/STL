...
using _Boost::math::quadrature::naive_monte_carlo;


template<class Real>
void test_pi_multithreaded()
{
    std::cout << "Testing pi is calculated correctly (multithreaded) using Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
...
void test_pi()
{
    std::cout << "Testing pi is calculated correctly using Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
        Real r = x[0]*x[0]+x[1]*x[1];
        if (r <= 1)
...
void test_constant()
{
    std::cout << "Testing constants are integrated correctly using Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
...
{
    std::cout << "Testing that a reasonable action is performed by the Monte-Carlo integrator when the integrand throws an exception on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
...
{
    std::cout << "Testing that cancellation and restarting works on naive Monte-Carlo integration on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real exact = _Boost::lexical_cast<Real>("1.3932039296856768591842462603255");
...
{
    std::cout << "Testing that finite singular boundaries work on naive Monte-Carlo integration on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    using std::pow;
    using std::log;
    auto g = [](std::vector<Real> const & x)->Real
    {
...
{
    std::cout << "Testing that variance computed by naive Monte-Carlo integration converges to integral formula on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
...
void test_product()
{
    std::cout << "Testing that product functions are integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [&](std::vector<Real> const & x)->Real
    {
        double y = 1;
...
{
    std::cout << "Testing that alternative RNGs work correctly using naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [&](std::vector<Real> const & x)->Real
    {
        double y = 1;
        for (uint64_t i = 0; i < x.size(); ++i)
...
{
    std::cout << "Testing that infinite upper bounds are integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
    ...
    auto task = mc.integrate();
    Real y = task.get();
    BOOST_CHECK_CLOSE_FRACTION(y, _Boost::math::constants::half_pi<Real>(), 0.01);
}

template<class Real>
void test_lower_bound_infinite()
{
    std::cout << "Testing that infinite lower bounds are integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
        return 1.0/(x[0]*x[0] + 1.0);
    };
...
{
    std::cout << "Testing that infinite lower bounds (2) are integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
    ...
    auto task = mc.integrate();
    Real y = task.get();
    BOOST_CHECK_CLOSE_FRACTION(y, _Boost::math::constants::half_pi<Real>()/_Boost::math::constants::root_two<Real>(), 0.01);
}

template<class Real>
void test_double_infinite()
{
    std::cout << "Testing that double infinite bounds are integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
        return 1.0/(x[0]*x[0] + 1.0);
    };
    ...
    auto task = mc.integrate();
    Real y = task.get();
    BOOST_CHECK_CLOSE_FRACTION(y, _Boost::math::constants::pi<Real>(), 0.01);
}
...
void test_radovic()
{
    ...
    std::cout << "Testing that the Radovic function is integrated correctly by naive Monte-Carlo on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    auto g = [](std::vector<Real> const & x)->Real
    {
        using std::abs;
