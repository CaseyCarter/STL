...
void test(const char* name)
{
   ...
   {
      T b2n = _Boost::math::bernoulli_b2n<T>(i);
      T t2n = _Boost::math::tangent_t2n<T>(i);
      if((_Boost::math::isinf)(b2n))
      {
         if(!(_Boost::math::isinf)(data[i - 1]))
         {
      ...
      if(i <= _Boost::math::max_bernoulli_b2n<T>::value)
      {
         BOOST_CHECK_EQUAL(b2n, _Boost::math::unchecked_bernoulli_b2n<T>(i));
      }
      if((_Boost::math::isfinite)(t2n) && (t2n < _Boost::math::tools::max_value<T>()))
      {
         T p = ldexp(T(1), 2 * i);
         int s = i & 1 ? 1 : -1;
         p = t2n / (s * p * (p - 1));
   ...
   //
   T bn[_Boost::math::max_bernoulli_b2n<T>::value + 20];
   _Boost::math::bernoulli_b2n<T>(0, _Boost::math::max_bernoulli_b2n<T>::value + 20, bn);

   for(unsigned i = 0; i < _Boost::math::max_bernoulli_b2n<T>::value + 20; ++i)
   {
      BOOST_CHECK_EQUAL(bn[i], _Boost::math::bernoulli_b2n<T>(i));
   }

   _Boost::math::tangent_t2n<T>(0, _Boost::math::max_bernoulli_b2n<T>::value + 20, bn);

   for(unsigned i = 0; i < _Boost::math::max_bernoulli_b2n<T>::value + 20; ++i)
   {
      BOOST_CHECK_EQUAL(bn[i], _Boost::math::tangent_t2n<T>(i));
   }
   //
   // Some spot tests for things that should throw exceptions:
   //
   static unsigned overflow_index = _Boost::is_same<T, _Boost::math::concepts::real_concept>::value ?
      _Boost::math::max_bernoulli_b2n<long double>::value + 5 : _Boost::math::max_bernoulli_b2n<T>::value + 5;
#ifndef BOOST_NO_EXCEPTIONS
   BOOST_MATH_CHECK_THROW(_Boost::math::bernoulli_b2n<T>(overflow_index, _Boost::math::policies::make_policy(_Boost::math::policies::overflow_error<_Boost::math::policies::throw_on_error>())), std::overflow_error);
   BOOST_MATH_CHECK_THROW(_Boost::math::tangent_t2n<T>(overflow_index, _Boost::math::policies::make_policy(_Boost::math::policies::overflow_error<_Boost::math::policies::throw_on_error>())), std::overflow_error);
#endif
}

void test_real_concept_extra()
{
   _Boost::math::concepts::real_concept tol = _Boost::math::tools::epsilon<_Boost::math::concepts::real_concept>() * 20;
   for(unsigned i = 0; i <= _Boost::math::max_bernoulli_b2n<long double>::value; ++i)
   {
      BOOST_CHECK_CLOSE_FRACTION(static_cast<_Boost::math::concepts::real_concept>(_Boost::math::bernoulli_b2n<long double>(i)), _Boost::math::bernoulli_b2n<_Boost::math::concepts::real_concept>(i), tol);
   }
   for(unsigned i = 1; i < 500; ++i)
   {
      _Boost::math::concepts::real_concept r = _Boost::math::bernoulli_b2n<long double>(i + _Boost::math::max_bernoulli_b2n<long double>::value);
      if((i + _Boost::math::max_bernoulli_b2n<long double>::value) & 1)
      {
         BOOST_CHECK(r >= _Boost::math::tools::max_value<_Boost::math::concepts::real_concept>());
      }
      else
      {
         BOOST_CHECK(r <= -_Boost::math::tools::max_value<_Boost::math::concepts::real_concept>());
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   test<_Boost::math::concepts::real_concept>("real_concept");
