...
T binomial_wrapper(T n, T k)
{
   ...
   return BINOMIAL_FUNCTION_TO_TEST(
      _Boost::math::itrunc(n),
      _Boost::math::itrunc(k));
#else
   return _Boost::math::binomial_coefficient<T>(
      _Boost::math::itrunc(n),
      _Boost::math::itrunc(k));
#endif
}
...
#include "binomial_data.ipp"
   ...
   _Boost::math::tools::test_result<T> result = _Boost::math::tools::test_hetero<T>(
      binomial_data, 
      bind_func<T>(f, 0, 1), 
      extract_result<T>(2));
...
#include "binomial_large_data.ipp"
   ...
   result = _Boost::math::tools::test_hetero<T>(
      binomial_large_data, 
      bind_func<T>(f, 0, 1), 
      extract_result<T>(2));
