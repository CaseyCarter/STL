...
using _Boost::math::beta_distribution;
...
{
   _Boost::math::beta_distribution<RealType> abeta(a, b);
   ...
   {  // We can only check this if P is not too close to 1,
      ...
      else
      {
           ...
           && (_Boost::is_floating_point<RealType>::value))
         {
            ...
            BOOST_CHECK(quantile(abeta, P) < _Boost::math::tools::epsilon<RealType>() * 10);
         }
      } // if k
      if(x != 0)
      {
      ...
      {  // Just check quantile is very small:
         ...
         {  // Limit where this is checked: if exponent range is very large we may
            // run out of iterations in our root finding algorithm.
            BOOST_CHECK(quantile(complement(abeta, Q)) < _Boost::math::tools::epsilon<RealType>() * 10);
         }
      } // if x
...
void test_spots(RealType)
{
  ...
  RealType tolerance = (std::max)
   ...
   cout << "Boost::math::tools::epsilon = " << _Boost::math::tools::epsilon<RealType>() <<endl;
  ...
  // RealType teneps = _Boost::math::tools::epsilon<RealType>() * 10;
 ...
 // No longer allow any parameter to be NaN or inf, so all these tests should throw.
  ...
  if (std::numeric_limits<RealType>::has_infinity)
  {
   ...
   check_out_of_range<_Boost::math::beta_distribution<RealType> >(1, 1); // (All) valid constructor parameter values.
   // and range and non-finite.

   // Not needed??????
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::beta_distribution<RealType>(0, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::beta_distribution<RealType>(-1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::beta_distribution<RealType>(1, 1), -1), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::beta_distribution<RealType>(1, 1), 2), std::domain_error);
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   // but that
   // _Boost::math::beta mybeta1(1., 1.); // Using typedef fails.
   // error C2039: 'beta' : is not a member of '_Boost::math'
