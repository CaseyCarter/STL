...
#include <boost/math/distributions/uniform.hpp>
    using _Boost::math::uniform_distribution;
...
void test_spots(RealType)
{
  ...
  // cdf complement
   ...
   tolerance = (std::max)(
      ...
      static_cast<RealType>(_Boost::math::tools::epsilon<double>())) * 5; // 5 eps as a fraction.
  ...
  { // BOOST_CHECK tests for infinity using std::numeric_limits<>::infinity()
   ...
   { // real_concept case, does has_infinfity == false, so can't check it throws.
     ...
     // BOOST_MATH_CHECK_THROW(pdf(distu01, _Boost::math::tools::max_value<RealType>() * 2),  std::domain_error); // Doesn't throw.
     BOOST_CHECK_EQUAL(pdf(distu01, _Boost::math::tools::max_value<RealType>()), 0); 
   }
   // Special cases:
...
Tolerance for type class _Boost::math::concepts::real_concept is 2e-005.
Tolerance (as fraction) for type class _Boost::math::concepts::real_concept is 1.11022e-015.
*** No errors detected
