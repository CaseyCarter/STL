...
#include <boost/math/distributions/students_t.hpp>
    using _Boost::math::students_t_distribution;
...
RealType naive_pdf(RealType v, RealType t)
{
   ...
   using _Boost::math::beta;

   //return pow(v / (v + t*t), (1+v) / 2) / (sqrt(v) * beta(v/2, RealType(0.5f)));
   RealType result = _Boost::math::tgamma_ratio((v+1)/2, v/2);
   result /= sqrt(v * _Boost::math::constants::pi<RealType>());
...
void test_spots(RealType)
{
  ...
  BOOST_MATH_CHECK_THROW(_Boost::math::quantile(
  ...
  BOOST_CHECK_EQUAL(_Boost::math::quantile(
         students_t_distribution<RealType>(1.),  // degrees_of_freedom (ignored).
  ...
  BOOST_CHECK_CLOSE(_Boost::math::quantile(
         students_t_distribution<RealType>(1.),  // degrees_of_freedom (ignored).
         static_cast<RealType>(0.5)),  //  probability == half - special case.
         static_cast<RealType>(0), // t == zero.
         tolerance);
   ...
   BOOST_CHECK_CLOSE(
      ...
      //
      tolerance = _Boost::math::tools::epsilon<RealType>() * 5000;
      BOOST_CHECK_CLOSE(_Boost::math::quantile(
      ...
      if(_Boost::is_floating_point<RealType>::value)
      {
         BOOST_CHECK_CLOSE(_Boost::math::cdf(
  ...
  // for PDF checks, use 100 eps tolerance expressed as a percent:
   tolerance = _Boost::math::tools::epsilon<RealType>() * 10000;
   ...
   for(unsigned i = 1; i < 20; i += 3)
   {
    ...
    RealType tol2 = _Boost::math::tools::epsilon<RealType>() * 5;
 ...
 //// was     , static_cast<RealType>(8.0L / 6.0L), tol2);
    ...
    using std::log;
    using std::sqrt;
    RealType expected_entropy = (RealType(9)/2)*(_Boost::math::digamma(RealType(9)/2) - _Boost::math::digamma(RealType(4))) + log(sqrt(RealType(8))*_Boost::math::beta(RealType(4), RealType(1)/2));
    BOOST_CHECK_CLOSE(
       entropy(dist)
...
//    if (type != "class _Boost::math::concepts::real_concept") fails for gcc
    if (typeid(RealType) != typeid(_Boost::math::concepts::real_concept))
    { // Ordinary floats only.
      RealType limit = 1/ _Boost::math::tools::epsilon<RealType>();
      ...
      _Boost::math::normal_distribution<RealType> n(0, 1); // 
      students_t_distribution<RealType> st(_Boost::math::tools::max_value<RealType>()); // Well over the switchover point,
      // PDF
...
../libs/math/test/test_students_t.cpp(552): error: "pole_error" has already been declared in the current scope
  ...
  {
  // Mean
    BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_students_t(-1))));
    BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_students_t(0))));
    BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_students_t(1))));

    // Variance
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(std::numeric_limits<RealType>::quiet_NaN()))));
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(-1))));
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(0))));
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(1))));
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(static_cast<RealType>(1.7L)))));
    BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_students_t(2))));

  // Skewness
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(std::numeric_limits<RealType>::quiet_NaN()))));
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(-1))));
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(0))));
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(1))));
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(2))));
    BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_students_t(3))));

  // Kurtosis 
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(std::numeric_limits<RealType>::quiet_NaN()))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(-1))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(0))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(1))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(2))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(static_cast<RealType>(2.0001L)))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(3))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_students_t(4))));
 
    // Kurtosis excess
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(std::numeric_limits<RealType>::quiet_NaN()))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(-1))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(0))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(1))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(2))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(static_cast<RealType>(2.0001L)))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(3))));
    BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_students_t(4))));
  } // has_quiet_NaN

  BOOST_CHECK(_Boost::math::isfinite(mean(ignore_error_students_t(1 + std::numeric_limits<RealType>::epsilon()))));
  BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_students_t(2 + 2 * std::numeric_limits<RealType>::epsilon()))));
  BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_students_t(static_cast<RealType>(2.0001L)))));
  BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_students_t(2 + 2 * std::numeric_limits<RealType>::epsilon()))));
  BOOST_CHECK(_Boost::math::isfinite(skewness(ignore_error_students_t(3 + 3 * std::numeric_limits<RealType>::epsilon()))));
  BOOST_CHECK(_Boost::math::isfinite(kurtosis(ignore_error_students_t(4 + 4 * std::numeric_limits<RealType>::epsilon()))));
  BOOST_CHECK(_Boost::math::isfinite(kurtosis(ignore_error_students_t(static_cast<RealType>(4.0001L)))));
...
Tolerance for type class _Boost::math::concepts::real_concept is 0.0001 %
*** No errors detected
