...
typedef std::pair<_Boost::regex, std::pair<_Boost::uintmax_t, _Boost::uintmax_t> > expected_data_type;
...
inline void add_expected_result(
   ...
   _Boost::uintmax_t max_peek_error, 
   _Boost::uintmax_t max_mean_error)
{
   std::string re("(?:");
   re += compiler;
   re += ")";
   ...
   get_expected_data().push_back(
      std::make_pair(_Boost::regex(re, _Boost::regex::perl | _Boost::regex::icase), 
...
inline const std::pair<_Boost::uintmax_t, _Boost::uintmax_t>&
   get_max_errors(const char* type_name, const char* test_name, const char* group_name)
{
   static const std::pair<_Boost::uintmax_t, _Boost::uintmax_t> defaults(1, 1);
...
#pragma warning(disable:4127)
   ...
   if((result.max)() != 0)
   {
      ...
      for(unsigned i = 0; i < worst.size(); ++i)
      {
         ...
         std::cout << _Boost::math::tools::real_cast<double>(worst[i]);
#else
         std::cout << worst[i];
#endif
      }
   ...
   //
   std::pair<_Boost::uintmax_t, _Boost::uintmax_t> const& bounds = get_max_errors(type_name, test_name, group_name);
...
template <class T, class Seq>
void print_test_result(const _Boost::math::tools::test_result<T>& result,
