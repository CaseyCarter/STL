...
void test_spots(RealType)
{
   ...
   RealType tolerance = _Boost::math::tools::epsilon<RealType>() * 30; // most OK with 3 eps tolerance.
   ...
   BOOST_CHECK_CLOSE_FRACTION(owens_t(static_cast<RealType>(0), static_cast<RealType>(2L)), atan(static_cast<RealType>(2L)) / (_Boost::math::constants::pi<RealType>() * 2), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(owens_t(static_cast<RealType>(0), static_cast<RealType>(0.5L)), atan(static_cast<RealType>(0.5L)) / (_Boost::math::constants::pi<RealType>() * 2), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(owens_t(static_cast<RealType>(0), static_cast<RealType>(2000L)), atan(static_cast<RealType>(2000L)) / (_Boost::math::constants::pi<RealType>() * 2), tolerance);
...
void check_against_T7(RealType)
{
   ...
   // Basic sanity checks, test data is as accurate as long double,
   // so set tolerance to a few epsilon expressed as a fraction.
   RealType tolerance = _Boost::math::tools::epsilon<RealType>() * 150; // most OK with 3 eps tolerance.
   cout << "Tolerance = " << tolerance << "." << endl;
   ...
   for(RealType a = static_cast<RealType>(-10.0l); a < static_cast<RealType>(3l); a += static_cast<RealType>(0.2l))
      for(RealType h = static_cast<RealType>(-10.0l); h < static_cast<RealType>(3.5l); h += static_cast<RealType>(0.2l))
      {
         ...
         const RealType t = _Boost::math::owens_t(exph, expa);
         RealType t7 = _Boost::math::owens_t_T7(exph, expa);
         //if(!(_Boost::math::isnormal)(t) || !(_Boost::math::isnormal)(t7))
...
void do_test_owens_t(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::owens_t<value_type>;
#else
   pg funcp = _Boost::math::owens_t;
#endif
