...
using std::cos;
using std::string;
using _Boost::math::quadrature::gauss_kronrod;
...
using _Boost::multiprecision::debug_adaptor;
using _Boost::multiprecision::number;
...
void test_linear()
{
    std::cout << "Testing linear functions are integrated properly by gauss_kronrod on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = _Boost::math::tools::epsilon<Real>() * 10;
    Real error;
...
void test_quadratic()
{
    std::cout << "Testing quadratic functions are integrated properly by Gauss Kronrod on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = _Boost::math::tools::epsilon<Real>() * 10;
    Real error;
...
void test_three_quadrature_schemes_examples()
{
    ...
    // Example 1:
    auto f1 = [](const Real& t)->Real { return t*_Boost::math::log1p(t); };
...
void test_integration_over_real_line()
{
    ...
    Q = gauss_kronrod<Real, Points>::integrate(f1, -_Boost::math::tools::max_value<Real>(), _Boost::math::tools::max_value<Real>(), 0, 0, &error, &L1);
    Q_expected = pi<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
}
...
{
    std::cout << "Testing right limit infinite for Gauss Kronrod in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    // Example 11:
    auto f1 = [](const Real& t)->Real { return 1/(1+t*t);};
    Q = gauss_kronrod<Real, Points>::integrate(f1, 0, _Boost::math::tools::max_value<Real>(), 0, 0, &error, &L1);
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);

    auto f4 = [](const Real& t)->Real { return 1/(1+t*t); };
    Q = gauss_kronrod<Real, Points>::integrate(f4, 1, _Boost::math::tools::max_value<Real>(), 0, 0, &error, &L1);
    Q_expected = pi<Real>()/4;
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
{
    std::cout << "Testing left limit infinite for Gauss Kronrod in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    // Example 11:
    auto f1 = [](const Real& t)->Real { return 1/(1+t*t);};
    Q = gauss_kronrod<Real, Points>::integrate(f1, -_Boost::math::tools::max_value<Real>(), Real(0), 0);
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
{
    std::cout << "Testing that complex-valued integrands are integrated correctly by Gaussian quadrature on type " << _Boost::typeindex::type_id<Complex>().pretty_name() << "\n";
    ...
    _Boost::math::quadrature::gauss_kronrod<Real, 61> integrator;
    Complex Q = integrator.integrate(lw, (Real) 0, pi<Real>());
    BOOST_CHECK_CLOSE_FRACTION(Q.real(), _Boost::lexical_cast<Real>("1.09007653448579084630177782678166964987102108635357778056449870727913321296238687023915522935120701763447787503167111962008709116746523970476893277703"), tol);
    BOOST_CHECK_CLOSE_FRACTION(Q.imag(), _Boost::lexical_cast<Real>("0.530139720774838801426860213574121741928705631382703178297940568794784362495390544411799468140433404536019992695815009036975117285537382995180319280835"), tol);
}
...
#ifdef BOOST_HAS_FLOAT128
    test_complex_lambert_w<_Boost::multiprecision::complex128>();
#endif
#endif
}
