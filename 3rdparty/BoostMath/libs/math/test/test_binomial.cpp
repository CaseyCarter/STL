...
using _Boost::math::binomial_distribution;
...
{
   _Boost::math::binomial_distribution<RealType> bn(N, p);
   BOOST_CHECK_CLOSE(
      cdf(bn, k), P, tol);
   if((P < 0.99) && (Q < 0.99))
   {
   ...
   else
   {
      ...
      RealType tol = (std::max)(2 * Q, _Boost::math::tools::epsilon<RealType>());
      BOOST_CHECK(sum < tol);
   }
}
...
void test_spots(RealType T)
{
  ...
  tolerance *= 100 * 1000;
  RealType tol2 = _Boost::math::tools::epsilon<RealType>() * 5 * 100;  // 5 eps as a percent
...

  {
    ...
    //cout << "mean(my8dist) = " << _Boost::math::mean(my8dist) << endl; // mean(my8dist) = 2
   ...
   //{
   //   int n = static_cast<int>(_Boost::math::tools::real_cast<double>(my8dist.trials()));
   //   RealType sumcdf = 0.;
   //   for (int k = 0; k <= n; k++)
   //   {
...
#include "binomial_quantile.ipp"
  ...
  for(unsigned i = 0; i < binomial_quantile_data.size(); ++i)
  {
     ...
     RealType tol = _Boost::math::tools::epsilon<RealType>() * 500;
   ...
   check_out_of_range<_Boost::math::binomial_distribution<RealType> >(1, 1); // (All) valid constructor parameter values.
...
BOOST_AUTO_TEST_CASE( test_main )
{
  ...
  // but that
   // _Boost::math::binomial mybn1(1., 0.5); // Using typedef fails
  // error C2039: 'binomial' : is not a member of '_Boost::math'
...
Output is:
  ...
  Tolerance for type class _Boost::math::concepts::real_concept is 2.22045e-011 %
