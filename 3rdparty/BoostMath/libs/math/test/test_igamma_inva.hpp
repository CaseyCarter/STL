...
void do_test_gamma_2(const T& data, const char* type_name, const char* test_name)
{
   ...
   for(unsigned i = 0; i < data.size(); ++i)
   {
      ...
      //
      if(Real(data[i][5]) == 0)
         BOOST_CHECK_EQUAL(_Boost::math::gamma_p_inva(Real(data[i][1]), Real(data[i][5])), std::numeric_limits<value_type>::has_infinity ? std::numeric_limits<value_type>::infinity() : _Boost::math::tools::max_value<value_type>());
      else if((1 - Real(data[i][5]) > 0.001) && (fabs(Real(data[i][5])) > 2 * _Boost::math::tools::min_value<value_type>()))
      {
         value_type inv = _Boost::math::gamma_p_inva(Real(data[i][1]), Real(data[i][5]));
         BOOST_CHECK_CLOSE_EX(Real(data[i][0]), inv, precision, i);
      }
      else if(1 == Real(data[i][5]))
         BOOST_CHECK_EQUAL(_Boost::math::gamma_p_inva(Real(data[i][1]), Real(data[i][5])), _Boost::math::tools::min_value<value_type>());
      else if(Real(data[i][5]) > 2 * _Boost::math::tools::min_value<value_type>())
      {
      ...
      if(Real(data[i][3]) == 0)
         BOOST_CHECK_EQUAL(_Boost::math::gamma_q_inva(Real(data[i][1]), Real(data[i][3])), _Boost::math::tools::min_value<value_type>());
      else if((1 - Real(data[i][3]) > 0.001) 
         && (fabs(Real(data[i][3])) > 2 * _Boost::math::tools::min_value<value_type>()) 
         && (fabs(Real(data[i][3])) > 2 * _Boost::math::tools::min_value<double>()))
      {
         value_type inv = _Boost::math::gamma_q_inva(Real(data[i][1]), Real(data[i][3]));
         BOOST_CHECK_CLOSE_EX(Real(data[i][0]), inv, precision, i);
      }
      else if(1 == Real(data[i][3]))
         BOOST_CHECK_EQUAL(_Boost::math::gamma_q_inva(Real(data[i][1]), Real(data[i][3])), std::numeric_limits<value_type>::has_infinity ? std::numeric_limits<value_type>::infinity() : _Boost::math::tools::max_value<value_type>());
      else if(Real(data[i][3]) > 2 * _Boost::math::tools::min_value<value_type>()) 
      {
         // not enough bits in our input to get back to x, but we should be in
         // the same ball park:
         value_type inv = _Boost::math::gamma_q_inva(Real(data[i][1]), Real(data[i][3]));
...
void do_test_gamma_inva(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::gamma_p_inva<value_type, value_type>;
#else
   pg funcp = _Boost::math::gamma_p_inva;
#endif
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   funcp = _Boost::math::gamma_q_inva<value_type, value_type>;
#else
   funcp = _Boost::math::gamma_q_inva;
#endif
   result = _Boost::math::tools::test_hetero<Real>(
      data,
      bind_func<Real>(funcp, 0, 1),
