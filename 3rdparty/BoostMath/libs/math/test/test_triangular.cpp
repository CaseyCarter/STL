...
using _Boost::math::triangular_distribution;
...
void test_spots(RealType)
{
  ...
  RealType tolerance = _Boost::math::tools::epsilon<RealType>() * 5; // 5 eps as a fraction.
  RealType tol5eps = _Boost::math::tools::epsilon<RealType>() * 5; // 5 eps as a fraction.
  ...
  tolerance = (std::max)(
    ...
    static_cast<RealType>(_Boost::math::tools::epsilon<double>())) * 10; // 10 eps as a fraction.
  ...
  { // real_concept case, does has_infinfity == false, so can't check it throws.
    ...
    // << (_Boost::math::fpclassify)(std::numeric_limits<RealType>::infinity()) << endl;
    // value of std::numeric_limits<RealType>::infinity() is zero, so FPclassify is zero,
    // so (_Boost::math::isfinite)(std::numeric_limits<RealType>::infinity()) does not detect infinity.
    ...
    // BOOST_MATH_CHECK_THROW(pdf(tridef, _Boost::math::tools::max_value<RealType>() * 2),  std::domain_error); // Doesn't throw.
    BOOST_CHECK_EQUAL(pdf(tridef, _Boost::math::tools::max_value<RealType>()), 0);
  }
  // Special cases:
...
Tolerance for type class _Boost::math::concepts::real_concept is 1.11022e-015.
*** No errors detected
