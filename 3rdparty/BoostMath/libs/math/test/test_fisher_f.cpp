...
using _Boost::math::fisher_f_distribution;
...
RealType naive_pdf(RealType df1, RealType df2, RealType x)
{
   ...
   RealType e = _Boost::math::lgamma((df1 + df2) / 2);
   e += log(df1) * df1 / 2;
   e += log(df2) * df2 / 2;
   e += log(x) * ((df1 / 2) - 1);
   e -= _Boost::math::lgamma(df1 / 2);
   e -= _Boost::math::lgamma(df2 / 2);
...
{
   _Boost::math::fisher_f_distribution<RealType> dist(df1, df2);
   BOOST_CHECK_CLOSE(
      cdf(dist, cs), P, tol);
   BOOST_CHECK_CLOSE(
...
void test_spots(RealType)
{
  ...
  for(unsigned i = 0; i < sizeof(upper_critical_values) / sizeof(upper_critical_values[0]); ++i)
  {
   ...
   tolerance = _Boost::math::tools::epsilon<RealType>() * 5 * 100;
   cout << "Tolerance = " << tolerance << "%." << endl;
   BOOST_CHECK_CLOSE(
...
BOOST_AUTO_TEST_CASE( test_main )
{
  ...
  // Check that can generate fisher distribution using the two convenience methods:
   _Boost::math::fisher_f myf1(1., 2); // Using typedef
