...
RealType naive_pdf(RealType v, RealType lam, RealType x)
{
   ...
   RealType eps = _Boost::math::tools::epsilon<RealType>();
   term = sum = pdf(_Boost::math::chi_squared_distribution<RealType>(v), x);
   for(int i = 1;; ++i)
   {
      prefix *= lam / (2 * i);
      term = prefix * pdf(_Boost::math::chi_squared_distribution<RealType>(v + 2 * i), x);
      sum += term;
      if(term / sum < eps)
         break;
   }
...
{
   _Boost::math::non_central_chi_squared_distribution<RealType> dist(df, ncp);
   BOOST_CHECK_CLOSE(
      cdf(dist, cs), P, tol);
   ...
   if((P < 0.99) && (Q < 0.99))
   {
      ...
      BOOST_CHECK_CLOSE(
         dist.find_degrees_of_freedom(_Boost::math::complement(ncp, cs, Q)), df, tol * 10);
      BOOST_CHECK_CLOSE(
         dist.find_non_centrality(df, cs, P), ncp, tol * 10);
      BOOST_CHECK_CLOSE(
         dist.find_non_centrality(_Boost::math::complement(df, cs, Q)), ncp, tol * 10);
   }
}
...
void test_spots(RealType)
{
   ...
   RealType tolerance = (std::max)(
      ...
      (RealType)_Boost::math::tools::epsilon<double>() * 5) * 150;
   //
   ...
   using _Boost::math::chi_squared_distribution;
   using  ::_Boost::math::chi_squared;
   ...
   RealType tol2 = _Boost::math::tools::epsilon<RealType>() * 5 * 100; // 5 eps as a percentage
   _Boost::math::non_central_chi_squared_distribution<RealType> dist(static_cast<RealType>(8), static_cast<RealType>(12));
   RealType x = 7;
   ...
   BOOST_CHECK_CLOSE(
      median(dist),
      quantile(
      _Boost::math::non_central_chi_squared_distribution<RealType>(
   ...
   check_out_of_range<_Boost::math::non_central_chi_squared_distribution<RealType> >(1, 1);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_chi_squared_distribution<RealType>(0, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_chi_squared_distribution<RealType>(-1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_chi_squared_distribution<RealType>(1, -1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_chi_squared_distribution<RealType>(1, 1), -1), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_chi_squared_distribution<RealType>(1, 1), 2), std::domain_error);
#endif
...
{
   return cdf(_Boost::math::non_central_chi_squared_distribution<T>(df, nc), x);
}

template <class T>
T nccs_ccdf(T df, T nc, T x)
{
   return cdf(complement(_Boost::math::non_central_chi_squared_distribution<T>(df, nc), x));
}
...
void quantile_sanity_check(T& data, const char* type_name, const char* test)
{
   ...
   for(unsigned i = 0; i < data.size(); ++i)
   {
      if(Real(data[i][3]) == 0)
      {
         BOOST_CHECK(0 == quantile(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), data[i][3]));
      }
      else if(data[i][3] < 0.9999f)
      {
         value_type p = quantile(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), data[i][3]);
         value_type pt = data[i][2];
         BOOST_CHECK_CLOSE_EX(pt, p, precision, i);
      }
      if(data[i][4] == 0)
      {
         BOOST_CHECK(0 == quantile(complement(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), data[i][3])));
      }
      else if(data[i][4] < 0.9999f)
      {
         value_type p = quantile(complement(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), data[i][4]));
         value_type pt = data[i][2];
         BOOST_CHECK_CLOSE_EX(pt, p, precision, i);
      }
...
#ifndef BOOST_NO_EXCEPTIONS
         try{
            value_type m = mode(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]));
            value_type p = pdf(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), m);
            BOOST_CHECK_EX(pdf(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), m * (1 + sqrt(precision) * 50)) <= p, i);
            BOOST_CHECK_EX(pdf(_Boost::math::non_central_chi_squared_distribution<value_type>(data[i][0], data[i][1]), m * (1 - sqrt(precision)) * 50) <= p, i);
         }
         ...
         try{
            ...
            if((data[i][3] < 0.99) && (data[i][3] != 0))
            {
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_chi_squared_distribution<value_type>::find_degrees_of_freedom(data[i][1], data[i][2], data[i][3]),
                  data[i][0], precision, i);
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_chi_squared_distribution<value_type>::find_non_centrality(data[i][0], data[i][2], data[i][3]),
                  data[i][1], precision, i);
            }
            if((data[i][4] < 0.99) && (data[i][4] != 0))
            {
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_chi_squared_distribution<value_type>::find_degrees_of_freedom(_Boost::math::complement(data[i][1], data[i][2], data[i][4])),
                  data[i][0], precision, i);
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_chi_squared_distribution<value_type>::find_non_centrality(_Boost::math::complement(data[i][0], data[i][2], data[i][4])),
                  data[i][1], precision, i);
            }
#ifndef BOOST_NO_EXCEPTIONS
         }
