...
{
   T derivative;
   _Boost::math::detail::ibeta_imp(a, b, x, _Boost::math::policies::policy<>(), false, true, &derivative);
   return derivative;
}
...
void do_test_beta(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::ibeta_derivative<value_type, value_type, value_type>;
#else
   pg funcp = _Boost::math::ibeta_derivative;
#endif
   ...
   if(_Boost::math::tools::digits<value_type>() > 40)
   {
      //
      // test ibeta_derivative against data:
      //
...
{
   using std::ldexp;
   T tolerance = _Boost::math::tools::epsilon<T>() * 40000;
      BOOST_CHECK_CLOSE(
