...
RealType NaivePDF(RealType shape, RealType scale, RealType x)
{
   ...
   using namespace std;
   RealType result = log(x) * (shape - 1) - x / scale - _Boost::math::lgamma(shape) - log(scale) * shape;
   return exp(result);
}
...
void test_spots(RealType)
{
   ...
   check_gamma(
    ...
    using std::log;
    RealType expected_entropy = RealType(8) + log(RealType(3)) + _Boost::math::lgamma(RealType(8)) - 7*_Boost::math::digamma(RealType(8));
    BOOST_CHECK_CLOSE(
  ...
  // Rely on default definition in derived accessors.
   ...
   // error tests
   check_out_of_range<_Boost::math::gamma_distribution<RealType> >(1, 1);
   BOOST_MATH_CHECK_THROW(_Boost::math::gamma_distribution<RealType>(0, 1), std::domain_error);
   BOOST_MATH_CHECK_THROW(_Boost::math::gamma_distribution<RealType>(-1, 1), std::domain_error);
   BOOST_MATH_CHECK_THROW(_Boost::math::gamma_distribution<RealType>(1, 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(_Boost::math::gamma_distribution<RealType>(1, -1), std::domain_error);
...
Tolerance for type class _Boost::math::concepts::real_concept is 5e-012 %
Tolerance for type class _Boost::math::concepts::real_concept is 0.001 %
*** No errors detected
