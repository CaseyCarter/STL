...
#ifndef BOOST_MATH_NO_LONG_DOUBLE_MATH_FUNCTIONS
   if(_Boost::math::tools::digits<long double>() == 64)
   {
      //
...
{
   _Boost::math::non_central_beta_distribution<RealType> dist(a, b, ncp);
   BOOST_CHECK_CLOSE(
      cdf(dist, cs), P, tol);
   //
   ...
   // float precision:
   //
   if(!_Boost::is_same<float, RealType>::value)
   {
      BOOST_CHECK_CLOSE(
...
void test_spots(RealType)
{
   RealType tolerance = (std::max)(
      ...
      (RealType)1e-6) * 100;
   RealType abs_tolerance = _Boost::math::tools::epsilon<RealType>() * 100;
   ...
   _Boost::math::non_central_beta_distribution<RealType> dist(100, 3, 63);
   ...
   BOOST_MATH_CHECK_THROW(skewness(dist), _Boost::math::evaluation_error);
   BOOST_MATH_CHECK_THROW(kurtosis(dist), _Boost::math::evaluation_error);
   BOOST_MATH_CHECK_THROW(kurtosis_excess(dist), _Boost::math::evaluation_error);
...
#if !BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
#ifdef TEST_REAL_CONCEPT
   test_accuracy(_Boost::math::concepts::real_concept(0.), "real_concept"); // Test real concept.
#endif
#endif
#endif
