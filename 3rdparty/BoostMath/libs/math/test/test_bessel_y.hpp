...
void do_test_cyl_neumann_y(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::cyl_neumann<value_type, value_type>;
#else
   pg funcp = _Boost::math::cyl_neumann;
#endif
...
T cyl_neumann_int_wrapper(T v, T x)
{
   ...
   return static_cast<T>(BESSEL_YN_FUNCTION_TO_TEST(_Boost::math::itrunc(v), x));
#else
   return static_cast<T>(_Boost::math::cyl_neumann(_Boost::math::itrunc(v), x));
#endif
}
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   pg funcp = _Boost::math::sph_neumann<value_type>;
#else
   pg funcp = _Boost::math::sph_neumann;
#endif
...
void test_bessel(T, const char* name)
{
   ...
   // 
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 11> yv_data = {{
