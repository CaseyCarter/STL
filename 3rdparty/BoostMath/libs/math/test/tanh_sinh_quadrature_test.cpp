...
using std::cos;
using std::string;
using _Boost::multiprecision::cpp_bin_float_50;
using _Boost::multiprecision::cpp_bin_float_100;
using _Boost::multiprecision::cpp_dec_float_50;
using _Boost::multiprecision::cpp_dec_float_100;
using _Boost::multiprecision::cpp_bin_float_quad;
using _Boost::math::sinc_pi;
using _Boost::math::quadrature::tanh_sinh;
using _Boost::math::quadrature::detail::tanh_sinh_detail;
using _Boost::math::constants::pi;
using _Boost::math::constants::half_pi;
using _Boost::math::constants::two_div_pi;
using _Boost::math::constants::two_pi;
using _Boost::math::constants::half;
using _Boost::math::constants::third;
using _Boost::math::constants::half;
using _Boost::math::constants::third;
using _Boost::math::constants::catalan;
using _Boost::math::constants::ln_two;
using _Boost::math::constants::root_two;
using _Boost::math::constants::root_two_pi;
using _Boost::math::constants::root_pi;
...
{
   return _Boost::math::tools::root_epsilon<Real>();
}
...
void test_linear()
{
    std::cout << "Testing linear functions are integrated properly by tanh_sinh on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10*_Boost::math::tools::epsilon<Real>();
...
void test_quadratic()
{
    std::cout << "Testing quadratic functions are integrated properly by tanh_sinh on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10*_Boost::math::tools::epsilon<Real>();
    auto integrator = get_integrator<Real>();
...
void test_singular()
{
    std::cout << "Testing integration of endpoint singularities on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10*_Boost::math::tools::epsilon<Real>();
    Real error;
    Real L1;
    auto integrator = get_integrator<Real>();
...
void test_ca()
{
    std::cout << "Testing integration of C(a) on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real error;
    Real L1;
    ...
    tol = sqrt(_Boost::math::tools::epsilon<Real>());
...
{
    std::cout << "Testing integral in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    ...
    auto integrator = get_integrator<Real>();
    // Example 1:
    auto f1 = [](const Real& t) { return t*_Boost::math::log1p(t); };
    ...
    Q_expected = _Boost::math::expm1(half_pi<Real>())*half<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
...
{
    std::cout << "Testing integrals over entire real line in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
    ...
    Q = integrator.integrate(f1, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = pi<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);

    auto f2 = [](const Real& t) { return exp(-t*t*half<Real>()); };
    Q = integrator.integrate(f2, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    ...
    //Q = integrator.integrate(_Boost::math::sinc_pi<Real>, -std::numeric_limits<Real>::infinity(), std::numeric_limits<Real>::infinity(), &error, &L1);
    ...
    Q = integrator.integrate(f4, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = pi<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
...
{
    std::cout << "Testing right limit infinite for tanh_sinh in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
    ...
    // Example 11:
    auto f1 = [](const Real& t) { return 1/(1+t*t);};
    Q = integrator.integrate(f1, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);

    // Example 12
    auto f2 = [](const Real& t) { return exp(-t)/sqrt(t); };
    Q = integrator.integrate(f2, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = root_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 1000*tol);

    auto f3 = [](const Real& t) { return exp(-t)*cos(t); };
    Q = integrator.integrate(f3, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = half<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);

    auto f4 = [](const Real& t) { return 1/(1+t*t); };
    Q = integrator.integrate(f4, 1, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = pi<Real>()/4;
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
{
    std::cout << "Testing left limit infinite for tanh_sinh in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    auto integrator = get_integrator<Real>();
    ...
    // Example 11:
    auto f1 = [](const Real& t) { return 1/(1+t*t);};
    Q = integrator.integrate(f1, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), Real(0));
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
void test_horrible()
{
    std::cout << "Testing Trefenthen's horrible integral on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    Q_expected = _Boost::lexical_cast<Real>("0.33673283478172753598559003181355241139806404130031017259552729882281");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
...
{
    std::cout << "Testing singular integrals from NR 4.5.4 on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
    ...
    Q = integrator.integrate(f2, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>());
    Q_expected = half<Real>()*_Boost::math::tgamma((Real) 5/ (Real) 14);
...
{
    std::cout << "Testing Clenshaw & Curtis's example of integrand which fools termination schemes on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
...
void test_crc()
{
    std::cout << "Testing CRC formulas on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
    ...
    //Q_expected = pow(two_pi<Real>(), 3*half<Real>())/pow(_Boost::math::tgamma((Real) 1/ (Real) 4), 2);
    Q_expected = _Boost::lexical_cast<Real>("1.198140234735592207439922492280323878227212663215651558263674952946405214143915670835885556489793389375907225");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    ...
    for (int n = 0; n < 3; ++n) {
        for (int m = 0; m < 3; ++m) {
            ...
            Q_expected = _Boost::lexical_cast<Real>((tgamma(np1)/pow(mp1, np1)).str());
            BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
        }
    }
    ...
    if (std::numeric_limits<Real>::max_exponent > std::numeric_limits<double>::max_exponent)
    {
       for (Real p = Real (-0.99); p < 1; p += Real(0.1)) {
          auto f = [&](Real x, Real cx)->Real
          {
             ...
             return cx < 0 ? exp(p * (log(x) - _Boost::math::log1p(-x))) : pow(x, p) / pow(cx, p);
          };
          Q = integrator.integrate(f, (Real)0, (Real)1, get_convergence_tolerance<Real>(), &error, &L1);
    ...
    for (Real p = Real(0.99); p > 0; p -= Real(0.1)) {
       auto f = [&](Real x)->Real
       {
          return exp(-x * (1 - p) + p * log(-_Boost::math::expm1(-x)));
       };
       Q = integrator.integrate(f, 0, _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
       Q_expected = 1 / sinc_pi(p*pi<Real>());
       BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, 10 * tol);
    }
    // and for p < 1:
    for (Real p = Real (-0.99); p < 0; p += Real(0.1)) {
       auto f = [&](Real x)->Real
       {
          return exp(-p * log(-_Boost::math::expm1(-x)) - (1 + p) * x);
       };
       Q = integrator.integrate(f, 0, _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
       Q_expected = 1 / sinc_pi(p*pi<Real>());
       BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, 10 * tol);
    ...
    for (Real i = 80; i < 100; ++i) {
       ...
       auto f = [&](Real x)->Real 
       { 
          if (x > -log(_Boost::math::tools::epsilon<Real>()))
             return exp((h - 1) * x);
          else
          {
             Real et = exp(-x);
             // Need to deal with numeric instability causing et to be greater than PI/2:
             return et >= _Boost::math::constants::half_pi<Real>() ? 0 : et * pow(1 / tan(et), h);
          }
       };
       Q = integrator.integrate(f, -log(half_pi<Real>()), _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
       Q_expected = half_pi<Real>() / cos(h*half_pi<Real>());
...
void test_sf()
{
   ...
   std::cout << "Testing special functions on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
   Real tol = 10 * _Boost::math::tools::epsilon<Real>();
   auto integrator = get_integrator<Real>();
   ...
   {
      Real a(100), b(15);
      auto f = [&](Real x)->Real { return _Boost::math::ibeta_derivative(a, b, x); };
      BOOST_CHECK_CLOSE_FRACTION(integrator.integrate(f, 0, Real(0.25)), _Boost::math::ibeta(100, 15, Real(0.25)), tol * 10);
   ...
   Real Q = integrator.integrate([&](const Real& t)->Real { return 1 / (sqrt(t + x) * (t + y)); }, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>()) / 2;
   BOOST_CHECK_CLOSE_FRACTION(Q, _Boost::math::ellint_rc(x, y), tol);
   // Elliptic Integral RJ:
   BOOST_CHECK_CLOSE_FRACTION(Real((Real(3) / 2) * integrator.integrate([&](Real t)->Real { return 1 / (sqrt((t + x) * (t + y) * (t + z)) * (t + p)); }, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>())), _Boost::math::ellint_rj(x, y, z, p), tol);
   ...
   BOOST_CHECK_CLOSE_FRACTION(integrator.integrate([&](Real t)->Real { using std::pow; using std::exp; return t > 10000 ? Real(0) : Real(pow(t, z - 1) * exp(-t)); }, 0, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>()),
      _Boost::math::tgamma(z), tol);
   BOOST_CHECK_CLOSE_FRACTION(integrator.integrate([](const Real& t)->Real {  using std::exp; return exp(-t*t); }, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>()),
      _Boost::math::constants::root_pi<Real>(), tol);
}
...
void test_2_arg()
{
   ...
   std::cout << "Testing 2 argument functors on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
   Real tol = 10 * _Boost::math::tools::epsilon<Real>();
   ...
   BOOST_CHECK_CLOSE_FRACTION(Q, _Boost::math::constants::pi<Real>(), tol);
   Q = integrator.integrate([&](const Real& t, const Real & tc)->Real
   {
      return tc < 0 ? 1 / _Boost::math::cbrt(t * (1-t)) : 1 / _Boost::math::cbrt(t * tc);
   }, 0, 1);
   BOOST_CHECK_CLOSE_FRACTION(Q, _Boost::math::pow<2>(_Boost::math::tgamma(Real(2) / 3)) / _Boost::math::tgamma(Real(4) / 3), tol * 3);
   //
   ...
   Q = integrator.integrate([&](const Real& t, const Real & tc)->Real
   {
      return t < 0 ? 1 / _Boost::math::cbrt(-tc * (1-t)) : 1 / _Boost::math::cbrt((t + 1) * tc);
   }, -1, 1);
   BOOST_CHECK_CLOSE_FRACTION(Q, sqrt(_Boost::math::constants::pi<Real>()) * _Boost::math::tgamma(Real(2) / 3) / _Boost::math::tgamma(Real(7) / 6), tol * 10);
   Q = integrator.integrate([&](const Real& t, const Real & tc)->Real
   {
      return t < 0 ? 1 / _Boost::math::cbrt(-tc * (1-t)) : 1 / _Boost::math::cbrt((t + 1) * tc);
   });
...
void test_complex()
{
   ...
   //
   value_type tol = 10 * _Boost::math::tools::epsilon<value_type>();
   ...
   //
   Complex expected(_Boost::lexical_cast<value_type>("-0.2911081612888249710582867318081776512805281815037891183828405999609246645054069649838607112484426042883371996"),
      _Boost::lexical_cast<value_type>("0.4507983563969959578849120188097153649211346293694903758252662015991543519595834937475296809912196906074655385"));
   ...
   Q = integrator.integrate(f2, value_type(0), _Boost::math::constants::half_pi<value_type>(), get_convergence_tolerance<value_type>());

   expected = Complex(_Boost::lexical_cast<value_type>("0.8893822921008980697856313681734926564752476188106405688951257340480164694708337246829840859633322683740376134733"),
      -_Boost::lexical_cast<value_type>("2.381380802906111364088958767973164614925936185337231718483495612539455538280372745733208000514737758457795502168"));
   expected -= _Boost::math::constants::half_pi<value_type>();
...
BOOST_AUTO_TEST_CASE(tanh_sinh_quadrature_test)
{
   ...
   print_levels(generate_constants<cpp_bin_float_100>(10, 8), "L");
    ...
    test_sf<_Boost::multiprecision::number<_Boost::multiprecision::cpp_bin_float<150> > >();

#endif
#ifndef BOOST_MATH_NO_LONG_DOUBLE_MATH_FUNCTIONS
#ifdef TEST6

    test_right_limit_infinite<_Boost::math::concepts::real_concept>();
    test_left_limit_infinite<_Boost::math::concepts::real_concept>();
    test_linear<_Boost::math::concepts::real_concept>();
    test_quadratic<_Boost::math::concepts::real_concept>();
    test_singular<_Boost::math::concepts::real_concept>();
    test_ca<_Boost::math::concepts::real_concept>();
    test_three_quadrature_schemes_examples<_Boost::math::concepts::real_concept>();
    test_horrible<_Boost::math::concepts::real_concept>();
    test_integration_over_real_line<_Boost::math::concepts::real_concept>();
    test_nr_examples<_Boost::math::concepts::real_concept>();
    test_early_termination<_Boost::math::concepts::real_concept>();
    test_sf<_Boost::math::concepts::real_concept>();
    test_2_arg<_Boost::math::concepts::real_concept>();
#endif
#ifdef TEST6A
    test_crc<_Boost::math::concepts::real_concept>();
    ...
    test_right_limit_infinite<_Boost::multiprecision::float128>();
    test_left_limit_infinite<_Boost::multiprecision::float128>();
    test_linear<_Boost::multiprecision::float128>();
    test_quadratic<_Boost::multiprecision::float128>();
    test_singular<_Boost::multiprecision::float128>();
    test_ca<_Boost::multiprecision::float128>();
    test_three_quadrature_schemes_examples<_Boost::multiprecision::float128>();
    test_horrible<_Boost::multiprecision::float128>();
    test_integration_over_real_line<_Boost::multiprecision::float128>();
    test_nr_examples<_Boost::multiprecision::float128>();
    test_early_termination<_Boost::multiprecision::float128>();
    test_crc<_Boost::multiprecision::float128>();
    test_sf<_Boost::multiprecision::float128>();
    test_2_arg<_Boost::multiprecision::float128>();
