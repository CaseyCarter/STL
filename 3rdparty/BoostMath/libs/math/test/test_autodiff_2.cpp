...
BOOST_AUTO_TEST_CASE_TEMPLATE(pow, T, bin_float_types) {
  ...
  for (auto j : _Boost::irange(std::size_t(1), n + 1)) {
    BOOST_CHECK_CLOSE(z1.derivative(0u, j), pow(log(cx), j) * pow(cx, cy), eps);
  }

  for (auto i : _Boost::irange(std::size_t(1), m + 1)) {
    for (auto j : _Boost::irange(n + 1)) {
  ...
  BOOST_CHECK_CLOSE(z2.derivative(2u, 1u),
                    ...
                    eps);
  for (auto j : _Boost::irange(std::size_t(2u), n + 1)) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ylogx, T, all_float_types) {
  ...
  for (auto i : _Boost::irange(1u, static_cast<unsigned>(m + 1))) {
    BOOST_CHECK_CLOSE(z.derivative(i, 0u),
                      pow(-1, i - 1) * _Boost::math::factorial<T>(i - 1) * cy /
                          pow(cx, i),
                      eps);
    BOOST_CHECK_CLOSE(
        z.derivative(i, 1u),
        pow(-1, i - 1) * _Boost::math::factorial<T>(i - 1) / pow(cx, i), eps);
    for (auto j : _Boost::irange(std::size_t(2), n + 1)) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(ldexp_test, T, all_float_types) {
  ...
  using _Boost::multiprecision::ldexp;
  constexpr auto m = 3u;
  const T cx = 3.5;
  const auto x = make_fvar<T, m>(cx);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(cos_and_sin, T, bin_float_types) {
  ...
  constexpr std::size_t m = 5;
  const T cx = _Boost::math::constants::third_pi<T>();
  const auto x = make_fvar<T, m>(cx);
  auto cos5 = cos(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(acosh_test, T, bin_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(1u),
                    1 / _Boost::math::constants::root_three<T>(), eps);
  BOOST_CHECK_CLOSE(y.derivative(2u),
                    -2 / (3 * _Boost::math::constants::root_three<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(3u),
                    1 / _Boost::math::constants::root_three<T>(), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u),
                    -22 / (9 * _Boost::math::constants::root_three<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(5u),
                    227 / (27 * _Boost::math::constants::root_three<T>()),
                    2 * eps);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(asin_infinity, T, all_float_types) {
  ...
  BOOST_CHECK_CLOSE(y.derivative(0u), _Boost::math::constants::half_pi<T>(),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(asinh_test, T, bin_float_types) {
  const T eps = 300 * std::numeric_limits<T>::epsilon(); // percent
  using _Boost::math::asinh;
  constexpr std::size_t m = 5;
  const T cx = 1;
  auto x = make_fvar<T, m>(cx);
  auto y = asinh(x);
  BOOST_CHECK_CLOSE(y.derivative(0u), asinh(static_cast<T>(x)), eps);
  BOOST_CHECK_CLOSE(y.derivative(1u), 1 / _Boost::math::constants::root_two<T>(),
                    eps);
  BOOST_CHECK_CLOSE(y.derivative(2u),
                    -1 / (2 * _Boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(3u),
                    1 / (4 * _Boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(4u),
                    3 / (8 * _Boost::math::constants::root_two<T>()), eps);
  BOOST_CHECK_CLOSE(y.derivative(5u),
                    -39 / (16 * _Boost::math::constants::root_two<T>()), eps);
}
