...
void do_test(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = &_Boost::math::log1p<value_type>;
#else
   pg funcp = &_Boost::math::log1p;
#endif
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   funcp = _Boost::math::expm1<value_type>;
#else
   funcp = _Boost::math::expm1;
#endif
   result = _Boost::math::tools::test_hetero<Real>(
      data, 
      bind_func<Real>(funcp, 0), 
...
#  include "log1p_expm1_data.ipp"
   ...
   BOOST_CHECK_EQUAL(_Boost::math::log1p(zero), zero);
   BOOST_CHECK_EQUAL(_Boost::math::log1p(T(-zero)), zero);
   BOOST_CHECK_EQUAL(_Boost::math::expm1(zero), zero);
   if(std::numeric_limits<T>::has_infinity)
   {
      BOOST_CHECK_EQUAL(_Boost::math::log1p(m_one), -std::numeric_limits<T>::infinity());
      BOOST_CHECK_EQUAL(_Boost::math::expm1(T(-std::numeric_limits<T>::infinity())), m_one);
      BOOST_CHECK_EQUAL(_Boost::math::expm1(std::numeric_limits<T>::infinity()), std::numeric_limits<T>::infinity());
#ifndef __BORLANDC__
      ...
      BOOST_MATH_CHECK_THROW(_Boost::math::log1p(m_one, pol()), std::overflow_error);
      BOOST_MATH_CHECK_THROW(_Boost::math::expm1(std::numeric_limits<T>::infinity(), pol()), std::overflow_error);
#endif
#endif
   }
}
