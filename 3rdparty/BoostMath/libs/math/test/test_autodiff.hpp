...
namespace mp11 = _Boost::mp11;
namespace bmp = _Boost::multiprecision;
...
using namespace _Boost::math::differentiation;
...
auto isNearZero(const T& t) noexcept -> typename std::enable_if<!detail::is_fvar<T>::value, bool>::type
{
  ...
  using std::sqrt;
  return fpclassify(fabs(t)) == FP_ZERO || fpclassify(fabs(t)) == FP_SUBNORMAL || _Boost::math::fpc::is_small(fabs(t), sqrt(std::numeric_limits<T>::epsilon()));
}
...
template <typename T>
T phi(const T& x) {
  return _Boost::math::constants::one_div_root_two_pi<T>() * exp(-0.5 * x * x);
}

// Standard normal cumulative distribution function
template <typename T>
T Phi(const T& x) {
  return 0.5 * erfc(-_Boost::math::constants::one_div_root_two<T>() * x);
}
