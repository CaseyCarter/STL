...
using _Boost::math::interpolators::cardinal_cubic_hermite;
using _Boost::math::interpolators::cardinal_cubic_hermite_aos;
...
void test_constant()
{
    ...
    _Boost::circular_buffer<Real> x_buf(x.size());
    for (auto & t : x) {
        x_buf.push_back(t);
    }

    _Boost::circular_buffer<Real> y_buf(x.size());
    for (auto & t : y) {
        y_buf.push_back(t);
    }

    _Boost::circular_buffer<Real> dydx_buf(x.size());
    for (auto & t : dydx) {
