...
#include <boost/math/distributions/rayleigh.hpp>
    using _Boost::math::rayleigh_distribution;
...
void test_spots(RealType T)
{
  ...
  using namespace _Boost::math::constants;
   ...
   BOOST_CHECK_CLOSE(
      ::_Boost::math::kurtosis_excess(rayleigh_distribution<RealType>(2)),
      ::_Boost::math::kurtosis(rayleigh_distribution<RealType>(2)) -3,
         tolerance* 100); // %


   RealType expected_entropy = 1 + log(_Boost::math::constants::root_two<RealType>()) + _Boost::math::constants::euler<RealType>()/2;
   BOOST_CHECK_CLOSE(
      ::_Boost::math::entropy(rayleigh_distribution<RealType>(2)),
      expected_entropy,
         tolerance* 100);
   return;
...
BOOST_AUTO_TEST_CASE( test_main )
{
  // Check that can generate rayleigh distribution using the two convenience methods:
   _Boost::math::rayleigh ray1(1.); // Using typedef
  ...
  // Double only tests.
   ...
   BOOST_CHECK_CLOSE_FRACTION(
      ::_Boost::math::quantile(complement(
      rayleigh_distribution<double>(4.),
...
Tolerance for type class _Boost::math::concepts::real_concept is 2.22045e-013 %
*** No errors detected
