...
{
   float expected = static_cast<float>((_Boost::math::hypot)(
...
#endif
   float found = (_Boost::math::hypot)(x, y);
...
void test_spots()
{
   ...
   for(unsigned i = 0; i < sizeof(boundaries)/sizeof(float); ++i)
   {
      BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[i], zero), std::fabs(boundaries[i]));
      BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[i], zero), std::fabs(-boundaries[i]));
      BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[i], -zero), std::fabs(boundaries[i]));
      BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[i], -zero), std::fabs(-boundaries[i]));
      for(unsigned j = 0; j < sizeof(boundaries)/sizeof(float); ++j)
      {
         BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[i], boundaries[j]), _Boost::math::hypot(boundaries[j], boundaries[i]));
         BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[i], boundaries[j]), _Boost::math::hypot(boundaries[i], -boundaries[j]));
         BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[i], -boundaries[j]), _Boost::math::hypot(-boundaries[j], -boundaries[i]));
         BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[i], -boundaries[j]), _Boost::math::hypot(-boundaries[i], boundaries[j]));
      }
   }
   if((std::numeric_limits<float>::has_infinity) && (std::numeric_limits<float>::has_quiet_NaN))
   {
      ...
      BOOST_CHECK_EQUAL(_Boost::math::hypot(inf, nan), inf);
      BOOST_CHECK_EQUAL(_Boost::math::hypot(-inf, nan), inf);
      BOOST_CHECK_EQUAL(_Boost::math::hypot(nan, inf), inf);
      BOOST_CHECK_EQUAL(_Boost::math::hypot(nan, -inf), inf);
      for(unsigned j = 0; j < sizeof(boundaries)/sizeof(float); ++j)
      {
         BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[j], inf), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[j], inf), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(inf, boundaries[j]), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(inf, -boundaries[j]), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(boundaries[j], -inf), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(-boundaries[j], -inf), inf);
         BOOST_CHECK_EQUAL(_Boost::math::hypot(-inf, boundaries[j]), inf);
