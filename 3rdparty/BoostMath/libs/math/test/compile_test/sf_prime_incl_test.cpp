...
{
   check_result<_Boost::uint32_t>(_Boost::math::prime(u));
   //
   // Add constexpr tests here:
   //
#ifdef BOOST_MATH_HAVE_CONSTEXPR_TABLES
   constexpr _Boost::uint32_t ce_f = _Boost::math::prime(_Boost::math::max_prime);
   static_assert(ce_f == 104729, "max_prime had incorrect value");
   check_result<_Boost::uint32_t>(ce_f);
#endif
}
