...
void test_inverses(const T& data)
{
   ...
   for(unsigned i = 0; i < data.size(); ++i)
   {
      ...
      //
      if(Real(data[i][5]) == 0)
         BOOST_CHECK_EQUAL(_Boost::math::ibeta_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][5])), value_type(0));
      ...
      {
         value_type inv = _Boost::math::ibeta_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][5]));
         BOOST_CHECK_CLOSE(Real(data[i][2]), inv, precision);
      }
      else if(1 == Real(data[i][5]))
         BOOST_CHECK_EQUAL(_Boost::math::ibeta_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][5])), value_type(1));

      if(Real(data[i][6]) == 0)
         BOOST_CHECK_EQUAL(_Boost::math::ibetac_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][6])), value_type(1));
      ...
      {
         value_type inv = _Boost::math::ibetac_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][6]));
         BOOST_CHECK_CLOSE(Real(data[i][2]), inv, precision);
      }
      else if(Real(data[i][6]) == 1)
         BOOST_CHECK_EQUAL(_Boost::math::ibetac_inv(Real(data[i][0]), Real(data[i][1]), Real(data[i][6])), value_type(0));
   }
}
...
void test_inverses2(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::ibeta_inv<value_type, value_type, value_type>;
#else
   pg funcp = _Boost::math::ibeta_inv;
#endif
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   funcp = _Boost::math::ibetac_inv<value_type, value_type, value_type>;
#else
   funcp = _Boost::math::ibetac_inv;
#endif
   result = _Boost::math::tools::test_hetero<Real>(
      data,
      bind_func<Real>(funcp, 0, 1, 2),
...
void test_spots(T)
{
   ...
   //
   T tolerance = _Boost::math::tools::epsilon<T>() * 10000;
   BOOST_CHECK_CLOSE(
      ::_Boost::math::ibeta_inv(
         static_cast<T>(1),
         static_cast<T>(2),
         static_cast<T>(0.5)),
