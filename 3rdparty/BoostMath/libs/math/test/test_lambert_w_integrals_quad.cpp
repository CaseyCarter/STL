...
using _Boost::math::lambert_wm1;
using _Boost::math::lambert_w0;
...
// Error in function _Boost::math::quadrature::exp_sinh<double>::integrate:
...
T debug_integration_proc(T x)
{
  ...
  try
  {
   // Assign function call to result in here...
    if (x <= sqrt(_Boost::math::tools::min_value<T>()) )
    {
      result = 0;
    }
    else
...
void test_integrals()
{
  ...
  { //  // Integrate for function lambert_W0(z);
    tanh_sinh<Real> ts;
    Real a = 0;
    Real b = _Boost::math::constants::e<Real>();
    auto f = [](Real z)->Real
    {
      return lambert_w0<Real>(z);
    };
    Real z = ts.integrate(f, a, b); // OK without any decltype(f)
    BOOST_CHECK_CLOSE_FRACTION(z, _Boost::math::constants::e<Real>() - 1, tol);
  }
  {
    ...
    BOOST_CHECK_CLOSE_FRACTION(z, 2 * _Boost::math::constants::root_two_pi<Real>(), tol);
  }
  {
    // Integrate for function lambert_W0(1/z^2).
    exp_sinh<Real> es;
    //const Real sqrt_min = sqrt(_Boost::math::tools::min_value<Real>()); // 1.08420217e-19 fo 32-bit float.
    // error C3493: 'sqrt_min' cannot be implicitly captured because no default capture mode has been specified
    auto f = [](Real z)->Real
    {
      if (z <= sqrt(_Boost::math::tools::min_value<Real>()) )
    ...
    BOOST_CHECK_CLOSE_FRACTION(z, _Boost::math::constants::root_two_pi<Real>(), tol);
  }
...
BOOST_AUTO_TEST_CASE( integrals )
{
  ...
  float max_w_f = _Boost::math::lambert_w_detail::lambert_w0_approx((std::numeric_limits<float>::max)()); // Corless equation 4.19, page 349, and Chapeau-Blondeau equation 20, page 2162.
  std::cout << "w max_f " << max_w_f << std::endl; // 84.2879
  Real max_w = _Boost::math::lambert_w_detail::lambert_w0_approx((std::numeric_limits<Real>::max)()); // Corless equation 4.19, page 349, and Chapeau-Blondeau equation 20, page 2162.
  ...
  std::cout << "ULP = " << _Boost::math::ulp(1., policy<digits2<> >()) << std::endl; // ULP = 2.2204460492503131e-16
  std::cout << "ULP = " << _Boost::math::ulp(1e-10, policy<digits2<> >()) << std::endl; // ULP = 2.2204460492503131e-16
  std::cout << "ULP = " << _Boost::math::ulp(1., policy<digits2<11> >()) << std::endl; // ULP = 2.2204460492503131e-16
  std::cout << "epsilon =  " << std::numeric_limits<Real>::epsilon() << std::endl; //
  std::cout << "sqrt(max) =  " << sqrt(_Boost::math::tools::max_value<float>() ) << std::endl; // sqrt(max) =  1.8446742974197924e+19
  std::cout << "sqrt(min) =  " << sqrt(_Boost::math::tools::min_value<float>() ) << std::endl; // sqrt(min) =  1.0842021724855044e-19
...
Real x;
{
  ...
  // Function to be integrated, lambert_w0(1/z^2).
    ...
    //};
    // Diagnostic is:
    // Error in function _Boost::math::lambert_w0<Real>: Expected a finite value but got inf

    auto f = [](Real z)->Real
    { // Debug with diagnostics for underflow and subsequent divide by zero and other bad things.
      return debug_integration_proc(z);
    };
    // Exception Error in function _Boost::math::lambert_w0<double>: Expected a finite value but got inf.
    ...
    BOOST_CHECK_CLOSE_FRACTION(x, _Boost::math::constants::root_two_pi<Real>(), tol);
