...
RealType naive_pdf(RealType df, RealType x)
{
   ...
   RealType e = log(x) * ((df / 2) - 1) - (x / 2) - _Boost::math::lgamma(df/2);
...
{
   _Boost::math::chi_squared_distribution<RealType> dist(df);
   BOOST_CHECK_CLOSE(
      cdf(dist, cs), P, tol);
   BOOST_CHECK_CLOSE(
   ...
   _Boost::math::non_central_chi_squared_distribution<RealType> dist2(df, 0);
   BOOST_CHECK_CLOSE(
      cdf(dist2, cs), P, tol);
   BOOST_CHECK_CLOSE(
...
void test_spots(RealType T)
{
  ...
  for(unsigned i = 0; i < sizeof(lower_critical_values) / sizeof(lower_critical_values[0]); ++i)
  {
    ...
    check_out_of_range<_Boost::math::chi_squared_distribution<RealType> >(1); // (All) valid constructor parameter values.
...
Description: Autorun "J:\Cpp\MathToolkit\test\Math_test\Debug\test_chi_squared.exe"
  ...
  Tolerance for type class _Boost::math::concepts::real_concept is 0.1 %
