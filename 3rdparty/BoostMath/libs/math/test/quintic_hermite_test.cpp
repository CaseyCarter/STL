...
using _Boost::math::interpolators::quintic_hermite;
using _Boost::math::interpolators::cardinal_quintic_hermite;
using _Boost::math::interpolators::cardinal_quintic_hermite_aos;
...
void test_linear()
{
    ...
    _Boost::random::uniform_real_distribution<Real> dis(0.5,1);
    x.resize(512);
    x[0] = dis(rng);
    Real xmin = x[0];
...
void test_interpolation_condition()
{
    for (size_t n = 4; n < 50; ++n) {
        ...
        _Boost::random::mt19937 rd; 
        _Boost::random::uniform_real_distribution<Real> dis(0,1);
        Real x0 = dis(rd);
        x[0] = x0;
        y[0] = dis(rd);
...
void test_cardinal_constant()
{
    ...
    while (i++ < samples)
    {
        ...
        tlo = _Boost::math::nextafter(tlo, std::numeric_limits<Real>::max());
        thi = _Boost::math::nextafter(thi, std::numeric_limits<Real>::lowest());
    }
}
