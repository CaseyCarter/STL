...
using _Boost::multiprecision::cpp_complex_50;
...
std::vector<T> generate_random_vector(size_t size, size_t seed)
{
    ...
    else if constexpr (_Boost::is_complex<T>::value)
    {
    ...
    else if constexpr (_Boost::multiprecision::number_category<T>::value == _Boost::multiprecision::number_kind_complex)
    {
    ...
    else if constexpr (_Boost::multiprecision::number_category<T>::value == _Boost::multiprecision::number_kind_floating_point)
    {
        std::normal_distribution<long double> dis(0, 1);
        for (size_t i = 0; i < v.size(); ++i)
        {
...
void test_integer_mean()
{
    ...
    double mu = _Boost::math::statistics::mean(v);
    BOOST_TEST(abs(mu - 3) < tol);

    // Work with std::array?
    std::array<Z, 5> w{1,2,3,4,5};
    mu = _Boost::math::statistics::mean(w);
    BOOST_TEST(abs(mu - 3) < tol);

    v = generate_random_vector<Z>(global_size, global_seed);
    Z scale = 2;

    double m1 = scale*_Boost::math::statistics::mean(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    double m2 = _Boost::math::statistics::mean(v);
...
void test_mean()
{
    ...
    Real mu = _Boost::math::statistics::mean(v.begin(), v.end());
    BOOST_TEST(abs(mu - 3) < tol);

    // Does range call work?
    mu = _Boost::math::statistics::mean(v);
    BOOST_TEST(abs(mu - 3) < tol);

    // Can we successfully average only part of the vector?
    mu = _Boost::math::statistics::mean(v.begin(), v.begin() + 3);
    BOOST_TEST(abs(mu - 2) < tol);

    // Does it work when we const qualify?
    mu = _Boost::math::statistics::mean(v.cbegin(), v.cend());
    BOOST_TEST(abs(mu - 3) < tol);

    // Does it work for std::array?
    std::array<Real, 7> u{1,2,3,4,5,6,7};
    mu = _Boost::math::statistics::mean(u.begin(), u.end());
    BOOST_TEST(abs(mu - 4) < 10*tol);

    // Does it work for a forward iterator?
    std::forward_list<Real> l{1,2,3,4,5,6,7};
    mu = _Boost::math::statistics::mean(l.begin(), l.end());
    BOOST_TEST(abs(mu - 4) < tol);

    // Does it work with ublas vectors?
    _Boost::numeric::ublas::vector<Real> w(7);
    for (size_t i = 0; i < w.size(); ++i)
    {
        w[i] = i+1;
    }
    mu = _Boost::math::statistics::mean(w.cbegin(), w.cend());
    BOOST_TEST(abs(mu - 4) < tol);

    v = generate_random_vector<Real>(global_size, global_seed);
    Real scale = 2;
    Real m1 = scale*_Boost::math::statistics::mean(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    Real m2 = _Boost::math::statistics::mean(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
    ...
    for (size_t i = 1; i < 30; ++i)
    {
        ...
        auto higham_ = _Boost::math::statistics::mean(v);
...
void test_complex_mean()
{
    ...
    auto mu = _Boost::math::statistics::mean(v.begin(), v.end());
...
void test_variance()
{
    ...
    Real sigma_sq = _Boost::math::statistics::variance(v.begin(), v.end());
    BOOST_TEST(abs(sigma_sq) < tol);

    sigma_sq = _Boost::math::statistics::variance(v);
    BOOST_TEST(abs(sigma_sq) < tol);

    Real s_sq = _Boost::math::statistics::sample_variance(v);
    BOOST_TEST(abs(s_sq) < tol);

    std::vector<Real> u{1};
    sigma_sq = _Boost::math::statistics::variance(u.cbegin(), u.cend());
    BOOST_TEST(abs(sigma_sq) < tol);

    std::array<Real, 8> w{0,1,0,1,0,1,0,1};
    sigma_sq = _Boost::math::statistics::variance(w.begin(), w.end());
    BOOST_TEST(abs(sigma_sq - 1.0/4.0) < tol);

    sigma_sq = _Boost::math::statistics::variance(w);
    BOOST_TEST(abs(sigma_sq - 1.0/4.0) < tol);

    std::forward_list<Real> l{0,1,0,1,0,1,0,1};
    sigma_sq = _Boost::math::statistics::variance(l.begin(), l.end());
    BOOST_TEST(abs(sigma_sq - 1.0/4.0) < tol);

    v = generate_random_vector<Real>(global_size, global_seed);
    Real scale = 2;
    Real m1 = scale*scale*_Boost::math::statistics::variance(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    Real m2 = _Boost::math::statistics::variance(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
...
void test_integer_variance()
{
    ...
    double sigma_sq = _Boost::math::statistics::variance(v);
    BOOST_TEST(abs(sigma_sq) < tol);
    ...
    v = generate_random_vector<Z>(global_size, global_seed);
    Z scale = 2;
    double m1 = scale*scale*_Boost::math::statistics::variance(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    double m2 = _Boost::math::statistics::variance(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
}
...
void test_integer_skewness()
{
    ...
    double skew = _Boost::math::statistics::skewness(v);
    BOOST_TEST(abs(skew) < tol);

    // Dataset is symmetric about the mean:
    v = {1,2,3,4,5};
    skew = _Boost::math::statistics::skewness(v);
    BOOST_TEST(abs(skew) < tol);
    ...
    v = generate_random_vector<Z>(global_size, global_seed);
    Z scale = 2;
    double m1 = _Boost::math::statistics::skewness(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    double m2 = _Boost::math::statistics::skewness(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
...
void test_skewness()
{
    ...
    Real skew = _Boost::math::statistics::skewness(v);
    BOOST_TEST(abs(skew) < tol);
    ...
    skew = _Boost::math::statistics::skewness(w1);
    BOOST_TEST(abs(skew - Real(3)/Real(2)) < tol);

    std::forward_list<Real> w2{0,0,0,0,5};
    skew = _Boost::math::statistics::skewness(w2);
    BOOST_TEST(abs(skew - Real(3)/Real(2)) < tol);

    v = generate_random_vector<Real>(global_size, global_seed);
    Real scale = 2;
    Real m1 = _Boost::math::statistics::skewness(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    Real m2 = _Boost::math::statistics::skewness(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
}
...
void test_kurtosis()
{
    Real tol = std::numeric_limits<Real>::epsilon();
    std::vector<Real> v{1,1,1};
    Real kurt = _Boost::math::statistics::kurtosis(v);
    BOOST_TEST(abs(kurt) < tol);

    v = {1,2,3,4,5};
    // mu =1, sigma^2 = 2, kurtosis = 17/10
    kurt = _Boost::math::statistics::kurtosis(v);
    ...
    kurt = _Boost::math::statistics::kurtosis(v1);
    BOOST_TEST(abs(kurt - Real(13)/Real(4)) < tol);

    std::forward_list<Real> v2{0,0,0,0,5};
    kurt = _Boost::math::statistics::kurtosis(v2);
    BOOST_TEST(abs(kurt - Real(13)/Real(4)) < tol);
    ...
    kurt = _Boost::math::statistics::kurtosis(v3);
    ...
    auto excess_kurtosis = _Boost::math::statistics::excess_kurtosis(v3);
    BOOST_TEST(abs(excess_kurtosis + 6.0/5.0) < 0.2);

    v = generate_random_vector<Real>(global_size, global_seed);
    Real scale = 2;
    Real m1 = _Boost::math::statistics::kurtosis(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    Real m2 = _Boost::math::statistics::kurtosis(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
    ...
    //    v3[i] = edis(gen);
    //}
    //excess_kurtosis = _Boost::math::statistics::kurtosis(v3) - 3;
...
void test_integer_kurtosis()
{
    double tol = std::numeric_limits<double>::epsilon();
    std::vector<Z> v{1,1,1};
    double kurt = _Boost::math::statistics::kurtosis(v);
    BOOST_TEST(abs(kurt) < tol);
    ...
    v = generate_random_vector<Z>(global_size, global_seed);
    Z scale = 2;
    double m1 = _Boost::math::statistics::kurtosis(v);
    for (auto & x : v)
    {
        x *= scale;
    }
    double m2 = _Boost::math::statistics::kurtosis(v);
    BOOST_TEST(abs(m1 - m2) < tol*abs(m1));
}
...
void test_first_four_moments()
{
    ...
    std::vector<Real> v{1,1,1};
    auto [M1_1, M2_1, M3_1, M4_1] = _Boost::math::statistics::first_four_moments(v);
    ...
    v = {1, 2, 3, 4, 5};
    auto [M1_2, M2_2, M3_2, M4_2] = _Boost::math::statistics::first_four_moments(v);
...
void test_median()
{
    ...
    Real m = _Boost::math::statistics::median(v.begin(), v.end());
    BOOST_TEST_EQ(m, 4);

    std::shuffle(v.begin(), v.end(), g);
    // Does range call work?
    m = _Boost::math::statistics::median(v);
    BOOST_TEST_EQ(m, 4);

    v = {1,2,3,3,4,5};
    m = _Boost::math::statistics::median(v.begin(), v.end());
    BOOST_TEST_EQ(m, 3);
    std::shuffle(v.begin(), v.end(), g);
    m = _Boost::math::statistics::median(v.begin(), v.end());
    BOOST_TEST_EQ(m, 3);
    ...
    m = _Boost::math::statistics::median(w);
    BOOST_TEST_EQ(m, 2);

    // Does it work with ublas?
    _Boost::numeric::ublas::vector<Real> w1(3);
    w1[0] = 1;
    w1[1] = 2;
    w1[2] = 3;
    m = _Boost::math::statistics::median(w);
...
void test_median_absolute_deviation()
{
    ...
    Real m = _Boost::math::statistics::median_absolute_deviation(v.begin(), v.end(), 0);
    BOOST_TEST_EQ(m, 4);

    std::mt19937 g(12);
    std::shuffle(v.begin(), v.end(), g);
    m = _Boost::math::statistics::median_absolute_deviation(v, 0);
    BOOST_TEST_EQ(m, 4);

    v = {1, -2, -3, 3, -4, -5};
    m = _Boost::math::statistics::median_absolute_deviation(v.begin(), v.end(), 0);
    BOOST_TEST_EQ(m, 3);
    std::shuffle(v.begin(), v.end(), g);
    m = _Boost::math::statistics::median_absolute_deviation(v.begin(), v.end(), 0);
    BOOST_TEST_EQ(m, 3);
    ...
    m = _Boost::math::statistics::median_absolute_deviation(v.begin(), v.end());
    BOOST_TEST_EQ(m, 1);

    m = _Boost::math::statistics::median_absolute_deviation(v);
    BOOST_TEST_EQ(m, 1);
    ...
    std::array<Real, 3> w{1, 2, -3};
    m = _Boost::math::statistics::median_absolute_deviation(w, 0);
    BOOST_TEST_EQ(m, 2);

    // boost.ublas vector?
    _Boost::numeric::ublas::vector<Real> u(6);
    u[0] = 1;
    u[1] = 2;
    u[2] = -3;
    u[3] = 1;
    u[4] = 2;
    u[5] = -3;
    m = _Boost::math::statistics::median_absolute_deviation(u, 0);
    BOOST_TEST_EQ(m, 2);
}
...
void test_sample_gini_coefficient()
{
    ...
    Real gini = _Boost::math::statistics::sample_gini_coefficient(v.begin(), v.end());
    BOOST_TEST(abs(gini - 1) < tol);

    gini = _Boost::math::statistics::sample_gini_coefficient(v);
    BOOST_TEST(abs(gini - 1) < tol);

    v[0] = 1;
    v[1] = 1;
    v[2] = 1;
    gini = _Boost::math::statistics::sample_gini_coefficient(v.begin(), v.end());
    ...
    gini = _Boost::math::statistics::sample_gini_coefficient(w);
    BOOST_TEST(abs(gini) < tol);
}
...
void test_gini_coefficient()
{
    Real tol = std::numeric_limits<Real>::epsilon();
    std::vector<Real> v{1,0,0};
    Real gini = _Boost::math::statistics::gini_coefficient(v.begin(), v.end());
    Real expected = Real(2)/Real(3);
    BOOST_TEST(abs(gini - expected) < tol);

    gini = _Boost::math::statistics::gini_coefficient(v);
    BOOST_TEST(abs(gini - expected) < tol);

    v[0] = 1;
    v[1] = 1;
    v[2] = 1;
    gini = _Boost::math::statistics::gini_coefficient(v.begin(), v.end());
    BOOST_TEST(abs(gini) < tol);
    ...
    std::array<Real, 3> w{0,0,0};
    gini = _Boost::math::statistics::gini_coefficient(w);
    BOOST_TEST(abs(gini) < tol);
    ...
    w1[1] = 1;
    w1[2] = 1;
    gini = _Boost::math::statistics::gini_coefficient(w1);
    BOOST_TEST(abs(gini) < tol);
...
void test_integer_gini_coefficient()
{
    ...
    double gini = _Boost::math::statistics::gini_coefficient(v.begin(), v.end());
    ...
    _Boost::numeric::ublas::vector<Z> w1(3);
    w1[0] = 1;
    w1[1] = 1;
    w1[2] = 1;
    gini = _Boost::math::statistics::gini_coefficient(w1);
...
void test_interquartile_range()
{
    ...
    iqr = _Boost::math::statistics::interquartile_range(v);
