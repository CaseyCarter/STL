...
BOOST_TYPEOF_REGISTER_TYPE(_Boost::math::concepts::real_concept)
...
void test_pow(T base)
{
    ...
    else
    {
       BOOST_CHECK_CLOSE(math::pow<N>(base),
              ...
              _Boost::math::tools::epsilon<result_type>() * 100 * 400); // 400 eps as a %
    }
}
...
void test_error_policy()
{
    ...
    BOOST_CHECK(pow<-2>(
                    0.0,
                    policy< ::_Boost::math::policies::overflow_error<user_error> >()
                )
                == 123.45);

    BOOST_CHECK(pow<0>(
                    0.0,
                    policy< ::_Boost::math::policies::indeterminate_result_error<user_error> >()
                )
                == 456.78);
}
...
BOOST_AUTO_TEST_CASE( test_main )
{
    ...
    test_with_small_exponents<_Boost::math::concepts::real_concept, 1>();
    ...
    test_with_big_exponents<_Boost::math::concepts::real_concept, 1>();
