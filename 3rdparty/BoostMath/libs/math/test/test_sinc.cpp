...
{
   T transition = 3.3f * _Boost::math::tools::forth_root_epsilon<T>();
   T val = transition;

   for (unsigned i = 0; i < 100; ++i)
   {
      _Boost::multiprecision::cpp_bin_float_50 extended = val;
      extended = sin(extended) / extended;
      T expected = extended.template convert_to<T>();
      T result = _Boost::math::sinc_pi(val);
      BOOST_CHECK_LE(_Boost::math::epsilon_difference(result, expected), 3);
      result = _Boost::math::sinc_pi(-val);
      BOOST_CHECK_LE(_Boost::math::epsilon_difference(result, expected), 3);
      val = _Boost::math::float_prior(val);
   }
   for (unsigned i = 0; i < 100; ++i)
   {
      _Boost::multiprecision::cpp_bin_float_50 extended = val;
      ...
      result = _Boost::math::sinc_pi(-val);
      BOOST_CHECK_LE(_Boost::math::epsilon_difference(result, expected), 3);
      val = _Boost::math::float_next(val);
   }
}
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   test_sinc(_Boost::math::concepts::real_concept(0.1), "real_concept");
#endif
#else
