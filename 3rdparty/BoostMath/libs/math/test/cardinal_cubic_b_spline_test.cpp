...
void test_b3_spline()
{
    std::cout << "Testing evaluation of spline basis functions on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    // Outside the support:
    Real eps = std::numeric_limits<Real>::epsilon();
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline<Real>(2.5), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline<Real>(-2.5), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_prime<Real>(2.5), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_prime<Real>(-2.5), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_double_prime<Real>(2.5), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_double_prime<Real>(-2.5), (Real) 0);


    // On the boundary of support:
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline<Real>(2), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline<Real>(-2), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_prime<Real>(2), (Real) 0);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_prime<Real>(-2), (Real) 0);

    // Special values:
    BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline<Real>(-1), third<Real>()*half<Real>(), eps);
    BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline<Real>( 1), third<Real>()*half<Real>(), eps);
    BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline<Real>(0), 2*third<Real>(), eps);

    BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline_prime<Real>(-1), half<Real>(), eps);
    BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline_prime<Real>( 1), -half<Real>(), eps);
    BOOST_CHECK_SMALL(_Boost::math::interpolators::detail::b3_spline_prime<Real>(0), eps);

    // Properties: B3 is an even function, B3' is an odd function.
    for (size_t i = 1; i < 200; ++i)
    {
        Real arg = i*0.01;
        BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline<Real>(arg), _Boost::math::interpolators::detail::b3_spline<Real>(arg), eps);
        BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline_prime<Real>(-arg), -_Boost::math::interpolators::detail::b3_spline_prime<Real>(arg), eps);
        BOOST_CHECK_CLOSE(_Boost::math::interpolators::detail::b3_spline_double_prime<Real>(-arg), _Boost::math::interpolators::detail::b3_spline_double_prime<Real>(arg), eps);
    }
...
{
    using std::sqrt;
    std::cout << "Testing interpolation condition for cubic b splines on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    std::random_device rd;
    std::mt19937 gen(rd());
    _Boost::random::uniform_real_distribution<Real> dis(1, 10);
    ...
    Real step = 0.01;
    Real a = 5;
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> spline(v.data(), v.size(), a, step);
...
{
    std::cout << "Testing that constants are interpolated correctly by cubic b splines on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    spline = _Boost::math::interpolators::cardinal_cubic_b_spline<Real>(v.data(), v.size(), a, step, 0, 0);
    ...
    //
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> spline2(v.begin(), v.end(), a, step);
    ...
    // Test that correctly specified left and right-derivatives work properly:
    spline2 = _Boost::math::interpolators::cardinal_cubic_b_spline<Real>(v.begin(), v.end(), a, step, 0, 0);
...
{
    using std::sqrt;
    std::cout << "Testing that affine functions are interpolated correctly by cubic b splines on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    std::vector<Real> v(500);
    Real a = 10;
    Real b = 8;
    Real step = 0.005;
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> spline(v.data(), v.size(), 0, step);
    ...
    // Test that correctly specified left and right-derivatives work properly:
    spline = _Boost::math::interpolators::cardinal_cubic_b_spline<Real>(v.data(), v.size(), 0, step, a, a);
...
{
    using std::sqrt;
    std::cout << "Testing that quadratic functions are interpolated correctly by cubic b splines on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    std::vector<Real> v(500);
    Real a = 1.2;
    Real b = -3.4;
    Real c = -8.6;
...
{
    std::cout << "Testing that sine functions are interpolated correctly by cubic b splines on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    std::mt19937 gen;
    std::vector<Real> v(500);
    Real x0 = 1;
    Real step = 0.125;
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> spline(v.data(), v.size(), x0, step);

    _Boost::random::uniform_real_distribution<Real> absissa(x0, x0 + 499 * step);
...
void test_copy_move()
{
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> d;
    d = _Boost::math::interpolators::cardinal_cubic_b_spline<Real>(v.data(), v.size(), x0, step);
    ...
    _Boost::math::interpolators::cardinal_cubic_b_spline<Real> c = d;
    BOOST_CHECK_CLOSE(c(x0), sin(x0), 0.01);

    // Test with std::bind:
    auto h = std::bind(&_Boost::math::interpolators::cardinal_cubic_b_spline<double>::operator(), &s, std::placeholders::_1);
