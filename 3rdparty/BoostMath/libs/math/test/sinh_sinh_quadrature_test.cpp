...
using _Boost::math::quadrature::sinh_sinh;
using _Boost::math::constants::pi;
using _Boost::math::constants::pi_sqr;
...
std::pair<std::vector<std::vector<Real>>, std::vector<std::vector<Real>> > generate_constants(unsigned max_rows)
{
   ...
   Real t_max = log(2 * two_div_pi<Real>()*log(2 * two_div_pi<Real>()*sqrt(_Boost::math::tools::max_value<TargetType>())));
...
void test_nr_examples()
{
    ...
    Real integration_limit = sqrt(_Boost::math::tools::epsilon<Real>());
    ...
    auto f2 = [](const Real& x)->Real { return fabs(x) > _Boost::math::tools::log_max_value<Real>() ? 0 : exp(-x*x); };
#else
...
void test_dirichlet_eta()
{
  ...
  std::cout << "Testing Dirichlet eta function on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
  Real tol = 10 * _Boost::math::tools::epsilon<Real>();
  Complex Q;
  sinh_sinh<Real> integrator(10);
  ...
  Complex Q_expected = {_Boost::lexical_cast<Real>("0.726559775062463263201495728547241386311129502735725787103568290594808442332084045617744978600192784188182345866652233650512117834307254514480657408096"),
                        _Boost::lexical_cast<Real>("0.158095863901207324355426285544321998253687969756843115763682522207208309489794631247865357375538028170751576870244296106203144195376645765556607038775")};
...
BOOST_AUTO_TEST_CASE(sinh_sinh_quadrature_test)
{
    ...
    */
    ...
    test_dirichlet_eta<_Boost::multiprecision::cpp_complex_quad>();
#ifndef BOOST_MATH_NO_LONG_DOUBLE_MATH_FUNCTIONS
