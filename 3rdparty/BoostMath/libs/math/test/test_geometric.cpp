...
using _Boost::math::geometric_distribution;
using _Boost::math::geometric; // using typedef for geometric_distribution<double>
...
{
   _Boost::math::geometric_distribution<RealType> g(p);
  ...
  if((P < 0.99) && (Q < 0.99))
  {
    ...
    else
    {
        ...
        && (_Boost::is_floating_point<RealType>::value))
      {
        ...
        BOOST_CHECK(quantile(g, P) < _Boost::math::tools::epsilon<RealType>() * 10);
      }
    }
    if(k != 0)
    {
    ...
    else
    {
        ...
        && (_Boost::is_floating_point<RealType>::value))
      {
        // Limit where this is checked: if exponent range is very large we may
        // run out of iterations in our root finding algorithm.
        BOOST_CHECK(quantile(complement(g, Q)) < _Boost::math::tools::epsilon<RealType>() * 10);
      }
    }
  } //   if((P < 0.99) && (Q < 0.99))
...
void test_spots(RealType)
{
  ...
  RealType tol1eps = _Boost::math::tools::epsilon<RealType>(); // Very tight, suit exact values.
  //RealType tol2eps = _Boost::math::tools::epsilon<RealType>() * 2; // Tight,  values.
