...
#include <boost/math/distributions/logistic.hpp>
    using _Boost::math::logistic_distribution;
...
void test_spots(RealType T)
{
   ...
   BOOST_CHECK_EQUAL(cdf(logistic_distribution<RealType>(), +_Boost::math::tools::max_value<RealType>()), 1); // x = + infinity, cdf = 1
   BOOST_CHECK_EQUAL(cdf(logistic_distribution<RealType>(), -_Boost::math::tools::max_value<RealType>()), 0); // x = - infinity, cdf = 0
   BOOST_CHECK_EQUAL(cdf(complement(logistic_distribution<RealType>(), +_Boost::math::tools::max_value<RealType>())), 0); // x = + infinity, c cdf = 0
   BOOST_CHECK_EQUAL(cdf(complement(logistic_distribution<RealType>(), -_Boost::math::tools::max_value<RealType>())), 1); // x = - infinity, c cdf = 1
   BOOST_CHECK_EQUAL(pdf(logistic_distribution<RealType>(), +_Boost::math::tools::max_value<RealType>()), 0); // x = + infinity, pdf = 0
   BOOST_CHECK_EQUAL(pdf(logistic_distribution<RealType>(), -_Boost::math::tools::max_value<RealType>()), 0); // x = - infinity, pdf = 0
  ...
  // Added test for Trac https://svn.boost.org/trac/boost/ticket/9126#comment:1
   ...
   //skewness
   BOOST_CHECK_CLOSE(
      ::_Boost::math::skewness(
      logistic_distribution<RealType>(2,1)      
      ),//x
      static_cast<RealType>(0),              // probability
      tolerance);
   BOOST_CHECK_CLOSE(
      ::_Boost::math::kurtosis_excess(
      logistic_distribution<RealType>(2,1)      
      ),//x
...
BOOST_AUTO_TEST_CASE( test_main )
{
  // Check that can generate logistic distribution using the two convenience methods:
   _Boost::math::logistic mycexp1(1.); // Using typedef
