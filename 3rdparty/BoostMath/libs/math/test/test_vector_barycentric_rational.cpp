...
void test_agreement_with_1d()
{
    std::cout << "Testing with 1D interpolation on type "
              << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    ...
    _Boost::random::uniform_real_distribution<Real> dis2(t[0], t[t.size()-1]);
    _Boost::math::vector_barycentric_rational<decltype(t), decltype(y)> interpolator(std::move(t), std::move(y));
    _Boost::math::barycentric_rational<Real> scalar_interpolator0(std::move(t_copy0), std::move(y_copy0));
    _Boost::math::barycentric_rational<Real> scalar_interpolator1(std::move(t_copy1), std::move(y_copy1));
...
void test_interpolation_condition_ublas()
{
    ...
    _Boost::random::uniform_real_distribution<Real> dis(0.1f, 1);
    std::vector<Real> t(100);
    std::vector<_Boost::numeric::ublas::vector<Real>> y(100);
    t[0] = dis(gen);
    y[0].resize(2);
    y[0][0] = dis(gen);
    y[0][1] = dis(gen);
    ...
    std::vector<Real> t_copy = t;
    std::vector<_Boost::numeric::ublas::vector<Real>> y_copy = y;

    _Boost::math::vector_barycentric_rational<decltype(t), decltype(y)> interpolator(std::move(t), std::move(y));

    _Boost::numeric::ublas::vector<Real> z(2);
    for (size_t i = 0; i < t_copy.size(); ++i)
    {
        interpolator(z, t_copy[i]);
        BOOST_CHECK_CLOSE(z[0], y_copy[i][0], 100*numeric_limits<Real>::epsilon());
...
void test_interpolation_condition_high_order()
{
    ...
    std::vector<Eigen::Vector2d> y_copy = y;
    std::vector<Real> t_copy = t;
    _Boost::math::vector_barycentric_rational<decltype(t), decltype(y)> interpolator(std::move(t), std::move(y), 5);
...
void test_weights()
{
    ...
    std::vector<Eigen::Vector2d> y_copy = y;
    std::vector<Real> t_copy = t;
    _Boost::math::detail::vector_barycentric_rational_imp<decltype(t), decltype(y)> interpolator(std::move(t), std::move(y), 1);
