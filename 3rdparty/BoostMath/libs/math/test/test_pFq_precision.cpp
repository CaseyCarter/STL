...
void test_spots_1F0()
{
   ...
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(2), 20), mp_type(-1), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(4), 20), mp_type(-27), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(0.5), 20), mp_type(0.125), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(0.5), 20), mp_type(8), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(2), 20), mp_type(-1), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(4), 20), mp_type(mp_type(-1) / 27), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(-0.5), 20), pow(mp_type(1.5), -3), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(-2), 20), mp_type(1 / mp_type(27)), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(-4), 20), mp_type(mp_type(1) / 125), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(-0.5), 20), pow(mp_type(1.5), 3), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(-2), 20), mp_type(27), tolerance);
   BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3) }, {}, mp_type(-4), 20), mp_type(125), tolerance);

   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({ mp_type(3) }, {}, mp_type(1), 20), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({ mp_type(3.25) }, {}, mp_type(1), 20), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({ mp_type(3.25) }, {}, mp_type(2), 20), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({ mp_type(-3.25) }, {}, mp_type(2), 20), std::domain_error);
}
...
void test_spots_0F1()
{
   ...
   BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(3) }, mp_type(0), 20), 1);
   BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(-3) }, mp_type(0), 20), 1);
   //BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(0) }, mp_type(0), 20), 1);

   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(0) }, mp_type(-1), 20), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(-1) }, mp_type(-1), 20), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq_precision({}, { mp_type(-10) }, mp_type(-5), 20), std::domain_error);

   static const _Boost::array<_Boost::array<mp_type, 3>, 35> hypergeometric_pFq_integer_data = { {
   ...
   {
      BOOST_CHECK_CLOSE_FRACTION(_Boost::math::hypergeometric_pFq_precision({}, { (*row)[0] }, (*row)[1], 20), (*row)[2], tolerance);
   }
}
...
#include "hypergeometric_1F1.ipp"
   ...
   for (auto row = hypergeometric_1F1.begin(); row != hypergeometric_1F1.end(); ++row)
   {
      try {
         mp_type norm;
         mp_type result = _Boost::math::hypergeometric_pFq_precision({ (*row)[0] }, { (*row)[1] }, (*row)[2], 20);
...
#include "hypergeometric_2F1.ipp"
   ...
   {
      try {
         mp_type result = _Boost::math::hypergeometric_pFq_precision({ (*row)[0], (*row)[1] }, { (*row)[2] }, (*row)[3], 20);
...
#include "hypergeometric_0F2.ipp"
   ...
   {
      try {
         T result = _Boost::math::hypergeometric_pFq_precision({}, { (*row)[0], (*row)[1] }, (*row)[2], 20);
         BOOST_CHECK_CLOSE_FRACTION(result, (*row)[3], tolerance);
      }
      catch (const _Boost::math::evaluation_error&) {}
   }
...
#include "hypergeometric_2F2.ipp"
   ...
   {
      try {
         mp_type result = _Boost::math::hypergeometric_pFq_precision({ (*row)[0], (*row)[1] }, { (*row)[2], (*row)[3] }, (*row)[4], 20);
