...
using _Boost::math::quadrature::exp_sinh;
...
std::pair<std::vector<std::vector<Real>>, std::vector<std::vector<Real>> > generate_constants(unsigned max_rows)
{
   ...
   Real tmp = (Real(_Boost::math::tools::log_min_value<TargetType>()) + log(Real(_Boost::math::tools::epsilon<TargetType>())))*0.5f;
   ...
   const Real t_max = log(2 * two_div_pi<Real>()*log(2 * two_div_pi<Real>()*sqrt(Real(_Boost::math::tools::max_value<TargetType>()))));
...
void test_right_limit_infinite()
{
    ...
    Q = integrator.integrate(f3, 10, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("-6.6976341310426674140007086979326069121526743314567805278252392932e-6");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, 10 * tol);
    // Integrating through zero risks precision loss:
    Q = integrator.integrate(f3, -10, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("-15232.3213626280525704332288302799653087046646639974940243044623285817777006");
    ...
    Q = integrator.integrate(f4, 20, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("0.0499583957219427614100062870348448814912770804235071744108534548299835954767");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
    Q = integrator.integrate(f4, 500, std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>(), get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("0.0019999973333397333150476759363217553199063513829126652556286269630");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
}
...
{
    std::cout << "Testing left limit infinite for 1/(1+t^2) on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    Real Q;
    Real Q_expected;
    Real error;
    ...
    Q = integrator.integrate(f1, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), 0, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
    Q = integrator.integrate(f1, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), -20, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("0.0499583957219427614100062870348448814912770804235071744108534548299835954767");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
    Q = integrator.integrate(f1, std::numeric_limits<Real>::has_infinity ? -std::numeric_limits<Real>::infinity() : -_Boost::math::tools::max_value<Real>(), -500, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::lexical_cast<Real>("0.0019999973333397333150476759363217553199063513829126652556286269630");
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
}
...
void test_nr_examples()
{
    ...
    using std::exp;
    using std::sqrt;
    std::cout << "Testing type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    ...
    auto f2 = [](Real x)->Real { return x > _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(pow(x, -(Real) 2/(Real) 7)*exp(-x*x)); };
    Q = integrator.integrate(f2, get_convergence_tolerance<Real>(), &error, &L1);
    ...
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, 10*_Boost::math::tools::epsilon<Real>());
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, 10*_Boost::math::tools::epsilon<Real>());

    auto f4 = [](const Real& t)->Real { return  t > _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(exp(-t*t*half<Real>())); };
...
void test_crc()
{
    ...
    using std::log;
    using std::cos;
    std::cout << "Testing integral from CRC handbook on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = 10 * _Boost::math::tools::epsilon<Real>();
    std::cout << std::setprecision(std::numeric_limits<Real>::digits10);
    Real Q;
    Real Q_expected;
    ...
    auto f0 = [](const Real& x)->Real { return x > _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(log(x)*exp(-x)); };
    Q = integrator.integrate(f0, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = -_Boost::math::constants::euler<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    ...
    Q = integrator.integrate(f1, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::math::tgamma(12.0f);
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    ...
    auto f2 = [](Real t)->Real {
        Real x = 12*cosh(t);
        if (x > _Boost::math::tools::log_max_value<Real>())
        {
            return (Real) 0;
        }
    ...
    Q = integrator.integrate(f2, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = _Boost::math::cyl_bessel_k<int, Real>(5, 12);
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    ...
    if (std::numeric_limits<Real>::is_specialized)
    {
       ...
       auto f4 = [&](Real t)->Real {
          ...
          return _Boost::math::cyl_bessel_j(0, t) * exp(-x);
       };
    ...
    auto f6 = [](const Real& t)->Real { return t > _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(exp(-t*t)*log(t));};
    Q = integrator.integrate(f6, get_convergence_tolerance<Real>(), &error, &L1);
    Q_expected = -_Boost::math::constants::root_pi<Real>()*(_Boost::math::constants::euler<Real>() + 2*ln_two<Real>())/4;
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    ...
    // over (0, INF).
    tol *= _Boost::math::tools::digits<Real>() > 100 ? 100000 : 75;
    for (Real pn = 99; pn > 0; pn -= 10) {
       Real p = pn / 100;
       auto f = [&](Real x)->Real
       {
          return x > 1000 * _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(exp(-x * (1 - p) + p * log(-_Boost::math::expm1(-x))));
       };
       Q = integrator.integrate(f, get_convergence_tolerance<Real>(), &error, &L1);
       Q_expected = 1 / _Boost::math::sinc_pi(p*pi<Real>());
       /*
    ...
    for (Real p = -0.99; p < 0; p += 0.1) {
       auto f = [&](Real x)->Real
       {
          return x > 1000 * _Boost::math::tools::log_max_value<Real>() ? Real(0) : Real(exp(-p * log(-_Boost::math::expm1(-x)) - (1 + p) * x));
       };
       Q = integrator.integrate(f, get_convergence_tolerance<Real>(), &error, &L1);
       Q_expected = 1 / _Boost::math::sinc_pi(p*pi<Real>());
       BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
...
{
    std::cout << "Testing complex modified Bessel function on type " << _Boost::typeindex::type_id<Complex>().pretty_name() << "\n";
    typedef typename Complex::value_type Real;
    Real tol = 100 * _Boost::math::tools::epsilon<Real>();
    Real error;
    Real L1;
    auto integrator = get_integrator<Real>();
    ...
    const auto f = [&z](const Real& t)->Complex
    {
        ...
        using std::exp;
        Real cosht = cosh(t);
        if (cosht > _Boost::math::tools::log_max_value<Real>())
        {
            return Complex{0, 0};
        }
        Complex arg = -z*cosht;
    ...
    Real K0_x_expected = _Boost::lexical_cast<Real>("-0.08296852656762551490517953520589186885781541203818846830385526187936132191822538822296497597191327722262903004145527496422090506197776994");
    Real K0_y_expected = _Boost::lexical_cast<Real>("0.027949603635183423629723306332336002340909030265538548521150904238352846705644065168365102147901993976999717171115546662967229050834575193041");
...
void test_complex_exponential_integral_E1(){
    std::cout << "Testing complex exponential integral on type " << _Boost::typeindex::type_id<Complex>().pretty_name() << "\n";
    typedef typename Complex::value_type Real;
    Real tol = 100 * _Boost::math::tools::epsilon<Real>();
    Real error;
    Real L1;
    ...
    Real inf = std::numeric_limits<Real>::has_infinity ? std::numeric_limits<Real>::infinity() : _Boost::math::tools::max_value<Real>();
   ...
   // Mathematica code: N[ExpIntegral[1,1.5 + 0.5 I],140]
    Real E1_real_expected = _Boost::lexical_cast<Real>("0.071702995463938694845949672113596046091766639758473558841839765788732549949008866887694451956003503764943496943262401868244277788066634858393");
    Real E1_imag_expected = _Boost::lexical_cast<Real>("-0.065138628279238400564373880665751377423524428792583839078600260273866805818117625959446311737353882269129094759883720722150048944193926087208");
...
BOOST_AUTO_TEST_CASE(exp_sinh_quadrature_test)
{
   ...
   */
...
#endif
#ifdef TEST6
    test_left_limit_infinite<_Boost::multiprecision::cpp_bin_float_50>();
    test_right_limit_infinite<_Boost::multiprecision::cpp_bin_float_50>();
    test_nr_examples<_Boost::multiprecision::cpp_bin_float_50>();
    test_crc<_Boost::multiprecision::cpp_bin_float_50>();
#endif
#ifdef TEST7
    test_left_limit_infinite<_Boost::multiprecision::cpp_dec_float_50>();
    test_right_limit_infinite<_Boost::multiprecision::cpp_dec_float_50>();
    test_nr_examples<_Boost::multiprecision::cpp_dec_float_50>();
    //
    ...
    test_crc<_Boost::multiprecision::cpp_dec_float_50>();
#endif
#endif
#ifdef TEST8
    ...
    #ifdef BOOST_HAS_FLOAT128
        test_complex_modified_bessel<_Boost::multiprecision::complex128>();
    #endif
    test_complex_modified_bessel<_Boost::multiprecision::cpp_complex_quad>();
#endif
#ifdef TEST9
    ...
    test_complex_exponential_integral_E1<std::complex<long double>>();
      #ifdef BOOST_HAS_FLOAT128
         test_complex_exponential_integral_E1<_Boost::multiprecision::complex128>();
      #endif
    test_complex_exponential_integral_E1<_Boost::multiprecision::cpp_complex_quad>();
#endif
}
