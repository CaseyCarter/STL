...
using _Boost::math::negative_binomial_distribution;

#include <boost/math/special_functions/gamma.hpp>
  using _Boost::math::lgamma;  // log gamma
...
{
   _Boost::math::negative_binomial_distribution<RealType> bn(N, p);
  ...
  if((P < 0.99) && (Q < 0.99))
  {
    ...
    else
    {
        ...
        && (_Boost::is_floating_point<RealType>::value))
      {
        ...
        BOOST_CHECK(quantile(bn, P) < _Boost::math::tools::epsilon<RealType>() * 10);
      }
    }
    if(k != 0)
    {
    ...
    else
    {
        ...
        && (_Boost::is_floating_point<RealType>::value))
      {
        // Limit where this is checked: if exponent range is very large we may
        // run out of iterations in our root finding algorithm.
        BOOST_CHECK(quantile(complement(bn, Q)) < _Boost::math::tools::epsilon<RealType>() * 10);
      }
    }
...
void test_spots(RealType)
{
  ...
  RealType tol1eps = _Boost::math::tools::epsilon<RealType>() * 2; // Very tight, suit exact values.
  //RealType tol2eps = _Boost::math::tools::epsilon<RealType>() * 2; // Tight, suit exact values.
  RealType tol5eps = _Boost::math::tools::epsilon<RealType>() * 5; // Wider 5 epsilon.
  ...
  using _Boost::math::negative_binomial_distribution;
  using  ::_Boost::math::negative_binomial;
...
{ // An extreme value test that takes 3 minutes using the real concept type
 ...
 // End of single spot tests using RealType
  ...
  2 * _Boost::math::tools::epsilon<RealType>() ); // Expect exact, but not quite.
  ...
  negative_binomial_distribution<RealType>(static_cast<RealType>(8), static_cast<RealType>(0.25)),
  static_cast<RealType>(_Boost::math::tools::min_value<RealType>())),
  static_cast<RealType>(0));

  BOOST_CHECK_CLOSE_FRACTION(
  quantile(  // Small P < cdf(0) so should be near zero.
  negative_binomial_distribution<RealType>(static_cast<RealType>(8), static_cast<RealType>(0.25)),
  static_cast<RealType>(_Boost::math::tools::epsilon<RealType>())), // 
  static_cast<RealType>(0),
    tol5eps);
  ...
  //negative_binomial_distribution<RealType>(static_cast<RealType>(8), static_cast<RealType>(0.25)),
  //static_cast<RealType>(1 - _Boost::math::tools::epsilon<RealType>())) <=
  ...
  { // BOOST_CHECK tests for infinity using std::numeric_limits<>::infinity()
    ...
    negative_binomial_distribution<RealType>(static_cast<RealType>(8), static_cast<RealType>(0.25)),
    static_cast<RealType>(1)) ==
    //static_cast<RealType>(_Boost::math::tools::infinity<RealType>())
...
#include "negative_binomial_quantile.ipp"
  ...
  for(unsigned i = 0; i < negative_binomial_quantile_data.size(); ++i)
  {
     ...
     typedef policy<discrete_quantile<_Boost::math::policies::real> > P1;
     ...
     RealType tol = _Boost::math::tools::epsilon<RealType>() * 700;
