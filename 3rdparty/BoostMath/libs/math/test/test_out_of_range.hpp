...
{ // Checks that support and function calls are within expected limits.
   ...
   if (Infinite == false)
   {
     if ((_Boost::math::isfinite)(range(d).first) && (range(d).first != -_Boost::math::tools::max_value<value_type>()))
     { // If possible, check that a random variable value just less than the bottom of the supported range throws domain errors.
       value_type m = (range(d).first == 0) ? -_Boost::math::tools::min_value<value_type>() : _Boost::math::float_prior(range(d).first);
     ...
     if ((_Boost::math::isfinite)(range(d).second) && (range(d).second != _Boost::math::tools::max_value<value_type>()))
     { // If possible, check that a random variable value just more than the top of the supported range throws domain errors.
       value_type m = (range(d).second == 0) ? _Boost::math::tools::min_value<value_type>() : _Boost::math::float_next(range(d).second);
       BOOST_ASSERT(m != range(d).first);
       BOOST_ASSERT(m > range(d).first);
       BOOST_MATH_CHECK_THROW(pdf(d, m), std::domain_error);
       BOOST_MATH_CHECK_THROW(cdf(d, m), std::domain_error);
     ...
     { // Infinity is available,
       if ((_Boost::math::isfinite)(range(d).second))
       ...
       if ((_Boost::math::isfinite)(range(d).first))
