...
#define BOOST_CHECK_CLOSE_EX(a, b, prec, i) \
      {\
      unsigned int failures = _Boost::unit_test::results_collector.results( _Boost::unit_test::framework::current_test_case().p_id ).p_assertions_failed;\
      BOOST_CHECK_CLOSE(a, b, prec); \
      if(failures != _Boost::unit_test::results_collector.results( _Boost::unit_test::framework::current_test_case().p_id ).p_assertions_failed)\
            {\
...
RealType naive_skewness(RealType v, RealType delta)
{
   ...
   r /= _Boost::math::constants::root_two<RealType>()
   ...
   RealType tol)   // Test tolerance
{
   ...
   _Boost::math::non_central_t_distribution<RealType> dist(df, ncp);
   BOOST_CHECK_CLOSE(
      cdf(dist, t), P, tol);
   ...
   */
   if((P < 0.99) && (Q < 0.99))
   {
      ...
      BOOST_CHECK_CLOSE(
      dist.find_degrees_of_freedom(_Boost::math::complement(ncp, t, Q)), df, tol * 10);
      BOOST_CHECK_CLOSE(
      dist.find_non_centrality(df, t, P), ncp, tol * 10);
      BOOST_CHECK_CLOSE(
      dist.find_non_centrality(_Boost::math::complement(df, t, Q)), ncp, tol * 10);
      */
   }
}
...
void test_spots(RealType)
{
   ...
   RealType tolerance = (std::max)(
      _Boost::math::tools::epsilon<RealType>(),
      (RealType)5e-12f) * 100;
   //
   ...
   //
   if(_Boost::math::tools::digits<RealType>() < 50)
      tolerance *= 50;
   if(_Boost::is_floating_point<RealType>::value != 1)
   ...
   */
   ...
   _Boost::math::non_central_t_distribution<RealType> dist(static_cast<RealType>(8), static_cast<RealType>(12));
   BOOST_CHECK_CLOSE(pdf(dist, 12), static_cast<RealType>(1.235329715425894935157684607751972713457e-1L), tolerance);
   BOOST_CHECK_CLOSE(pdf(_Boost::math::non_central_t_distribution<RealType>(126, -2), -4), static_cast<RealType>(5.797932289365814702402873546466798025787e-2L), tolerance);
   BOOST_CHECK_CLOSE(pdf(_Boost::math::non_central_t_distribution<RealType>(126, 2), 4), static_cast<RealType>(5.797932289365814702402873546466798025787e-2L), tolerance);
   BOOST_CHECK_CLOSE(pdf(_Boost::math::non_central_t_distribution<RealType>(126, 2), 0), static_cast<RealType>(5.388394890639957139696546086044839573749e-2L), tolerance);

   // Error handling checks:
   //check_out_of_range<_Boost::math::non_central_t_distribution<RealType> >(1, 1);  // Fails one check because df for this distribution *can* be infinity.
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_t_distribution<RealType>(0, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_t_distribution<RealType>(-1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_t_distribution<RealType>(1, 1), -1), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_t_distribution<RealType>(1, 1), 2), std::domain_error);
} // template <class RealType>void test_spots(RealType)

template <class T>
T nct_cdf(T df, T nc, T x)
{
   return cdf(_Boost::math::non_central_t_distribution<T>(df, nc), x);
}

template <class T>
T nct_ccdf(T df, T nc, T x)
{
   return cdf(complement(_Boost::math::non_central_t_distribution<T>(df, nc), x));
}
...
void quantile_sanity_check(T& data, const char* type_name, const char* test)
{
   ...
   //
   if(!_Boost::is_floating_point<value_type>::value)
      return;
   ...
   //
   value_type precision = static_cast<value_type>(ldexp(1.0, 1 - _Boost::math::policies::digits<value_type, _Boost::math::policies::policy<> >() / 2)) * 100;
   if(_Boost::math::policies::digits<value_type, _Boost::math::policies::policy<> >() < 50)
   ...
   for(unsigned i = 0; i < data.size(); ++i)
   {
      if(data[i][3] == 0)
      {
         BOOST_CHECK(0 == quantile(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), data[i][3]));
      }
      else if(data[i][3] < 0.9999f)
      {
         value_type p = quantile(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), data[i][3]);
         value_type pt = data[i][2];
         BOOST_CHECK_CLOSE_EX(pt, p, precision, i);
      }
      if(data[i][4] == 0)
      {
         BOOST_CHECK(0 == quantile(complement(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), data[i][3])));
      }
      else if(data[i][4] < 0.9999f)
      {
         value_type p = quantile(complement(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), data[i][4]));
         value_type pt = data[i][2];
         BOOST_CHECK_CLOSE_EX(pt, p, precision, i);
      }
      if(_Boost::math::tools::digits<value_type>() > 50)
      {
         //
...
#ifndef BOOST_NO_EXCEPTIONS
         try{
            value_type m = mode(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]));
            value_type p = pdf(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), m);
            value_type delta = (std::max)(fabs(m * sqrt(precision) * 50), sqrt(precision) * 50);
            BOOST_CHECK_EX(pdf(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), m + delta) <= p, i);
            BOOST_CHECK_EX(pdf(_Boost::math::non_central_t_distribution<value_type>(data[i][0], data[i][1]), m - delta) <= p, i);
         }
         catch(const _Boost::math::evaluation_error&) {}
#endif
#if 0
         //
         ...
         try{
            if((data[i][3] < 0.99) && (data[i][3] != 0))
            {
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_t_distribution<value_type>::find_degrees_of_freedom(data[i][1], data[i][2], data[i][3]),
                  data[i][0], precision, i);
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_t_distribution<value_type>::find_non_centrality(data[i][0], data[i][2], data[i][3]),
                  data[i][1], precision, i);
            }
            if((data[i][4] < 0.99) && (data[i][4] != 0))
            {
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_t_distribution<value_type>::find_degrees_of_freedom(_Boost::math::complement(data[i][1], data[i][2], data[i][4])),
                  data[i][0], precision, i);
               BOOST_CHECK_CLOSE_EX(
                  _Boost::math::non_central_t_distribution<value_type>::find_non_centrality(_Boost::math::complement(data[i][0], data[i][2], data[i][4])),
                  data[i][1], precision, i);
            }
         }
...
void test_big_df(RealType)
{
   ...
   if(typeid(RealType) != typeid(_Boost::math::concepts::real_concept))
   { // Ordinary floats only.
      // Could also test if (std::numeric_limits<RealType>::is_specialized);

      RealType tolerance = 10 * _Boost::math::tools::epsilon<RealType>(); // static_cast<RealType>(1e-14); //
      ...
      RealType inf =
         ...
         :
         _Boost::math::tools::max_value<RealType>();
      RealType nan = std::numeric_limits<RealType>::quiet_NaN();

      // Tests for df = max_value and infinity.
      RealType max_val = _Boost::math::tools::max_value<RealType>();
      ...
      BOOST_CHECK_CLOSE_FRACTION(pdf(maxdf, 0), _Boost::math::constants::one_div_root_two_pi<RealType>(), tolerance);
      BOOST_CHECK_CLOSE_FRACTION(pdf(infdf, 0), _Boost::math::constants::one_div_root_two_pi<RealType>(), tolerance);
      BOOST_CHECK_CLOSE_FRACTION(cdf(infdf, 0), _Boost::math::constants::half<RealType>(), tolerance);
      BOOST_CHECK_CLOSE_FRACTION(cdf(maxdf, 0), _Boost::math::constants::half<RealType>(), tolerance);
...
void test_ignore_policy(RealType)
{
   ...
   { // Ordinary floats only.
      ...
      typedef policy<
         _Boost::math::policies::domain_error<_Boost::math::policies::ignore_error>,
         _Boost::math::policies::overflow_error<_Boost::math::policies::ignore_error>,
         _Boost::math::policies::underflow_error<_Boost::math::policies::ignore_error>,
         _Boost::math::policies::denorm_error<_Boost::math::policies::ignore_error>,
         _Boost::math::policies::pole_error<_Boost::math::policies::ignore_error>,
         _Boost::math::policies::evaluation_error<_Boost::math::policies::ignore_error>
      > ignore_all_policy;
      ...
      {
         // Mean
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(-nan, 0))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(+nan, 0))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(-1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(0, 0))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(2, nan))));
         BOOST_CHECK((_Boost::math::isnan)(mean(ignore_error_non_central_t(nan, nan))));
         BOOST_CHECK(_Boost::math::isfinite(mean(ignore_error_non_central_t(2, 0)))); // OK

         // Variance
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(nan, 0))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(1, nan))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(2, nan))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(-1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(0, 0))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(static_cast<RealType>(1.7L), 0))));
         BOOST_CHECK((_Boost::math::isnan)(variance(ignore_error_non_central_t(2, 0))));

         // Skewness
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(std::numeric_limits<RealType>::quiet_NaN(), 0))));
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(-1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(0, 0))));
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(2, 0))));
         BOOST_CHECK((_Boost::math::isnan)(skewness(ignore_error_non_central_t(3, 0))));

         // Kurtosis
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(std::numeric_limits<RealType>::quiet_NaN(), 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(-1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(0, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(2, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(static_cast<RealType>(2.0001L), 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(3, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis(ignore_error_non_central_t(4, 0))));

         // Kurtosis excess
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(std::numeric_limits<RealType>::quiet_NaN(), 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(-1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(0, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(1, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(2, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(static_cast<RealType>(2.0001L), 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(3, 0))));
         BOOST_CHECK((_Boost::math::isnan)(kurtosis_excess(ignore_error_non_central_t(4, 0))));
      } // has_quiet_NaN
      BOOST_CHECK(_Boost::math::isfinite(mean(ignore_error_non_central_t(1 + std::numeric_limits<RealType>::epsilon(), 0))));
      BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_non_central_t(2 + 2 * std::numeric_limits<RealType>::epsilon(), 0))));
      BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_non_central_t(static_cast<RealType>(2.0001L), 0))));
      BOOST_CHECK(_Boost::math::isfinite(variance(ignore_error_non_central_t(2 + 2 * std::numeric_limits<RealType>::epsilon(), 0))));
      BOOST_CHECK(_Boost::math::isfinite(skewness(ignore_error_non_central_t(3 + 3 * std::numeric_limits<RealType>::epsilon(), 0))));
      BOOST_CHECK(_Boost::math::isfinite(kurtosis(ignore_error_non_central_t(4 + 4 * std::numeric_limits<RealType>::epsilon(), 0))));
      BOOST_CHECK(_Boost::math::isfinite(kurtosis(ignore_error_non_central_t(static_cast<RealType>(4.0001L), 0))));
