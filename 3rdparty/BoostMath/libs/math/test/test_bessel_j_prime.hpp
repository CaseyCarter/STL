...
void do_test_cyl_bessel_j_prime(const T& data, const char* type_name, const char* test_name)
{
   ...
   pg funcp = _Boost::math::cyl_bessel_j_prime<value_type, value_type>;
#else
   pg funcp = _Boost::math::cyl_bessel_j_prime;
#endif
...
T cyl_bessel_j_prime_int_wrapper(T v, T x)
{
   ...
   return static_cast<T>(BESSEL_JPN_FUNCTION_TO_TEST(_Boost::math::itrunc(v), x));
#else
   return static_cast<T>(_Boost::math::cyl_bessel_j_prime(_Boost::math::itrunc(v), x));
#endif
}
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   pg funcp = _Boost::math::sph_bessel_prime<value_type>;
#else
   pg funcp = _Boost::math::sph_bessel_prime;
#endif
...
void test_bessel_prime(T, const char* name)
{
   ...
   // 
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 8> j0_data = {{
    ...
    static const _Boost::array<_Boost::array<T, 3>, 6> j0_tricky = {{
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 8> j1_data = {{
    ...
    static const _Boost::array<_Boost::array<T, 3>, 5> j1_tricky = {{
        // Big numbers make the accuracy of std::sin the limiting factor:
    ...
    static const _Boost::array<_Boost::array<typename table_type<T>::type, 3>, 17> jn_data = {{
    ...
    static const _Boost::array<_Boost::array<T, 3>, 21> jv_data = {{
         {{ T(22.5), T(0), SC_(0.0) }},
         ...
         {{ SC_(8.5), _Boost::math::constants::pi<T>() * 4, SC_(-0.194590144622675911618596506265006877277074) }},
         {{ SC_(-8.5), _Boost::math::constants::pi<T>() * 4, SC_(-0.014516314554743677558496402742690038592728) }},
    }};
    do_test_cyl_bessel_j_prime<T>(jv_data, name, "Bessel J': Mathworld Data");
    static const _Boost::array<_Boost::array<T, 3>, 4> jv_large_data = {{
...
#include "sph_bessel_prime_data.ipp"
    ...
    // Some special cases:
    //
    BOOST_CHECK_EQUAL(_Boost::math::cyl_bessel_j_prime(T(1), T(0)), T(0.5));
    BOOST_CHECK_EQUAL(_Boost::math::cyl_bessel_j_prime(T(-1), T(0)), T(-0.5));
    BOOST_CHECK_EQUAL(_Boost::math::cyl_bessel_j_prime(T(2), T(0)), T(0));

    //
    // Special cases that are errors:
    //
    BOOST_MATH_CHECK_THROW(_Boost::math::sph_bessel_prime(1, T(0)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::sph_bessel_prime(100000, T(0)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_prime(T(-2.5), T(0)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_prime(T(-2.5), T(-2)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_prime(T(2.5), T(-2)), std::domain_error);
}
