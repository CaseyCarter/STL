...
BOOST_AUTO_TEST_CASE_TEMPLATE(atan_test, T, all_float_types) {
  ...
  auto y = atan(x);
  const auto eps = _Boost::math::tools::epsilon<T>();
  BOOST_CHECK_CLOSE(y.derivative(0u), _Boost::math::constants::pi<T>() / 4, eps);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(erf_test, T, all_float_types) {
  ...
  const T eps = 300 * 100 * _Boost::math::tools::epsilon<T>(); // percent
  const T cx = 1.0;
  constexpr unsigned m = 5;
  const auto x = make_fvar<T, m>(cx);
  auto y = erf(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sinc_test, T, bin_float_types) {
  BOOST_MATH_STD_USING
  const T eps = 20000 * _Boost::math::tools::epsilon<T>(); // percent
  const T cx = 1;
  constexpr unsigned m = 5;
  auto x = make_fvar<T, m>(cx);
  auto y = sinc(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sinh_and_cosh, T, bin_float_types) {
  BOOST_MATH_STD_USING
  const T eps = 300 * _Boost::math::tools::epsilon<T>(); // percent
  const T cx = 1;
  constexpr unsigned m = 5;
  auto x = make_fvar<T, m>(cx);
  auto s = sinh(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(tanh_test, T, all_float_types) {
  ...
  auto x = make_fvar<T, m>(cx);
  auto t = tanh(x);
  for (auto i : _Boost::irange(tanh_derivatives.size())) {
    BOOST_TEST_WARN(isNearZero(t.derivative(i) -
                               _Boost::lexical_cast<T>(tanh_derivatives[i])));
  }
}
#endif
...
BOOST_AUTO_TEST_CASE_TEMPLATE(tan_test, T, bin_float_types) {
  BOOST_MATH_STD_USING
  const T eps = 800 * _Boost::math::tools::epsilon<T>(); // percent
  const T cx = _Boost::math::constants::third_pi<T>();
  const T root_three = _Boost::math::constants::root_three<T>();
  constexpr unsigned m = 5;
  const auto x = make_fvar<T, m>(cx);
  auto y = tan(x);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(lambert_w0_test, T, all_float_types) {
  const T eps = 1000 * _Boost::math::tools::epsilon<T>(); // percent
  ...
  auto y = lambert_w0(x);
  for (auto i : _Boost::irange(m + 1)) {
    const T answer = _Boost::lexical_cast<T>(answers[i]);
    BOOST_CHECK_CLOSE(y.derivative(i), answer, eps);
  }
  // const T cx0 = -1 / _Boost::math::constants::e<T>();
...
BOOST_AUTO_TEST_CASE_TEMPLATE(tgamma2_test, T, all_float_types) {
  //const T eps = 5000 * _Boost::math::tools::epsilon<T>(); // ok for non-multiprecision
  const T eps = 500000 * _Boost::math::tools::epsilon<T>(); // percent
  constexpr unsigned m = 10;
  const T cx = -1.5;
