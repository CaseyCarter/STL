...
#include <boost/math/distributions/weibull.hpp>
    using _Boost::math::weibull_distribution;
...
void test_spots(RealType)
{
   ...
   tolerance = (std::max)(
      ...
      static_cast<RealType>(_Boost::math::tools::epsilon<double>())) * 5 * 100; // 5 eps as a percentage
   cout << "Tolerance for type " << typeid(RealType).name()  << " is " << tolerance << " %" << endl;
   ...
   BOOST_CHECK_CLOSE(
      mean(dist)
      , dist.scale() * _Boost::math::tgamma(1 + 1 / dist.shape()), tolerance);
   // variance:
   BOOST_CHECK_CLOSE(
      variance(dist)
      , dist.scale() * dist.scale() * _Boost::math::tgamma(1 + 2 / dist.shape()) - mean(dist) * mean(dist), tolerance);
   // std deviation:
   BOOST_CHECK_CLOSE(
   ...
   BOOST_CHECK_CLOSE(
    skewness(dist), 
    (_Boost::math::tgamma(1 + 3/dist.shape()) * pow(dist.scale(), RealType(3)) - 3 * mean(dist) * variance(dist) - pow(mean(dist), RealType(3))) / pow(standard_deviation(dist), RealType(3)), 
    tolerance * 100);
   // kurtosis:
   BOOST_CHECK_CLOSE(
    kurtosis(dist)
   ...
   BOOST_CHECK_CLOSE(
    ...
    (pow(dist.scale(), RealType(4)) * _Boost::math::tgamma(1 + 4/dist.shape()) 
   ...
   RealType expected_entropy = _Boost::math::constants::euler<RealType>()*(1-1/dist.shape()) + log(dist.scale()/dist.shape()) + 1;
   BOOST_CHECK_CLOSE(
    entropy(dist)
...
Output:
  ...
  Tolerance for type class _Boost::math::concepts::real_concept is 0.002 %
  Tolerance for type class _Boost::math::concepts::real_concept is 1.11022e-013 %
