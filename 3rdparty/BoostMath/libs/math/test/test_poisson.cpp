...
void test_spots(RealType)
{
 ...
 BOOST_CHECK_CLOSE(
  ...
  BOOST_CHECK_CLOSE(_Boost::math::quantile( // inverse of cdf above.
  ...
  BOOST_CHECK_EQUAL(_Boost::math::quantile( // Check case when probability < cdf(0) (== pdf(0))
...
#include "poisson_quantile.ipp"
  ...
  for(unsigned i = 0; i < poisson_quantile_data.size(); ++i)
  {
     ...
     RealType tol = _Boost::math::tools::epsilon<RealType>() * 20;
...
BOOST_AUTO_TEST_CASE( test_main )
{
  ...
  for (int i = 0; i <= 50; i++)
  {
   ...
   << cdf(poisson_distribution<double>(mean), static_cast<double>(i)) << ' ';
     {
       cout << _Boost::math::gamma_q<double>(i+1, mean); // cdf
       double diff = _Boost::math::gamma_q<double>(i+1, mean) - sum; // cdf -sum
