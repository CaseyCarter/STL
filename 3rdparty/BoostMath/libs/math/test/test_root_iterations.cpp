...
struct ibeta_roots_1   // for first order algorithms
{
   ...
   {
      return _Boost::math::detail::ibeta_imp(a, b, x, Policy(), invert, true) - target;
   }
private:
   T a, b, target;
   bool invert;
...
struct ibeta_roots_2   // for second order algorithms
{
   ...
   _Boost::math::tuple<T, T> operator()(const T& x)
   {
      typedef _Boost::math::lanczos::lanczos<T, Policy> S;
      ...
      return _Boost::math::make_tuple(f, f1);
   }
private:
   T a, b, target;
   bool invert;
...
struct ibeta_roots_3   // for third order algorithms
{
   ...
   _Boost::math::tuple<T, T, T> operator()(const T& x)
   {
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   double arg = 1e-50;
   _Boost::uintmax_t iters;
   double guess;
   double dr;

   while(arg < 1e50)
   {
      double result = _Boost::math::cbrt(arg);
      //
      ...
      iters = 1000;
      // TOMS algo first:
      std::pair<double, double> r = _Boost::math::tools::bracket_and_solve_root(cbrt_functor_noderiv(arg), guess, 2.0, true, _Boost::math::tools::eps_tolerance<double>(), iters);
      BOOST_CHECK_CLOSE_FRACTION((r.first + r.second) / 2, result, std::numeric_limits<double>::epsilon() * 4);
      BOOST_CHECK_LE(iters, 14);
      // Newton next:
      iters = 1000;
      dr = _Boost::math::tools::newton_raphson_iterate(cbrt_functor_deriv(arg), guess, guess / 2, result * 10, newton_limits, iters);
      BOOST_CHECK_CLOSE_FRACTION(dr, result, std::numeric_limits<double>::epsilon() * 2);
      BOOST_CHECK_LE(iters, 12);
      // Halley next:
      iters = 1000;
      dr = _Boost::math::tools::halley_iterate(cbrt_functor_2deriv(arg), guess, result / 10, result * 10, newton_limits, iters);
      BOOST_CHECK_CLOSE_FRACTION(dr, result, std::numeric_limits<double>::epsilon() * 2);
      BOOST_CHECK_LE(iters, 7);
      // Schroder next:
      iters = 1000;
      dr = _Boost::math::tools::schroder_iterate(cbrt_functor_2deriv(arg), guess, result / 10, result * 10, newton_limits, iters);
      BOOST_CHECK_CLOSE_FRACTION(dr, result, std::numeric_limits<double>::epsilon() * 2);
      ...
      iters = 1000;
      guess = result * 5;
      r = _Boost::math::tools::bracket_and_solve_root(cbrt_functor_noderiv(arg), guess, 2.0, true, _Boost::math::tools::eps_tolerance<double>(), iters);
      BOOST_CHECK_CLOSE_FRACTION((r.first + r.second) / 2, result, std::numeric_limits<double>::epsilon() * 4);
      BOOST_CHECK_LE(iters, 14);
      // Newton next:
      iters = 1000;
      dr = _Boost::math::tools::newton_raphson_iterate(cbrt_functor_deriv(arg), guess, result / 10, result * 10, newton_limits, iters);
      BOOST_CHECK_CLOSE_FRACTION(dr, result, std::numeric_limits<double>::epsilon() * 2);
      BOOST_CHECK_LE(iters, 12);
      // Halley next:
      iters = 1000;
...
#  include "ibeta_small_data.ipp"
   ...
   for (unsigned i = 0; i < ibeta_small_data.size(); ++i)
   {
      ...
      {
         iters = 1000;
         dr = _Boost::math::tools::newton_raphson_iterate(ibeta_roots_2<double, _Boost::math::policies::policy<> >(ibeta_small_data[i][0], ibeta_small_data[i][1], ibeta_small_data[i][5]), 0.5, 0.0, 1.0, 53, iters);
         BOOST_CHECK_EQUAL(dr, 0.0);
         BOOST_CHECK_LE(iters, 27);
         iters = 1000;
         dr = _Boost::math::tools::halley_iterate(ibeta_roots_3<double, _Boost::math::policies::policy<> >(ibeta_small_data[i][0], ibeta_small_data[i][1], ibeta_small_data[i][5]), 0.5, 0.0, 1.0, 53, iters);
         BOOST_CHECK_EQUAL(dr, 0.0);
         BOOST_CHECK_LE(iters, 10);
      }
      else if ((1 - ibeta_small_data[i][5] > 0.001)
         && (fabs(ibeta_small_data[i][5]) > 2 * _Boost::math::tools::min_value<double>()))
      {
         iters = 1000;
