...
using std::cos;
using std::string;
using _Boost::math::quadrature::gauss;
...
void test_linear()
{
    std::cout << "Testing linear functions are integrated properly by gauss on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
...
void test_quadratic()
{
    std::cout << "Testing quadratic functions are integrated properly by Gaussian quadrature on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    Real tol = _Boost::math::tools::epsilon<Real>() * 10;
...
void test_integration_over_real_line()
{
    ...
    Q = gauss<Real, Points>::integrate(f1, -_Boost::math::tools::max_value<Real>(), _Boost::math::tools::max_value<Real>(), &L1);
    Q_expected = pi<Real>();
    BOOST_CHECK_CLOSE_FRACTION(Q, Q_expected, tol);
    BOOST_CHECK_CLOSE_FRACTION(L1, Q_expected, tol);
}
...
{
    std::cout << "Testing right limit infinite for Gaussian quadrature in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    // Example 11:
    auto f1 = [](const Real& t) { return 1/(1+t*t);};
    Q = gauss<Real, Points>::integrate(f1, 0, _Boost::math::tools::max_value<Real>(), &L1);
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);

    auto f4 = [](const Real& t) { return 1/(1+t*t); };
    Q = gauss<Real, Points>::integrate(f4, 1, _Boost::math::tools::max_value<Real>(), &L1);
    Q_expected = pi<Real>()/4;
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
{
    std::cout << "Testing left limit infinite for Gaussian quadrature in 'A Comparison of Three High Precision Quadrature Schemes' on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    ...
    // Example 11:
    auto f1 = [](const Real& t) { return 1/(1+t*t);};
    Q = gauss<Real, Points>::integrate(f1, -_Boost::math::tools::max_value<Real>(), Real(0));
    Q_expected = half_pi<Real>();
    BOOST_CHECK_CLOSE(Q, Q_expected, 100*tol);
}
...
#ifdef BOOST_HAS_FLOAT128
    test_left_limit_infinite<_Boost::multiprecision::float128, 30>();
    test_complex_lambert_w<_Boost::multiprecision::complex128>();
#endif
    test_complex_lambert_w<_Boost::multiprecision::cpp_complex_quad>();
#endif
}
