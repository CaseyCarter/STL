...
void test_bessel_zeros(RealType)
{
  ...
  // and is an arbitrary zero for all tests.
   RealType tolerance = 5 * (std::max)(
     static_cast<RealType>(_Boost::math::tools::epsilon<long double>()),
   ...
   using _Boost::math::cyl_bessel_j_zero; // (nu, j)
  ...
  if (std::numeric_limits<RealType>::has_infinity)
  {
    ...
    // class _Boost::exception_detail::clone_impl<struct _Boost::exception_detail::error_info_injector<class std::domain_error> >:
    // Error in function _Boost::math::cyl_bessel_j_zero<double>(double, int): Order argument is 1.#INF, but must be finite >= 0 !
  ...
  //BOOST_MATH_CHECK_THROW(cyl_bessel_j_zero(_Boost::math::tools::max_value<RealType>(), 1), std::domain_error);
  // unknown location(0): fatal error in "test_main":
  // class _Boost::exception_detail::clone_impl<struct _Boost::exception_detail::error_info_injector<class _Boost::math::evaluation_error> >:
  // Error in function _Boost::math::bessel_jy<double>(double,double): Order of Bessel function is too large to evaluate: got 3.4028234663852886e+038

  BOOST_MATH_CHECK_THROW(cyl_bessel_j_zero(_Boost::math::tools::max_value<RealType>(), 1), _Boost::math::evaluation_error);

  BOOST_CHECK_CLOSE_FRACTION(cyl_bessel_j_zero(_Boost::math::tools::min_value<RealType>(), 1),
...
20 | 496.39435037938252557535375498577989720272298310802
  ...
  */
  ...
  // Zero'th cases.
  BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(0), 0), std::domain_error); // Zero'th zero of J0(x).
  BOOST_CHECK(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(1), 0) == 0); // Zero'th zero of J1(x).
  BOOST_CHECK(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(2), 0) == 0); // Zero'th zero of J2(x).
  ...
  //  45.597624026432090522996531982029164361723758769649
   ...
   // 49.624678304306778749502719837270544976331123155017
    ...
    v -= _Boost::math::constants::third<RealType>();
  ...
  BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(0), -1), std::domain_error);
  // unknown location(0): fatal error in "test_main":
  // class _Boost::exception_detail::clone_impl<struct _Boost::exception_detail::error_info_injector<class std::domain_error> >:
  // Error in function _Boost::math::cyl_bessel_j_zero<double>(double, int): Requested the -1'th zero, but must be > 0 !
  ...
  if (std::numeric_limits<RealType>::has_quiet_NaN)
  {
    ...
    BOOST_CHECK((_Boost::math::isnan<RealType>)(cyl_bessel_j_zero(std::numeric_limits<RealType>::quiet_NaN(), 1, ignore_all_policy())) );
    BOOST_MATH_CHECK_THROW(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(std::numeric_limits<RealType>::quiet_NaN()), -1), std::domain_error);
  }
  else
  { // real_concept bad m returns zero.
    //std::cout << _Boost::math::cyl_bessel_j_zero(static_cast<RealType>(0), -1, ignore_all_policy()) << std::endl; // 0 for real_concept.
      BOOST_CHECK_EQUAL(_Boost::math::cyl_bessel_j_zero(static_cast<RealType>(0), -1, ignore_all_policy() ), 0);
  }
  ...
  if (std::numeric_limits<RealType>::has_infinity)
  {
    ...
    BOOST_CHECK((_Boost::math::isnan<RealType>)(cyl_bessel_j_zero(std::numeric_limits<RealType>::infinity(), 1, ignore_all_policy())) );
  }
...
*/
  ...
  // Test Data for airy_ai_zero and airy_bi_zero functions.
   ...
   using _Boost::math::airy_ai_zero; //
  ...
  { // If ignore errors, return NaN.
    BOOST_CHECK((_Boost::math::isnan)(airy_ai_zero<RealType>(0, ignore_all_policy())));
    BOOST_CHECK((_Boost::math::isnan)(airy_ai_zero<RealType>((std::numeric_limits<unsigned>::min)() , ignore_all_policy())));
  ...
  BOOST_CHECK_CLOSE_FRACTION(airy_ai_zero<RealType>((std::numeric_limits<_Boost::int32_t>::max)()), -static_cast<RealType>(4678579.33301973093739L), tolerance);
  ...
  using _Boost::math::airy_bi_zero;

  BOOST_MATH_CHECK_THROW(airy_bi_zero<RealType>(0), std::domain_error);

  if (std::numeric_limits<RealType>::has_quiet_NaN)
  { // return NaN.
    BOOST_CHECK((_Boost::math::isnan)(airy_bi_zero<RealType>(0, ignore_all_policy())));
    BOOST_CHECK((_Boost::math::isnan)(airy_bi_zero<RealType>((std::numeric_limits<unsigned>::min)() , ignore_all_policy())));
    // Can't abuse with NaN as won't compile.
    // BOOST_MATH_CHECK_THROW(airy_bi_zero<RealType>(std::numeric_limits<RealType>::quiet_NaN()), std::domain_error);
    // cannot convert parameter 1 from '_Boost::math::concepts::real_concept' to 'unsigned int'.
  }
  else
  { // real_concept NaN not available, so return zero.
  ...
  BOOST_CHECK_CLOSE_FRACTION(airy_bi_zero<RealType>((std::numeric_limits<_Boost::int32_t>::max)()), -static_cast<RealType>(4678579.33229351984573L), tolerance * 300);
 ...
 // Table[N[AiryBiZero[n], 51], {n, 1000, 1001, 1}]
  ...
  // Check the multi-root versions.
  {
    ...
    _Boost::math::airy_ai_zero<RealType>(2U, n_roots, std::back_inserter(roots));
    BOOST_CHECK_CLOSE_FRACTION(roots[0], static_cast<RealType>(-4.08794944413097061663698870145739106022476469910853L), tolerance);
  }
  {
    unsigned int n_roots = 1U;
    std::vector<RealType> roots;
    _Boost::math::airy_bi_zero<RealType>(2U, n_roots, std::back_inserter(roots));
...
BOOST_AUTO_TEST_CASE(test_main)
{
   ...
   test_bessel_zeros(_Boost::math::concepts::real_concept(0.1));
#endif
#else
