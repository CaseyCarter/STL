...
using std::abs;
using std::pow;
using _Boost::math::differentiation::finite_difference_derivative;
using _Boost::math::differentiation::complex_step_derivative;
using _Boost::math::cyl_bessel_j;
using _Boost::math::cyl_bessel_j_prime;
using _Boost::math::constants::half;

template<class Real, size_t order>
void test_order(size_t points_to_test)
{
    std::cout << "Testing order " <<  order  << " derivative error estimate on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    std::cout << std::setprecision(std::numeric_limits<Real>::digits10);
    //std::cout << std::fixed << std::scientific;
    auto f = [](Real t) { return _Boost::math::cyl_bessel_j<Real>(1, t); };
    Real min = -100000.0;
    Real max = -min;
    Real x = min;
    Real max_error = 0;
...
void test_bessel()
{
    std::cout << "Testing numerical derivatives of Bessel's function on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    std::cout << std::setprecision(std::numeric_limits<Real>::digits10);

    Real eps = std::numeric_limits<Real>::epsilon();
    Real x = static_cast<Real>(25.1);
    auto f = [](Real t) { return _Boost::math::cyl_bessel_j(12, t); };
    ...
    error_estimate = abs(f(x))*pow(eps, _Boost::math::constants::two_thirds<Real>());
    //std::cout << std::setprecision(std::numeric_limits<Real>::digits10);
    //std::cout << "cyl_bessel_j_prime: " << expected << std::endl;
