...
using _Boost::math::interpolators::cardinal_trigonometric;
...
void test_constant_q()
{
    ...
    for(size_t n = 1; n < 20; ++n)
    {
      ...
      auto ct = cardinal_trigonometric<decltype(v)>(v, t0, h);
      CHECK_ULP_CLOSE(_Boost::multiprecision::float128(c), _Boost::multiprecision::float128(ct(0.3)), 3);
      CHECK_ULP_CLOSE(_Boost::multiprecision::float128(c*h*n), _Boost::multiprecision::float128(ct.integrate()), 3);
    }
}
#endif
