...
void test_hoyer_sparsity()
{
    ...
    Real hs = _Boost::math::statistics::hoyer_sparsity(v.begin(), v.end());
    BOOST_TEST(abs(hs - 1) < tol);

    hs = _Boost::math::statistics::hoyer_sparsity(v);
    BOOST_TEST(abs(hs - 1) < tol);

    // Does it work with constant iterators?
    hs = _Boost::math::statistics::hoyer_sparsity(v.cbegin(), v.cend());
    BOOST_TEST(abs(hs - 1) < tol);
    ...
    hs = _Boost::math::statistics::hoyer_sparsity(w);
    BOOST_TEST(abs(hs) < tol);
    ...
    hs = _Boost::math::statistics::hoyer_sparsity(v);
    Real expected = (1.0 - _Boost::math::constants::root_three<Real>()/2)/(1.0 - 1.0/sqrt(v.size()));
    BOOST_TEST(abs(expected - hs) < 0.01);

    // Does it work with a forward list?
    std::forward_list<Real> u1{1, 1, 1};
    hs = _Boost::math::statistics::hoyer_sparsity(u1);
    BOOST_TEST(abs(hs) < tol);

    // Does it work with a boost ublas vector?
    _Boost::numeric::ublas::vector<Real> u2(3);
    u2[0] = 1;
    u2[1] = 1;
    u2[2] = 1;
    hs = _Boost::math::statistics::hoyer_sparsity(u2);
    BOOST_TEST(abs(hs) < tol);
...
void test_integer_hoyer_sparsity()
{
    ...
    double hs = _Boost::math::statistics::hoyer_sparsity(v);
    BOOST_TEST(abs(hs - 1) < tol);
...
{
    using _Boost::math::statistics::absolute_gini_coefficient;
    using _Boost::math::statistics::sample_absolute_gini_coefficient;
...
void test_oracle_snr()
{
    ...
    noisy_signal[0] += 1;
    Real snr = _Boost::math::statistics::oracle_snr(signal, noisy_signal);
    Real snr_db = _Boost::math::statistics::oracle_snr_db(signal, noisy_signal);
...
void test_integer_oracle_snr()
{
    ...
    noisy_signal[0] += 1;
    double snr = _Boost::math::statistics::oracle_snr(signal, noisy_signal);
    double snr_db = _Boost::math::statistics::oracle_snr_db(signal, noisy_signal);
    BOOST_TEST(abs(snr - length) < tol);
    BOOST_TEST(abs(snr_db - 10*log10(length)) < tol);
}
...
void test_m2m4_snr_estimator()
{
    ...
    auto m2m4_db = _Boost::math::statistics::m2m4_snr_estimator_db(x, 1.5);
    auto oracle_snr_db = _Boost::math::statistics::mean_invariant_oracle_snr_db(signal, x);
    ...
    m2m4_db = _Boost::math::statistics::m2m4_snr_estimator_db(x, 1.5, 1.8);
    oracle_snr_db = _Boost::math::statistics::mean_invariant_oracle_snr_db(signal, x);
    ...
    Real ka = 1.5;
    Real kw = 1.8;
    auto m2m4 = _Boost::math::statistics::m2m4_snr_estimator(x.begin(), x.end(), ka, kw);
    for(size_t i = 0; i < x.size(); ++i)
    {
        x[i] *= 4096;
    }
    auto m2m4_2 = _Boost::math::statistics::m2m4_snr_estimator(x.begin(), x.end(), ka, kw);
