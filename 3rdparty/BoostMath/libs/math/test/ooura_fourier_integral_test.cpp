...
using _Boost::math::quadrature::ooura_fourier_sin;
using _Boost::math::quadrature::ooura_fourier_cos;
...
void test_ooura_eta()
{
    using _Boost::math::quadrature::detail::ooura_eta;
    std::cout << "Testing eta function on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    {
        Real x = 0;
        Real alpha = 7;
...
{
    using _Boost::math::quadrature::detail::ooura_sin_node_and_weight;
    using _Boost::math::quadrature::detail::ooura_eta;
    std::cout << "Testing nodes and weights on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    {
        long n = 1;
        Real alpha = 1;
        Real h = 1;
...
void test_ooura_alpha() {
    std::cout << "Testing Ooura alpha on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::sqrt;
    using std::log1p;
    using _Boost::math::quadrature::detail::calculate_ooura_alpha;
...
void test_node_weight_precision_agreement()
{
    ...
    for (long n = -1; n > -6*int_max; --n) {
        ...
        auto node_dist = abs(_Boost::math::float_distance(node_quad, node_dbl));
        if ( (weight_quad < 0 && weight_dbl > 0) || (weight_dbl < 0 && weight_quad > 0) ){
            std::cout << "Weights at different precisions have different signs!\n";
        } else {
            auto weight_dist = abs(_Boost::math::float_distance(weight_quad, weight_dbl));
            if (weight_dist > 100) {
                std::cout << std::fixed;
...
void test_sinc()
{
    std::cout << "Testing sinc integral on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
...
void test_exp()
{
    std::cout << "Testing exponential integral on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::exp;
    using std::numeric_limits;
    Real tol = 50*numeric_limits<Real>::epsilon();
    auto integrator = get_sin_integrator<Real>();
...
void test_root()
{
    std::cout << "Testing integral of sin(kx)/sqrt(x) on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::sqrt;
    using std::numeric_limits;
...
{
    std::cout << "Testing double oscillation on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::sqrt;
    using std::numeric_limits;
    auto integrator = get_sin_integrator<Real>();
    Real lambda = 7;
...
void test_zero_integrand()
{
    ...
    std::cout << "Testing zero integrand on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::sqrt;
    using std::numeric_limits;
    auto integrator = get_sin_integrator<Real>();
...
// void test_log()
// {
//     std::cout << "Testing integral of log(x)sin(x) on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
//     using std::log;
//     using std::exp;
//     using std::numeric_limits;
//     using _Boost::math::constants::euler;
...
{
    std::cout << "Testing integral of cos(x)/(x*x+1) on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::exp;
    using _Boost::math::constants::half_pi;
    using _Boost::math::constants::e;
    using std::numeric_limits;
    Real tol = 10*numeric_limits<Real>::epsilon();
...
{
    std::cout << "Testing integral of exp(-a*x) on type " << _Boost::typeindex::type_id<Real>().pretty_name()  << "\n";
    using std::exp;
    using _Boost::math::constants::half_pi;
    using _Boost::math::constants::e;
    using std::numeric_limits;
