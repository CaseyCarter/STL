...
void do_test_ellint_d2(const T& data, const char* type_name, const char* test)
{
   ...
   value_type(*fp2)(value_type, value_type) = ELLINT_D2_FUNCTION_TO_TEST;
    ...
    value_type (*fp2)(value_type, value_type) = _Boost::math::ellint_d<value_type, value_type>;
#else
    value_type (*fp2)(value_type, value_type) = _Boost::math::ellint_d;
#endif
...
#elif defined(BOOST_MATH_NO_DEDUCED_FUNCTION_POINTERS)
   value_type (*fp1)(value_type) = _Boost::math::ellint_d<value_type>;
#else
   value_type (*fp1)(value_type) = _Boost::math::ellint_d;
#endif
   result = _Boost::math::tools::test_hetero<Real>(
      data,
...
void test_spots(T, const char* type_name)
{
    ...
    static const _Boost::array<_Boost::array<T, 3>, 11> data1 = {{
...
#include "ellint_d2_data.ipp"
    ...
    // Function values calculated on http://functions.wolfram.com/
    // Note that Mathematica's EllipticE accepts k^2 as the second parameter.
    static const _Boost::array<_Boost::array<T, 2>, 3> data2 = {{
       { { SC_(0.5), SC_(0.87315258189267554964563356323264341) } },
       { { SC_(1.0) / 1024, SC_(0.78539844427788694671464428063604776) } },
       { { _Boost::math::tools::root_epsilon<T>(), SC_(0.78539816339744830961566084581987572) } }
    }};
...
#include "ellint_d_data.ipp"
    ...
    BOOST_MATH_CHECK_THROW(_Boost::math::ellint_d(T(1)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::ellint_d(T(-1)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::ellint_d(T(1.5)), std::domain_error);
    BOOST_MATH_CHECK_THROW(_Boost::math::ellint_d(T(-1.5)), std::domain_error);
}
