...
#endif
#ifndef BOOST_MATH_NO_LONG_DOUBLE_MATH_FUNCTIONS
   if((_Boost::math::tools::digits<long double>() > _Boost::math::tools::digits<double>())
      && (_Boost::math::tools::digits<long double>() < 100))
   {
      //
...
template<>
inline unsigned make_unsigned(_Boost::math::concepts::real_concept x)
{
...
{
   _Boost::math::hypergeometric_distribution<T> d(make_unsigned(r), make_unsigned(n), make_unsigned(N));
   return pdf(d, x);
}
...
void do_test_hypergeometric_quantile(const T& data, const char* type_name, const char* test_name)
{
   ...
   if(_Boost::math::tools::digits<value_type>() > 50)
   {
      for(unsigned i = 0; i < data.size(); ++i)
      {
         ...
         _Boost::math::hypergeometric_distribution<value_type, 
            policy<discrete_quantile<integer_round_up> > > du(r, n, N);

         if((cp < 0.9) && (cp > _Boost::math::tools::min_value<value_type>()))
         {
            BOOST_CHECK_EX(quantile(du, cp) >= x);
         }
         if((ccp < 0.9) && (ccp > _Boost::math::tools::min_value<value_type>()))
         {
               ...
               RealType p, RealType cp, RealType ccp, RealType tol)
{
   ...
   (void)tol;
#if !defined(TEST_QUANT) || (TEST_QUANT == 0)
   _Boost::math::hypergeometric_distribution<RealType> d(r, n, N);
   ...
   if(_Boost::math::tools::digits<RealType>() > 50)
   {
      ...
      _Boost::math::hypergeometric_distribution<RealType, 
         policy<discrete_quantile<integer_round_up> > > du(r, n, N);
...
#include "hypergeometric_dist_data2.ipp"
   ...
   _Boost::math::hypergeometric_distribution<RealType> d(50, 200, 500);
   ...
   BOOST_MATH_CHECK_THROW(d = _Boost::math::hypergeometric_distribution<RealType>(501, 40, 500), std::domain_error);
   BOOST_MATH_CHECK_THROW(d = _Boost::math::hypergeometric_distribution<RealType>(40, 501, 500), std::domain_error);
}
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   test_spots(_Boost::math::concepts::real_concept(0), "real_concept"); // Test real_concept.
#endif
#else
