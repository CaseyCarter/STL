...
struct ibeta_roots_1   // for first order algorithms
{
   ...
   {
      return _Boost::math::detail::ibeta_imp(a, b, (neg ? -x : x), Policy(), invert, true) - target;
   }
private:
   T a, b, target;
   bool invert, neg;
...
struct ibeta_roots_2   // for second order algorithms
{
   ...
   _Boost::math::tuple<T, T> operator()(const T& xx)
   {
      typedef typename _Boost::math::lanczos::lanczos<T, Policy>::type L;
      T x = neg ? -xx : xx;
      T f = _Boost::math::detail::ibeta_imp(a, b, x, Policy(), invert, true) - target;
      T f1 = invert ?
         -_Boost::math::detail::ibeta_power_terms(b, a, 1 - x, x, L(), true, Policy())
               : _Boost::math::detail::ibeta_power_terms(a, b, x, 1 - x, L(), true, Policy());
      T y = 1 - x;
      if(y == 0)
         y = _Boost::math::tools::min_value<T>() * 8;
      f1 /= y * x;

      // make sure we don't have a zero derivative:
      if(f1 == 0)
         f1 = (invert ? -1 : 1) * _Boost::math::tools::min_value<T>() * 64;

      return _Boost::math::make_tuple(f, neg ? -f1 : f1);
   }
private:
   T a, b, target;
   bool invert, neg;
...
struct ibeta_roots_3   // for third order algorithms
{
   ...
   _Boost::math::tuple<T, T, T> operator()(const T& xx)
   {
      typedef typename _Boost::math::lanczos::lanczos<T, Policy>::type L;
      T x = neg ? -xx : xx;
      T f = _Boost::math::detail::ibeta_imp(a, b, x, Policy(), invert, true) - target;
      ...
      return _Boost::math::make_tuple(f, f1, f2);
   }
private:
   T a, b, target;
   bool invert, neg;
...
{
   typedef _Boost::math::policies::policy<> pol;
   bool invert = false;
   ...
   double min = 0;
   double max = 1;
   _Boost::math::tools::eps_tolerance<double> tol(precision);
   return _Boost::math::tools::bisect(ibeta_roots_1<double, pol>(a, b, z, invert), min, max, tol).first;
}
...
double inverse_ibeta_bisect_neg(double a, double b, double z)
{
   ...
   double max = 0;
   _Boost::math::tools::eps_tolerance<double> tol(precision);
   return -_Boost::math::tools::bisect(ibeta_roots_1<double, pol>(a, b, z, invert, true), min, max, tol).first;
}
...
double inverse_ibeta_newton(double a, double b, double z)
{
   ...
   double min = 0;
   double max = 1;
   return _Boost::math::tools::newton_raphson_iterate(ibeta_roots_2<double, _Boost::math::policies::policy<> >(a, b, z, invert), guess, min, max, precision);
}
...
double inverse_ibeta_newton_neg(double a, double b, double z)
{
   ...
   double min = -1;
   double max = 0;
   return -_Boost::math::tools::newton_raphson_iterate(ibeta_roots_2<double, _Boost::math::policies::policy<> >(a, b, z, invert, true), -guess, min, max, precision);
}
...
double inverse_ibeta_halley(double a, double b, double z)
{
   ...
   double min = 0;
   double max = 1;
   return _Boost::math::tools::halley_iterate(ibeta_roots_3<double, _Boost::math::policies::policy<> >(a, b, z, invert), guess, min, max, precision);
}
...
double inverse_ibeta_halley_neg(double a, double b, double z)
{
   ...
   double min = -1;
   double max = 0;
   return -_Boost::math::tools::halley_iterate(ibeta_roots_3<double, _Boost::math::policies::policy<> >(a, b, z, invert, true), guess, min, max, precision);
}
...
double inverse_ibeta_schroder(double a, double b, double z)
{
   ...
   double min = 0;
   double max = 1;
   return _Boost::math::tools::schroder_iterate(ibeta_roots_3<double, _Boost::math::policies::policy<> >(a, b, z, invert), guess, min, max, precision);
}
...
void test_inverses(const T& data)
{
   ...
   value_type precision = static_cast<value_type>(ldexp(1.0, 1-_Boost::math::policies::digits<value_type, _Boost::math::policies::policy<> >()/2)) * 150;
   ...
   for(unsigned i = 0; i < data.size(); ++i)
   {
      ...
      if(data[i][5] == 0)
      {
         ...
         && (fabs(data[i][5]) > 2 * _Boost::math::tools::min_value<value_type>())
         && (fabs(data[i][5]) > 2 * _Boost::math::tools::min_value<double>()))
      {
         ...
         BOOST_ASSERT(_Boost::math::isfinite(inv));
         BOOST_CHECK_CLOSE_EX(Real(data[i][2]), inv, precision, i);
...
void test_complex_newton()
{
    ...
    std::cout << "Testing complex Newton's Method on type " << _Boost::typeindex::type_id<Real>().pretty_name() << "\n";
    using std::abs;
    using std::sqrt;
    using _Boost::math::tools::complex_newton;
    using _Boost::math::tools::polynomial;
    ...
    // p'(z) = 2z + i
    p_prime = p.prime();
    guess = Complex(0,-_Boost::math::constants::half<Real>());
...
{
    Real tol = std::numeric_limits<Real>::epsilon();
    using _Boost::math::tools::quadratic_roots;
...
void test_failures()
{
   ...
   // There is no root:
   BOOST_CHECK_THROW(_Boost::math::tools::newton_raphson_iterate([](double x) { return std::make_pair(x * x + 1, 2 * x); }, 10.0, -12.0, 12.0, 52), _Boost::math::evaluation_error);
   BOOST_CHECK_THROW(_Boost::math::tools::newton_raphson_iterate([](double x) { return std::make_pair(x * x + 1, 2 * x); }, -10.0, -12.0, 12.0, 52), _Boost::math::evaluation_error);
   // There is a root, but a bad guess takes us into a local minima:
   BOOST_CHECK_THROW(_Boost::math::tools::newton_raphson_iterate([](double x) { return std::make_pair(_Boost::math::pow<6>(x) - 2 * _Boost::math::pow<4>(x) + x + 0.5, 6 * _Boost::math::pow<5>(x) - 8 * _Boost::math::pow<3>(x) + 1); }, 0.75, -20., 20., 52), _Boost::math::evaluation_error);

   // There is no root:
   BOOST_CHECK_THROW(_Boost::math::tools::halley_iterate([](double x) { return std::make_tuple(x * x + 1, 2 * x, 2); }, 10.0, -12.0, 12.0, 52), _Boost::math::evaluation_error);
   BOOST_CHECK_THROW(_Boost::math::tools::halley_iterate([](double x) { return std::make_tuple(x * x + 1, 2 * x, 2); }, -10.0, -12.0, 12.0, 52), _Boost::math::evaluation_error);
   // There is a root, but a bad guess takes us into a local minima:
   BOOST_CHECK_THROW(_Boost::math::tools::halley_iterate([](double x) { return std::make_tuple(_Boost::math::pow<6>(x) - 2 * _Boost::math::pow<4>(x) + x + 0.5, 6 * _Boost::math::pow<5>(x) - 8 * _Boost::math::pow<3>(x) + 1, 30 * _Boost::math::pow<4>(x) - 24 * _Boost::math::pow<2>(x)); }, 0.75, -20., 20., 52), _Boost::math::evaluation_error);
#endif
}
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   test_complex_newton<_Boost::multiprecision::cpp_complex_100>();
   test_daubechies_fails();
    ...
    test_solve_real_quadratic<_Boost::multiprecision::cpp_bin_float_50>();
