...
void check_pFq_result(const T& result, const T& norm, const T& expect, const std::initializer_list<T>& a, const std::initializer_list<T>& b, const T& z)
{
   ...
   //
   T err = _Boost::math::relative_difference(result, expect);
   T found_err = norm / fabs(result);
   T fudge_factor = 25;
   if (is_small_a(a))
      fudge_factor *= 4;  // not sure why??
   if ((has_negative_ab(a, b)) || ((a.size() == 2) && (b.size() == 1)) || (_Boost::math::tools::epsilon<T>() < _Boost::math::tools::epsilon<double>()))
   {
      T min_err = _Boost::math::tools::epsilon<T>() * 600 / found_err;
      fudge_factor = (std::max)(fudge_factor, min_err);
   }
   if ((((err > fudge_factor * found_err) && (found_err < 1)) || (_Boost::math::isnan)(found_err)) && (!(_Boost::math::isinf)(result)))
   {
      ...
      BOOST_CHECK(!(_Boost::math::isnan)(found_err));
   }
}
...
void test_spots_1F0(T, const char*)
{
   ...
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(2)), T(-1), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(4)), T(-27), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(0.5)), T(0.125), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(0.5)), T(8), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(2)), T(-1), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(4)), T(T(-1) / 27), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(-0.5)), pow(T(1.5), -3), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(-2)), T(1 / T(27)), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(-4)), T(T(1) / 125), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(-0.5)), pow(T(1.5), 3), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(-2)), T(27), tolerance);
   BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(-4)), T(125), tolerance);

   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(3) }, {}, T(1)), std::domain_error);
   //BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(-3) }, {}, T(1)), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(3.25) }, {}, T(1)), std::domain_error);
   //BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(-3.25) }, {}, T(1)), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(3.25) }, {}, T(2)), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({ T(-3.25) }, {}, T(2)), std::domain_error);
}
...
{
   T tolerance = _Boost::math::tools::epsilon<T>() * 50000;

   BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq({},  { T(3) }, T(0)), 1);
   BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq({}, { T(-3) }, T(0)), 1);
   //BOOST_CHECK_EQUAL(_Boost::math::hypergeometric_pFq({}, { T(0) }, T(0)), 1);

   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({}, { T(0) }, T(-1)), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({}, { T(-1) }, T(-1)), std::domain_error);
   BOOST_CHECK_THROW(_Boost::math::hypergeometric_pFq({}, { T(-10) }, T(-5)), std::domain_error);

   static const _Boost::array<_Boost::array<T, 3>, 35> hypergeometric_pFq_integer_data = { {
   ...
   {
      BOOST_CHECK_CLOSE(_Boost::math::hypergeometric_pFq({},  { (*row)[0] }, (*row)[1]), (*row)[2], tolerance);
   }
}
...
#include "hypergeometric_1F1.ipp"
   ...
   for (auto row = hypergeometric_1F1.begin(); row != hypergeometric_1F1.end(); ++row)
   {
      try {
         T norm;
         T result = _Boost::math::hypergeometric_pFq({ (*row)[0] }, { (*row)[1] }, (*row)[2], &norm);
...
#include "hypergeometric_2F1.ipp"
   ...
   for (auto row = hypergeometric_2F1.begin(); row != hypergeometric_2F1.end(); ++row)
   {
      try {
         T norm;
         T result = _Boost::math::hypergeometric_pFq({ (*row)[0], (*row)[1] }, { (*row)[2] }, (*row)[3], &norm);
...
#include "hypergeometric_2F2.ipp"
   ...
   for (auto row = hypergeometric_2F2.begin(); row != hypergeometric_2F2.end(); ++row)
   {
      try {
         T norm;
         T result = _Boost::math::hypergeometric_pFq({ (*row)[0], (*row)[1] }, { (*row)[2], (*row)[3] }, (*row)[4], &norm);
