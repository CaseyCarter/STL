...
void test_airy(T, const char* name)
{
   ...
   static const _Boost::array<_Boost::array<T, 5>, 8> data = 
   {{
   ...
   T tol = _Boost::math::tools::epsilon<T>() * 800;
   for(unsigned i = 0; i < data.size(); ++i)
   {
      BOOST_CHECK_CLOSE_FRACTION(data[i][1], _Boost::math::airy_ai(data[i][0]), tol);
      if(_Boost::math::isfinite(data[i][2]))
         BOOST_CHECK_CLOSE_FRACTION(data[i][2], _Boost::math::airy_bi(data[i][0]), tol);
      BOOST_CHECK_CLOSE_FRACTION(data[i][3], _Boost::math::airy_ai_prime(data[i][0]), tol);
      if(_Boost::math::isfinite(data[i][4]))
         BOOST_CHECK_CLOSE_FRACTION(data[i][4], _Boost::math::airy_bi_prime(data[i][0]), tol);
   }
}
...
BOOST_AUTO_TEST_CASE( test_main )
{
   ...
   test_airy(_Boost::math::concepts::real_concept(0), "real_concept");
#else
