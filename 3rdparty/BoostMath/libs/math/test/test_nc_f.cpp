...
RealType naive_pdf(RealType v1, RealType v2, RealType l, RealType f)
{
   ...
   for(int i = 0; ; ++i)
   {
      ...
      term += _Boost::math::lgamma(v2/2 + v1/2+i) - (_Boost::math::lgamma(v2/2) + _Boost::math::lgamma(v1/2+i));
      term -= _Boost::math::lgamma(RealType(i+1));
      term += log(v1/v2) * (v1/2+i) + log(v2 / (v2 + v1 * f)) * ((v1 + v2) / 2 + i);
      term += log(f) * (v1/2 - 1 + i);
      term = exp(term);
      sum += term;
      if((term/sum < _Boost::math::tools::epsilon<RealType>()) || (term == 0))
         break;
   }
   return sum;
}
...
{
   _Boost::math::non_central_f_distribution<RealType> dist(a, b, ncp);
   BOOST_CHECK_CLOSE(
      cdf(dist, x), P, tol);
   BOOST_CHECK_CLOSE(
      pdf(dist, x), D, tol);
...
{
   RealType tolerance = _Boost::math::tools::epsilon<RealType>() * 10000;
   ...
   //
   tolerance = (std::max)(
      _Boost::math::tools::epsilon<RealType>() * 100,
      (RealType)1e-6) * 100;
   ...
   //
   RealType tol2 = (std::max)(_Boost::math::tools::epsilon<RealType>() * 500, RealType(1e-25));
   RealType x = 2;

   _Boost::math::non_central_f_distribution<RealType> dist(20, 15, 30);
   // mean:
   BOOST_CHECK_CLOSE(
      mean(dist)
   ...
   check_out_of_range<_Boost::math::non_central_f_distribution<RealType> >(1, 1, 1);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_f_distribution<RealType>(0, 1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_f_distribution<RealType>(-1, 1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_f_distribution<RealType>(1, 0, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(pdf(_Boost::math::non_central_f_distribution<RealType>(1, -1, 1), 0), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_f_distribution<RealType>(1, 1, 1), -1), std::domain_error);
   BOOST_MATH_CHECK_THROW(quantile(_Boost::math::non_central_f_distribution<RealType>(1, 1, 1), 2), std::domain_error);
