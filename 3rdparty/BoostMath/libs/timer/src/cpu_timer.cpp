...
namespace
{
  ...
  //  be as low as 10, although will be 15 for many common platforms.
  {
    if (places > 9)
      ...
      places = _Boost::timer::default_places;
 
    _Boost::io::ios_flags_saver ifs(os);
    _Boost::io::ios_precision_saver ips(os);
...
# if defined(BOOST_POSIX_API)
  _Boost::int_least64_t tick_factor() // multiplier to convert ticks
                                     //  to nanoseconds; -1 if unknown
  {
    static _Boost::int_least64_t tick_factor = 0;
    if (!tick_factor)
    {
...
# endif
  ...
  void get_cpu_times(_Boost::timer::cpu_times& current)
  {
    _Boost::chrono::duration<_Boost::int64_t, _Boost::nano>
      x (_Boost::chrono::high_resolution_clock::now().time_since_epoch());
...
#  endif
    {
      current.system = current.user = _Boost::timer::nanosecond_type(-1);
    }
# else
    tms tm;
    clock_t c = ::times(&tm);
    ...
    {
      current.system = _Boost::timer::nanosecond_type(tm.tms_stime + tm.tms_cstime);
      current.user = _Boost::timer::nanosecond_type(tm.tms_utime + tm.tms_cutime);
      _Boost::int_least64_t factor;
      ...
      {
        current.user = current.system = _Boost::timer::nanosecond_type(-1);
      }
    }
# endif
  }
