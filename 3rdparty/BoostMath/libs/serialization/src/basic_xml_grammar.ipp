...
struct append_string {
    ...
    #if 0
        typedef _Boost::archive::iterators::xml_unescape<Iterator> translator;
        contents.append(
...
bool basic_xml_grammar<CharType>::my_parse(
    ...
    for(;;){
        ...
        if(is.fail()){
            _Boost::serialization::throw_exception(
                _Boost::archive::archive_exception(
    ...
    parse_info<typename std::basic_string<CharType>::iterator> 
        result = _Boost::spirit::classic::parse(arg.begin(), arg.end(), rule_);
    return result.hit;
}
