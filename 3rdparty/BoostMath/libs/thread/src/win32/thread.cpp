...
#else
        __declspec(thread) _Boost::detail::thread_data_base* current_thread_data_base;
#endif

        void create_current_thread_tls_key()
        {
            _Boosttss_cleanup_implemented(); // if anyone uses TSS, we need the cleanup linked in
#if !BOOST_PLAT_WINDOWS_RUNTIME
...
// Windows CE doesn't define _beginthreadex
        ...
        {
            _Boost::csbl::unique_ptr<ThreadProxyData> data(reinterpret_cast<ThreadProxyData*>(args));
        ...
        bool win32::scoped_winrt_thread::start(thread_func address, void *parameter, unsigned int *thrdId)
        {
            ...
            *thrdId = ++threadCount;
                ...
                ([address, parameter, completionHandle](ABI::Windows::Foundation::IAsyncAction *)
            {
                ...
                ::_Boost::detail::thread_data_base* const thread_info(reinterpret_cast<::_Boost::detail::thread_data_base*>(parameter));
            ...
            if(current_thread_data)
            {
                while(! current_thread_data->tss_data.empty() || current_thread_data->thread_exit_callbacks)
                {
                    while(current_thread_data->thread_exit_callbacks)
                    {
                        ...
                        {
                            (*current_node->func)();
                            _Boost::detail::heap_delete(current_node->func);
                        }
                        _Boost::detail::heap_delete(current_node);
                    }
...
#pragma warning(pop)
        ...
        bool interruptible_wait(detail::win32::handle handle_to_wait_for, detail::internal_platform_timepoint const &timeout)
        {
            ...
            {
                _Boost::intmax_t const time_left_msec = (timeout - detail::internal_platform_clock::now()).getMs();
            ...
            _Boost::intmax_t time_left_msec(INFINITE);
    ...
    BOOST_THREAD_DECL void __cdecl _Booston_process_enter()
    {}

    BOOST_THREAD_DECL void __cdecl _Booston_thread_enter()
    {}

    BOOST_THREAD_DECL void __cdecl _Booston_process_exit()
    {
        _Boost::cleanup_tls_key();
    }

    BOOST_THREAD_DECL void __cdecl _Booston_thread_exit()
    {
        _Boost::run_thread_exit_callbacks();
    }
