...
namespace pt = _Boost::filesystem::path_traits;
namespace fs = _Boost::filesystem;
namespace bs = _Boost::system;
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      ...
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "_Boost::filesystem::path codecvt to wstring"));
    }
...
//--------------------------------------------------------------------------------------//
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      //std::cout << " result is " << static_cast<int>(res) << std::endl;
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "_Boost::filesystem::path codecvt to string"));
    }
    target.append(to, to_next);
  }
...
namespace _Boost { namespace filesystem { namespace path_traits {
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    {
      _Boost::scoped_array< wchar_t > buf(new wchar_t [buf_size]);
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    if (buf_size > default_codecvt_buf_size)
    {
      _Boost::scoped_array< char > buf(new char [buf_size]);
      convert_aux(from, from_end, buf.get(), buf.get()+buf_size, to, cvt);
    }
    else
    {
...
}}} // namespace _Boost::filesystem::path_traits
