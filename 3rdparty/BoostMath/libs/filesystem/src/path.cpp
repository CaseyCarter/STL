...
# elif defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__) \
  || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__HAIKU__)
    ...
    std::locale global_loc = std::locale();
    return std::locale(global_loc, new _Boost::filesystem::detail::utf8_codecvt_facet);
# else  // Other POSIX
...
#endif
    BOOST_ASSERT_MSG(&path_locale(), "_Boost::filesystem::path locale initialization error");
