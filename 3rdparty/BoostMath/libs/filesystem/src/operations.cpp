...
#     define BOOST_STATVFS_F_FRSIZE static_cast<_Boost::uintmax_t>(vfs.f_bsize)
#   endif
#   include <unistd.h>
#   include <fcntl.h>
...
namespace fs = _Boost::filesystem;
using _Boost::filesystem::path;
using _Boost::filesystem::filesystem_error;
using _Boost::filesystem::perms;
using _Boost::system::error_code;
using _Boost::system::system_category;
...
_Boost::uintmax_t remove_all_aux(const path& p, fs::file_type type,
  error_code* ec)
{
  _Boost::uintmax_t count = 0;
  ...
  const std::string& to_p, bool fail_if_exists)
{
  ...
  _Boost::scoped_array<char> buf(new char [buf_sz]);
...
inline std::wstring wgetenv(const wchar_t* name)
{
  ...
  if (size > 0)
  {
    _Boost::scoped_array<wchar_t> buf(new wchar_t[size]);
...
bool is_reparse_point_a_symlink(const path& p)
{
  ...
  _Boost::scoped_array<char> buf(new char [MAXIMUM_REPARSE_DATA_BUFFER_SIZE]);
...
fs::file_status process_status_failure(const path& p, error_code* ec)
{
  ...
  if (ec == 0)
    BOOST_FILESYSTEM_THROW(filesystem_error("_Boost::filesystem::status",
...
BOOL resize_file_api(const wchar_t* p, _Boost::uintmax_t size)
{
...
PtrCreateHardLinkW create_hard_link_api = PtrCreateHardLinkW(
  _Boost::winapi::get_proc_address(
    _Boost::winapi::GetModuleHandleW(L"kernel32.dll"), "CreateHardLinkW"));
...
PtrCreateSymbolicLinkW create_symbolic_link_api = PtrCreateSymbolicLinkW(
  _Boost::winapi::get_proc_address(
    _Boost::winapi::GetModuleHandleW(L"kernel32.dll"), "CreateSymbolicLinkW"));
...
path canonical(const path& p, const path& base, system::error_code* ec)
{
  ...
  if (stat.type() == fs::file_not_found)
  {
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error(
        "_Boost::filesystem::canonical", source,
  ...
  else if (local_ec)
  {
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error(
        "_Boost::filesystem::canonical", source, local_ec));
    *ec = local_ec;
    return result;
  }
...
void copy(const path& from, const path& to, system::error_code* ec)
{
  ...
  {
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error("_Boost::filesystem::copy",
...
# endif
  error(!BOOST_COPY_DIRECTORY(from.c_str(), to.c_str()) ? BOOST_ERRNO : 0,
    from, to, ec, "_Boost::filesystem::copy_directory");
}
...
void copy_file(const path& from, const path& to, copy_option option, error_code* ec)
{
  error(!BOOST_COPY_FILE(from.c_str(), to.c_str(),
    option == fail_if_exists) ? BOOST_ERRNO : 0,
      from, to, ec, "_Boost::filesystem::copy_file");
}
...
# if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0600
  error(BOOST_ERROR_NOT_SUPPORTED, new_symlink, existing_symlink, ec,
    "_Boost::filesystem::copy_symlink");
...
bool create_directories(const path& p, system::error_code* ec)
{
 if (p.empty())
 {
   if (ec == 0)
     BOOST_FILESYSTEM_THROW(filesystem_error(
       "_Boost::filesystem::create_directories", p,
  ...
  if (!parent.empty())
  {
    ...
    if (parent_status.type() == file_not_found)
    {
      ...
      if (local_ec)
      {
        if (ec == 0)
          BOOST_FILESYSTEM_THROW(filesystem_error(
            "_Boost::filesystem::create_directories", parent, local_ec));
        else
          *ec = local_ec;
        return false;
      }
...
bool create_directory(const path& p, error_code* ec)
{
  ...
  if (ec == 0)
    BOOST_FILESYSTEM_THROW(filesystem_error("_Boost::filesystem::create_directory",
      p, error_code(errval, system_category())));
  else
...
# if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
  ...
  error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
    "_Boost::filesystem::create_directory_symlink");
# else
...
#   endif
  ...
  error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(),
    ...
    to, from, ec, "_Boost::filesystem::create_directory_symlink");
# endif
}
...
# if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0500  // SDK earlier than Win 2K
  ...
  error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
    "_Boost::filesystem::create_hard_link");
# else
...
#   if defined(BOOST_WINDOWS_API) && _WIN32_WINNT >= 0x0600
      // see if actually supported by Windows runtime dll
      if (error(!create_symbolic_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to, from, ec,
          "_Boost::filesystem::create_symlink"))
        return;
#   endif

  error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(), 0) ? BOOST_ERRNO : 0,
    to, from, ec, "_Boost::filesystem::create_symlink");
# endif
}
...
#   if defined(__MSL__) && (defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__))
          && err != 0
#   endif
        ) ? err : 0, ec, "_Boost::filesystem::current_path");
    }
  };
  ...
  else if (BOOST_LIKELY(!local::getcwd_error(ec)))
  {
    for (std::size_t path_max = sizeof(small_buf);; path_max *= 2u) // loop 'til buffer large enough
    {
      ...
      {
        if (ec == 0)
          BOOST_FILESYSTEM_THROW(filesystem_error("_Boost::filesystem::current_path",
      ...
      _Boost::scoped_array<char> buf(new char[path_max]);
...
# else
  ...
  _Boost::scoped_array<path::value_type> buf(new path::value_type[sz]);
  error(::GetCurrentDirectoryW(sz, buf.get()) == 0 ? BOOST_ERRNO : 0, ec,
    "_Boost::filesystem::current_path");
  return path(buf.get());
# endif
}
...
# else
  error(!BOOST_SET_CURRENT_DIRECTORY(p.c_str()) ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::current_path");
# endif
}
...
# ifdef BOOST_POSIX_API
  ...
  if (e1 != 0 || e2 != 0)
  {
    ...
    error (e1 != 0 && e2 != 0, p1, p2, ec, "_Boost::filesystem::equivalent");
    return false;
  }
...
# else  // Windows
  ...
  if (error(!::GetFileInformationByHandle(h1.handle, &info1) ? BOOST_ERRNO : 0,
    p1, p2, ec, "_Boost::filesystem::equivalent"))
    return  false;
...
BOOST_FILESYSTEM_DECL
_Boost::uintmax_t file_size(const path& p, error_code* ec)
{
# ifdef BOOST_POSIX_API

  struct stat path_stat;
  if (error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
      p, ec, "_Boost::filesystem::file_size"))
    return static_cast<_Boost::uintmax_t>(-1);
  if (error(!S_ISREG(path_stat.st_mode) ? EPERM : 0,
      p, ec, "_Boost::filesystem::file_size"))
    return static_cast<_Boost::uintmax_t>(-1);

  return static_cast<_Boost::uintmax_t>(path_stat.st_size);
...
# else  // Windows
  ...
  if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad)== 0
    ? BOOST_ERRNO : 0, p, ec, "_Boost::filesystem::file_size"))
        return static_cast<_Boost::uintmax_t>(-1);

  if (error((fad.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)!= 0
    ? ERROR_NOT_SUPPORTED : 0, p, ec, "_Boost::filesystem::file_size"))
    return static_cast<_Boost::uintmax_t>(-1);

  return (static_cast<_Boost::uintmax_t>(fad.nFileSizeHigh)
            << (sizeof(fad.nFileSizeLow)*8)) + fad.nFileSizeLow;
# endif
}

BOOST_FILESYSTEM_DECL
_Boost::uintmax_t hard_link_count(const path& p, system::error_code* ec)
{
# ifdef BOOST_POSIX_API

  struct stat path_stat;
  return error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
                p, ec, "_Boost::filesystem::hard_link_count")
         ? 0
         : static_cast<_Boost::uintmax_t>(path_stat.st_nlink);
...
# ifdef BOOST_POSIX_API
  ...
  if (error(::stat(p.c_str(), &path_stat)!= 0,
      p, ec, "_Boost::filesystem::is_empty"))
    return false;
...
# else
  ...
  WIN32_FILE_ATTRIBUTE_DATA fad;
  if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad)== 0
    ? BOOST_ERRNO : 0, p, ec, "_Boost::filesystem::is_empty"))
      return false;
...
# ifdef BOOST_POSIX_API
  ...
  struct stat path_stat;
  if (error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::last_write_time"))
      return std::time_t(-1);
...
#   if _POSIX_C_SOURCE >= 200809L
  ...
  {
    error(BOOST_ERRNO, p, ec, "_Boost::filesystem::last_write_time");
    return;
  }
...
# ifdef BOOST_POSIX_API
  ...
  if (local_ec)
  {
    if (ec == 0)
    BOOST_FILESYSTEM_THROW(filesystem_error(
        "_Boost::filesystem::permissions", p, local_ec));
    else
      *ec = local_ec;
    return;
  }
...
# endif
  {
    ...
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error(
        "_Boost::filesystem::permissions", p,
...
# else  // Windows
  ...
  if (error(attr == 0 ? BOOST_ERRNO : 0, p, ec, "_Boost::filesystem::permissions"))
    return;
  ...
  error(::SetFileAttributesW(p.c_str(), attr) == 0 ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::permissions");
# endif
}
...
# ifdef BOOST_POSIX_API
  ...
  fail:
    const int err = errno;
    if (ec == 0)
      BOOST_FILESYSTEM_THROW(filesystem_error("_Boost::filesystem::read_symlink",
...
# elif _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
  error(BOOST_ERROR_NOT_SUPPORTED, p, ec,
        "_Boost::filesystem::read_symlink");
# else  // Vista and Server 2008 SDK, or later
  ...
  if (error(h.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::read_symlink"))
      return symlink_path;
...
path relative(const path& p, const path& base, error_code* ec)
{
  ...
  if (error(tmp_ec.value(), base, ec, "_Boost::filesystem::relative"))
    return path();
...
BOOST_FILESYSTEM_DECL
_Boost::uintmax_t remove_all(const path& p, error_code* ec)
{
  error_code tmp_ec;
  file_type type = query_file_type(p, &tmp_ec);
  if (error(type == status_error ? tmp_ec.value() : 0, p, ec,
    "_Boost::filesystem::remove_all"))
    return 0;
...
# if defined(BOOST_POSIX_API)
  if (BOOST_UNLIKELY(size > static_cast< uintmax_t >((std::numeric_limits< off_t >::max)()))) {
    error(system::errc::file_too_large, p, ec, "_Boost::filesystem::resize_file");
    return;
  }
# endif
  error(!BOOST_RESIZE_FILE(p.c_str(), size) ? BOOST_ERRNO : 0, p, ec,
    "_Boost::filesystem::resize_file");
}
...
# ifdef BOOST_POSIX_API
  ...
  if (!error(::BOOST_STATVFS(p.c_str(), &vfs) ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::space"))
  {
    info.capacity
      = static_cast<_Boost::uintmax_t>(vfs.f_blocks)* BOOST_STATVFS_F_FRSIZE;
    info.free
      = static_cast<_Boost::uintmax_t>(vfs.f_bfree)* BOOST_STATVFS_F_FRSIZE;
    info.available
      = static_cast<_Boost::uintmax_t>(vfs.f_bavail)* BOOST_STATVFS_F_FRSIZE;
  }
...
# else
  ...
  {
    info.capacity
      = (static_cast<_Boost::uintmax_t>(total.HighPart)<< 32)
        + total.LowPart;
    info.free
      = (static_cast<_Boost::uintmax_t>(free.HighPart)<< 32)
        + free.LowPart;
    info.available
      = (static_cast<_Boost::uintmax_t>(avail.HighPart)<< 32)
        + avail.LowPart;
  }
...
#   endif
  ...
  {
    error(ENOTDIR, p, ec, "_Boost::filesystem::temp_directory_path");
    return p;
  }
...
# else  // Windows
  ...
  if (p.empty())
  {
    ...
    getwindir_error:
      int errval = ::GetLastError();
      error(errval, ec, "_Boost::filesystem::temp_directory_path");
      return path();
    }
...
# else
  ...
  if (error(len == 0 ? BOOST_ERRNO : 0, p, ec, "_Boost::filesystem::system_complete"))
    return path();

  if (len < buf_size)// len does not include null termination character
    return path(&buf[0]);

  _Boost::scoped_array<wchar_t> big_buf(new wchar_t[len]);

  return error(get_full_path_name(p, len , big_buf.get(), &pfn)== 0 ? BOOST_ERRNO : 0,
    p, ec, "_Boost::filesystem::system_complete")
    ? path()
    : path(big_buf.get());
# endif
}
...
path weakly_canonical(const path& p, system::error_code* ec)
{
  ...
  for (; !head.empty(); --itr)
  {
    ...
    if (error(head_status.type() == fs::status_error,
      head, ec, "_Boost::filesystem::weakly_canonical"))
      return path();
  ...
  if (error(tmp_ec.value(), head, ec, "_Boost::filesystem::weakly_canonical"))
    return path();
  return tail.empty()
    ? head
