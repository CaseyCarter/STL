...
void fail(int err, _Boost::system::error_code* ec)
{
  if (ec == 0)
    BOOST_FILESYSTEM_THROW( _Boost::system::system_error(err,
      _Boost::system::system_category(),
      "_Boost::filesystem::unique_path"));

  ec->assign(err, _Boost::system::system_category());
  return;
}
...
void system_crypt_random(void* buf, std::size_t len, _Boost::system::error_code* ec)
{
# ifdef BOOST_POSIX_API
...
namespace _Boost { namespace filesystem { namespace detail {
