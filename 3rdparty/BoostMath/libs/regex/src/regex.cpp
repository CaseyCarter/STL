 ...
 */
  ...
  *   DESCRIPTION: Misc _Boost::regbase member funnctions.
  */
...
regex_error::regex_error(regex_constants::error_type err) 
   : std::runtime_error(::_Boost::BOOST_REGEX_DETAIL_NS::get_default_error_string(err))
   , m_error_code(err)
   , m_position(0) 
{
}
...
{
   ::_Boost::throw_exception(ex);
}
//
// error checking API:
//
BOOST_REGEX_DECL void BOOST_REGEX_CALL verify_options(_Boost::regex::flag_type /*ef*/, match_flag_type mf)
{
