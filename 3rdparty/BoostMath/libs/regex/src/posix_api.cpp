...
typedef _Boost::basic_regex<char, c_regex_traits<char> > c_regex_type;
...
#endif
   // set default flags:
   _Boost::uint_fast32_t flags = (f & REG_PERLEX) ? 0 : ((f & REG_EXTENDED) ? regex::extended : regex::basic);
...
BOOST_REGEX_DECL regsize_t BOOST_REGEX_CCALL regerrorA(int code, const regex_tA* e, char* buf, regsize_t buf_size)
{
   ...
   if(code <= (int)REG_E_UNKNOWN)
   {
      ...
      if((e) && (e->re_magic == magic_value))
         p = static_cast<c_regex_type*>(e->guts)->get_traits().error_string(static_cast< ::_Boost::regex_constants::error_type>(code));
      else
      {
