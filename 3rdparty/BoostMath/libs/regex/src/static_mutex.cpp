...
BOOST_STATIC_ASSERT(sizeof(LONG) == sizeof(_Boost::int32_t));
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
      ...
      while(0 != InterlockedCompareExchange(reinterpret_cast<void**>((_Boost::uint_least16_t*)&(m_mutex.m_mutex)), (void*)1, 0))
#else
...
_Boost::recursive_mutex* static_mutex::m_pmutex = 0;
_Boost::once_flag static_mutex::m_once = BOOST_ONCE_INIT;

extern "C" BOOST_REGEX_DECL void _Boost_regex_free_static_mutex()
{
...
{
   m_pmutex = new _Boost::recursive_mutex();
   int r = atexit(_Boost_regex_free_static_mutex);
   BOOST_ASSERT(0 == r);
}
...
void scoped_static_mutex_lock::lock()
{
   if(0 == m_have_lock)
   {
       _Boost::call_once(static_mutex::m_once,&static_mutex::init);
      if(0 == m_plock)
         m_plock = new _Boost::unique_lock<_Boost::recursive_mutex>(*static_mutex::m_pmutex, _Boost::defer_lock);
      m_plock->lock();
      m_have_lock = true;
   }
}
