...
//    const std::list<T>::iterator prev = _Boost::prior(p);
//    const std::list<T>::iterator next = _Boost::next(prev, 2);
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        _Boost::iterators::reverse_iterator< T > rx(x);
        _Boost::iterators::advance(rx, n);
        return rx.base();
    }
};
