...
template <class T> struct is_copy_constructible : public _Boost::is_constructible<T, const T&>{};
...
// _Boost::non_copyable as a base class as a special case:
//
...
namespace _Boost {
   ...
   namespace detail
   {
      ...
      template <class T, bool b> struct is_copy_constructible_imp : public _Boost::is_constructible<T, const T&>{};
...
namespace _Boost {
   ...
   namespace detail{
      ...
      struct is_copy_constructible_impl2 {
         ...
         template <class T1>
         static _Boost::type_traits::yes_type test(const T1&, _Boost::mpl::int_<sizeof(T1(_Boost::declval<const T1&>()))>* = 0);
#else
         template <class T1>
         static _Boost::type_traits::yes_type test(const T1&, decltype(T1(_Boost::declval<const T1&>()))* = 0);
#endif

         static _Boost::type_traits::no_type test(...);
#else
         template <class T1>
         static _Boost::type_traits::no_type test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
         static _Boost::type_traits::yes_type test(...);
#endif
         ...
         //      struct T: private _Boost::noncopyable {
         //          ...
         //      private:
         //          T(const T &);
         //          ...
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
            _Boost::declval<BOOST_DEDUCED_TYPENAME _Boost::add_reference<T const>::type>()
            )) == sizeof(_Boost::type_traits::yes_type)
            &&
            !_Boost::is_rvalue_reference<T>::value
            && !_Boost::is_array<T>::value
            ));
      };
      ...
      struct is_copy_constructible_impl {
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            _Boost::detail::is_copy_constructible_impl2<
            _Boost::is_noncopyable<T>::value,
            T
            >::value
            ));
      };
   ...
   template <class T> struct is_copy_constructible : public integral_constant<bool, ::_Boost::detail::is_copy_constructible_impl<T>::value>{};
   template <> struct is_copy_constructible<void> : public false_type{};
