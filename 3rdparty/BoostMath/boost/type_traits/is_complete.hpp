...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::_Boost::is_function<typename _Boost::remove_reference<T>::type>::value || (sizeof(_Boost::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(_Boost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : _Boost::integral_constant<bool, ::_Boost::is_function<typename _Boost::remove_reference<T>::type>::value || ::_Boost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : _Boost::is_complete<T> {};
   
#else

      template <class T> struct is_complete
         : public _Boost::integral_constant<bool, true> {};
