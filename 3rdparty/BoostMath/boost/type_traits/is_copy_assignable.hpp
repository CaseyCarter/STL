...
// https://connect.microsoft.com/VisualStudio/feedback/details/800328/std-is-copy-constructible-is-broken
    ...
    typedef _Boost::type_traits::yes_type yes_type;
    typedef _Boost::type_traits::no_type  no_type;

    template <class U>
    static decltype(::_Boost::declval<U&>() = ::_Boost::declval<const U&>(), yes_type() ) test(int);
...
#else
    static BOOST_DEDUCED_TYPENAME _Boost::add_reference<T>::type produce();

    template <class T1>
    static _Boost::type_traits::no_type test(T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
    ...
    BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(produce())) == sizeof(_Boost::type_traits::yes_type)
    ));
   #endif
};
...
struct is_copy_assignable_impl {
    ...
    //types derived from _Boost::noncopyable and types defined as BOOST_MOVEABLE_BUT_NOT_COPYABLE
    typedef BOOST_DEDUCED_TYPENAME _Boost::remove_reference<T>::type unreferenced_t;
    BOOST_STATIC_CONSTANT(bool, value = (
        _Boost::detail::is_copy_assignable_impl2<
            _Boost::is_noncopyable<T>::value
            || _Boost::is_const<unreferenced_t>::value || _Boost::is_array<unreferenced_t>::value
            ,T
        >::value
    ));
    #else
    BOOST_STATIC_CONSTANT(bool, value = (
        _Boost::detail::is_copy_assignable_impl2<
            _Boost::is_noncopyable<T>::value,T
        >::value
    ));
    #endif
};
...
template <class T> struct is_copy_assignable : public integral_constant<bool, ::_Boost::detail::is_copy_assignable_impl<T>::value>{};
