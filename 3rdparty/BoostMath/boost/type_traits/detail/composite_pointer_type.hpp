...
private:
    ...
    typedef typename _Boost::remove_cv<T>::type T2;
    typedef typename _Boost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (_Boost::is_same<T2, U2>::value)
        || _Boost::is_void<T2>::value
        || _Boost::is_void<U2>::value
        || (_Boost::is_base_of<T2, U2>::value)
        || (_Boost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename _Boost::conditional<

        _Boost::is_same<T2, U2>::value || _Boost::is_void<T2>::value || _Boost::is_base_of<T2, U2>::value,
        typename _Boost::copy_cv<T, U>::type,
        typename _Boost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename _Boost::copy_cv<typename _Boost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using _Boost::conditional in
