...
//
namespace _Boost{ namespace type_traits_detail{
...
{
  template<typename Q> static _Boost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static _Boost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename _Boost::enable_if_<has_one_operator_call<T>::value>::type> :
     _Boost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace _Boost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public _Boost::integral_constant<bool, false> {};
}}
