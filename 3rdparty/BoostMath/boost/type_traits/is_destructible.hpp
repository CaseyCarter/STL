...
namespace _Boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(_Boost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(_Boost::detail::is_destructible_imp::test<T>(0)) == sizeof(_Boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(_Boost::is_complete<T>::value, "Arguments to is_destructible must be complete types");
   };
