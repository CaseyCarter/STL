...
namespace _Boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(_Boost::declval<T>() = _Boost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(_Boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(_Boost::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
