    ...
    Char delim)
{
    ...
    if (entry) {
        quoted_state<Char> state = _Boost::io::detail::quoted_start(string,
            escape, delim);
        std::basic_streambuf<Char, Traits>& buf = *os.rdbuf();
        std::size_t width = static_cast<std::size_t>(os.width());
        if (width <= state.count) {
            if (!_Boost::io::detail::quoted_put(buf, state.string, state.size,
            ...
            if (!_Boost::io::detail::quoted_put(buf, state.string, state.size,
                    state.count, escape, delim) ||
                !_Boost::io::detail::buffer_fill(buf, os.fill(),
                    width - state.count)) {
                return os;
            }
        } else if (!_Boost::io::detail::buffer_fill(buf, os.fill(),
                width - state.count) ||
            !_Boost::io::detail::quoted_put(buf, state.string, state.size,
                state.count, escape, delim)) {
            return os;
        }
        os.width(0);
...
{
    return _Boost::io::detail::quoted_out(os, proxy.string, proxy.escape,
        proxy.delim);
}
    ...
    const quoted_proxy<std::basic_string<Char, Traits, Alloc>*, Char>& proxy)
{
    ...
    {
        _Boost::io::ios_flags_saver ifs(is);
        std::noskipws(is);
        proxy.string->clear();
