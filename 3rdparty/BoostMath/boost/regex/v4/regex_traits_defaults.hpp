...
inline bool is_separator(charT c)
{
   return BOOST_REGEX_MAKE_BOOL(
      ...
      || (static_cast<_Boost::uint16_t>(c) == 0x2028u)
      || (static_cast<_Boost::uint16_t>(c) == 0x2029u)
      || (static_cast<_Boost::uint16_t>(c) == 0x85u));
}
template <>
...
_Boost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   (void)t; // warning suppression
   _Boost::intmax_t limit = (std::numeric_limits<_Boost::intmax_t>::max)() / radix;
   _Boost::intmax_t next_value = t.value(*p1, radix);
   if((p1 == p2) || (next_value < 0) || (next_value >= radix))
      return -1;
   _Boost::intmax_t result = 0;
   while(p1 != p2)
   {
...
template <class charT>
inline typename _Boost::enable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
...
template <class charT>
inline typename _Boost::disable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
