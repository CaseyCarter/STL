...
#pragma warning(push)
// '_Boost::atomics::atomic_ref<T>' : multiple assignment operators specified
...
private:
    ...
    {
        storage_type old_value = operations::load(this->storage(), _Boost::memory_order_relaxed);
        while (true)
        {
            ...
            bool res = operations::compare_exchange_weak(this->storage(), old_value, new_value, _Boost::memory_order_relaxed, _Boost::memory_order_relaxed);
            if (BOOST_LIKELY(res))
                break;
        }
    }
...
public:
    ...
    BOOST_STATIC_ASSERT_MSG(sizeof(value_type) > 0u, "_Boost::atomic_ref<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    BOOST_STATIC_ASSERT_MSG(atomics::detail::is_trivially_copyable< value_type >::value, "_Boost::atomic_ref<T> requires T to be a trivially copyable type");
#endif
