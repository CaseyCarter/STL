...
// forward declaration
namespace _Boost { template<class T> class optional; }
...
namespace _Boost { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const _Boost::any& value_store) const
    {
        const T* value = _Boost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                _Boost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                _Boost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const _Boost::any& value);
    }
    ...
    */
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            _Boost::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(_Boost::any& v, 
    ...
    {
        if (v.empty()) {
            v = _Boost::any(std::vector<T>());
        }
        std::vector<T>* tv = _Boost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(_Boost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                _Boost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
                  ...
                  int)
    {
        ...
        _Boost::any a;
        validate(a, s, (T*)0, 0);
        v = _Boost::any(_Boost::optional<T>(_Boost::any_cast<T>(a)));
    }

    template<class T, class charT>
    void 
    typed_value<T, charT>::
    xparse(_Boost::any& value_store, 
    ...
    value()
    {
        ...
        return _Boost::program_options::value<T>(0);
    }
