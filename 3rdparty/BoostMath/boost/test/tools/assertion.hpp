...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(_Boost::declval<Lhs>() oper _Boost::declval<Rhs>() ) optype; \
    typedef typename _Boost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _, _i )                     \
    template<typename T>                                        \
    binary_expr<ExprType,typename _Boost::decay<T const>::type,  \
        op::name<ValType,typename _Boost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename _Boost::decay<T const>::type Rhs;       \
