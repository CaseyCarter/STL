...
public:
    ...
    void            setup( const const_string& stream_name,
                           _Boost::function<void ()> const &cleaner_callback = _Boost::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = _Boost::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(_Boost::function<void ()> cleaner_callback)
        ...
        _Boost::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    _Boost::shared_ptr<callback_cleaner>   m_cleaner;
