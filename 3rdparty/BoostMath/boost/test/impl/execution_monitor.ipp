...
extract( _Boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = _Boost::get_error_info<ErrorInfo>( *ex );
...
#endif
report_error( execution_exception::error_code ec, _Boost::exception const* be, char const* format, va_list* args )
{
...
#endif
report_error( execution_exception::error_code ec, _Boost::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return _Boost::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    _Boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( _Boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( _Boost::function<int ()> const& F )
{
    ...
    // _Boost::exception (before std::exception, with extended diagnostic)
    catch( _Boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", _Boost::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<_Boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( _Boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    _Boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( _Boost::function<void ()> const& F )
{
