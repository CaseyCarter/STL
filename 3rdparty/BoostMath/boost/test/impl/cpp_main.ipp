...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_I_TRY {
        _Boost::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::_Boost::execution_monitor ex_mon;
        ...
        if( result == 0 )
            result = ::_Boost::exit_success;
        else if( result != ::_Boost::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::_Boost::exit_failure;
        }
    }
    BOOST_TEST_I_CATCH( ::_Boost::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::_Boost::exit_exception_failure;
    }
    BOOST_TEST_I_CATCH( ::_Boost::system_error, ex ) {
    ...
    if( result != ::_Boost::exit_success ) {
        ...
        ::_Boost::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
...
{
    return ::_Boost::prg_exec_monitor_main( &cpp_main, argc, argv );
}
