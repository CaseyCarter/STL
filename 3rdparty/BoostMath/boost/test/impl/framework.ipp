...
private:
    bool            filter_unit( test_unit const& tu )
    {
        ...
        // look for match
        using namespace _Boost::placeholders;
        return std::find_if( filters.begin(), filters.end(), bind( &component::pass, _1, _Boost::ref(tu) ) ) != filters.end();
    }
...
public:
                                        ...
                                        random_generator_helper const * const p_random_generator = 0)
    {
        ...
        BOOST_TEST_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
            ...
            result = unit_test_monitor.execute_and_translate( _Boost::bind( &test_unit_fixture::setup, F ) );
        ...
        if( result == unit_test_monitor_t::test_ok ) {
            ...
            _Boost::unit_test::timer::timer tu_timer;
        ...
        if( !unit_test_monitor.is_critical_error( result ) ) {
            ...
            BOOST_TEST_REVERSE_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
                ut_detail::test_unit_id_restore restore_current_test_unit(m_curr_test_unit, tu.p_id);
                result = (std::min)( result, unit_test_monitor.execute_and_translate( _Boost::bind( &test_unit_fixture::teardown, F ), 0 ) );
    ...
    _Boost::execution_monitor m_aux_em;
...
setup_loggers()
{
    ...
    BOOST_TEST_I_TRY {
        ...
        if( !has_combined_logger ) {
            ...
            if( runtime_config::has( runtime_config::btrt_log_sink ) ) {
                ...
                _Boost::function< void () > log_cleaner = _Boost::bind( &unit_test_log_t::set_stream,
                                                                      &unit_test_log,
                                                                      _Boost::ref(std::cout)
                                                                      );
            ...
            BOOST_TEST_FOREACH( const_string, current_multi_config, v_output_format ) {
                ...
                BOOST_TEST_FOREACH( std::string const&, current_config, v_processed_tokens ) {
                    ...
                    BOOST_TEST_I_ASSRT( format != OF_INVALID,
                                        _Boost::runtime::access_to_missing_argument()
    ...
    BOOST_TEST_I_CATCH( _Boost::runtime::init_error, ex ) {
        BOOST_TEST_SETUP_ASSERT( false, ex.msg );
    }
    BOOST_TEST_I_CATCH( _Boost::runtime::input_error, ex ) {
        std::cerr << ex.msg << "\n\n";

        BOOST_TEST_I_THROW( framework::nothing_to_test( _Boost::exit_exception_failure ) );
    }
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    if( runtime_config::has( runtime_config::btrt_report_sink ) ) {
        _Boost::function< void () > report_cleaner = _Boost::bind( &results_reporter::set_stream,
                                                                 _Boost::ref(std::cerr)
                                                                );
    ...
    BOOST_TEST_I_TRY {
        s_frk_state().m_aux_em.vexecute( _Boost::bind( &impl::invoke_init_func, init_func ) );
    }
...
int
add_context( ::_Boost::unit_test::lazy_ostream const& context_descr, bool sticky )
{
    ...
    if( call_start_finish ) {
        ...
        BOOST_TEST_FOREACH( test_observer*, to, impl::s_frk_state().m_observers ) {
            BOOST_TEST_I_TRY {
                ...
                unit_test_monitor_t::error_level result = unit_test_monitor.execute_and_translate( _Boost::bind( &test_observer::test_start, to, tcc.p_count, id ) );
                if( init_ok ) {
                    if( result != unit_test_monitor_t::test_ok ) {
                        init_ok = false;
                    }
