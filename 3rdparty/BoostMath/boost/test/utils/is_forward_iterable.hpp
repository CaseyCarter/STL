...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(_Boost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(_Boost::declval<U&>())); // does not work with _Boost::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(_Boost::declval<U&>())); // does not work with _Boost::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(_Boost::declval<
        typename _Boost::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename _Boost::add_const<T>::type T_const;
    typedef typename _Boost::add_pointer<T_const>::type const_iterator;
