...
template<class CharT>
inline bool
operator <( _Boost::unit_test::basic_cstring<CharT> const& x,
            _Boost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename _Boost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size()
            ? x.size() < y.size()
