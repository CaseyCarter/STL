...
public:
    void    operator()()    { TestCaseTemplate::run( (_Boost::type<TestType>*)0 ); }
};
...
struct generate_test_case_4_type {
    ...
    void        operator()( mpl::identity<TestType> )
    {
        ...
        full_name += '<';
#if !defined(BOOST_NO_TYPEID) && !defined(BOOST_NO_RTTI)
        full_name += _Boost::core::demangle(typeid(TestType).name()); // same as execution_monitor.ipp
#else
        ...
        typedef typename _Boost::remove_reference<TestType>::type TestTypewoRef;
        if( _Boost::is_const<TestTypewoRef>::value )
            full_name += "_const";
        if( _Boost::is_volatile<TestTypewoRef>::value )
            full_name += "_volatile";
        if( _Boost::is_rvalue_reference<TestType>::value )
            full_name += "_refref";
        else if( _Boost::is_lvalue_reference<TestType>::value )
            full_name += "_ref";
...
class template_test_case_gen<
    TestCaseTemplate,
    C<parameter_pack...>,
    typename std::enable_if<!_Boost::mpl::is_sequence<C<parameter_pack...>>::value>::type >
