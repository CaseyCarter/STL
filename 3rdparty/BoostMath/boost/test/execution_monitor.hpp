...
///
/// All symbols in the Execution Monitor implementation are located in the namespace _Boost. To use the Execution Monitor you need to:
...
typedef _Boost::shared_ptr<translator_holder_base> translator_holder_base_ptr;

class BOOST_TEST_DECL translator_holder_base {
protected:
    typedef _Boost::unit_test::const_string const_string;
public:
    // Constructor
    ...
    virtual int operator()( _Boost::function<int ()> const& F ) = 0;
    ...
    translator_holder_base_ptr erase( translator_holder_base_ptr this_, _Boost::type<ExceptionType>* = 0 )
    {
        if( m_next )
...
public:
    ...
    /// @see vexecute
    int         execute( _Boost::function<int ()> const& F );
    ...
    /// @param[in] F  Function to monitor
    /// @see execute
    void         vexecute( _Boost::function<void ()> const& F );
    // @}
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, const_string tag = const_string(), _Boost::type<ExceptionType>* = 0 );
    ...
    template<typename ExceptionType>
    void        erase_exception_translator( _Boost::type<ExceptionType>* = 0 )
    {
...
private:
    ...
    int         catch_signals( _Boost::function<int ()> const& F );

    // Data members
    detail::translator_holder_base_ptr  m_custom_translators;
    _Boost::scoped_array<char>           m_alt_stack;
}; // execution_monitor
...
public:
    ...
    // translator holder interface
    virtual int operator()( _Boost::function<int ()> const& F )
    {
        BOOST_TEST_I_TRY {
        ...
        BOOST_TEST_I_CATCH( ExceptionType, e ) {
            m_translator( e );
            return _Boost::exit_exception_failure;
        }
    }
#ifndef BOOST_NO_RTTI
...
template<typename ExceptionType, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, const_string tag, _Boost::type<ExceptionType>* )
{
...
//!@internal
#define BOOST_TEST_SYS_ASSERT( cond ) BOOST_TEST_I_ASSRT( cond, ::_Boost::system_error( BOOST_STRINGIZE( exp ) ) )
