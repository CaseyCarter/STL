...
public:
    ...
    shared_array( _Boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT : px( 0 ), pn()
    {
    }
...
// ... except in C++0x, move disables the implicit copy
    ...
    shared_array( shared_array<Y> const & r, typename _Boost::detail::sp_enable_if_convertible< Y[], T[] >::type = _Boost::detail::sp_empty() )
...
template<class T> inline bool operator==( shared_array<T> const & p, _Boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( _Boost::detail::sp_nullptr_t, shared_array<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
