...
struct sp_array_element {
    typedef typename _Boost::remove_cv<typename
        _Boost::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<_Boost::alignment_of<T>::value,
            _Boost::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename _Boost::type_with_alignment<sp_array_alignment<T,
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        _Boost::alloc_construct_n(state_.allocator(),
            _Boost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        _Boost::alloc_construct_n(state_.allocator(),
            _Boost::first_scalar(start), state_.size() * count,
            _Boost::first_scalar(&list), count);
    }
    ...
    virtual void dispose() BOOST_SP_NOEXCEPT {
        _Boost::alloc_destroy_n(state_.allocator(),
            _Boost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return _Boost::allocate_shared<T>(_Boost::noinit_adapt(allocator), count);
}
...
{
    return _Boost::allocate_shared<T>(_Boost::noinit_adapt(allocator));
}
