...
public:
    ...
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename _Boost::detail::sp_enable_if_convertible<U,T>::type = _Boost::detail::sp_empty() )
...
// Move support
    ...
    intrusive_ptr(intrusive_ptr<U> && rhs, typename _Boost::detail::sp_enable_if_convertible<U,T>::type = _Boost::detail::sp_empty())
...
template<class T> inline bool operator==( intrusive_ptr<T> const & p, _Boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( _Boost::detail::sp_nullptr_t, intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return p.get() == 0;
}
...
template< class T > std::size_t hash_value( _Boost::intrusive_ptr<T> const & p ) BOOST_SP_NOEXCEPT
{
    return _Boost::hash< T* >()( p.get() );
}
