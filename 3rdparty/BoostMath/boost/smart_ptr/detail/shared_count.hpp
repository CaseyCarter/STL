...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( _Boost::addressof( t ) )
    {
    }
...
public:
...
#endif
    {
        ...
        {
            _Boost::checked_delete( p );
            _Boost::throw_exception( std::bad_alloc() );
        }
    ...
    template<class Y, class D>
    explicit shared_count( _Boost::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        , id_(shared_count_id)
#endif
    {
        ...
        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename _Boost::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
#endif
{
    ...
    {
        _Boost::throw_exception( _Boost::bad_weak_ptr() );
    }
}
