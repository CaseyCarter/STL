...
#define BOOST_INTRUSIVE_HAS_MEMBER_FUNCTION_CALLABLE_WITH_NS_BEG namespace _Boost { namespace container { namespace dtl {
...
struct allocator_traits
{
   ...
   typedef typename allocator_type::value_type value_type;
   ...
   #else
      //pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(_Boost::container::dtl::, Allocator,
         pointer, value_type*)
            pointer;
      //const_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(_Boost::container::dtl::, Allocator,
         const_pointer, typename _Boost::intrusive::pointer_traits<pointer>::template
      ...
      //void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(_Boost::container::dtl::, Allocator,
         void_pointer, typename _Boost::intrusive::pointer_traits<pointer>::template
            rebind_pointer<void>)
               void_pointer;
      //const_void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(_Boost::container::dtl::, Allocator,
         const_void_pointer, typename _Boost::intrusive::pointer_traits<pointer>::template
      ...
      //rebind_alloc & rebind_traits
         ...
         //C++11
         template <typename T> using rebind_alloc  = typename _Boost::intrusive::pointer_rebind<Allocator, T>::type;
         template <typename T> using rebind_traits = allocator_traits< rebind_alloc<T> >;
      #else    // #if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
         //Some workaround for C++03 or C++11 compilers with no template aliases
         template <typename T>
         struct rebind_alloc : _Boost::intrusive::pointer_rebind<Allocator,T>::type
         {
            typedef typename _Boost::intrusive::pointer_rebind<Allocator,T>::type Base;
            #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
               template <typename... Args>
               rebind_alloc(BOOST_FWD_REF(Args)... args) : Base(_Boost::forward<Args>(args)...) {}
         ...
         struct rebind_traits
            : allocator_traits<typename _Boost::intrusive::pointer_rebind<Allocator, T>::type>
         {};
      ...
      template <class T>
      struct portable_rebind_alloc
      {  typedef typename _Boost::intrusive::pointer_rebind<Allocator, T>::type type;  };
   ...
   {
      const bool value = _Boost::container::dtl::
   ...
   select_on_container_copy_construction(const Allocator &a)
   {
      ...
      {
         static const bool value = ::_Boost::move_detail::and_
            < dtl::is_not_std_allocator<Allocator>
            , _Boost::container::dtl::has_member_function_callable_with_construct
                  < Allocator, T*, Args... >
            >::value;
         dtl::bool_<value> flag;
         allocator_traits::priv_construct(flag, a, p, ::_Boost::forward<Args>(args)...);
      }
   #endif
   ...
   template<class T>
   BOOST_CONTAINER_FORCEINLINE static void priv_construct(dtl::false_type, Allocator &, T *p, const ::_Boost::container::default_init_t&)
