...
class node_handle
{
   ...
   typedef ::_Boost::aligned_storage
      < sizeof(nallocator_type)
      , _Boost::alignment_of<nallocator_type>::value>              nalloc_storage_t;

   node_pointer      m_ptr;
   nalloc_storage_t  m_nalloc_storage;

   void move_construct_alloc(nallocator_type &al)
   {  ::new(m_nalloc_storage.address(), boost_container_new_t()) nallocator_type(::_Boost::move(al));   }

   void destroy_deallocate_node()
   {
      nator_traits::destroy(this->node_alloc(), _Boost::movelib::to_raw_pointer(m_ptr));
   ...
   {
      if(m_ptr){
         ::new (m_nalloc_storage.address(), boost_container_new_t()) nallocator_type(::_Boost::move(nh.node_alloc()));
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            ...
            if(nator_traits::propagate_on_container_move_assignment::value){
               this->node_alloc() = ::_Boost::move(nh.node_alloc());
            }
         }
         else{
      ...
      BOOST_NOEXCEPT_IF(nator_traits::propagate_on_container_swap::value || nator_traits::is_always_equal::value)
   {
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            if(nator_traits::propagate_on_container_swap::value){
               ::_Boost::adl_move_swap(this->node_alloc(), nh.node_alloc());
            }
         }
         else{
            this->move_construct_alloc(nh.node_alloc());
      ...
      ::_Boost::adl_move_swap(m_ptr, nh.m_ptr);
   }
...
struct insert_return_type_base
{
   ...
   insert_return_type_base(BOOST_RV_REF(insert_return_type_base) other)
      : inserted(other.inserted), position(other.position), node(_Boost::move(other.node))
   {}

   template<class RelatedIt, class RelatedNode>
   insert_return_type_base(bool insert, RelatedIt it, BOOST_RV_REF(RelatedNode) node)
      : inserted(insert), position(it), node(_Boost::move(node))
   {}
