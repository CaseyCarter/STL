            ...
            portable_rebind_alloc<typename ICont::value_type>::type   //NodeAlloc
{
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x)
      : NodeAlloc(_Boost::move(x.node_alloc()))
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x, const val_compare &c)
      : NodeAlloc(_Boost::move(x.node_alloc())), m_icont(typename ICont::key_compare(c))
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      BOOST_TRY{
         ::new(_Boost::movelib::iterator_to_raw_pointer(p), boost_container_new_t()) Node;
         allocator_traits<NodeAlloc>::construct
            (this->node_alloc()
            , p->get_real_data_ptr(), _Boost::forward<Args>(args)...);
      }
      BOOST_CATCH(...) {
         p->destroy_header();
   ...
   NodePtr create_node_from_it(const It &it)
   {
      ...
      BOOST_TRY{
         ::new(_Boost::movelib::iterator_to_raw_pointer(p), boost_container_new_t()) Node;
         ::_Boost::container::construct_in_place(this->node_alloc(), p->get_real_data_ptr(), it);
      }
      BOOST_CATCH(...) {
         p->destroy_header();
         this->node_alloc().deallocate(p, 1);
   ...
   NodePtr create_node_from_key(BOOST_FWD_REF(KeyConvertible) key)
   {
      ...
      BOOST_TRY{
         ...
         node_allocator_traits_type::construct
            (na, dtl::addressof(p->get_real_data().first), _Boost::forward<KeyConvertible>(key));
         BOOST_TRY{
   ...
   {
      allocator_traits<NodeAlloc>::destroy(this->node_alloc(), _Boost::movelib::to_raw_pointer(nodep));
      ...
      (FwdIterator beg, difference_type n, Inserter inserter)
   {
      if(n){
         ...
         Node *p = 0;
            ...
            while(n){
               --n;
               p = _Boost::movelib::iterator_to_raw_pointer(itbeg);
               ++itbeg; //Increment iterator before overwriting pointed memory
               //This does not throw
               p = ::new(p, boost_container_new_t()) Node;
               node_deallocator.set(p);
               //This can throw
               _Boost::container::construct_in_place(nalloc, p->get_real_data_ptr(), beg);
               sdestructor.set(p);
               ++beg;
   ...
   void clear(version_2)
   {
      ...
      //BOOST_STATIC_ASSERT((::_Boost::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
   ...
   struct move_cloner
   {
      ...
      {  //Use get_real_data() instead of get_real_data to allow moving const key in [multi]map
         return m_holder.create_node(::_Boost::move(other.get_real_data()));
      }
