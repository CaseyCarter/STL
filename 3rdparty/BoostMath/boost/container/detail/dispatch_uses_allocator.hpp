...
// Check if we can detect is_convertible using advanced SFINAE expressions
   ...
   struct is_constructible
   {
      ...
      template<class X>
      static decltype(X(_Boost::move_detail::declval<Args>()...), true_type()) test(int);
...
{
   (void)arg_alloc;
   allocator_traits<ConstructAlloc>::construct(construct_alloc, p, ::_Boost::forward<Args>(args)...);
}
   ...
   ( ConstructAlloc& construct_alloc, BOOST_FWD_REF(ArgAlloc) arg_alloc, T* p, BOOST_FWD_REF(Args) ...args)
{
   allocator_traits<ConstructAlloc>::construct
      ...
      , ::_Boost::forward<ArgAlloc>(arg_alloc), ::_Boost::forward<Args>(args)...);
}
...
{
   allocator_traits<ConstructAlloc>::construct
      (construct_alloc, p, ::_Boost::forward<Args>(args)..., ::_Boost::forward<ArgAlloc>(arg_alloc));
}
...
#define BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE(N) \
   ...
   {\
      allocator_traits<ConstructAlloc>::construct\
         (construct_alloc, p, allocator_arg, ::_Boost::forward<ArgAlloc>(arg_alloc) BOOST_MOVE_I##N BOOST_MOVE_FWD##N);\
   }\
//
BOOST_MOVE_ITERATE_0TO9(BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE)
#undef BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE
...
#define BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE(N) \
   ...
   {\
      allocator_traits<ConstructAlloc>::construct\
         (construct_alloc, p BOOST_MOVE_I##N BOOST_MOVE_FWD##N, ::_Boost::forward<ArgAlloc>(arg_alloc));\
   }\
//
BOOST_MOVE_ITERATE_0TO9(BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE)
#undef BOOST_CONTAINER_SCOPED_ALLOCATOR_DISPATCH_USES_ALLOCATOR_CODE
...
{
   (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->first), ::_Boost::forward<U>(x));
   BOOST_TRY{
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->second), ::_Boost::forward<V>(y));
   }
   BOOST_CATCH(...){
      allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(p->first));
      BOOST_RETHROW
...
typename dtl::enable_if_and
   < void
   , dtl::is_pair<Pair>
   , dtl::not_<_Boost::move_detail::is_reference<Pair2> > >::type //This is needed for MSVC10 and ambiguous overloads
   dispatch_uses_allocator
   (ConstructAlloc & construct_alloc
      , BOOST_FWD_REF(ArgAlloc) arg_alloc
      , Pair* p, BOOST_RV_REF_BEG Pair2 BOOST_RV_REF_END x)
{  (dispatch_uses_allocator)(construct_alloc, arg_alloc, p, ::_Boost::move(x.first), ::_Boost::move(x.second));  }
...
//piecewise construction from Std Tuple
   ...
   {
      (void)t1; (void)t2;
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(pair->first), ::_Boost::forward<Args1>(get<Indexes1>(t1))...);
      BOOST_TRY{
         (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(pair->second), ::_Boost::forward<Args2>(get<Indexes2>(t2))...);
      }
      BOOST_CATCH(...){
         allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(pair->first));
         BOOST_RETHROW
...
{
   (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->first), ::_Boost::forward<KeyType>(k));
   BOOST_TRY{
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->second), ::_Boost::forward<Args>(args)...);
   }
   BOOST_CATCH(...) {
      allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(p->first));
      BOOST_RETHROW
