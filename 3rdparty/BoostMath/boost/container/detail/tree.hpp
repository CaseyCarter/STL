...
using _Boost::intrusive::tree_value_compare;

template<class VoidPointer, _Boost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct intrusive_tree_hook;

template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, _Boost::container::red_black_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, _Boost::container::avl_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, _Boost::container::scapegoat_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, _Boost::container::splay_tree, OptimizeSize>
{
   typedef typename dtl::bi::make_bs_set_base_hook
      < dtl::bi::void_pointer<VoidPointer>
      , dtl::bi::link_mode<dtl::bi::normal_link>
...
{
   typedef pair<typename _Boost::move_detail::remove_const<T1>::type, T2> type;
};

//The node to be store in the tree
template <class T, class VoidPointer, _Boost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct tree_node
   :  public intrusive_tree_hook<VoidPointer, tree_type_value, OptimizeSize>::type
{
   ...
   typedef typename _Boost::container::dtl::aligned_storage
      <sizeof(T), _Boost::container::dtl::alignment_of<T>::value>::type storage_t;
   storage_t m_storage;
   ...
   {
      const_cast<T1&>(this->get_real_data().first) = ::_Boost::move(p.first);
      this->get_real_data().second = ::_Boost::move(p.second);
   }
   ...
   {  this->get_real_data() = ::_Boost::move(v); }
};
...
template< class NodeType, class NodeCompareType
        ...
        , _Boost::container::tree_type_enum tree_type_value>
struct intrusive_tree_dispatch;

template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, _Boost::container::red_black_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, _Boost::container::avl_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, _Boost::container::scapegoat_tree>
{
...
template<class NodeType, class NodeCompareType, class SizeType, class HookType>
struct intrusive_tree_dispatch
   <NodeType, NodeCompareType, SizeType, HookType, _Boost::container::splay_tree>
{
...
template<class Allocator, class ValueCompare, _Boost::container::tree_type_enum tree_type_value, bool OptimizeSize>
...
template<_Boost::container::tree_type_enum tree_type_value>
...
struct key_node_compare
   :  public _Boost::intrusive::detail::ebo_functor_holder<KeyCompare>
{
   BOOST_CONTAINER_FORCEINLINE explicit key_node_compare(const KeyCompare &comp)
      :  base_t(comp)
   {}

   typedef _Boost::intrusive::detail::ebo_functor_holder<KeyCompare> base_t;
...
template<class T1, class T2>
struct real_key_of_value<_Boost::container::pair<T1, T2>, int>
{
   typedef dtl::select1st<T1> type;
};
      ...
      >
{
   ...
   typedef _Boost::container::reverse_iterator
         ...
         )
   {
      ...
      this->allocate_many_and_construct
         ( first, _Boost::container::iterator_distance(first, last)
      ...
      :  AllocHolder(x.value_comp(), a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->icont().clone_from
            (_Boost::move(x.icont()), typename AllocHolder::move_cloner(*this), Destroyer(this->node_alloc()));
      }
      //AllocHolder clears in case of exception
   }
   ...
   tree& operator=(BOOST_COPY_ASSIGN_REF(tree) x)
   {
      if (BOOST_LIKELY(this != &x)) {
         ...
         //automatically
         Icont other_tree(::_Boost::move(this->icont()));
                           ...
                           _Boost::container::dtl::is_nothrow_move_assignable<Compare>::value)
   {
      if (BOOST_LIKELY(this != &x)) {
         ...
         if(propagate_alloc || allocators_equal){
            ...
            //Obtain resources
            this->icont() = _Boost::move(x.icont());
         }
   ...
   {
      NodePtr tmp = AllocHolder::create_node(_Boost::forward<MovableConvertible>(v));
   ...
   std::pair<iterator,bool> insert_unique(BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      if(ret.second){
         ret.first = this->insert_unique_commit(_Boost::forward<MovableConvertible>(v), data);
      }
      return ret;
   }
   ...
   {
      NodePtr tmp = AllocHolder::create_node(_Boost::forward<KeyConvertible>(key), _Boost::forward<M>(obj));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   {  return this->emplace_unique_impl(AllocHolder::create_node(_Boost::forward<Args>(args)...));   }

   template <class... Args>
   BOOST_CONTAINER_FORCEINLINE iterator emplace_hint_unique(const_iterator hint, BOOST_FWD_REF(Args)... args)
   {  return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(_Boost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_equal(BOOST_FWD_REF(Args)... args)
   {
      NodePtr tmp(AllocHolder::create_node(_Boost::forward<Args>(args)...));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      ...
      (const_iterator hint, BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(Args)... args)
   {
      ...
      if(ret.second){
         ret.first = this->icont().insert_unique_commit
            (*AllocHolder::create_node(try_emplace_t(), _Boost::forward<KeyType>(key), _Boost::forward<Args>(args)...), data);
      }
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);
   }
   ...
   {\
      ...
      if(ret.second){\
         ret.first = this->icont().insert_unique_commit\
            (*AllocHolder::create_node(try_emplace_t(), _Boost::forward<KeyType>(key) BOOST_MOVE_I##N BOOST_MOVE_FWD##N), data);\
      }\
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);\
   }\
   //
   ...
   iterator insert_unique_convertible(const_iterator hint, BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      if(!ret.second)
         return ret.first;
      return this->insert_unique_commit(_Boost::forward<MovableConvertible>(v), data);
   }
   ...
   std::pair<iterator, bool> insert_or_assign(const_iterator hint, BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(M) obj)
   {
      ...
      std::pair<iiterator, bool> ret =
         hint == const_iterator() ? this->icont().insert_unique_check(k, KeyNodeCompare(key_comp()), data)
                                  : this->icont().insert_unique_check(hint.get(), k, KeyNodeCompare(key_comp()), data);
      if(ret.second){
         ret.first = this->priv_insert_or_assign_commit(_Boost::forward<KeyType>(key), _Boost::forward<M>(obj), data);
      }
      else{
         ret.first->get_data().second = _Boost::forward<M>(obj);
      }
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);
   }
   ...
   {
      return this->insert_unique_node(this->end(), _Boost::move(nh));
   }
   ...
   BOOST_CONTAINER_FORCEINLINE friend bool operator<(const tree& x, const tree& y)
   {  return ::_Boost::container::algo_lexicographical_compare(x.begin(), x.end(), y.begin(), y.end());  }
...
struct has_trivial_destructor_after_move
   < 
      ::_Boost::container::dtl::tree
         <T, KeyOfValue, Compare, Allocator, Options>
   >
{
   typedef typename ::_Boost::container::dtl::tree<T, KeyOfValue, Compare, Allocator, Options>::allocator_type allocator_type;
   ...
   static const bool value = ::_Boost::has_trivial_destructor_after_move<allocator_type>::value &&
                             ...
                             ::_Boost::has_trivial_destructor_after_move<Compare>::value;
};
