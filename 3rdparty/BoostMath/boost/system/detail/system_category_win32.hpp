...
inline _Boost::winapi::UINT_ message_cp_win32()
{
#if defined(BOOST_SYSTEM_USE_UTF8)

    return _Boost::winapi::CP_UTF8_;

#else

    return _Boost::winapi::CP_ACP_;
...
inline char const * system_category_message_win32( int ev, char * buffer, std::size_t len ) BOOST_NOEXCEPT
{
    ...
    _Boost::winapi::UINT_ const code_page = message_cp_win32();

    int r = 0;

#if !defined(BOOST_NO_ANSI_APIS)

    if( code_page == _Boost::winapi::CP_ACP_ )
    {
        using namespace _Boost::winapi;

        DWORD_ retval = _Boost::winapi::FormatMessageA(
...

    {
        ...
        DWORD_ retval = _Boost::winapi::FormatMessageW(
        ...
        if( retval != 0 )
        {
            r = _Boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, buffer, static_cast<int>( len ), NULL, NULL );
            _Boost::winapi::LocalFree( lpMsgBuf );
...
inline std::string system_category_message_win32( int ev )
{
    ...
    int r = _Boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, 0, 0, NULL, NULL );
    ...
    r = _Boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, &buffer[0], r, NULL, NULL );
