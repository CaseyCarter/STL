...
private:
    ...
    _Boost::system::error_category const * pc_;

public:

    explicit std_category( _Boost::system::error_category const * pc, unsigned id ): pc_( pc )
    {
        if( id != 0 )
        {
...
inline std::error_category const & to_std_category( _Boost::system::error_category const & cat ) BOOST_SYMBOL_VISIBLE;
#endif

struct cat_ptr_less
{
    bool operator()( _Boost::system::error_category const * p1, _Boost::system::error_category const * p2 ) const BOOST_NOEXCEPT
    {
        return *p1 < *p2;
    }
};

inline std::error_category const & to_std_category( _Boost::system::error_category const & cat )
{
    if( cat == _Boost::system::system_category() )
    {
        static const std_category system_instance( &cat, 0x1F4D7 );
        return system_instance;
    }
    else if( cat == _Boost::system::generic_category() )
    {
    ...
    {
        typedef std::map< _Boost::system::error_category const *, std::unique_ptr<std_category>, cat_ptr_less > map_type;
...
inline bool std_category::equivalent( int code, const std::error_condition & condition ) const BOOST_NOEXCEPT
{
    ...
    {
        _Boost::system::error_condition bn( condition.value(), *pc_ );
        return pc_->equivalent( code, bn );
    }
    else if( condition.category() == std::generic_category() || condition.category() == _Boost::system::generic_category() )
    {
        _Boost::system::error_condition bn( condition.value(), _Boost::system::generic_category() );
        return pc_->equivalent( code, bn );
    }
    ...
    {
        _Boost::system::error_condition bn( condition.value(), *pc2->pc_ );
        return pc_->equivalent( code, bn );
    }
...
inline bool std_category::equivalent( const std::error_code & code, int condition ) const BOOST_NOEXCEPT
{
    ...
    {
        _Boost::system::error_code bc( code.value(), *pc_ );
        return pc_->equivalent( bc, condition );
    }
    else if( code.category() == std::generic_category() || code.category() == _Boost::system::generic_category() )
    {
        _Boost::system::error_code bc( code.value(), _Boost::system::generic_category() );
        return pc_->equivalent( bc, condition );
    }
    ...
    {
        _Boost::system::error_code bc( code.value(), *pc2->pc_ );
        return pc_->equivalent( bc, condition );
    }
#endif

    else if( *pc_ == _Boost::system::generic_category() )
    {
