...
namespace date_time {
  ...
  */
  ...
  class BOOST_SYMBOL_VISIBLE time_duration : private
      _Boost::less_than_comparable<T
    , _Boost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, _Boost::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(_Boost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    // The argument (ss) must be an integral type
    template <typename T>
    BOOST_CXX14_CONSTEXPR explicit subsecond_duration(T const& ss,
                                                      typename _Boost::enable_if<_Boost::is_integral<T>,
