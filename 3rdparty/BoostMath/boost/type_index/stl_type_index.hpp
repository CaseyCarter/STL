...
/// \brief Contains _Boost::typeindex::stl_type_index class.
///
/// _Boost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined _Boost::typeindex::ctti is usually used instead of _Boost::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "_Boost::typeindex::detail::cvr_saver<";
    ...
    const _Boost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        _Boost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return _Boost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME _Boost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME _Boost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME _Boost::conditional<
            _Boost::is_signed<no_cvr_prefinal_t>::value,
            _Boost::make_signed<no_cvr_prefinal_t>,
            _Boost::type_identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME _Boost::conditional<
        _Boost::is_reference<T>::value ||  _Boost::is_const<T>::value || _Boost::is_volatile<T>::value,
        detail::cvr_saver<T>,
        T
    >::type type;
