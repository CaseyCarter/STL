...
std::complex<T> acos(const std::complex<T>& z)
{
   ...
   static const T s_pi = _Boost::math::constants::pi<T>();
...
#pragma warning(disable:4127)
   ...
   else if((_Boost::math::isnan)(x))
   {
      if((_Boost::math::isinf)(y))
         return std::complex<T>(x, ((_Boost::math::signbit)(z.imag())) ? std::numeric_limits<T>::infinity() :  -std::numeric_limits<T>::infinity());
   ...
   else
   {
      ...
      //
      if((y == 0) && (x <= one))
         return std::complex<T>((x == 0) ? half_pi : std::acos(z.real()), (_Boost::math::changesign)(z.imag()));
      //
      ...
      if((x < safe_max) && (x > safe_min) && (y < safe_max) && (y > safe_min))
      {
         ...
         if(a <= a_crossover)
         {
            ...
            imag = _Boost::math::log1p(am1 + std::sqrt(am1 * (a + one)));
         }
         else
         {
      ...
      else
      {
         ...
         if(y <= (std::numeric_limits<T>::epsilon() * std::fabs(xm1)))
         {
            ...
            else
            {
               ...
               if(((std::numeric_limits<T>::max)() / xp1) > xm1)
               {
                  ...
                  imag = _Boost::math::log1p(xm1 + std::sqrt(xp1*xm1));
               }
               else
               {
                  real = y / x;
         ...
         else if(x > one)
         {
            real = std::atan(y/x);
            T xoy = x/y;
            imag = log_two + std::log(y) + half * _Boost::math::log1p(xoy*xoy);
         }
         else
         {
            real = half_pi;
            T a = std::sqrt(one + y*y);
            imag = half * _Boost::math::log1p(static_cast<T>(2)*y*(y+a));
         }
      }
   }
