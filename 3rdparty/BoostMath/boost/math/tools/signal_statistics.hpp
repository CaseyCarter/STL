...
{
    return _Boost::math::tools::absolute_gini_coefficient(v.begin(), v.end());
}
...
auto sample_absolute_gini_coefficient(ForwardIterator first, ForwardIterator last)
{
    ...
    return n*_Boost::math::tools::absolute_gini_coefficient(first, last)/(n-1);
}

template<class RandomAccessContainer>
inline auto sample_absolute_gini_coefficient(RandomAccessContainer & v)
{
    return _Boost::math::tools::sample_absolute_gini_coefficient(v.begin(), v.end());
}
...
{
    return _Boost::math::tools::hoyer_sparsity(v.cbegin(), v.cend());
}
...
auto mean_invariant_oracle_snr(Container const & signal, Container const & noisy_signal)
{
    ...
    Real mu = _Boost::math::tools::mean(signal);
    Real numerator = 0;
    Real denominator = 0;
    for (size_t i = 0; i < signal.size(); ++i)
    {
...
{
    using std::log10;
    return 10*log10(_Boost::math::tools::mean_invariant_oracle_snr(signal, noisy_signal));
}
...
{
    using std::log10;
    return 10*log10(_Boost::math::tools::oracle_snr(signal, noisy_signal));
}
...
auto m2m4_snr_estimator(ForwardIterator first, ForwardIterator last, decltype(*first) estimated_signal_kurtosis=1, decltype(*first) estimated_noise_kurtosis=3)
{
    ...
    if constexpr (std::is_floating_point<Real>::value || std::numeric_limits<Real>::max_exponent)
    {
        ...
        auto [M1, M2, M3, M4] = _Boost::math::tools::first_four_moments(first, last);
        if (M4 == 0)
        {
