...
#include <boost/type_traits/is_floating_point.hpp> // for _Boost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for _Boost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for _Boost::is_convertible
#include <boost/type_traits/is_same.hpp>// for _Boost::is_same
#include <boost/type_traits/remove_cv.hpp>// for _Boost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for _Boost::mpl::if_c.
#include <boost/mpl/and.hpp> // for _Boost::mpl::if_c.
#include <boost/mpl/or.hpp> // for _Boost::mpl::if_c.
#include <boost/mpl/not.hpp> // for _Boost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_c<
          ...
          typename mpl::if_c<!is_floating_point<T2P>::value && ::_Boost::is_convertible<T1P, T2P>::value, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT_MSG((0 == ::_Boost::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
