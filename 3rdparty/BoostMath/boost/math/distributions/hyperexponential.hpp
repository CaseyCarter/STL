...
bool check_probabilities(char const* function, std::vector<RealT> const& probabilities, RealT* presult, PolicyT const& pol)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(_Boost::math::isfinite)(probabilities[i]))
        {
...
{
    if (p < 0 || p > 1 || (_Boost::math::isnan)(p))
    {
...
RealT quantile_impl(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& p, bool comp)
{
    ...
    static const char* function = comp ? "_Boost::math::quantile(const _Boost::math::complemented2_type<_Boost::math::hyperexponential_distribution<%1%>, %1%>&)"
                                       : "_Boost::math::quantile(const _Boost::math::hyperexponential_distribution<%1%>&, %1%)";
...
class hyperexponential_distribution
{
    ...
    {
        RealT err;
        hyperexp_detail::check_dist("_Boost::math::hyperexponential_distribution<%1%>::hyperexponential_distribution",
                                    probs_,
                                    rates_,
                                    &err,
                                    PolicyT());
    ...
    public: template <typename ProbRangeT, typename RateRangeT>
            hyperexponential_distribution(ProbRangeT const& prob_range,
                                          ...
                                          typename _Boost::disable_if_c<_Boost::has_pre_increment<ProbRangeT>::value || _Boost::has_pre_increment<RateRangeT>::value>::type* = 0)
    : probs_(_Boost::begin(prob_range), _Boost::end(prob_range)),
      rates_(_Boost::begin(rate_range), _Boost::end(rate_range))
    {
        hyperexp_detail::normalize(probs_);
    ...
    public: template <typename RateIterT, typename RateIterT2>
            hyperexponential_distribution(RateIterT const& rate_first, 
                                          ...
                                          typename _Boost::enable_if_c<_Boost::has_pre_increment<RateIterT>::value || _Boost::has_pre_increment<RateIterT2>::value>::type* = 0)
...
public: hyperexponential_distribution(std::initializer_list<RealT> l1)
    ...
    : probs_(_Boost::size(rate_range), 1), // will be normalized below
      rates_(_Boost::begin(rate_range), _Boost::end(rate_range))
    {
        hyperexp_detail::normalize(probs_);
...
RealT pdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("_Boost::math::pdf(const _Boost::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("_Boost::math::cdf(const _Boost::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(complemented2_type<hyperexponential_distribution<RealT,PolicyT>, RealT> const& c)
{
    ...
    if (!hyperexp_detail::check_x("_Boost::math::cdf(_Boost::math::complemented2_type<const _Boost::math::hyperexponential_distribution<%1%>&, %1%>)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT variance(hyperexponential_distribution<RealT, PolicyT> const& dist)
{
    ...
    const RealT mean = _Boost::math::mean(dist);
