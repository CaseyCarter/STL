...
public:
   ...
   { // Default is a 1,1 inverse_gaussian distribution.
     static const char* function = "_Boost::math::inverse_gaussian_distribution<%1%>::inverse_gaussian_distribution";
...
{ // Probability Density Function
   ...
   RealType result = 0;
   static const char* function = "_Boost::math::pdf(const inverse_gaussian_distribution<%1%>&, %1%)";
...
{ // Cumulative Density Function.
   ...
   RealType scale = dist.scale();
   RealType mean = dist.mean();
   static const char* function = "_Boost::math::cdf(const inverse_gaussian_distribution<%1%>&, %1%)";
   RealType result = 0;
   if(false == detail::check_scale(function, scale, &result, Policy()))
   {
      return result;
...
{ 
  ...
  inverse_gaussian_quantile_functor(const _Boost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }
  _Boost::math::tuple<RealType, RealType> operator()(RealType const& x)
  {
    ...
    return _Boost::math::make_tuple(fx, dx);
  }
  private:
  const _Boost::math::inverse_gaussian_distribution<RealType, Policy> distribution;
  RealType prob; 
};
...
{ 
    inverse_gaussian_quantile_complement_functor(const _Boost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }
  _Boost::math::tuple<RealType, RealType> operator()(RealType const& x)
...
namespace detail
{
  ...
  { // guess at random variate value x for inverse gaussian quantile.
    ...
    { // phi < 2 so much less symmetrical with long tail,
      ...
      using _Boost::math::gamma_distribution;
...
inline RealType quantile(const inverse_gaussian_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType mean = dist.mean();
   RealType scale = dist.scale();
   static const char* function = "_Boost::math::quantile(const inverse_gaussian_distribution<%1%>&, %1%)";
  ...
  _Boost::uintmax_t m = policies::get_max_root_iterations<Policy>(); // and max iterations.
  using _Boost::math::tools::newton_raphson_iterate;
  result =
...
inline RealType cdf(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType x = c.param;
   static const char* function = "_Boost::math::cdf(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
...
inline RealType quantile(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType scale = c.dist.scale();
   RealType mean = c.dist.mean();
   static const char* function = "_Boost::math::quantile(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
   RealType result = 0;
   if(false == detail::check_scale(function, scale, &result, Policy()))
      return result;
   if(false == detail::check_location(function, mean, &result, Policy()))
  ...
  _Boost::uintmax_t m = policies::get_max_root_iterations<Policy>();
  using _Boost::math::tools::newton_raphson_iterate;
  result =
