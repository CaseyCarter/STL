...
{ // Error checks:
  ...
  {
     if((sigma <= 0) || (!(_Boost::math::isfinite)(sigma)))
     {
...
public:
   ...
   {
      RealType err;
      detail::verify_sigma("_Boost::math::rayleigh_distribution<%1%>::rayleigh_distribution", l_sigma, &err, Policy());
...
inline RealType pdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   static const char* function = "_Boost::math::pdf(const rayleigh_distribution<%1%>&, %1%)";
...
inline RealType cdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "_Boost::math::cdf(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   ...
   result = -_Boost::math::expm1(-x * x / ( 2 * sigma * sigma), Policy());
   return result;
} // cdf
...
inline RealType quantile(const rayleigh_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "_Boost::math::quantile(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
      return result;
   ...
   result = sqrt(-2 * sigma * sigma * _Boost::math::log1p(-p, Policy()));
   return result;
} // quantile
...
inline RealType mean(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "_Boost::math::mean(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using _Boost::math::constants::root_half_pi;
...
inline RealType variance(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "_Boost::math::variance(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using _Boost::math::constants::four_minus_pi;
...
{
   using _Boost::math::constants::root_ln_four;
