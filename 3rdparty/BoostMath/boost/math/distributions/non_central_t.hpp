...
namespace _Boost
{
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_t2_q(T v, T delta, T x, T y, const Policy& pol, T init_val)
         {
            ...
            if((k < (int)(max_factorial<T>::value)) && (d2 < tools::log_max_value<T>()) && (log(d2) * k < tools::log_max_value<T>()))
            {
               ...
               pois /= _Boost::math::tgamma(T(k + 1 + 0.5), pol);
         ...
         T non_central_t_cdf(T v, T delta, T t, bool invert, const Policy& pol)
         {
            ...
            if(x < cross)
            {
               ...
               if(x != 0)
               {
                  ...
                  result = 0;
               result += cdf(_Boost::math::normal_distribution<T, Policy>(), -delta);
            }
            else
            {
               //
               ...
               if(x != 0)
               {
                  ...
                  result = cdf(complement(_Boost::math::normal_distribution<T, Policy>(), -delta));
            }
            if(invert)
               result = 1 - result;
            return result;
     ...
     // now passed as function
            ...
            value_type guess = 0;
            if ( ((_Boost::math::isinf)(v)) || (v > 1 / _Boost::math::tools::epsilon<T>()) )
            ...
            int s;
            if(p < q)
               s = _Boost::math::sign(p - pzero);
            else
               s = _Boost::math::sign(pzero - q);
            if(s != _Boost::math::sign(guess))
            {
         ...
         T mean(T v, T delta, const Policy& pol)
         {
            ...
            BOOST_MATH_STD_USING
            if (v > 1 / _Boost::math::tools::epsilon<T>() )
            {
              //normal_distribution<T, Policy> n(delta, 1);
              //return _Boost::math::mean(n); 
              return delta;
            }
            else
            {
         ...
         T skewness(T v, T delta, const Policy& pol)
         {
            ...
            T mean = _Boost::math::detail::mean(v, delta, pol);
            T l2 = delta * delta;
      ...
      public:
         ...
         { 
            const char* function = "_Boost::math::non_central_t_distribution<%1%>::non_central_t_distribution(%1%,%1%)";
            RealType r;
      ...
      { 
         const char* function = "_Boost::math::cdf(non_central_t_distribution<%1%>&, %1%)";
//   was const char* function = "_Boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
         typedef typename policies::evaluation<RealType, Policy>::type value_type;
         typedef typename policies::normalise<
            Policy, 
            policies::promote_float<false>, 
  ...
  // was       const char* function = "_Boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
         const char* function = "_Boost::math::cdf(const complement(non_central_t_distribution<%1%>&), %1%)";
         typedef typename policies::evaluation<RealType, Policy>::type value_type;
         typedef typename policies::normalise<
            Policy, 
            policies::promote_float<false>, 
