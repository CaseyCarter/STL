...
namespace _Boost
{
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_beta_p(T a, T b, T lam, T x, T y, const Policy& pol, T init_val = 0)
         {
            ...
            for(int i = k + 1; ; ++i)
            {
               ...
               if(static_cast<_Boost::uintmax_t>(count + i - k) > max_iter)
               {
         ...
         T non_central_beta_q(T a, T b, T lam, T x, T y, const Policy& pol, T init_val = 0)
         {
            ...
            for(int i = k; i >= 0; --i)
            {
               ...
               if(static_cast<_Boost::uintmax_t>(count + k - i) > max_iter)
               {
                  return policies::raise_evaluation_error(
                     "cdf(non_central_beta_distribution<%1%>, %1%)",
                     "Series did not converge, closest value was %1%", sum, pol);
         ...
         inline RealType non_central_beta_cdf(RealType x, RealType y, RealType a, RealType b, RealType l, bool invert, const Policy&)
         {
            ...
            if(l == 0)
               result = cdf(_Boost::math::beta_distribution<RealType, Policy>(a, b), x);
            else if(x > cross)
            {
            ...
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               result,
               "_Boost::math::non_central_beta_cdf<%1%>(%1%, %1%, %1%)");
         }
         ...
         std::pair<T, T> bracket_and_solve_root_01(F f, const T& guess, T factor, bool rising, Tol tol, _Boost::uintmax_t& max_iter, const Policy& pol)
         {
            BOOST_MATH_STD_USING
               static const char* function = "_Boost::math::tools::bracket_and_solve_root_01<%1%>";
            //
            ...
            //
            _Boost::uintmax_t count = max_iter - 1;
            ...
            if((fa < 0) == (guess < 0 ? !rising : rising))
            {
               ...
               // until we find it:
               //
               while((_Boost::math::sign)(fb) == (_Boost::math::sign)(fa))
               {
                  if(count == 0)
                  {
         ...
         RealType nc_beta_pdf(const non_central_beta_distribution<RealType, Policy>& dist, const RealType& x)
         {
            ...
            if(l == 0)
               return pdf(_Boost::math::beta_distribution<RealType, Policy>(dist.alpha(), dist.beta()), x);
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
         ...
         T hypergeometric_2F2(T a1, T a2, T b1, T b2, T z, const Policy& pol)
         {
            ...
            const char* function = "_Boost::math::detail::hypergeometric_2F2<%1%>(%1%,%1%,%1%,%1%,%1%)";

            hypergeometric_2F2_sum<value_type> s(a1, a2, b1, b2, z);
            _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
            value_type zero = 0;
            value_type result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<value_type, Policy>(), max_iter, zero);
#else
            value_type result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<value_type, Policy>(), max_iter);
#endif
      ...
      public:
         ...
         {
            const char* function = "_Boost::math::non_central_beta_distribution<%1%>::non_central_beta_distribution(%1%,%1%)";
            RealType r;
      ...
      { // skewness = sqrt(l).
         const char* function = "_Boost::math::non_central_beta_distribution<%1%>::skewness()";
      ...
      {
         const char* function = "_Boost::math::non_central_beta_distribution<%1%>::kurtosis_excess()";
         typedef typename Policy::assert_undefined_type assert_type;
         BOOST_STATIC_ASSERT(assert_type::value == 0);
      ...
      {
         const char* function = "_Boost::math::non_central_beta_distribution<%1%>::cdf(%1%)";
            RealType a = dist.alpha();
            RealType b = dist.beta();
            RealType l = dist.non_centrality();
            RealType r;
