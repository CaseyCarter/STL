...
namespace _Boost
{
   namespace math
   {
      ...
      public:
         ...
         {
            const char* function = "_Boost::math::non_central_f_distribution<%1%>::non_central_f_distribution(%1%,%1%)";
            RealType r;
            detail::check_df(
               function,
               v1, &r, Policy());
      ...
      inline RealType kurtosis_excess(const non_central_f_distribution<RealType, Policy>& dist)
      {
         ...
         RealType result = (3 * (m - 4) * (n * (m + n - 2)
            ...
            * (m + n - 2) * (2 * m + 3 * n - 4) * l2
            ...
            + (10 + m) * l4))
            /
            ((-8 + m) * (-6 + m) * _Boost::math::pow<2>(n * (-2 + m + n)
            + 2 * (-2 + m + n) * l + l2));
            return result;
      } // kurtosis_excess
      ...
      { // Probability Density/Mass Function.
         ...
         value_type r = pdf(_Boost::math::non_central_beta_distribution<value_type, forwarding_policy>(alpha, beta, dist.non_centrality()), y / (1 + y));
      ...
      { // Complemented Cumulative Distribution Function
         ...
         if((c.param < 0) || !(_Boost::math::isfinite)(c.param))
         {
            return policies::raise_domain_error<RealType>(
      ...
      { // Quantile (or Percent Point) function.
         RealType alpha = dist.degrees_of_freedom1() / 2;
         RealType beta = dist.degrees_of_freedom2() / 2;
         RealType x = quantile(_Boost::math::non_central_beta_distribution<RealType, Policy>(alpha, beta, dist.non_centrality()), p);
         if(x == 1)
      ...
      { // Quantile (or Percent Point) function.
         RealType alpha = c.dist.degrees_of_freedom1() / 2;
         RealType beta = c.dist.degrees_of_freedom2() / 2;
         RealType x = quantile(complement(_Boost::math::non_central_beta_distribution<RealType, Policy>(alpha, beta, c.dist.non_centrality()), c.param));
         if(x == 1)
            return policies::raise_overflow_error<RealType>(
