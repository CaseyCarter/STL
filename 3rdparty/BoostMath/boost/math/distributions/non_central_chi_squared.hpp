   ...
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_chi_square_q(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            T errtol = _Boost::math::policies::get_epsilon<T, Policy>();
            T sum = init_sum;
            //
            ...
            T poisf = _Boost::math::gamma_p_derivative(static_cast<T>(1 + k), lambda, pol);
            T poisb = poisf * k / lambda;
            // Initial forwards central chi squared term:
            T gamf = _Boost::math::gamma_q(del + k, y, pol);
            // Forwards and backwards recursion terms on the central chi squared:
            T xtermf = _Boost::math::gamma_p_derivative(del + 1 + k, y, pol);
            ...
            //
            int i;
            for(i = k; static_cast<_Boost::uintmax_t>(i-k) < max_iter; ++i)
            {
               T term = poisf * gamf;
               sum += term;
               poisf *= lambda / (i + 1);
            ...
            //Error check:
            if(static_cast<_Boost::uintmax_t>(i-k) >= max_iter)
         ...
         T non_central_chi_square_p_ding(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            if(x == 0)
               return 0;
            T tk = _Boost::math::gamma_p_derivative(f/2 + 1, x/2, pol);
            T lambda = theta / 2;
            T vk = exp(-lambda);
            T uk = vk;
            ...
            int i;
            T lterm(0), term(0);
            for(i = 1; static_cast<_Boost::uintmax_t>(i) < max_iter; ++i)
            {
               tk = tk * x / (f + 2 * i);
               uk = uk * lambda / i;
               vk = vk + uk;
            ...
            //Error check:
            if(static_cast<_Boost::uintmax_t>(i) >= max_iter)
               return policies::raise_evaluation_error(
                  "cdf(non_central_chi_squared_distribution<%1%>, %1%)",
                  "Series did not converge, closest value was %1%", sum, pol);
            return sum;
         ...
         T non_central_chi_square_p(T y, T n, T lambda, const Policy& pol, T init_sum)
         {
            ...
            T gamkf = _Boost::math::gamma_p(a, x, pol);
            ...
            T xtermf = _Boost::math::gamma_p_derivative(a, x, pol);
            ...
            do
            {
               ...
               sum += errorf;
               ++i;
            }while((fabs(errorf / sum) > errtol) && (static_cast<_Boost::uintmax_t>(i) < max_iter));
         ...
         inline RealType non_central_chi_squared_cdf(RealType x, RealType k, RealType l, bool invert, const Policy&)
         {
            ...
            value_type result;
            if(l == 0)
              return invert == false ? cdf(_Boost::math::chi_squared_distribution<RealType, Policy>(k), x) : cdf(complement(_Boost::math::chi_squared_distribution<RealType, Policy>(k), x));
            else if(x > k + l)
            {
            ...
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               result,
               "_Boost::math::non_central_chi_squared_cdf<%1%>(%1%, %1%, %1%)");
         }
         ...
         RealType nccs_quantile(const non_central_chi_squared_distribution<RealType, Policy>& dist, const RealType& p, bool comp)
         {
            ...
            if(guess < 0.005)
            {
               ...
               guess = pow(pow(value_type(2), (k / 2 - 1)) * exp(l / 2) * pp * k * _Boost::math::tgamma(k / 2, forwarding_policy()), (2 / k));
               if(guess == 0)
         ...
         if(l == 0)
            return pdf(_Boost::math::chi_squared_distribution<RealType, forwarding_policy>(dist.degrees_of_freedom()), x);
         ...
         else
         {
            ...
            else
            {
               r = exp(r);
               r = 0.5f * r
                  * _Boost::math::cyl_bessel_i(k/2 - 1, sqrt(l * x), forwarding_policy());
            }
         }
         return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               r,
            ...
            RealType lam, RealType x, RealType p, RealType q, const Policy& pol)
         {
            ...
            _Boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();
            //
      ...
      public:
         ...
         {
            const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::non_central_chi_squared_distribution(%1%,%1%)";
            RealType r;
            detail::check_df(
               function,
               df, &r, Policy());
      ...
      { // Mean of poisson distribution = lambda.
         const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::mean()";
      ...
      { // variance.
         const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::variance()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      { // skewness = sqrt(l).
         const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::skewness()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      {
         const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::kurtosis_excess()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      {
         const char* function = "_Boost::math::non_central_chi_squared_distribution<%1%>::cdf(%1%)";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
