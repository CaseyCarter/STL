...
public:
   ...
   { // Constructor.
      ...
      detail::check_df_gt0_to_inf( // Checks that df > 0 or df == inf.
         "_Boost::math::students_t_distribution<%1%>::students_t_distribution", df_, &result, Policy());
...
inline RealType pdf(const students_t_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   if(false == detail::check_x_not_NaN(
      "_Boost::math::pdf(const students_t_distribution<%1%>&, %1%)", x, &error_result, Policy()))
      return error_result;
   RealType df = dist.degrees_of_freedom();
   if(false == detail::check_df_gt0_to_inf( // Check that df > 0 or == +infinity.
      "_Boost::math::pdf(const students_t_distribution<%1%>&, %1%)", df, &error_result, Policy()))
      return error_result;

   RealType result;
   if ((_Boost::math::isinf)(x))
   { // - or +infinity.
   ...
   { // 
     ...
     if(basem1 < 0.125)
     {
        result = exp(-_Boost::math::log1p(basem1, Policy()) * (1+df) / 2);
     }
     else
     {
        result = pow(1 / (1 + basem1), (df + 1) / 2);
     }
     result /= sqrt(df) * _Boost::math::beta(df / 2, RealType(0.5f), Policy());
   }
   return result;
} // pdf
...
inline RealType cdf(const students_t_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType df = dist.degrees_of_freedom();
   if (false == detail::check_df_gt0_to_inf(  // Check that df > 0 or == +infinity.
     "_Boost::math::cdf(const students_t_distribution<%1%>&, %1%)", df, &error_result, Policy()))
   {
     return error_result;
   }
   // Check for bad x first.
   if(false == detail::check_x_not_NaN(
      "_Boost::math::cdf(const students_t_distribution<%1%>&, %1%)", x, &error_result, Policy()))
   { 
      return error_result;
   }
   if (x == 0)
...
inline RealType quantile(const students_t_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType df = dist.degrees_of_freedom();
   static const char* function = "_Boost::math::quantile(const students_t_distribution<%1%>&, %1%)";
   RealType error_result;
   if(false == (detail::check_df_gt0_to_inf( // Check that df > 0 or == +infinity.
   ...
   //
   return _Boost::math::detail::fast_students_t_quantile(df, probability, Policy());
} // quantile
...
RealType students_t_distribution<RealType, Policy>::find_degrees_of_freedom(
      ...
      RealType hint)
{
   static const char* function = "_Boost::math::students_t_distribution<%1%>::find_degrees_of_freedom";
   //
   // Check for domain errors:
   //
   RealType error_result;
...
{  // Revised for https://svn.boost.org/trac/boost/ticket/7177
   RealType df = dist.degrees_of_freedom();
   if(((_Boost::math::isnan)(df)) || (df <= 1) ) 
   { // mean is undefined for moment <= 1!
      return policies::raise_domain_error<RealType>(
      "_Boost::math::mean(students_t_distribution<%1%> const&, %1%)",
...
{
    RealType df = dist.degrees_of_freedom();
   if( ((_Boost::math::isnan)(df)) || (dist.degrees_of_freedom() <= 3))
   { // Undefined for moment k = 3.
      return policies::raise_domain_error<RealType>(
         "_Boost::math::skewness(students_t_distribution<%1%> const&, %1%)",
...
{
   RealType df = dist.degrees_of_freedom();
   if(((_Boost::math::isnan)(df)) || (df <= 4))
   { // Undefined or infinity for moment k = 4.
      return policies::raise_domain_error<RealType>(
       "_Boost::math::kurtosis(students_t_distribution<%1%> const&, %1%)",
...
inline RealType kurtosis_excess(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   { // Undefined or infinity for moment k = 4.
     return policies::raise_domain_error<RealType>(
       "_Boost::math::kurtosis_excess(students_t_distribution<%1%> const&, %1%)",
