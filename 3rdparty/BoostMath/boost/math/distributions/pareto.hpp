...
namespace _Boost
{
  namespace math
  {
    ...
    public:
      ...
      { // Constructor.
        RealType result = 0;
        detail::check_pareto("_Boost::math::pareto_distribution<%1%>::pareto_distribution", l_scale, l_shape, &result, Policy());
      }
    ...
    inline RealType pdf(const pareto_distribution<RealType, Policy>& dist, const RealType& x)
    {
      ...
      static const char* function = "_Boost::math::pdf(const pareto_distribution<%1%>&, %1%)";
    ...
    {
      BOOST_MATH_STD_USING  // for ADL of std function pow.
      static const char* function = "_Boost::math::cdf(const pareto_distribution<%1%>&, %1%)";
      RealType scale = dist.scale();
      RealType shape = dist.shape();
      RealType result = 0;
      ...
      result = -_Boost::math::powm1(scale/x, shape, Policy()); // should be more accurate.
      return result;
    } // cdf
    ...
    {
      BOOST_MATH_STD_USING  // for ADL of std function pow.
      static const char* function = "_Boost::math::quantile(const pareto_distribution<%1%>&, %1%)";
      RealType result = 0;
      RealType scale = dist.scale();
      RealType shape = dist.shape();
    ...
    {
      RealType result = 0;
      static const char* function = "_Boost::math::mean(const pareto_distribution<%1%>&, %1%)";
    ...
    {
      RealType result = 0;
      static const char* function = "_Boost::math::median(const pareto_distribution<%1%>&, %1%)";
      if(false == detail::check_pareto(function, dist.scale(), dist.shape(), &result, Policy()))
      {
        return result;
      }
    ...
    inline RealType variance(const pareto_distribution<RealType, Policy>& dist)
    {
      ...
      RealType scale = dist.scale();
      RealType shape = dist.shape();
      static const char* function = "_Boost::math::variance(const pareto_distribution<%1%>&, %1%)";
