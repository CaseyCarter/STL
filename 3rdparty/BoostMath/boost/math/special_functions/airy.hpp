...
T airy_ai_imp(T x, const Policy& pol)
{
   ...
   if(x < 0)
   {
      T p = (-x * sqrt(-x) * 2) / 3;
      T v = T(1) / 3;
      T j1 = _Boost::math::cyl_bessel_j(v, p, pol);
      T j2 = _Boost::math::cyl_bessel_j(-v, p, pol);
      T ai = sqrt(-x) * (j1 + j2) / 3;
      //T bi = sqrt(-x / 3) * (j2 - j1);
      return ai;
   }
   ...
   {
      T tg = _Boost::math::tgamma(constants::twothirds<T>(), pol);
      T ai = 1 / (pow(T(3), constants::twothirds<T>()) * tg);
      //T bi = 1 / (sqrt(_Boost::math::cbrt(T(3))) * tg);
      return ai;
   }
   else
   {
      T p = 2 * x * sqrt(x) / 3;
      T v = T(1) / 3;
      //T j1 = _Boost::math::cyl_bessel_i(-v, p, pol);
      //T j2 = _Boost::math::cyl_bessel_i(v, p, pol);
      //
      ...
      //
      T ai = cyl_bessel_k(v, p, pol) * sqrt(x / 3) / _Boost::math::constants::pi<T>();  //sqrt(x) * (j1 - j2) / 3;
      //T bi = sqrt(x / 3) * (j1 + j2);
      return ai;
   }
}
...
T airy_ai_prime_imp(T x, const Policy& pol)
{
   ...
   {
      T tg = _Boost::math::tgamma(constants::third<T>(), pol);
      T aip = 1 / (_Boost::math::cbrt(T(3)) * tg);
      return -aip;
   }
   else
   {
      ...
      // as we're subtracting two very large values, so use the Bessel K relation instead:
      //
      T aip = -cyl_bessel_k(v, p, pol) * x / (_Boost::math::constants::root_three<T>() * _Boost::math::constants::pi<T>());
      return aip;
   }
}
...
T airy_bi_prime_imp(T x, const Policy& pol)
{
   ...
   {
      T tg = _Boost::math::tgamma(constants::third<T>(), pol);
      T bip = sqrt(_Boost::math::cbrt(T(3))) / tg;
      return bip;
   }
   else
   {
      ...
      T j1 = _Boost::math::cyl_bessel_i(-v, p, pol);
      T j2 = _Boost::math::cyl_bessel_i(v, p, pol);
      T aip = x * (j1 + j2) / _Boost::math::constants::root_three<T>();
      return aip;
   }
}
...
T airy_ai_zero_imp(int m, const Policy& pol)
{
   ...
   if(m < 0)
   {
      return policies::raise_domain_error<T>("_Boost::math::airy_ai_zero<%1%>(%1%, int)",
   ...
   if(m == 0U)
   {
      return policies::raise_domain_error<T>("_Boost::math::airy_ai_zero<%1%>(%1%,%1%)",
        "The requested rank of the zero is %1%, but must be 1 or more !", static_cast<T>(m), pol);
   }

   // Set up the initial guess for the upcoming root-finding.
   const T guess_root = _Boost::math::detail::airy_zero::airy_ai_zero_detail::initial_guess<T>(m);
   ...
   const T am =
      _Boost::math::tools::newton_raphson_iterate(
         _Boost::math::detail::airy_zero::airy_ai_zero_detail::function_object_ai_and_ai_prime<T, Policy>(pol),
         guess_root,
...
T airy_bi_zero_imp(int m, const Policy& pol)
{
   ...
   if(m < 0)
   {
      return policies::raise_domain_error<T>("_Boost::math::airy_bi_zero<%1%>(%1%, int)",
   ...
   if(m == 0U)
   {
      return policies::raise_domain_error<T>("_Boost::math::airy_bi_zero<%1%>(%1%,%1%)",
        "The requested rank of the zero is %1%, but must be 1 or more !", static_cast<T>(m), pol);
   }
   // Set up the initial guess for the upcoming root-finding.
   const T guess_root = _Boost::math::detail::airy_zero::airy_bi_zero_detail::initial_guess<T>(m);
   ...
   const T bm =
      _Boost::math::tools::newton_raphson_iterate(
         _Boost::math::detail::airy_zero::airy_bi_zero_detail::function_object_bi_and_bi_prime<T, Policy>(pol),
         guess_root,
         T(guess_root - tolerance),
         T(guess_root + tolerance),
         policies::digits<T, Policy>(),
...
inline typename tools::promote_args<T>::type airy_ai(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_ai_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "_Boost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_bi(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_bi_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "_Boost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_ai_prime(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_ai_prime_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "_Boost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_bi_prime(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_bi_prime_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "_Boost::math::airy<%1%>(%1%)");
}
...
inline T airy_ai_zero(int m, const Policy& /*pol*/)
{
   ...
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_ai_zero_imp<value_type>(m, forwarding_policy()), "_Boost::math::airy_ai_zero<%1%>(unsigned)");
}
                         ...
                         const Policy& pol)
{
   ...
   {
      *out_it = _Boost::math::airy_ai_zero<result_type>(start_index + i, pol);
      ++out_it;
   }
   return out_it;
}
...
inline T airy_bi_zero(int m, const Policy& /*pol*/)
{
   ...
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_bi_zero_imp<value_type>(m, forwarding_policy()), "_Boost::math::airy_bi_zero<%1%>(unsigned)");
}
                         ...
                         const Policy& pol)
{
   ...
   for(unsigned i = 0; i < number_of_zeros; ++i)
   {
      *out_it = _Boost::math::airy_bi_zero<result_type>(start_index + i, pol);
      ++out_it;
   }
   return out_it;
}
