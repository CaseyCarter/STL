...
T expm1_imp(T x, const _Boost::integral_constant<int, 0>&, const Policy& pol)
{
   ...
   {
      return policies::raise_domain_error<T>("_Boost::math::expm1<%1%>(%1%)", "expm1 requires a finite argument, but got %1%", a, pol);
   }
   if(a > T(0.5f))
   {
      ...
      {
         if(x > 0)
            return policies::raise_overflow_error<T>("_Boost::math::expm1<%1%>(%1%)", 0, pol);
         return -1;
      }
      return exp(x) - T(1);
   }
...
#endif
   policies::check_series_iterations<T>("_Boost::math::expm1<%1%>(%1%)", max_iter, pol);
   return result;
}

template <class T, class P>
T expm1_imp(T x, const _Boost::integral_constant<int, 53>&, const P& pol)
{
   BOOST_MATH_STD_USING
...
template <class T, class P>
T expm1_imp(T x, const _Boost::integral_constant<int, 64>&, const P& pol)
{
   BOOST_MATH_STD_USING
...
template <class T, class P>
T expm1_imp(T x, const _Boost::integral_constant<int, 113>&, const P& pol)
{
   BOOST_MATH_STD_USING
...
inline typename tools::promote_args<T>::type expm1(T x, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expm1_imp(
      ...
      tag_type(), forwarding_policy()), "_Boost::math::expm1<%1%>(%1%)");
}
