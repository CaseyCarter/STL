...
T log1p_imp(T const & x, const Policy& pol, const _Boost::integral_constant<int, 0>&)
{ // The function returns the natural logarithm of 1 + x.
   typedef typename tools::promote_args<T>::type result_type;
   BOOST_MATH_STD_USING

   static const char* function = "_Boost::math::log1p<%1%>(%1%)";

   if((x < -1) || (_Boost::math::isnan)(x))
...
template <class T, class Policy>
T log1p_imp(T const& x, const Policy& pol, const _Boost::integral_constant<int, 53>&)
{ // The function returns the natural logarithm of 1 + x.
   BOOST_MATH_STD_USING
...
template <class T, class Policy>
T log1p_imp(T const& x, const Policy& pol, const _Boost::integral_constant<int, 64>&)
{ // The function returns the natural logarithm of 1 + x.
   BOOST_MATH_STD_USING
...
template <class T, class Policy>
T log1p_imp(T const& x, const Policy& pol, const _Boost::integral_constant<int, 24>&)
{ // The function returns the natural logarithm of 1 + x.
   BOOST_MATH_STD_USING
...
struct log1p_initializer
{
   struct init
   {
      ...
      {
         _Boost::math::log1p(static_cast<T>(0.25), Policy());
      }
...
{ 
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::log1p_imp(static_cast<value_type>(x), forwarding_policy(), tag_type()), "_Boost::math::log1p<%1%>(%1%)");
}
...
inline float log1p(float x, const Policy& pol)
{
   return static_cast<float>(_Boost::math::log1p(static_cast<double>(x), pol));
}
#ifndef _WIN32_WCE
//
...
{
   return _Boost::math::log1p(x, policies::policy<>());
}
//
// Compute log(1+x)-x:
//
   ...
   log1pmx(T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   BOOST_MATH_STD_USING
   static const char* function = "_Boost::math::log1pmx<%1%>(%1%)";
   ...
   if(a < tools::epsilon<result_type>())
      return -x * x / 2;
   _Boost::math::detail::log1p_series<T> s(x);
   s();
   _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T zero = 0;
   T result = _Boost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = _Boost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>(function, max_iter, pol);
   return result;
}
