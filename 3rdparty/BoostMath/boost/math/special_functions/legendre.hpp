...
{
   static const char* function = "_Boost::math::legrendre_p<%1%>(unsigned, %1%)";
   // Error handling:
   if((x < -1) || (x > 1))
   ...
   if(second)
   {
      ...
      p0 = (_Boost::math::log1p(x, pol) - _Boost::math::log1p(-x, pol)) / 2;
      p1 = x * p0 - 1;
   }
   else
   {
   ...
   {
      std::swap(p0, p1);
      p1 = _Boost::math::legendre_next(n, x, p0, p1);
      ++n;
   }
   return p1;
}
...
{
   static const char* function = "_Boost::math::legrendre_p_prime<%1%>(unsigned, %1%)";
   // Error handling:
   if ((x < -1) || (x > 1))
      return policies::raise_domain_error<T>(
         function,
   ...
   if (l == 0)
    ...
    {
        std::swap(p0, p1);
        *Pn = _Boost::math::legendre_next(n, x, p0, p1);
    }
    return p_prime;
}
   ...
   legendre_p(int l, T x, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::legendre_p<%1%>(unsigned, %1%)";
   if(l < 0)
   ...
   legendre_p_prime(int l, T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   static const char* function = "_Boost::math::legendre_p_prime<%1%>(unsigned, %1%)";
   if(l < 0)
...
{
   return _Boost::math::legendre_p(l, x, policies::policy<>());
}
...
{
   return _Boost::math::legendre_p_prime(l, x, policies::policy<>());
}
...
{
   return _Boost::math::legendre_p_zeros<T>(l, policies::policy<>());
}
   ...
   legendre_q(unsigned l, T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::legendre_imp(l, static_cast<value_type>(x), pol, true), "_Boost::math::legendre_q<%1%>(unsigned, %1%)");
}
...
{
   return _Boost::math::legendre_q(l, x, policies::policy<>());
}
...
T legendre_p_imp(int l, int m, T x, T sin_theta_power, const Policy& pol)
{
   ...
   if((x < -1) || (x > 1))
      return policies::raise_domain_error<T>(
      "_Boost::math::legendre_p<%1%>(int, int, %1%)",
   ...
   {
      int sign = (m&1) ? -1 : 1;
      return sign * _Boost::math::tgamma_ratio(static_cast<T>(l+m+1), static_cast<T>(l+1-m), pol) * legendre_p_imp(l, -m, x, sin_theta_power, pol);
   }
   // Special cases:
   if(m > l)
      return 0;
   if(m == 0)
      return _Boost::math::legendre_p(l, x, pol);

   T p0 = _Boost::math::double_factorial<T>(2 * m - 1, pol) * sin_theta_power;
   ...
   {
      std::swap(p0, p1);
      p1 = _Boost::math::legendre_next(n, m, x, p0, p1);
      ++n;
   }
   return p1;
}
   ...
   legendre_p(int l, int m, T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::legendre_p_imp(l, m, static_cast<value_type>(x), pol), "_Boost::math::legendre_p<%1%>(int, int, %1%)");
}
...
{
   return _Boost::math::legendre_p(l, m, x, policies::policy<>());
}
