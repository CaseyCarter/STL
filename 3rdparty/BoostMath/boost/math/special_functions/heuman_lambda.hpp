...
T heuman_lambda_imp(T phi, T k, const Policy& pol)
{
    ...
    const char* function = "_Boost::math::heuman_lambda<%1%>(%1%, %1%)";
...
inline typename tools::promote_args<T1, T2>::type heuman_lambda(T1 k, T2 phi, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::heuman_lambda_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "_Boost::math::heuman_lambda<%1%>(%1%,%1%)");
}

template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type heuman_lambda(T1 k, T2 phi)
{
   return _Boost::math::heuman_lambda(k, phi, policies::policy<>());
}
