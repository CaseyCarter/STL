...
inline T spherical_harmonic_prefix(unsigned n, unsigned m, T theta, const Policy& pol)
{
   ...
   T prefix = _Boost::math::tgamma_delta_ratio(static_cast<T>(n - m + 1), static_cast<T>(2 * m), pol);
...
T spherical_harmonic_r(unsigned n, int m, T theta, T phi, const Policy& pol)
{
   ...
   if(m&1)
   {
      ...
      T mod = _Boost::math::tools::fmod_workaround(theta, T(2 * constants::pi<T>()));
      if(mod < 0)
...
std::complex<T> spherical_harmonic(unsigned n, int m, U theta, U phi, const Policy& pol)
{
   ...
   {
      // Check phase if theta is outside [0, PI]:
      U mod = _Boost::math::tools::fmod_workaround(theta, U(2 * constants::pi<U>()));
      if(mod < 0)
   ...
   if(i_sign)
      i = -i;
   static const char* function = "_Boost::math::spherical_harmonic<%1%>(int, int, %1%, %1%)";
...
{
   return _Boost::math::spherical_harmonic(n, m, theta, phi, policies::policy<>());
}
...
{
   return _Boost::math::spherical_harmonic_r(n, m, theta, phi, policies::policy<>());
}
   ...
   spherical_harmonic_i(unsigned n, int m, T1 theta, T2 phi, const Policy& pol)
{
   typedef typename tools::promote_args<T1, T2>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::spherical_harmonic_i(n, m, static_cast<value_type>(theta), static_cast<value_type>(phi), pol), "_Boost::math::spherical_harmonic_i<%1%>(unsigned, int, %1%, %1%)");
}
...
{
   return _Boost::math::spherical_harmonic_i(n, m, theta, phi, policies::policy<>());
}
