...
namespace _Boost {
   namespace math {
      ...
      namespace detail {
         ...
         {
            timed_iteration_terminator(_Boost::uintmax_t i, double t) : max_iter(i), max_time(t), start_time(std::chrono::system_clock::now()) {}

            bool operator()(_Boost::uintmax_t iter)const
            {
               if (iter > max_iter)
                  _Boost::throw_exception(_Boost::math::detail::pFq_termination_exception("pFq exceeded maximum permitted iterations."));
               if (std::chrono::duration<double>(std::chrono::system_clock::now() - start_time).count() > max_time)
                  _Boost::throw_exception(_Boost::math::detail::pFq_termination_exception("pFq exceeded maximum permitted evaluation time."));
               return false;
            }

            _Boost::uintmax_t max_iter;
            double max_time;
      ...
      inline typename tools::promote_args<Real, typename Seq::value_type>::type hypergeometric_pFq(const Seq& aj, const Seq& bj, const Real& z, Real* p_abs_error, const Policy& pol)
      {
         ...
         int scale = 0;
         std::pair<value_type, value_type> r = _Boost::math::detail::hypergeometric_pFq_checked_series_impl(aj, bj, value_type(z), pol, _Boost::math::detail::iteration_terminator(_Boost::math::policies::get_max_series_iterations<forwarding_policy>()), scale);
         r.first *= exp(Real(scale));
         r.second *= exp(Real(scale));
         if (p_abs_error)
            *p_abs_error = static_cast<Real>(r.second) * _Boost::math::tools::epsilon<Real>();
         return policies::checked_narrowing_cast<result_type, Policy>(r.first, "_Boost::math::hypergeometric_pFq<%1%>(%1%,%1%,%1%)");
      }
      ...
      {
         return hypergeometric_pFq(aj, bj, z, p_abs_error, _Boost::math::policies::policy<>());
      }
      ...
      Real hypergeometric_pFq_precision(const Seq& aj, const Seq& bj, Real z, unsigned digits10, double timeout, const Policy& pol)
      {
         ...
         do
         {
            ...
            {
               int scale = 0;
               std::pair<Real, Real> rp = _Boost::math::detail::hypergeometric_pFq_checked_series_impl(aa, bb, z, pol, _Boost::math::detail::timed_iteration_terminator(_Boost::math::policies::get_max_series_iterations<Policy>(), timeout), scale);
            ...
            catch (const detail::pFq_termination_exception& e)
            {
               ...
               throw _Boost::math::evaluation_error(e.what());
            }
         } while (true);
      ...
      {
         return hypergeometric_pFq_precision(aj, bj, z, digits10, timeout, _Boost::math::policies::policy<>());
      }
      ...
      {
         return hypergeometric_pFq_precision< std::initializer_list<Real>, Real>(aj, bj, z, digits10, timeout, _Boost::math::policies::policy<>());
      }
