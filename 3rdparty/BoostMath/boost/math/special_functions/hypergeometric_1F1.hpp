...
namespace _Boost { namespace math { namespace detail {
   ...
   T hypergeometric_1F1_divergent_fallback(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
   {
      ...
      if (b > 0)
      {
         ...
         {
            return _Boost::math::detail::hypergeometric_1f1_recurrence_on_z_minus_zero(a, b, T(z - k), k, pol, log_scaling);
         }
#endif
         if (z < b)
            return hypergeometric_1F1_backward_recurrence_for_negative_a(a, b, z, pol, function, log_scaling);
      ...
      else  // b < 0
      {
         if (a < 0)
         {
            ...
            else
            {
               ...
               //
               bool can_use_recursion = (z - b + 100 < _Boost::math::policies::get_max_series_iterations<Policy>()) && (100 - a < _Boost::math::policies::get_max_series_iterations<Policy>());
         ...
         else
         {
            ...
            if ((domain < 0) && ((a > 1) || (_Boost::math::policies::digits<T, Policy>() <= 64)))
               return hypergeometric_1F1_from_function_ratio_negative_b(a, b, z, pol, log_scaling);
            else if (domain > 0)
            {
               if (_Boost::math::policies::digits<T, Policy>() <= 64)
   ...
   T hypergeometric_1F1_imp(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
   {
      ...
      static const char* const function = "_Boost::math::hypergeometric_1F1<%1%,%1%,%1%>(%1%,%1%,%1%)";
      ...
      if ((a == 1) && (b == 2))
         return _Boost::math::expm1(z, pol) / z;
      ...
      if (z < 0)
      {
         if (hypergeometric_1F1_is_13_3_6_region(a, b, z))
         {
            ...
            T r = _Boost::math::detail::hypergeometric_1F1_AS_13_3_6(b_minus_a, b, T(-z), a, pol, log_scaling);
            int scale = itrunc(z, pol);
            log_scaling += scale;
      ...
      if (hypergeometric_1F1_need_kummer_reflection(a, b, z))
      {
         ...
         {
            if ((_Boost::math::sign(b_minus_a) == _Boost::math::sign(b)) && ((b > 0) || (b < -200)))
            {
      ...
      if (hypergeometric_1F1_is_13_3_6_region(b_minus_a, b, T(-z)))
      {
         ...
         // 13.3.6 appears to be the most efficient and often the most accurate method.
         return _Boost::math::detail::hypergeometric_1F1_AS_13_3_6(a, b, z, b_minus_a, pol, log_scaling);
      }
#if 0
      if ((a > 0) && (b > 0) && (a * z / b > 2))
      {
   ...
   inline T hypergeometric_1F1_imp(const T& a, const T& b, const T& z, const Policy& pol)
   {
    ...
    static const thread_local int max_scaling = itrunc(_Boost::math::tools::log_max_value<T>()) - 2;
    static const thread_local T max_scale_factor = exp(T(max_scaling));
#else
    int max_scaling = itrunc(_Boost::math::tools::log_max_value<T>()) - 2;
   ...
   inline T hypergeometric_1F1_regularized_imp(const T& a, const T& b, const T& z, const Policy& pol)
   {
      ...
      //
    int result_sign = 1;
    T scale = log_scaling - _Boost::math::lgamma(b, &result_sign, pol);
#ifndef BOOST_NO_CXX11_THREAD_LOCAL
      static const thread_local T max_scaling = _Boost::math::tools::log_max_value<T>() - 2;
    static const thread_local T max_scale_factor = exp(max_scaling);
#else
    T max_scaling = _Boost::math::tools::log_max_value<T>() - 2;
...
inline typename tools::promote_args<T1, T2, T3>::type hypergeometric_1F1(T1 a, T2 b, T3 z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::hypergeometric_1F1_imp<value_type>(
         ...
         forwarding_policy()),
      "_Boost::math::hypergeometric_1F1<%1%>(%1%,%1%,%1%)");
}
