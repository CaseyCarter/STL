...
struct big_int_type
{
   operator _Boost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_c<
      _Boost::is_convertible<big_int_type, T>::value,
      _Boost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   if((_Boost::math::isinf)(z) || (z == 0))
      return z;
   if(!(_Boost::math::isfinite)(z))
   {
      return policies::raise_domain_error("_Boost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }
   ...
   typedef typename mpl::divides<prec, _Boost::integral_constant<int, 3> >::type prec3;
   typedef typename mpl::plus<prec3, _Boost::integral_constant<int, 3> >::type new_prec;
