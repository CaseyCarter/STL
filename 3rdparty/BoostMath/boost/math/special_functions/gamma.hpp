...
template <class T>
inline bool is_odd(T v, const _Boost::true_type&)
{
   int i = static_cast<int>(v);
   return i&1;
}
template <class T>
inline bool is_odd(T v, const _Boost::false_type&)
{
...
{
   return is_odd(v, ::_Boost::is_convertible<T, int>());
}
...
T sinpx(T z)
{
   ...
   if(dist > 0.5)
      dist = 1 - dist;
   T result = sin(dist*_Boost::math::constants::pi<T>());
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
...
#endif
   static const char* function = "_Boost::math::tgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) < _Boost::math::constants::pi<T>()))
            return -_Boost::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result = -_Boost::math::constants::pi<T>() / result;
         if(result == 0)
            return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);
         if((_Boost::math::fpclassify)(result) == (int)FP_SUBNORMAL)
   ...
   else
   {
      ...
      T zgh = (z + static_cast<T>(Lanczos::g()) - _Boost::math::constants::half<T>());
      T lzgh = log(zgh);
      BOOST_MATH_INSTRUMENT_VARIABLE(result);
      ...
      {
         BOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(pow(zgh, z - _Boost::math::constants::half<T>()));
         BOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
         result *= pow(zgh, z - _Boost::math::constants::half<T>()) / exp(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   }
   return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
   ...
   static const char* function = "_Boost::math::lgamma<%1%>(%1%)";
   ...
   if(z <= -tools::root_epsilon<T>())
   {
      ...
      result = log(_Boost::math::constants::pi<T>()) - lgamma_imp(z, pol, l) - log(t);
   }
   else if (z < tools::root_epsilon<T>())
   {
      if (0 == z)
   ...
   else if(z < 15)
   {
      ...
      typedef _Boost::integral_constant<int,
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + Lanczos::g() - _Boost::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
      //
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + _Boost::math::tools::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   T factor = policies::get_epsilon<T, Policy>();
   T result = _Boost::math::tools::sum_series(s, factor, max_iter, init_value);
   policies::check_series_iterations<T>("_Boost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
std::size_t highest_bernoulli_index()
{
   const float digits10_of_type = (std::numeric_limits<T>::is_specialized
                                      ...
                                      : static_cast<float>(_Boost::math::tools::digits<T>() * 0.301F));
...
int minimum_argument_for_bernoulli_recursion()
{
   ...
   const float limit = std::ceil(std::pow(1.0f / std::ldexp(1.0f, 1-_Boost::math::tools::digits<T>()), 1.0f / 20.0f));
...
T scaled_tgamma_no_lanczos(const T& z, const Policy& pol, bool islog = false)
{
   ...
   T sum = (_Boost::math::bernoulli_b2n<T>(1) / 2) * one_over_x_pow_two_n_minus_one;
   const T target_epsilon_to_break_loop = sum * _Boost::math::tools::epsilon<T>();
   const T half_ln_two_pi_over_z = sqrt(_Boost::math::constants::two_pi<T>() / z);
   T last_term = 2 * sum;
   ...
   for (std::size_t n = 2U;; ++n)
   {
      ...
      const T term = (_Boost::math::bernoulli_b2n<T>(static_cast<int>(n)) * one_over_x_pow_two_n_minus_one) / (n2 * (n2 - 1U));
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if((_Boost::math::isnan)(z) || (is_at_zero) || ((_Boost::math::isinf)(z) && (z < 0)))
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < _Boost::math::max_factorial<T>::value))
   {
      return _Boost::math::unchecked_factorial<T>(itrunc(z) - 1);
   }
   ...
   {
      const T a0(1);
      const T a1(_Boost::math::constants::euler<T>());
      const T six_euler_squared((_Boost::math::constants::euler<T>() * _Boost::math::constants::euler<T>()) * 6);
      const T a2((six_euler_squared -  _Boost::math::constants::pi_sqr<T>()) / 12);
   ...
   {
      n_recur = _Boost::math::itrunc(min_arg_for_recursion - zz) + 1;
   ...
   if(b_neg)
   {
      ...
      const bool result_is_too_large_to_represent = (   (abs(gamma_value) < 1)
                                                     && ((tools::max_value<T>() * abs(gamma_value)) < _Boost::math::constants::pi<T>()));

      if(result_is_too_large_to_represent)
         return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);

      gamma_value = -_Boost::math::constants::pi<T>() / gamma_value;
      BOOST_MATH_INSTRUMENT_VARIABLE(gamma_value);

      if(gamma_value == 0)
         return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);

      if((_Boost::math::fpclassify)(gamma_value) == static_cast<int>(FP_SUBNORMAL))
...
inline T log_gamma_near_1(const T& z, Policy const& pol)
{
   ...
   do
   {
      term = power_term * _Boost::math::polygamma(n - 1, T(1));
      result += term;
      ++n;
      power_term *= z / n;
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&, int* sign)
{
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < _Boost::math::max_factorial<T>::value))
   {
      if (sign)
         *sign = 1;
      return log(_Boost::math::unchecked_factorial<T>(itrunc(z) - 1));
   }
   ...
   if(b_neg)
   {
      ...
      log_gamma_value = - log_gamma_value
                        + log(_Boost::math::constants::pi<T>())
                        - log(t);
   }
...
T tgammap1m1_imp(T dz, Policy const& pol, const Lanczos& l)
{
   ...
   if(dz < 0)
   {
      if(dz < -0.5)
      {
         ...
         result = _Boost::math::tgamma(1+dz, pol) - 1;
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
         // Use expm1 on lgamma:
         result = _Boost::math::expm1(-_Boost::math::log1p(dz, pol) 
   ...
   else
   {
      ...
      {
         // Use expm1 on lgamma:
         result = _Boost::math::expm1(lgamma_small_imp<T>(dz+1, dz, dz-1, tag_type(), pol, l), pol);
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
...
inline T tgammap1m1_imp(T z, Policy const& pol,
                 const ::_Boost::math::lanczos::undefined_lanczos&)
{
   BOOST_MATH_STD_USING // ADL of std names

   if(fabs(z) < 0.55)
   {
      return _Boost::math::expm1(log_gamma_near_1(z, pol));
   }
   return _Boost::math::expm1(_Boost::math::lgamma(1 + z, pol));
}
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   //
   if((_Boost::math::fpclassify)(prefix) == (int)FP_INFINITE)
      return policies::raise_overflow_error<T>("_Boost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
   ...
   else if((fabs(d*d*a) <= 100) && (a > 150))
   {
      ...
      prefix = a * _Boost::math::log1pmx(d, pol) + z * static_cast<T>(0.5 - Lanczos::g()) / agh;
      prefix = exp(prefix);
   }
   else
   {
   ...
   prefix *= sqrt(agh / _Boost::math::constants::e<T>()) / Lanczos::lanczos_sum_expG_scaled(a);
   return prefix;
}
//
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   if((a < 1) && (z < 1))
   {
      ...
      return pow(z, a) * exp(-z) / _Boost::math::tgamma(a, pol);
   }
   ...
   else
   {
      ...
      else
      {
         ...
         if ((0 == power_term_1) || (0 == power_term_2) || (0 == power_term_3) || (fabs(a + shift - z) > tools::log_max_value<T>()))
         {
            ...
            return exp(a * log(z) - z - _Boost::math::lgamma(a, pol));
         }
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   //
   T result;
   result = _Boost::math::tgamma1pm1(a, pol);
   if(pgam)
      *pgam = (result + 1) / a;
   T p = _Boost::math::powm1(x, a, pol);
   result -= p;
   result /= a;
   detail::small_gamma2_series<T> s(a, x);
   _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>() - 10;
   p += 1;
   if(pderivative)
      *pderivative = p / (*pgam * exp(x));
   T init_value = invert ? *pgam : 0;
   result = -p * tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
   policies::check_series_iterations<T>("_Boost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
   if(invert)
      result = -result;
   return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
   ...
   if(pderivative)
   {
      *pderivative = e * pow(x, a) / _Boost::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
   }
   return sum;
}
//
...
T finite_half_gamma_q(T a, T x, T* p_derivative, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T e = _Boost::math::erfc(sqrt(x), pol);
   if((e != 0) && (a > 1))
   {
...
T incomplete_tgamma_large_x(const T& a, const T& x, const Policy& pol)
{
   ...
   _Boost::uintmax_t max_iter = _Boost::math::policies::get_max_series_iterations<Policy>();
   T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
   _Boost::math::policies::check_series_iterations<T>("_Boost::math::tgamma<%1%>(%1%,%1%)", max_iter, pol);
   return result;
}
...
{
   static const char* function = "_Boost::math::gamma_p<%1%>(%1%, %1%)";
   if(a <= 0)
      ...
      else
      {
         ...
         if(result == 0)
         {
            if(invert)
            {
               ...
               result = log(result) - a + (a - 0.5f) * log(a) + log(_Boost::math::constants::root_two_pi<T>());
               if(p_derivative)
         ...
         {
            result = log(result) + _Boost::math::lgamma(a, pol);
         }
      }
   ...
   case 0:
      {
         ...
         if(normalised == false)
            result *= _Boost::math::tgamma(a, pol);
         break;
      }
   case 1:
      {
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = (normalised ? 1 : _Boost::math::tgamma(a, pol));
   ...
   case 6:
      {
         ...
         result = !normalised ? pow(x, a) / (a) : pow(x, a) / _Boost::math::tgamma(a + 1, pol);
         result *= 1 - a * x / (a + 1);
         if (p_derivative)
            *p_derivative = regularised_gamma_prefix(a, x, pol, lanczos_type());
         break;
   ...
   if(invert)
   {
      T gam = normalised ? 1 : _Boost::math::tgamma(a, pol);
      result = gam - result;
   }
   if(p_derivative)
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const Lanczos& l)
{
   ...
   if(z < tools::epsilon<T>())
   {
      ...
      //
      if(_Boost::math::max_factorial<T>::value < delta)
      {
         T ratio = tgamma_delta_ratio_imp_lanczos(delta, T(_Boost::math::max_factorial<T>::value - delta), pol, l);
         ratio *= z;
         ratio *= _Boost::math::unchecked_factorial<T>(_Boost::math::max_factorial<T>::value - 1);
         return 1 / ratio;
      }
      else
      {
         return 1 / (z * _Boost::math::tgamma(z + delta, pol));
      }
   }
   ...
   {
      if(fabs(delta) < 10)
         result = exp((constants::half<T>() - z) * _Boost::math::log1p(delta / zgh, pol));
      else
         result = 1;
   }
   else
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   if (numerator_shift == 0 && denominator_shift == 0)
   {
      ...
      result *= exp(z * _Boost::math::log1p(-delta / (z + delta), pol)) * pow((delta + z) / constants::e<T>(), -delta);
      return result;
   }
   //
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if((z <= 0) || (z + delta <= 0))
   {
      ...
      return _Boost::math::tgamma(z, pol) / _Boost::math::tgamma(z + delta, pol);
   }
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if((x <= 0) || (_Boost::math::isinf)(x))
      return policies::raise_domain_error<T>("_Boost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", x, pol);
   if((y <= 0) || (_Boost::math::isinf)(y))
      return policies::raise_domain_error<T>("_Boost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", y, pol);
   ...
   if((x < max_factorial<T>::value) && (y < max_factorial<T>::value))
   {
      ...
      return _Boost::math::tgamma(x, pol) / _Boost::math::tgamma(y, pol);
   }
   T prefix = 1;
   if(x < 1)
   {
      if(y < 2 * max_factorial<T>::value)
      {
         ...
         return prefix * _Boost::math::tgamma(x, pol) / _Boost::math::tgamma(y, pol);
      }
      //
      // result is almost certainly going to underflow to zero, try logs just in case:
      //
      return exp(_Boost::math::lgamma(x, pol) - _Boost::math::lgamma(y, pol));
   }
   if(y < 1)
   {
   ...
   //
   return _Boost::math::tgamma_delta_ratio(x, y - x, pol);
}
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      return policies::raise_domain_error<T>("_Boost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      return policies::raise_domain_error<T>("_Boost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 : policies::raise_overflow_error<T>("_Boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   //
   // Normal case:
   //
   ...
   {
      // overflow:
      return policies::raise_overflow_error<T>("_Boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   if(f1 == 0)
   {
...
inline typename tools::promote_args<T>::type 
   tgamma(T z, const Policy& /* pol */, const _Boost::true_type)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "_Boost::math::tgamma<%1%>(%1%)");
}
...
struct igamma_initializer
{
   struct init
   {
      ...
      template <int N>
      static void do_init(const _Boost::integral_constant<int, N>&)
      {
         ...
         {
            _Boost::math::gamma_p(static_cast<T>(400), static_cast<T>(400), Policy());
         }
      }
      static void do_init(const _Boost::integral_constant<int, 53>&){}
...
struct lgamma_initializer
{
   struct init
   {
      ...
      static void do_init(const _Boost::integral_constant<int, 64>&)
      {
         _Boost::math::lgamma(static_cast<T>(2.5), Policy());
         _Boost::math::lgamma(static_cast<T>(1.25), Policy());
         _Boost::math::lgamma(static_cast<T>(1.75), Policy());
      }
      static void do_init(const _Boost::integral_constant<int, 113>&)
      {
         _Boost::math::lgamma(static_cast<T>(2.5), Policy());
         _Boost::math::lgamma(static_cast<T>(1.25), Policy());
         _Boost::math::lgamma(static_cast<T>(1.5), Policy());
         _Boost::math::lgamma(static_cast<T>(1.75), Policy());
      }
      static void do_init(const _Boost::integral_constant<int, 0>&)
      {
      }
      void force_instantiate()const{}
   };
...
inline typename tools::promote_args<T1, T2>::type
   tgamma(T1 a, T2 z, const Policy&, const _Boost::false_type)
{
   BOOST_FPU_EXCEPTION_GUARD
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      ...
      forwarding_policy(), static_cast<value_type*>(0)), "_Boost::math::tgamma<%1%>(%1%, %1%)");
}

template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
   tgamma(T1 a, T2 z, const _Boost::false_type& tag)
{
   ...
   lgamma(T z, int* sign, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign), "_Boost::math::lgamma<%1%>(%1%)");
}
...
{
   return ::_Boost::math::lgamma(x, 0, pol);
}
...
inline typename tools::promote_args<T>::type 
   lgamma(T x)
{
   return ::_Boost::math::lgamma(x, 0, policies::policy<>());
}
   ...
   tgamma1pm1(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<typename remove_cv<result_type>::type, forwarding_policy>(detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "_Boost::math::tgamma1pm1<%!%>(%1%)");
}
...
{
   return detail::tgamma(a, z, pol, _Boost::false_type());
}
//
   ...
   tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()), "_Boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   tgamma_ratio(T1 a, T2 b, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(b), forwarding_policy()), "_Boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   gamma_p_derivative(T1 a, T2 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()), "_Boost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
