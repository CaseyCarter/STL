...
T beta_imp(T a, T b, const Lanczos&, const Policy& pol)
{
   ...
   if(a <= 0)
      return policies::raise_domain_error<T>("_Boost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got a=%1%).", a, pol);
   if(b <= 0)
      return policies::raise_domain_error<T>("_Boost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got b=%1%).", b, pol);
   ...
   */
   ...
   if((fabs(b * ambh) < (cgh * 100)) && (a > 100))
   {
      ...
      result *= exp(ambh * _Boost::math::log1p(-b / cgh, pol));
   }
   else
   {
   ...
   result *= sqrt(_Boost::math::constants::e<T>() / bgh);
...
T beta_imp(T a, T b, const lanczos::undefined_lanczos& l, const Policy& pol)
{
   ...
   {
      return _Boost::math::tgamma(a, pol) * (_Boost::math::tgamma(b, pol) / _Boost::math::tgamma(c));
   }
   else if(a < 1)
      return _Boost::math::tgamma(a, pol) * _Boost::math::tgamma_delta_ratio(b, a, pol);
   else if(b < 1)
      return _Boost::math::tgamma(b, pol) * _Boost::math::tgamma_delta_ratio(a, b, pol);
   else
   {
...
T ibeta_power_terms(T a,
                        ...
                        const Policy& pol,
                        T prefix = 1,
                        const char* function = "_Boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
   BOOST_MATH_STD_USING
   ...
   result *= sqrt(bgh / _Boost::math::constants::e<T>());
   result *= sqrt(agh / cgh);
   ...
   if(((std::min)(fabs(l1), fabs(l2)) < 0.2))
   {
      ...
      if((l1 * l2 > 0) || ((std::min)(a, b) < 1))
      {
         ...
         if(fabs(l1) < 0.1)
         {
            result *= exp(a * _Boost::math::log1p(l1, pol));
         ...
         if(fabs(l2) < 0.1)
         {
            result *= exp(b * _Boost::math::log1p(l2, pol));
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
      ...
      else if((std::max)(fabs(l1), fabs(l2)) < 0.5)
      {
         ...
         {
            T l3 = _Boost::math::expm1(ratio * _Boost::math::log1p(l2, pol), pol);
            l3 = l1 + l3 + l3 * l1;
            l3 = a * _Boost::math::log1p(l3, pol);
            result *= exp(l3);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
            T l3 = _Boost::math::expm1(_Boost::math::log1p(l1, pol) / ratio, pol);
            l3 = l2 + l3 + l3 * l2;
            l3 = b * _Boost::math::log1p(l3, pol);
            result *= exp(l3);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
      }
      else if(fabs(l1) < fabs(l2))
      {
         ...
         T l = a * _Boost::math::log1p(l1, pol)
            + b * log((y * cgh) / bgh);
      ...
      else
      {
         ...
         T l = b * _Boost::math::log1p(l2, pol)
            + a * log((x * cgh) / agh);
         if((l <= tools::log_min_value<T>()) || (l >= tools::log_max_value<T>()))
         {
            l += log(result);
...
T ibeta_power_terms(T a,
                        ...
                        T x,
                        T y,
                        const _Boost::math::lanczos::undefined_lanczos& l,
                        bool normalised,
                        const Policy& pol,
                        T prefix = 1,
                        const char* = "_Boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
   BOOST_MATH_STD_USING
   ...
   if ((shift_a == 0) && (shift_b == 0))
   {
      ...
      if (!(_Boost::math::isnormal)(power1) || !(_Boost::math::isnormal)(power2))
      {
         // We have to use logs :(
   ...
   if(!(_Boost::math::isnormal)(power1) || !(_Boost::math::isnormal)(power2) || !(_Boost::math::isnormal)(bet))
   {
      int shift_c = shift_a + shift_b;
      T result = ibeta_power_terms(T(a + shift_a), T(b + shift_b), x, y, l, normalised, pol, prefix);
      if ((_Boost::math::isnormal)(result))
      {
      ...
      else
      {
         ...
         if ((_Boost::math::isnormal)(bet))
            log_result -= log(bet);
         else
            log_result += _Boost::math::lgamma(c, pol) - _Boost::math::lgamma(a) - _Boost::math::lgamma(c, pol);
...
T ibeta_series(T a, T b, T x, T s0, const Lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   ...
   if(normalised)
   {
      ...
      {
         if(a * b < bgh * 10)
            result *= exp((b - 0.5f) * _Boost::math::log1p(a / bgh, pol));
         else
            result *= pow(cgh / bgh, b - 0.5f);
         result *= pow(x * cgh / agh, a);
         result *= sqrt(agh / _Boost::math::constants::e<T>());
   ...
   result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("_Boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (with lanczos)", max_iter, pol);
   return result;
}
//
...
//
template <class T, class Policy>
T ibeta_series(T a, T b, T x, T s0, const _Boost::math::lanczos::undefined_lanczos& l, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   BOOST_MATH_STD_USING
   ...
   if(normalised)
   {
      ...
      if ((shift_a == 0) && (shift_b == 0))
      {
         ...
         result = pow(x, a) / (_Boost::math::tgamma(a, pol) * _Boost::math::tgamma_delta_ratio(b, a, pol));
      else
      {
         T power = pow(x, a);
         T bet = beta_imp(a, b, l, pol);
         if (!(_Boost::math::isnormal)(power) || !(_Boost::math::isnormal)(bet))
         {
            result = exp(a * log(x) + _Boost::math::lgamma(c, pol) - _Boost::math::lgamma(a, pol) - _Boost::math::lgamma(b, pol));
         }
         else
            result = power / bet;
      }
   ...
   _Boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("_Boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without lanczos)", max_iter, pol);
   return result;
}
...
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised, T* p_derivative)
{
   ...
   T fract = _Boost::math::tools::continued_fraction_b(f, _Boost::math::policies::get_epsilon<T, Policy>());
...
struct Pn_size
{
   ...
   BOOST_STATIC_CONSTANT(unsigned, value =
      ::_Boost::math::max_factorial<T>::value >= 100 ? 50
   : ::_Boost::math::max_factorial<T>::value >= ::_Boost::math::max_factorial<double>::value ? 30
   : ::_Boost::math::max_factorial<T>::value >= ::_Boost::math::max_factorial<float>::value ? 15 : 1);
   BOOST_STATIC_ASSERT(::_Boost::math::max_factorial<T>::value >= ::_Boost::math::max_factorial<float>::value);
};
template <>
struct Pn_size<float>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 15); // ~8-15 digit accuracy
   BOOST_STATIC_ASSERT(::_Boost::math::max_factorial<float>::value >= 30);
};
template <>
struct Pn_size<double>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 30); // 16-20 digit accuracy
   BOOST_STATIC_ASSERT(::_Boost::math::max_factorial<double>::value >= 60);
};
template <>
struct Pn_size<long double>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 50); // ~35-50 digit accuracy
   BOOST_STATIC_ASSERT(::_Boost::math::max_factorial<long double>::value >= 100);
};
...
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy& pol, bool normalised)
{
   ...
   if(normalised)
   {
      prefix = h / _Boost::math::tgamma_delta_ratio(a, b, pol);
      prefix /= pow(t, b);
   }
   else
   {
   ...
   //
   T p[ ::_Boost::math::detail::Pn_size<T>::value ] = { 1 };  // see 9.3.
   //
   // Now an initial value for J, see 9.6:
   //
   T j = _Boost::math::gamma_q(b, u, pol) / h;
   //
   ...
   for(unsigned n = 1; n < sizeof(p)/sizeof(p[0]); ++n)
   {
      ...
      */
      ...
      {
         mbn = m * b - n;
         p[n] += mbn * p[n-m] / _Boost::math::unchecked_factorial<T>(tmp1);
         tmp1 += 2;
      }
      p[n] /= n;
      p[n] += bm1 / _Boost::math::unchecked_factorial<T>(tnp1);
      //
...
T binomial_ccdf(T n, T k, T x, T y)
{
   ...
   else
   {
      ...
      if(start <= k + 1)
         start = itrunc(k + 2);
      result = pow(x, start) * pow(y, n - start) * _Boost::math::binomial_coefficient<T>(itrunc(n), itrunc(start));
      if(result == 0)
      {
         ...
         {
            result += pow(x, (int)i) * pow(y, n - i) * _Boost::math::binomial_coefficient<T>(itrunc(n), itrunc(i));
         }
      }
      else
      {
...
{
   static const char* function = "_Boost::math::ibeta<%1%>(%1%, %1%, %1%)";
   typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
   ...
   if(x == 0)
   {
      ...
      return (invert ? (normalised ? T(1) : _Boost::math::beta(a, b, pol)) : T(0));
   }
   if(x == 1)
   {
      if(p_derivative)
      ...
      return (invert == 0 ? (normalised ? 1 : _Boost::math::beta(a, b, pol)) : 0);
   }
   if((a == 0.5f) && (b == 0.5f))
   {
   ...
   if(b == 1)
   {
      ...
      T p;
      if(y < 0.5)
         p = invert ? T(-_Boost::math::expm1(a * _Boost::math::log1p(-y, pol), pol)) : T(exp(a * _Boost::math::log1p(-y, pol)));
      else
         p = invert ? T(-_Boost::math::powm1(x, a, pol)) : T(pow(x, a));
      if(!normalised)
         p /= a;
      return p;
   }
   ...
   if((std::min)(a, b) <= 1)
   {
      ...
      if((std::max)(a, b) <= 1)
      {
         ...
         if((a >= (std::min)(T(0.2), b)) || (pow(x, a) <= 0.9))
         {
            ...
            {
               fract = -(normalised ? 1 : _Boost::math::beta(a, b, pol));
               invert = false;
   ...
   else
   {
      ...
      if(b < 40)
      {
         if((floor(a) == a) && (floor(b) == b) && (a < (std::numeric_limits<int>::max)() - 100) && (y != 1))
         {
            ...
            if(!normalised)
               fract *= _Boost::math::beta(a, b, pol);
            BOOST_MATH_INSTRUMENT_VARIABLE(fract);
         }
         else if(b * x <= 0.7)
         {
   ...
   return invert ? (normalised ? 1 : _Boost::math::beta(a, b, pol)) - fract : fract;
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
   ...
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 / _Boost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   else if(x == 1)
   {
      return (b > 1) ? 0 :
         (b == 1) ? 1 / _Boost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   //
...
inline typename tools::promote_args<RT1, RT2>::type
   beta(RT1 a, RT2 b, const Policy&, const _Boost::true_type*)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::beta_imp(static_cast<value_type>(a), static_cast<value_type>(b), evaluation_type(), forwarding_policy()), "_Boost::math::beta<%1%>(%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   beta(RT1 a, RT2 b, RT3 x, const _Boost::false_type*)
{
   return _Boost::math::beta(a, b, x, policies::policy<>());
}
} // namespace detail
   ...
   beta(RT1 a, RT2 b, A arg)
{
   ...
   return _Boost::math::detail::beta(a, b, arg, static_cast<tag*>(0));
}
...
inline typename tools::promote_args<RT1, RT2>::type
   beta(RT1 a, RT2 b)
{
   return _Boost::math::beta(a, b, policies::policy<>());
}
   ...
   beta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, false), "_Boost::math::beta<%1%>(%1%,%1%,%1%)");
}
   ...
   betac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, false), "_Boost::math::betac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   betac(RT1 a, RT2 b, RT3 x)
{
   return _Boost::math::betac(a, b, x, policies::policy<>());
}
   ...
   ibeta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, true), "_Boost::math::ibeta<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibeta(RT1 a, RT2 b, RT3 x)
{
   return _Boost::math::ibeta(a, b, x, policies::policy<>());
}
   ...
   ibetac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, true), "_Boost::math::ibetac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibetac(RT1 a, RT2 b, RT3 x)
{
   return _Boost::math::ibetac(a, b, x, policies::policy<>());
}
   ...
   ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy()), "_Boost::math::ibeta_derivative<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
   ibeta_derivative(RT1 a, RT2 b, RT3 x)
{
   return _Boost::math::ibeta_derivative(a, b, x, policies::policy<>());
}
