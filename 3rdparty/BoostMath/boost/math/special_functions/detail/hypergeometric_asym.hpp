...
#pragma warning(disable:4127)
  ...
  namespace detail {
     ...
     inline T hypergeometric_1F1_asym_large_z_series(T a, const T& b, T z, const Policy& pol, int& log_scaling)
     {
        ...
        static const char* function = "_Boost::math::hypergeometric_1F1_asym_large_z_series<%1%>(%1%, %1%, %1%)";
        T prefix;
        int e, s;
        if (z < 0)
        {
        ...
        {
           prefix *= pow(z, a) * pow(z, -b) * _Boost::math::tgamma(b, pol) / _Boost::math::tgamma(a, pol);
        }
        else
        {
           T t = log(z) * (a - b);
           ...
           t = _Boost::math::lgamma(b, &s, pol);
           e = itrunc(t, pol);
           log_scaling += e;
           prefix *= s * exp(t - e);

           t = _Boost::math::lgamma(a, &s, pol);
           e = itrunc(t, pol);
           log_scaling -= e;
           prefix /= s * exp(t - e);
        }
        ...
        do
        {
           ...
           a1_poch += 1;
           a2_poch += 1;
           if (fabs(sum) * _Boost::math::policies::get_epsilon<T, Policy>() > fabs(term))
              break;
           if(fabs(sum) / abs_sum < _Boost::math::policies::get_epsilon<T, Policy>())
              return _Boost::math::policies::raise_evaluation_error<T>(function, "Large-z asymptotic approximation to 1F1 has destroyed all the digits in the result due to cancellation.  Current best guess is %1%", 
                 prefix * sum, Policy());
           if(k > _Boost::math::policies::get_max_series_iterations<Policy>())
              return _Boost::math::policies::raise_evaluation_error<T>(function, "1F1: Unable to locate solution in a reasonable time:"
                 " large-z asymptotic approximation.  Current best guess is %1%", prefix * sum, Policy());
           if((k > 10) && (fabs(term) > fabs(last_term)))
              return _Boost::math::policies::raise_evaluation_error<T>(function, "Large-z asymptotic approximation to 1F1 is divergent.  Current best guess is %1%", prefix * sum, Policy());
        } while (true);
