...
namespace _Boost { namespace math {
   ...
   namespace detail {
      ...
      int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
      {
         ...
         T spv = _Boost::math::sin_pi(v, pol);
         T spv2 = _Boost::math::sin_pi(v/2, pol);
         T xp = pow(x/2, v);
         ...
         policies::check_series_iterations<T>("_Boost::math::bessel_jy<%1%>(%1%,%1%) in temme_jy", k, pol);
         *Y = -sum;
         *Y1 = -2 * sum1 / x;
      ...
      int CF1_jy(T v, T x, T* fv, int* sign, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("_Boost::math::bessel_jy<%1%>(%1%,%1%) in CF1_jy", k / 100, pol);
         *fv = -f;
      ...
      int CF2_jy(T v, T x, T* p, T* q, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("_Boost::math::bessel_jy<%1%>(%1%,%1%) in CF2_jy", k, pol);
         *p = fr;
         *q = fi;
      ...
      int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
      {
         ...
         static const char* function = "_Boost::math::bessel_jy<%1%>(%1%,%1%)";
         ...
         {
            T z = (u + n % 2);
            cp = _Boost::math::cos_pi(z, pol);
            sp = _Boost::math::sin_pi(z, pol);
            if(u != 0)
         ...
         else if((x > 8) && hankel_PQ(v, x, &p, &q, pol))
         {
            ...
            //
            // chi = x - fmod(T(v / 2 + 0.25f), T(2)) * _Boost::math::constants::pi<T>();
            //
            ...
            T chi = _Boost::math::constants::root_two<T>() / (_Boost::math::constants::root_pi<T>() * sqrt(x)); //sqrt(2 / (_Boost::math::constants::pi<T>() * x));
            Yv = chi * (p * sc + q * cc);
            Jv = chi * (p * cc - q * sc);
         }
