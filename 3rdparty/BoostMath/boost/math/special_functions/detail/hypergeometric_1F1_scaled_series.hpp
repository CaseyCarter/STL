...
#include <boost/array.hpp>
  ...
  namespace _Boost{ namespace math{ namespace detail{
     ...
     T hypergeometric_1F1_scaled_series(const T& a, const T& b, T z, const Policy& pol, const char* function)
     {
        ...
        unsigned n = 0;
        _Boost::intmax_t log_scaling_factor = 1 - itrunc(_Boost::math::tools::log_max_value<T>());
        T scaling_factor = exp(T(log_scaling_factor));
        _Boost::intmax_t current_scaling = 0;
        ...
        do
        {
           ...
           ++n;
           diff = fabs(term / sum);
        } while (diff > _Boost::math::policies::get_epsilon<T, Policy>());
