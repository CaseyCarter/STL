...
#include <boost/math/policies/error_handling.hpp>
  ...
  inline T sum_pFq_series(detail::hypergeometric_pFq_generic_series_term<T, p, q>& term, const Policy& pol)
  {
    ...
    const T zero = 0;
    const T result = _Boost::math::tools::sum_series(term, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
    const T result = _Boost::math::tools::sum_series(term, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
    policies::check_series_iterations<T>("_Boost::math::hypergeometric_pFq_generic_series<%1%>(%1%,%1%,%1%)", max_iter, pol);
    return result;
  }
  ...
  inline T log_pochhammer(T z, unsigned n, const Policy pol, int* s = 0)
  {
...
#endif
     {
        ...
        int s1, s2;
        T r = _Boost::math::lgamma(T(z + n), &s1, pol) - _Boost::math::lgamma(z, &s2, pol);
        if(s)
           *s = s1 * s2;
        return r;
     }
  ...
  {
     BOOST_MATH_STD_USING
     T sum(0), term(1), upper_limit(sqrt(_Boost::math::tools::max_value<T>())), diff;
     T lower_limit(1 / upper_limit);
     unsigned n = 0;
     int log_scaling_factor = itrunc(_Boost::math::tools::log_max_value<T>()) - 2;
     ...
     if (summit_location > _Boost::math::policies::get_max_series_iterations<Policy>() / 4)
     {
        //
        ...
        //std::cout << term << " " << log_pochhammer(_Boost::multiprecision::mpfr_float(a), summit_location, pol, &s1) + summit_location * log(_Boost::multiprecision::mpfr_float(z)) - log_pochhammer(_Boost::multiprecision::mpfr_float(b), summit_location, pol, &s2) - lgamma(_Boost::multiprecision::mpfr_float(summit_location + 1), pol) << std::endl;
        local_scaling = itrunc(term);
        log_scaling += local_scaling;
        term = s1 * s2 * exp(term - local_scaling);
        //std::cout << term << " " << exp(log_pochhammer(_Boost::multiprecision::mpfr_float(a), summit_location, pol, &s1) + summit_location * log(_Boost::multiprecision::mpfr_float(z)) - log_pochhammer(_Boost::multiprecision::mpfr_float(b), summit_location, pol, &s2) - lgamma(_Boost::multiprecision::mpfr_float(summit_location + 1), pol) - local_scaling) << std::endl;
        n = summit_location;
     }
     else
        summit_location = 0;
     ...
     {
        sum += term;
        //std::cout << n << " " << term * exp(_Boost::multiprecision::mpfr_float(local_scaling)) << " " << rising_factorial(_Boost::multiprecision::mpfr_float(a), n) * pow(_Boost::multiprecision::mpfr_float(z), n) / (rising_factorial(_Boost::multiprecision::mpfr_float(b), n) * factorial<_Boost::multiprecision::mpfr_float>(n)) << std::endl;
        ...
        term_m1 = term;
        term *= (((a + n) / ((b + n) * (n + 1))) * z);
        if (n - summit_location > _Boost::math::policies::get_max_series_iterations<Policy>())
           return _Boost::math::policies::raise_evaluation_error(function, "Series did not converge, best value is %1%", sum, pol);
        ++n;
        diff = fabs(term / sum);
     } while ((diff > _Boost::math::policies::get_epsilon<T, Policy>()) || (fabs(term_m1) < fabs(term)) || (small_a && n < 10));
     ...
     if (summit_location)
     {
        ...
        do
        {
           ...
           term_m1 = term;
           term *= (b + (n - 1)) * n / ((a + (n - 1)) * z);
           if (summit_location - n > _Boost::math::policies::get_max_series_iterations<Policy>())
              return _Boost::math::policies::raise_evaluation_error(function, "Series did not converge, best value is %1%", sum, pol);
           --n;
           diff = fabs(term / sum);
        } while ((diff > _Boost::math::policies::get_epsilon<T, Policy>()) || (fabs(term_m1) < fabs(term)));
     }
     ...
     if (have_minima && n && summit_location)
     {
        ...
        do
        {
           ...
           //term_m1 = term;
           term *= (((a + n) / ((b + n) * (n + 1))) * z);
           if (n > _Boost::math::policies::get_max_series_iterations<Policy>())
              return _Boost::math::policies::raise_evaluation_error(function, "Series did not converge, best value is %1%", sum, pol);
           if (++n == backstop)
