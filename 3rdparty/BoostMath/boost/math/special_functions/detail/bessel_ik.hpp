...
inline T bessel_i_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / _Boost::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - _Boost::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if(prefix == 0)
      return prefix;
...
#endif
   policies::check_series_iterations<T>("_Boost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    T gp = _Boost::math::tgamma1pm1(v, pol);
    T gm = _Boost::math::tgamma1pm1(-v, pol);

    a = log(x / 2);
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ?
       T(1) : T(_Boost::math::sin_pi(v) / (v * pi<T>()));
    ...
    policies::check_series_iterations<T>("_Boost::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("_Boost::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("_Boost::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    static const char* function = "_Boost::math::bessel_ik<%1%>(%1%,%1%)";
    ...
    for (k = 1; k <= n; k++)                   // forward recurrence for K
    {
        ...
        if((tools::max_value<T>() - fabs(prev)) / fact < fabs(current))
        {
           prev /= current;
           scale /= current;
           scale_sign *= _Boost::math::sign(current);
           current = 1;
        }
    ...
    {
        T z = (u + n % 2);
        T fact = (2 / pi<T>()) * (_Boost::math::sin_pi(z) * Kv);
        if(fact == 0)
           *I = Iv;
