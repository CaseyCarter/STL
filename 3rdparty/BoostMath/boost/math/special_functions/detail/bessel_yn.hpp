...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    static const char* function = "_Boost::math::bessel_yn<%1%>(%1%,%1%)";
    ...
    if(x < policies::get_epsilon<T, Policy>())
    {
       ...
       if(tools::max_value<T>() * fabs(scale) < fabs(value))
          return _Boost::math::sign(scale) * _Boost::math::sign(value) * policies::raise_overflow_error<T>(function, 0, pol);
       value /= scale;
    }
    ...
    else
    {
       ...
       int k = 1;
       BOOST_ASSERT(k < n);
       policies::check_series_iterations<T>("_Boost::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
       T mult = 2 * k / x;
