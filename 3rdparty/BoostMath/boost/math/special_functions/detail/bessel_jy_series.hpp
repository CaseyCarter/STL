...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   *pscale = scale;
   ...
   T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>("_Boost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = _Boost::math::tgamma(-v, pol) * _Boost::math::cos_pi(v) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = _Boost::math::lgamma(-v, &sgn, pol) + p;
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   {
      T p = pow(z / 2, n);
      T result = -((_Boost::math::factorial<T>(n - 1) / constants::pi<T>()));
