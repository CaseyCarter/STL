...
struct gamma_inva_t
{
   ...
   T operator()(T a)
   {
      return invert ? p - _Boost::math::gamma_q(a, z, Policy()) : _Boost::math::gamma_p(a, z, Policy()) - p;
   }
private:
   T z, p;
   bool invert;
...
T inverse_poisson_cornish_fisher(T lambda, T p, T q, const Policy& pol)
{
   ...
   T x = _Boost::math::erfc_inv(p > q ? 2 * q : 2 * p, pol) * constants::root_two<T>();
   // Set the sign:
   if(p < 0.5)
      x = -x;
   T x2 = x * x;
...
T gamma_inva_imp(const T& z, const T& p, const T& q, const Policy& pol)
{
   ...
   if(p == 0)
   {
      return policies::raise_overflow_error<T>("_Boost::math::gamma_p_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(q == 0)
   {
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      return policies::raise_evaluation_error<T>("_Boost::math::gamma_p_inva<%1%>(%1%, %1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
   ...
   gamma_p_inva(T1 x, T2 p, const Policy& pol)
{
   ...
   if(p == 0)
   {
      policies::raise_overflow_error<result_type>("_Boost::math::gamma_p_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(p == 1)
   {
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "_Boost::math::gamma_p_inva<%1%>(%1%, %1%)");
}
   ...
   gamma_q_inva(T1 x, T2 q, const Policy& pol)
{
   ...
   if(q == 1)
   {
      policies::raise_overflow_error<result_type>("_Boost::math::gamma_q_inva<%1%>(%1%, %1%)", 0, Policy());
   }
   if(q == 0)
   {
      return tools::min_value<result_type>();
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "_Boost::math::gamma_q_inva<%1%>(%1%, %1%)");
}
...
{
   return _Boost::math::gamma_p_inva(x, p, policies::policy<>());
}
...
{
   return _Boost::math::gamma_q_inva(x, q, policies::policy<>());
}
