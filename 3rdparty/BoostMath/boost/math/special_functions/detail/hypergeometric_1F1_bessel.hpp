...
#include <boost/math/special_functions/bessel_iterators.hpp>
  ...
  namespace _Boost { namespace math { namespace detail {
     ...
     bool hypergeometric_1F1_is_tricomi_viable_positive_b(const T& a, const T& b, const T& z)
     {
        ...
        T v = b - 1;
        return log(_Boost::math::constants::e<T>() * x / (2 * v)) * v > tools::log_min_value<T>();
     }
     ...
     struct hypergeometric_1F1_AS_13_3_7_tricomi_series
     {
           ...
           two_a_minus_b(2 * a - b), pol(pol_), n(2)
        {
           ...
           bessel_cache[cache_size - 1] = bessel_arg > 0 ? _Boost::math::cyl_bessel_j(b_minus_1_plus_n - 1, 2 * sqrt(bessel_arg), pol) : _Boost::math::cyl_bessel_i(b_minus_1_plus_n - 1, 2 * sqrt(-bessel_arg), pol);
           ...
           if ((term * bessel_cache[cache_size - 1] < tools::min_value<T>() / (tools::epsilon<T>() * tools::epsilon<T>())) || !(_Boost::math::isfinite)(term) || (!std::numeric_limits<T>::has_infinity && (fabs(term) > tools::max_value<T>())))
           {
           ...
           {
              if (!(_Boost::math::isfinite)(bessel_cache[cache_size - 1]))
              ...
              if ((_Boost::math::isnan)(bessel_cache[cache_size - 1]) || (fabs(bessel_cache[cache_size - 1]) >= tools::max_value<T>()))
                 policies::raise_evaluation_error("hypergeometric_1F1_AS_13_3_7_tricomi_series<%1%>", "Expected finite Bessel function result but got %1%", bessel_cache[cache_size - 1], pol);
#else
           if ((std::numeric_limits<T>::has_infinity && !(_Boost::math::isfinite)(bessel_cache[cache_size - 1])) 
              || (!std::numeric_limits<T>::has_infinity && ((_Boost::math::isnan)(bessel_cache[cache_size - 1]) || (fabs(bessel_cache[cache_size - 1]) >= tools::max_value<T>()))))
              policies::raise_evaluation_error("hypergeometric_1F1_AS_13_3_7_tricomi_series<%1%>", "Expected finite Bessel function result but got %1%", bessel_cache[cache_size - 1], pol);
#endif
              ...
              if (b_minus_1_plus_n > 0)
              {
                 ...
                 for (int j = cache_size - 1; j >= 0; --j, ++i)
                 {
                    ...
                    if ((j < cache_size - 2) && (tools::max_value<T>() / fabs(64 * bessel_cache[j] / bessel_cache[j + 1]) < fabs(bessel_cache[j])))
                    {
                       ...
                       if (!((_Boost::math::isfinite)(rescale)))
     ...
     T hypergeometric_1F1_AS_13_3_7_tricomi(const T& a, const T& b, const T& z, const Policy& pol, int& log_scale)
     {
        ...
        try
        {
           prefix = _Boost::math::tgamma(b, pol);
           prefix *= exp(z / 2);
        }
        ...
        if (use_logs || (prefix == 0) || !(_Boost::math::isfinite)(prefix) || (!std::numeric_limits<T>::has_infinity && (fabs(prefix) >= tools::max_value<T>())))
        {
           use_logs = true;
           prefix = _Boost::math::lgamma(b, &prefix_sgn, pol) + z / 2;
           scale = itrunc(prefix);
           log_scale += scale;
           prefix -= scale;
        }
        ...
        try
        {
           ...
           try
           {
              ...
              result = 0;
              if((a < 0) && (b < 0))
                 result = _Boost::math::tools::checked_sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, result, norm);
              else
                 result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter, result);
              if (!(_Boost::math::isfinite)(result) || (result == 0) || (!std::numeric_limits<T>::has_infinity && (fabs(result) >= tools::max_value<T>())))
                 retry = true;
              if (norm / fabs(result) > 1 / _Boost::math::tools::root_epsilon<T>())
        ...
        _Boost::math::policies::check_series_iterations<T>("_Boost::math::hypergeometric_1F1_AS_13_3_7<%1%>(%1%,%1%,%1%)", max_iter, pol);
        if (use_logs)
        {
           int sgn = _Boost::math::sign(result);
     ...
     T cyl_bessel_i_large_x_scaled(const T& v, const T& x, int& log_scaling, const Policy& pol)
     {
        ...
        _Boost::math::policies::check_series_iterations<T>("_Boost::math::cyl_bessel_i_large_x<%1%>(%1%,%1%)", max_iter, pol);
        int scale = _Boost::math::itrunc(x);
        log_scaling += scale;
        return result * exp(x - scale) / sqrt(_Boost::math::constants::two_pi<T>() * x);
     }
     ...
     struct hypergeometric_1F1_AS_13_3_6_series
     {
           ...
           : b_minus_a(b_minus_a), half_z(z / 2), poch_1(2 * b_minus_a - 1), poch_2(b_minus_a - a), b_poch(b), term(1), last_result(1), sign(1), n(0), cache_offset(-cache_size), scale(0), pol(pol_)
        {
           bessel_i_cache[cache_size - 1] = half_z > tools::log_max_value<T>() ?
              cyl_bessel_i_large_x_scaled(T(b_minus_a - 1.5f), half_z, scale, pol) : _Boost::math::cyl_bessel_i(b_minus_a - 1.5f, half_z, pol);
           refill_cache();
        }
        T operator()()
        {
     ...
     T hypergeometric_1F1_AS_13_3_6(const T& a, const T& b, const T& z, const T& b_minus_a, const Policy& pol, int& log_scaling)
     {
        ...
        _Boost::uintmax_t max_iter = _Boost::math::policies::get_max_series_iterations<Policy>();
        T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
        _Boost::math::policies::check_series_iterations<T>("_Boost::math::hypergeometric_1F1_AS_13_3_6<%1%>(%1%,%1%,%1%)", max_iter, pol);
        result *= _Boost::math::tgamma(b_minus_a - 0.5f) * pow(z / 4, -b_minus_a + 0.5f);
     ...
     struct hypergeometric_1F1_AS_13_3_8_series
     {
        ...
        T operator()()
        {
           ...
           T result = C_minus_2 * power_term * _Boost::math::cyl_bessel_j(bessel_order, bessel_arg, pol);
           bessel_order += 1;
           power_term *= mult;
           ++n;
     ...
     {
        BOOST_MATH_STD_USING
        T prefix = exp(h * z) * _Boost::math::tgamma(b);
        hypergeometric_1F1_AS_13_3_8_series<T, Policy> s(a, b, z, h, pol);
        _Boost::uintmax_t max_iter = _Boost::math::policies::get_max_series_iterations<Policy>();
        T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
        _Boost::math::policies::check_series_iterations<T>("_Boost::math::hypergeometric_1F1_AS_13_3_8<%1%>(%1%,%1%,%1%)", max_iter, pol);
        result *= prefix;
        return result;
     }
     ...
     struct hypergeometric_1f1_13_11_1_series
     {
        ...
        T operator()()
        {
           T result = term * a_minus_half_plus_s * _Boost::math::cyl_bessel_i(a_minus_half_plus_s, half_z, pol);
     ...
     T hypergeometric_1f1_13_11_1(const T& a, const T& b, const T& z, const Policy& pol, int& log_scaling)
     {
        ...
        T prefix;
        int prefix_sgn = 1;
        if (true/*(a < _Boost::math::max_factorial<T>::value) && (a > 0)*/)
           prefix = _Boost::math::tgamma(a - 0.5f, pol);
        else
        {
           prefix = _Boost::math::lgamma(a - 0.5f, &prefix_sgn, pol);
           use_logs = true;
        }
        if (use_logs)
        {
        ...
        _Boost::uintmax_t max_iter = _Boost::math::policies::get_max_series_iterations<Policy>();
        T result = _Boost::math::tools::sum_series(s, _Boost::math::policies::get_epsilon<T, Policy>(), max_iter);
        _Boost::math::policies::check_series_iterations<T>("_Boost::math::hypergeometric_1f1_13_11_1<%1%>(%1%,%1%,%1%)", max_iter, pol);
        if (use_logs)
        {
