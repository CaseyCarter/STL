...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T u = log(p) + _Boost::math::lgamma(a + 1, pol);
   ...
   else
   {
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + _Boost::math::lgamma(ap1, pol);
            z = exp((v + w) / a);
            s = _Boost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = _Boost::math::log1p(z / ap1 * (1 + z / ap2), pol);
            z = exp((v + z - s) / a);
            s = _Boost::math::log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))), pol);
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   _Boost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      T f, f1;
      value_type ft;
      f = static_cast<T>(_Boost::math::detail::gamma_incomplete_imp(
      ...
      return _Boost::math::make_tuple(static_cast<T>(f - p), f1, f2);
   }
private:
   T a, p;
   bool invert;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::gamma_p_inv<%1%>(%1%, %1%)";
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::gamma_q_inv<%1%>(%1%, %1%)";
