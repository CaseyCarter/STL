...
#include <boost/math/special_functions/gamma.hpp>
  ...
  inline T hypergeometric_0F1_bessel(const T& b, const T& z, const Policy& pol)
  {
    ...
    const T bessel_mult = is_z_nonpositive ?
      _Boost::math::cyl_bessel_j(b - 1, 2 * sqrt_z, pol) :
      _Boost::math::cyl_bessel_i(b - 1, 2 * sqrt_z, pol) ;

    if (b > _Boost::math::max_factorial<T>::value)
    {
       const T lsqrt_z = log(sqrt_z);
       const T lsqrt_z_pow_b = (b - 1) * lsqrt_z;
       T lg = (_Boost::math::lgamma(b, pol) - lsqrt_z_pow_b);
       lg = exp(lg);
    ...
    else
    {
       ...
       return (_Boost::math::tgamma(b, pol) / sqrt_z_pow_b) * bessel_mult;
    }
  }
