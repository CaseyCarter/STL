...
#include <boost/math/special_functions/detail/hypergeometric_series.hpp>
  ...
  namespace _Boost { namespace math { namespace detail {
     ...
     std::pair<Real, Real> hypergeometric_pFq_checked_series_impl(const Seq& aj, const Seq& bj, const Real& z, const Policy& pol, const Terminal& termination, int& log_scale)
     {
        ...
        Real term = 1;
        Real term0 = 0;
        Real tol = _Boost::math::policies::get_epsilon<Real, Policy>();
        _Boost::uintmax_t k = 0;
        Real upper_limit(sqrt(_Boost::math::tools::max_value<Real>())), diff;
        Real lower_limit(1 / upper_limit);
        int log_scaling_factor = itrunc(_Boost::math::tools::log_max_value<Real>()) - 2;
        ...
        if ((aj.size() == 1) && (bj.size() == 0))
        {
           if (fabs(z) > 1)
           {
              ...
              {
                 Real r = policies::raise_domain_error("_Boost::math::hypergeometric_pFq", "Got p == 1 and q == 0 and |z| > 1, result is imaginary", z, pol);
        ...
        if (aj.size() > bj.size())
        {
           if (aj.size() == bj.size() + 1)
           {
              if (fabs(z) > 1)
              {
                 Real r = policies::raise_domain_error("_Boost::math::hypergeometric_pFq", "Got p == q+1 and |z| > 1, series does not converge", z, pol);
                 return std::make_pair(r, r);
              }
              if (fabs(z) == 1)
              {
                 ...
                 if ((z == 1) && (s <= 0))
                 {
                    Real r = policies::raise_domain_error("_Boost::math::hypergeometric_pFq", "Got p == q+1 and |z| == 1, in a situation where the series does not converge", z, pol);
                    return std::make_pair(r, r);
                 }
                 if ((z == -1) && (s <= -1))
                 {
           ...
           {
              Real r = policies::raise_domain_error("_Boost::math::hypergeometric_pFq", "Got p > q+1, series does not converge", z, pol);
              return std::make_pair(r, r);
           }
        }
        ...
        while (!termination(k))
        {
           ...
           for (auto bi = bj.begin(); bi != bj.end(); ++bi)
           {
              if (*bi + k == 0)
              {
                 ...
                 result = _Boost::math::policies::raise_domain_error("_Boost::math::hypergeometric_pFq<%1%>", "One of the b values was the negative integer %1%", *bi, pol);
           ...
           if (abs_result * tol > abs(result))
           {
              ...
              result = _Boost::math::policies::raise_evaluation_error("_Boost::math::hypergeometric_pFq<%1%>", "Cancellation is so severe that no bits in the reuslt are correct, last result was %1%", Real(result * exp(Real(log_scale))), pol);
              return std::make_pair(result, result);
           }
           term0 = term;
        }
        ...
        //std::cout << "Norm result = " << std::setprecision(35) << _Boost::multiprecision::mpfr_float_50(result) * exp(_Boost::multiprecision::mpfr_float_50(local_scaling)) << std::endl;
        //
        // We have to be careful when one of the b's crosses the origin:
        //
        if(bj.size() > BOOST_MATH_PFQ_MAX_B_TERMS)
           policies::raise_domain_error<Real>("_Boost::math::hypergeometric_pFq<%1%>(Seq, Seq, %1%)", 
        ...
        for (unsigned n = 0; n < N_crossovers; ++n)
        {
           if (k < crossover_locations[n])
           {
              ...
              //_Boost::multiprecision::mpfi_float err_est = 0;
              //
              ...
              for (auto ai = aj.begin(); ai != aj.end(); ++ai)
              {
                 ...
                 else
                 {
                    ...
                    //err_est += _Boost::multiprecision::mpfi_float(p);
                 }
              }
              ...
              if (term <= tools::log_min_value<Real>())
              {
               ...
               do
               {
                  ...
                  if (!trivial_small_series_check && !terms_are_growing)
                  {
                     ...
                     if (d < _Boost::math::policies::get_epsilon<Real, Policy>())
                        break;
                  }
               } while (!termination(k - s) && ((diff > _Boost::math::policies::get_epsilon<Real, Policy>()) || terms_are_growing));

               //std::cout << "Norm loop result = " << std::setprecision(35) << _Boost::multiprecision::mpfr_float_50(loop_result)* exp(_Boost::multiprecision::mpfr_float_50(loop_scale)) << std::endl;
               //
               ...
               //
               _Boost::uintmax_t next_backstop = k;
               ...
               do
               {
                  ...
                  diff = fabs(term / loop_result);
               } while (!termination(s - k) && ((diff > _Boost::math::policies::get_epsilon<Real, Policy>()) || (fabs(term) > fabs(term_m1))));
     ...
     {
        iteration_terminator(_Boost::uintmax_t i) : m(i) {}

        bool operator()(_Boost::uintmax_t v) const { return v >= m; }
     ...
     {
        BOOST_MATH_STD_USING
        iteration_terminator term(_Boost::math::policies::get_max_series_iterations<Policy>());
        ...
        //
        if (result.second * sqrt(_Boost::math::policies::get_epsilon<Real, Policy>()) > abs(result.first))
        {
           return _Boost::math::policies::raise_evaluation_error("_Boost::math::hypergeometric_pFq<%1%>", "Cancellation is so severe that fewer than half the bits in the result are correct, last result was %1%", Real(result.first * exp(Real(log_scale))), pol);
        }
        return result.first;
     }
     ...
     {
        _Boost::array<Real, 1> aj = { a };
        _Boost::array<Real, 1> bj = { b };
