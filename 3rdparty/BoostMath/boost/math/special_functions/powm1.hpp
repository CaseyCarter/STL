...
inline T powm1_imp(const T x, const T y, const Policy& pol)
{
   ...
   static const char* function = "_Boost::math::powm1<%1%>(%1%, %1%)";
   ...
   if (x > 0)
   {
      if ((fabs(y * (x - 1)) < 0.5) || (fabs(y) < 0.2))
      {
         ...
         T l = y * log(x);
         if (l < 0.5)
            return _Boost::math::expm1(l);
         if (l > _Boost::math::tools::log_max_value<T>())
   ...
   else
   {
      ...
      if (_Boost::math::trunc(y) != y)
         return _Boost::math::policies::raise_domain_error<T>(function, "For non-integral exponent, expected base > 0 but got %1%", x, pol);
      if (_Boost::math::trunc(y / 2) == y / 2)
