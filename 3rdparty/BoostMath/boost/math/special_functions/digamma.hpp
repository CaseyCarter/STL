...
//
inline unsigned digamma_large_lim(const _Boost::integral_constant<int, 0>*)
{  return 20;  }
inline unsigned digamma_large_lim(const _Boost::integral_constant<int, 113>*)
{  return 20;  }
...
//
template <class T>
inline T digamma_imp_large(T x, const _Boost::integral_constant<int, 113>*)
{
...
//
template <class T>
inline T digamma_imp_large(T x, const _Boost::integral_constant<int, 64>*)
{
   BOOST_MATH_STD_USING // ADL of std functions.
   static const T P[] = {
...
//
template <class T>
inline T digamma_imp_large(T x, const _Boost::integral_constant<int, 53>*)
{
   BOOST_MATH_STD_USING // ADL of std functions.
   static const T P[] = {
...
//
template <class T>
inline T digamma_imp_large(T x, const _Boost::integral_constant<int, 24>*)
{
   BOOST_MATH_STD_USING // ADL of std functions.
   static const T P[] = {
...
public:
   ...
   T operator()()
   {
      T result = term * _Boost::math::bernoulli_b2n<T>(k) / (2 * k);
      term /= xx;
      ++k;
      return result;
   }
...
inline T digamma_imp_large(T x, const Policy& pol, const _Boost::integral_constant<int, 0>*)
{
   ...
   result = -result;
   policies::check_series_iterations<T>("_Boost::math::digamma<%1%>(%1%)", max_iter, pol);
   return result;
}
//
...
//
template <class T>
T digamma_imp_1_2(T x, const _Boost::integral_constant<int, 113>*)
{
   //
...
//
template <class T>
T digamma_imp_1_2(T x, const _Boost::integral_constant<int, 64>*)
{
   //
   // Now the approximation, we use the form:
   //
...
//
template <class T>
T digamma_imp_1_2(T x, const _Boost::integral_constant<int, 53>*)
{
   //
   // Now the approximation, we use the form:
   //
...
//
template <class T>
inline T digamma_imp_1_2(T x, const _Boost::integral_constant<int, 24>*)
{
   //
   // Now the approximation, we use the form:
   //
...
T digamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x <= -1)
   {
      ...
      if(remainder == 0)
      {
         return policies::raise_pole_error<T>("_Boost::math::digamma<%1%>(%1%)", 0, (1-x), pol);
      }
      result = constants::pi<T>() / tan(constants::pi<T>() * remainder);
   }
   if(x == 0)
      return policies::raise_pole_error<T>("_Boost::math::digamma<%1%>(%1%)", 0, x, pol);
   //
...
template <class T, class Policy>
T digamma_imp(T x, const _Boost::integral_constant<int, 0>* t, const Policy& pol)
{
   //
   // This handles reflection of negative arguments, and all our
   // error handling, then forwards to the T-specific approximation.
...
struct digamma_initializer
{
   struct init
   {
      init()
      {
         ...
         do_init(_Boost::integral_constant<bool, precision_type::value && (precision_type::value <= 113)>());
      }
      void do_init(const _Boost::true_type&)
      {
         _Boost::math::digamma(T(1.5), Policy());
         _Boost::math::digamma(T(500), Policy());
      }
   ...
   digamma(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::digamma_imp(
      ...
      static_cast<const tag_type*>(0), forwarding_policy()), "_Boost::math::digamma<%1%>(%1%)");
}
