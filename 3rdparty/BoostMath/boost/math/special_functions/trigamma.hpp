...
template <class T, class Policy>
T trigamma_prec(T x, const _Boost::integral_constant<int, 53>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (offset + _Boost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 4)
   {
      T y = 1 / x;
...
template <class T, class Policy>
T trigamma_prec(T x, const _Boost::integral_constant<int, 64>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (offset_1_2 + _Boost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 8)
   {
      T y = 1 / x;
...
template <class T, class Policy>
T trigamma_prec(T x, const _Boost::integral_constant<int, 113>*, const Policy&)
{
   ...
   if(x <= 2)
   {
      return (2 + _Boost::math::tools::evaluate_polynomial(P_1_2, x) / tools::evaluate_polynomial(Q_1_2, x)) / (x * x);
   }
   else if(x <= 4)
   {
      return (y_offset_2_4 + _Boost::math::tools::evaluate_polynomial(P_2_4, x) / tools::evaluate_polynomial(Q_2_4, x)) / (x * x);
   }
   else if(x <= 8)
   {
      T y = 1 / x;
...
T trigamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x <= 0)
   {
      ...
      if(floor(x) == x)
      {
         return policies::raise_pole_error<T>("_Boost::math::trigamma<%1%>(%1%)", 0, (1-x), pol);
      }
      T s = fabs(x) < fabs(z) ? _Boost::math::sin_pi(x, pol) : _Boost::math::sin_pi(z, pol);
      return -trigamma_imp(z, t, pol) + _Boost::math::pow<2>(constants::pi<T>()) / (s * s);
   }
   if(x < 1)
   {
      result = 1 / (x * x);
...
template <class T, class Policy>
T trigamma_imp(T x, const _Boost::integral_constant<int, 0>*, const Policy& pol)
{
...
struct trigamma_initializer
{
   struct init
   {
      ...
      {
         _Boost::math::trigamma(T(2.5), Policy());
      }
      void do_init(const _Boost::false_type&){}
   ...
   trigamma(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::trigamma_imp(
      ...
      static_cast<const tag_type*>(0), forwarding_policy()), "_Boost::math::trigamma<%1%>(%1%)");
}
