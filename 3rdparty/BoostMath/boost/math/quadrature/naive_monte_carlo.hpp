...
namespace _Boost { namespace math { namespace quadrature {
...
template<class Real, class F, class RandomNumberGenerator = std::mt19937_64, class Policy = _Boost::math::policies::policy<>>
...
public:
                      ...
                      uint64_t seed = 0): m_num_threads{threads}, m_seed{seed}
    {
        ...
        m_volume = 1;
        static const char* function = "_Boost::math::quadrature::naive_monte_carlo<%1%>";
        for (uint64_t i = 0; i < n; ++i)
        {
            if (bounds[i].second <= bounds[i].first)
            {
                _Boost::math::policies::raise_domain_error(function, "The upper bound is <= the lower bound.\n", bounds[i].second, Policy());
                return;
            }
        ...
        m_thread_calls.reset(new _Boost::atomic<uint64_t>[threads]);
        m_thread_Ss.reset(new _Boost::atomic<Real>[threads]);
        m_thread_averages.reset(new _Boost::atomic<Real>[threads]);
   ...
   Real m_integrate()
   {
      ...
      do{
         ...
         do {
            ...
            for (uint64_t i = 0; i < m_num_threads; ++i)
            {
               uint64_t t_calls = m_thread_calls[i].load(_Boost::memory_order::consume);
               total_calls += t_calls;
            }
            Real variance = 0;
            Real avg = 0;
            for (uint64_t i = 0; i < m_num_threads; ++i)
            {
               ...
               avg += m_thread_averages[i].load(_Boost::memory_order::relaxed)*((Real)t_calls / (Real)total_calls);
               variance += m_thread_Ss[i].load(_Boost::memory_order::relaxed);
            }
            m_avg.store(avg, _Boost::memory_order::release);
            m_variance.store(variance / (total_calls - 1), _Boost::memory_order::release);
         ...
         } while (m_total_calls < 2048 || this->current_error_estimate() > m_error_goal.load(_Boost::memory_order::consume));
      ...
      return m_avg.load(_Boost::memory_order::consume);
    }
    ...
    void m_thread_monte(uint64_t thread_index, uint64_t seed)
    {
        ...
        try
        {
            ...
            Real M1 = m_thread_averages[thread_index].load(_Boost::memory_order::consume);
            Real S = m_thread_Ss[thread_index].load(_Boost::memory_order::consume);
            ...
            Real compensator = 0;
            uint64_t k = m_thread_calls[thread_index].load(_Boost::memory_order::consume);
            while (!m_done) // relaxed load
            {
                ...
                while (j++ < magic_calls_before_update)
                {
                    ...
                    if (!isfinite(f))
                    {
                        ...
                        static const char* function = "_Boost::math::quadrature::naive_monte_carlo<%1%>";
                        _Boost::math::policies::raise_domain_error(function, os.str().c_str(), /*this is a dummy arg to make it compile*/ 7.2, Policy());
                    }
                    ++k;
                    Real term = (f - M1)/k;
                ...
                m_thread_averages[thread_index].store(M1, _Boost::memory_order::release);
                m_thread_Ss[thread_index].store(S, _Boost::memory_order::release);
                m_thread_calls[thread_index].store(k, _Boost::memory_order::release);
            }
        }
        catch (...)
        {
    ...
    uint64_t m_seed;
    _Boost::atomic<Real> m_error_goal;
    _Boost::atomic<bool> m_done;
    std::vector<Real> m_lbs;
    std::vector<Real> m_dxs;
    std::vector<detail::limit_classification> m_limit_types;
    Real m_volume;
    _Boost::atomic<uint64_t> m_total_calls;
    // I wanted these to be vectors rather than maps,
    // but you can't resize a vector of atomics.
    std::unique_ptr<_Boost::atomic<uint64_t>[]> m_thread_calls;
    _Boost::atomic<Real> m_variance;
    std::unique_ptr<_Boost::atomic<Real>[]> m_thread_Ss;
    _Boost::atomic<Real> m_avg;
    std::unique_ptr<_Boost::atomic<Real>[]> m_thread_averages;
