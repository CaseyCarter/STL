...
public:
    ...
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_engine,
        UIntType, value)
    {
        minstd_rand0 intgen(static_cast<_Boost::uint32_t>(value));
    ...
    void discard(_Boost::uintmax_t z)
    {
        for(_Boost::uintmax_t j = 0; j < z; ++j) {
            (*this)();
        }
    }
...
public:
    ...
    BOOST_STATIC_CONSTANT(_Boost::uint32_t, default_seed = 331u);
     ...
     */
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_01_engine, _Boost::uint32_t, value)
    {
...
template<class RealType, int w, unsigned int p, unsigned int q>
const _Boost::uint32_t lagged_fibonacci_01_engine<RealType,w,p,q>::default_seed;
#endif
...
public:
    lagged_fibonacci_01() {}
    BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_01, _Boost::uint32_t, val)
    { this->seed(val); }
...
/** @copydoc _Boost::random::detail::lagged_fibonacci_doc */
