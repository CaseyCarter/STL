 ...
 *
 * Much of the code here taken by _Boost::math::hyperexponential_distribution.
...
namespace _Boost { namespace random {
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename _Boost::enable_if_c<_Boost::has_pre_increment<RateIterT>::value>::type* = 0)
    ...
    : dd_(std::vector<RealT>(_Boost::size(rate_range), 1)),
      rates_(_Boost::begin(rate_range), _Boost::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return _Boost::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    {
        dd_.param(typename _Boost::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    private: _Boost::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace _Boost::random
