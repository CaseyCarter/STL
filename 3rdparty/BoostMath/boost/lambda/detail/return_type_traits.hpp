...
namespace detail {
  ...
  class protect_conversion {
      typedef typename _Boost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
      _Boost::is_reference<T>::value && !_Boost::is_const<non_ref_T>::value,
...
// experimental feature
  ...
  typedef typename _Boost::remove_reference<A>::type non_ref_A;
  typedef typename _Boost::remove_reference<B>::type non_ref_B;
