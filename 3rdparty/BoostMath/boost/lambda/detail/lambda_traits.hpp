...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename _Boost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    _Boost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<class T> struct remove_reference_and_cv {
   typedef typename _Boost::remove_cv<
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     _Boost::tuples::access_traits<
       typename _Boost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<_Boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const _Boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile _Boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile _Boost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<_Boost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = _Boost::tuples::length<args_t>::value);
