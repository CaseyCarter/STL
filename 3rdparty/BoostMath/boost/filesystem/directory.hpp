...
{
public:
  typedef _Boost::filesystem::path::value_type value_type;   // enables class path ctor taking directory_entry

  directory_entry() BOOST_NOEXCEPT {}
  explicit directory_entry(const _Boost::filesystem::path& p) :
    m_path(p), m_status(file_status()), m_symlink_status(file_status())
  {
  }
  directory_entry(const _Boost::filesystem::path& p,
  ...
  void assign(const _Boost::filesystem::path& p,
  ...
  void replace_filename(const _Boost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const _Boost::filesystem::path& p, file_status st, file_status symlink_st)
  {
    replace_filename(p, st, symlink_st);
  }
# endif

  const _Boost::filesystem::path& path() const BOOST_NOEXCEPT { return m_path; }
  operator const _Boost::filesystem::path&() const BOOST_NOEXCEPT { return m_path; }
...
//  - a conversion to 'path' using 'operator const _Boost::filesystem::path&()',
...
struct dir_itr_imp :
  public _Boost::intrusive_ref_counter< dir_itr_imp >
{
...
class directory_iterator :
  public _Boost::iterator_facade<
    directory_iterator,
    directory_entry,
    _Boost::single_pass_traversal_tag
  >
{
  friend class _Boost::iterator_core_access;
...
private:
  ...
  _Boost::intrusive_ptr< detail::dir_itr_imp > m_imp;
};
...
//  namespace _Boost template specializations
template<typename C, typename Enabler>
struct range_mutable_iterator;

template<>
struct range_mutable_iterator<_Boost::filesystem::directory_iterator, void>
{
  typedef _Boost::filesystem::directory_iterator type;
};
...
template<>
struct range_const_iterator<_Boost::filesystem::directory_iterator, void>
{
  typedef _Boost::filesystem::directory_iterator type;
};
...
struct recur_dir_itr_imp :
  public _Boost::intrusive_ref_counter< recur_dir_itr_imp >
{
...
private:
  // intrusive_ptr provides the shallow-copy semantics required for single pass iterators
  // (i.e. InputIterators). The end iterator is indicated by is_end().
  _Boost::intrusive_ptr< detail::recur_dir_itr_imp > m_imp;
};
...
//  namespace _Boost template specializations
template<>
struct range_mutable_iterator<_Boost::filesystem::recursive_directory_iterator, void>
{
  typedef _Boost::filesystem::recursive_directory_iterator type;
};
template<>
struct range_const_iterator<_Boost::filesystem::recursive_directory_iterator, void>
{
  typedef _Boost::filesystem::recursive_directory_iterator type;
};
