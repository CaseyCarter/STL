...
namespace path_detail // intentionally don't use filesystem::detail to not bring internal Boost.Filesystem functions into ADL via path_constants
{
  ...
  public:
    ...
    path(Source const& source,
      typename _Boost::enable_if<path_traits::is_pathable<
        typename _Boost::decay<Source>::type> >::type* =0)
    {
...
# endif
    ...
    template <class Source>
      typename _Boost::enable_if<path_traits::is_pathable<
        typename _Boost::decay<Source>::type>, path&>::type
    ...
    template <class CharT>
      typename _Boost::enable_if<_Boost::is_integral<CharT>, path&>::type
    operator+=(CharT c)
    {
      CharT tmp[2];
      tmp[0] = c;
...
# endif
  ...
  private:
    ...
    friend class _Boost::filesystem::path;
    friend class _Boost::filesystem::path::reverse_iterator;
...
# endif
  ...
  //    use _Boost::io::quoted() to handle spaces in paths
  ...
  {
    return os
      << _Boost::io::quoted(p.template string<std::basic_string<Char> >(), static_cast<Char>('&'));
  }
  ...
  operator>>(std::basic_istream<Char, Traits>& is, path& p)
  {
    ...
    is >> _Boost::io::quoted(str, static_cast<Char>('&'));
    p = str;
    return is;
  }
