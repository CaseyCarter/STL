...
BOOST_STATIC_ASSERT((AdaptiveSortInsertionSortThreshold&(AdaptiveSortInsertionSortThreshold-1)) == 0);
   ...
   typedef ::_Boost::uintptr_t uintptr_t;
#else
...
{
   if(begin != with){
      ::_Boost::adl_move_swap_ranges(begin, end, with);
      ::_Boost::adl_move_swap(*key_next, *key_range2);
      if(key_next == key_mid){
         key_mid = key_range2;
      }
   ...
   (RandIt first1, RandIt last1, RandIt const last2, bool *const pis_range1_A, Compare comp)
{
   ...
   if(first1 != last1 && comp(*last1, last1[-1])){
      do{
         ...
         last1  = _Boost::movelib::lower_bound(last1, last2, *first1, comp);
   ...
   , Compare comp)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted_and_unique(key_first, key_first + key_count, key_comp));
   ...
   {  //Selection sort blocks
      ...
      for (RandIt f = first+l_irreg1; n_block_left; --n_block_left, ++key_range2, f += l_block, min_check -= min_check != 0, max_check -= max_check != 0) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(f, f+l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first_min, first_min + l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT((f == (first+l_irreg1)) || !comp(*f, *(f-l_block)));
      }
   }
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first+l_irreg1+n_bef_irreg2*l_block, first_irr2, comp));
   ...
   for(RandItKeys key_next = key_first; key_next != key_end; ++key_next){
      ...
      last1 += l_block;
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, first1, comp));
   }

   merge_bufferless(is_range1_A ? first1 : last1, first_irr2, last_irr2, comp);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, last_irr2, comp));
}
   ...
   if(max_collected){
      ...
      if(xbuf.capacity() >= max_collected){
         ...
         while(u != last && h < max_collected){
            typename XBuf::iterator const r = _Boost::movelib::lower_bound(ph0, xbuf.end(), *u, comp);
            //If key not found add it to [h, h+h0)
            if(r == xbuf.end() || comp(*u, *r) ){
               RandIt const new_h0 = _Boost::move(search_end, u, h0);
               search_end = u;
               ++search_end;
               ++h;
               xbuf.insert(r, u);
         ...
         _Boost::move_backward(first, h0, h0+h);
         _Boost::move(xbuf.data(), xbuf.end(), first);
      }
      else{
         while(u != last && h < max_collected){
            RandIt const r = _Boost::movelib::lower_bound(h0, search_end, *u, comp);
            //If key not found add it to [h, h+h0)
...
{
   _Boost::movelib::inplace_stable_sort(first, last, comp);
}
      ...
      , XBuf &xbuf)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, last, _Boost::movelib::unantistable(comp)));
}
                    ...
                    , XBuf & xbuf)
{
   ...
   BOOST_ASSERT(_Boost::movelib::is_sorted_and_unique(first, last, comp));
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left, ++key_first, min_check -= min_check != 0, max_check -= max_check != 0){
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first_reg, last_reg, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!next_key_idx || _Boost::movelib::is_sorted(first_min, last_min, comp));
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(orig_dest, dest, comp));

      if(first_reg == dest){
         dest = next_key_idx ? ::_Boost::adl_move_swap_ranges(first_min, last_min, first_reg)
                             : last_reg;
      }
      else{
...
//   BOOST_ASSERT(n_block_a || n_block_b);
   ...
   for (; n_block_left; --n_block_left, ++key_range2, min_check -= min_check != 0, max_check -= max_check != 0) {
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first1, last1, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || _Boost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         (void)unmerged;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first-l_block, unmerged, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first-l_block, buffer, comp));
   ...
   , RandItBuf const buf_first)
{
   ...
   for (; n_block_left; --n_block_left, ++key_range2, min_check -= min_check != 0, max_check -= max_check != 0) {
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(buffer_empty ? _Boost::movelib::is_sorted(first1, last1, comp) : _Boost::movelib::is_sorted(buffer, buffer_end, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || _Boost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first2, last2, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first_min, last_min, comp));
         first1 = first2;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, first1, comp));
      }
      else {
         ...
         first1 = unmerged;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, unmerged, comp));
      }
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(first, res, comp));
   BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   merge_blocks_w_fwd: ", len);
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(dest, last_irr2, comp));
      ...
      , Op op)
{
   ...
   for(; l_merged < l_build_buf && l_left_space >= l_merged; l_merged*=2){
      ...
      while((elements_in_blocks - p0) > 2*l_merged) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(pos-l_merged, pos+l_merged, comp));
         p0 += 2*l_merged;
         pos = first_block+p0;
      }
      if((elements_in_blocks-p0) > l_merged) {
         op_merge_left(pos-l_merged, pos, pos+l_merged, first_block+elements_in_blocks, comp, op);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(pos-l_merged, pos-l_merged+(first_block+elements_in_blocks-pos), comp));
      }
      else {
         op(forward_t(), pos, first_block+elements_in_blocks, pos-l_merged);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(_Boost::movelib::is_sorted(pos-l_merged, first_block+elements_in_blocks-l_merged, comp));
      }
