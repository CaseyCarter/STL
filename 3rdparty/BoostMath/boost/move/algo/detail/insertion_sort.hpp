...
{
   typedef typename _Boost::movelib::iterator_traits<BirdirectionalIterator>::value_type value_type;
   if (first != last){
      BirdirectionalIterator i = first;
      for (++i; i != last; ++i){
         BirdirectionalIterator j = i;
         if (comp(*i,  *--j)) {
            value_type tmp(::_Boost::move(*i));
            *i = ::_Boost::move(*j);
   ...
   , Compare comp)
{
   ...
   if (first1 != last1){
      ...
      ::new((iterator_to_raw_pointer)(last2), boost_move_new_t()) value_type(::_Boost::move(*first1));
      ...
      for (++last2; ++first1 != last1; ++last2){
         ...
         if (comp(*first1, *--k2)){
            ::new((iterator_to_raw_pointer)(j2), boost_move_new_t()) value_type(::_Boost::move(*k2));
            d.incr();
            for (--j2; k2 != first2 && comp(*first1, *--k2); --j2)
               *j2 = ::_Boost::move(*k2);
            *j2 = ::_Boost::move(*first1);
         }
         else{
            ::new((iterator_to_raw_pointer)(j2), boost_move_new_t()) value_type(::_Boost::move(*first1));
            d.incr();
         }
      }
      d.release();
