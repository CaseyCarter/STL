...
class adaptive_xbuf
{
   ...
   {
      if(n <= m_size){
         _Boost::move(first, first+n, m_ptr);
         ...
         RandRawIt result = _Boost::move(first, first+m_size, m_ptr);
         _Boost::uninitialized_move(first+m_size, first+n, result);
         m_size = n;
      }
   }
   ...
   void push_back(RandIt first, size_type n)
   {
      ...
      _Boost::uninitialized_move(first, first+n, m_ptr+m_size);
      m_size += n;
   }
   ...
   void insert(iterator pos, RandIt it)
   {
      if(pos == (m_ptr + m_size)){
         ...
         //m_size updated
         _Boost::move_backward(pos, m_ptr+m_size-2, m_ptr+m_size-1);
         *pos = _Boost::move(*it);
      }
   }
   ...
   void initialize_until(size_type const size, T &t)
   {
      ...
      if(m_size < size){
         BOOST_TRY
         {
            ::new((void*)&m_ptr[m_size]) T(::_Boost::move(t));
            ++m_size;
            for(; m_size != size; ++m_size){
               ::new((void*)&m_ptr[m_size]) T(::_Boost::move(m_ptr[m_size-1]));
            }
            t = ::_Boost::move(m_ptr[m_size-1]);
         }
         BOOST_CATCH(...)
         {
            while(m_size)
...
RandIt rotate_gcd(RandIt first, RandIt middle, RandIt last)
{
   ...
   if (middle == ret){
      _Boost::adl_move_swap_ranges(first, middle, middle);
   }
   else{
...
void merge_bufferless_ON2(RandIt first, RandIt middle, RandIt last, Compare comp)
{
   if((middle - first) < (last - middle)){
      while(first != middle){
         ...
         middle = _Boost::movelib::lower_bound(middle, last, *first, comp);
      ...
      while(middle != last){
         RandIt p = _Boost::movelib::upper_bound(first, middle, last[-1], comp);
   ...
   , Compare comp)
{
   ...
   while(1) {
      ...
      if (len1 > len2) {
         len11 = len1 / 2;
         first_cut +=  len11;
         second_cut = _Boost::movelib::lower_bound(middle, last, *first_cut, comp);
         len22 = size_type(second_cut - middle);
      }
      else {
         len22 = len2 / 2;
         second_cut += len22;
         first_cut = _Boost::movelib::upper_bound(first, middle, *second_cut, comp);
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new((iterator_to_raw_pointer)(dest_first)) value_type(::_Boost::move(*first));
            d.incr();
         }
         d.release();
         InputOutIterator end = ::_Boost::move(first, last, original_r_first);
         BOOST_ASSERT(end == r_last);
         (void)end;
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new((iterator_to_raw_pointer)(dest_first)) value_type(::_Boost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
   ...
   , Compare comp)
{
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new(&*dest_first) value_type(::_Boost::move(*first));
            d.incr();
         }
         d.release();
         BidirOutIterator end = ::_Boost::move(first, last, original_r_first);
         BOOST_ASSERT(end == r_last);
         (void)end;
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new(&*dest_first) value_type(::_Boost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
      ...
      typename iterator_traits<BidirectionalIterator1>::size_type buffer_size)
{
   if (len1 > len2 && len2 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = _Boost::move(middle, last, buffer);
         _Boost::move_backward(first, middle, last);
         return _Boost::move(buffer, buffer_end, first);
      }
      else
         return first;
   }
   else if (len1 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = _Boost::move(first, middle, buffer);
         BidirectionalIterator1 ret = _Boost::move(middle, last, first);
         _Boost::move(buffer, buffer_end, ret);
         return ret;
      }
      else
         return last;
                           ...
                           typename iterator_traits<BidirectionalIterator>::size_type uninitialized_len)
{
   ...
   if(uninitialized_len)
   {
      ...
      ::_Boost::movelib::adaptive_xbuf<value_type, RandRawIt> xbuf(uninitialized, uninitialized_len);
