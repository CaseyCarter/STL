...
_Boost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( _Boost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        _Boost::exception const * be=current_exception_cast<_Boost::exception const>();
        ...
        if( be || se )
            ...
            return "Dynamic exception type: "+_Boost::core::demangle(p->name());
#endif
        else
...
_Boost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( _Boost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<_Boost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) BOOST_NOEXCEPT_OR_NOTHROW
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce _Boost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
