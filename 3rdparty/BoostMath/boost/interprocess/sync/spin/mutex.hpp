...
class spin_mutex
{
   ...
   void lock();
   bool try_lock();
   bool timed_lock(const _Boost::posix_time::ptime &abs_time);
   void unlock();
   void take_ownership(){}
   private:
   volatile _Boost::uint32_t m_s;
};
...
inline void spin_mutex::lock(void)
{
	...
	_Boost::posix_time::ptime wait_time
		= microsec_clock::universal_time()
		+ _Boost::posix_time::milliseconds(BOOST_INTERPROCESS_TIMEOUT_WHEN_LOCKING_DURATION_MS);
...
{
   _Boost::uint32_t prev_s = ipcdetail::atomic_cas32(const_cast<_Boost::uint32_t*>(&m_s), 1, 0);
   return m_s == 1 && prev_s == 0;
}

inline bool spin_mutex::timed_lock(const _Boost::posix_time::ptime &abs_time)
{  return ipcdetail::try_based_timed_lock(*this, abs_time); }

inline void spin_mutex::unlock(void)
{  ipcdetail::atomic_cas32(const_cast<_Boost::uint32_t*>(&m_s), 0, 1);   }
