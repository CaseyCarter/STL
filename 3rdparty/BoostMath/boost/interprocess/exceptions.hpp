      ...
      :  m_err(err_info)
   {
      try{
         if(m_err.get_native_error() != 0){
            ...
            m_str = "_Boost::interprocess_exception::library_error";
         }
      }
      catch(...){}
   }
...
class BOOST_SYMBOL_VISIBLE lock_exception : public interprocess_exception
{
   ...
   virtual const char* what() const BOOST_NOEXCEPT_OR_NOTHROW
   {  return "_Boost::interprocess::lock_exception";  }
};
...
{
 public:
   bad_alloc() : interprocess_exception("::_Boost::interprocess::bad_alloc"){}
   virtual const char* what() const BOOST_NOEXCEPT_OR_NOTHROW
      {  return "_Boost::interprocess::bad_alloc";  }
};
