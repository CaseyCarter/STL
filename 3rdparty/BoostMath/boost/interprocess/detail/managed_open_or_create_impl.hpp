   ...
   : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(_Boost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::_Boost::container::dtl::alignment_of
                  < ::_Boost::container::dtl::max_align_t >::value)
            >::value;
   ...
   {
      managed_open_or_create_impl tmp(_Boost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            _Boost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<_Boost::uint32_t*>(region.get_address());
            _Boost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         _Boost::uint32_t *patomic_word = static_cast<_Boost::uint32_t*>(region.get_address());
         _Boost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = _Boost::move(dev);
      }
   }
