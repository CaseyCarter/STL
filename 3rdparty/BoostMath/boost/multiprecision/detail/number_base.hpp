...
namespace detail {
//
// Workaround for missing abs(_Boost::long_long_type) and abs(__int128) on some compilers:
//
template <class T>
...
#define BOOST_MP_USING_ABS using _Boost::multiprecision::detail::abs;
...
struct canonical_imp<Val, Backend, mpl::int_<0> >
{
   ...
   typedef typename mpl::eval_if<_Boost::is_same<iter_type, end_type>, mpl::identity<Val>, mpl::deref<iter_type> >::type type;
};
template <class Val, class Backend>
...
template <class T>
struct expression_storage : public expression_storage_base<T, _Boost::is_arithmetic<T>::value>
{};
...
struct expression<tag, Arg1, void, void, void>
{
   ...
   // an arithmetic type:
   //
   template <class T, typename _Boost::enable_if<is_same<T, bool>, int>::type = 0>
   explicit operator T() const
   {
      result_type r(*this);
      return static_cast<bool>(r);
   }
   template <class T, typename _Boost::disable_if_c<is_same<T, bool>::value || is_void<T>::value || is_number<T>::value, int>::type = 0>
   explicit operator T() const
   {
...
#ifndef __SUNPRO_CC
             ,
             typename _Boost::disable_if_c<is_number<T>::value || is_constructible<T const&, result_type>::value || !is_constructible<T, result_type>::value, int>::type = 0
#endif
             >
...
template <class S>
void format_float_string(S& str, _Boost::intmax_t my_exp, _Boost::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else if (!fixed || (my_exp >= 0))
   {
      ...
      //
      _Boost::intmax_t chars = str.size();
      chars                 = digits - chars;
      if (scientific)
         ++chars;
      if (chars > 0)
   ...
   {
      if (1 + my_exp > static_cast<_Boost::intmax_t>(str.size()))
      {
      ...
      else if (my_exp + 1 < static_cast<_Boost::intmax_t>(str.size()))
      {
         if (my_exp < 0)
         {
   ...
   else
   {
      ...
      S e = _Boost::lexical_cast<S>(abs(my_exp));
...
{};
template <class Num>
struct number_category : public number_category_base<Num, _Boost::is_class<Num>::value || _Boost::is_arithmetic<Num>::value, _Boost::is_abstract<Num>::value>
{};
template <class Backend, expression_template_option ExpressionTemplates>
...
template <>
struct number_category<_Boost::int128_type> : public mpl::int_<number_kind_integer>
{};
template <>
struct number_category<_Boost::uint128_type> : public mpl::int_<number_kind_integer>
{};
#endif
...
namespace _Boost { namespace math {
   namespace tools {
      ...
      template <class tag, class A1, class A2, class A3, class A4>
      struct promote_arg<_Boost::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
      {
         typedef typename _Boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type type;
      };

      template <class R, class B, _Boost::multiprecision::expression_template_option ET>
      inline R real_cast(const _Boost::multiprecision::number<B, ET>& val)
      {
         return val.template convert_to<R>();
      }

      template <class R, class tag, class A1, class A2, class A3, class A4>
      inline R real_cast(const _Boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
      {
         typedef typename _Boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type val_type;
         return val_type(val).template convert_to<R>();
      }

      template <class B, _Boost::multiprecision::expression_template_option ET>
      struct is_complex_type<_Boost::multiprecision::number<B, ET> > : public _Boost::mpl::bool_<_Boost::multiprecision::number_category<B>::value == _Boost::multiprecision::number_kind_complex> {};
...
template <class B, _Boost::multiprecision::expression_template_option ET>
struct is_explicitly_convertible_from_string<_Boost::multiprecision::number<B, ET> >
{
