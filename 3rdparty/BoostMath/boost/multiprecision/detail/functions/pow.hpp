...
inline void pow_imp(T& result, const T& t, const U& p, const mpl::false_&)
{
   ...
   typedef typename _Boost::multiprecision::detail::canonical<U, T>::type int_type;
...
{
   detail::pow_imp(result, t, p, _Boost::is_signed<U>());
}
...
void hyp0F0(T& H0F0, const T& x)
{
   ...
   BOOST_ASSERT(&H0F0 != &x);
   long tol = _Boost::multiprecision::detail::digits2<number<T, et_on> >::value();
   T    t;
   ...
   const unsigned series_limit =
       _Boost::multiprecision::detail::digits2<number<T, et_on> >::value() < 100
           ? 100
           : _Boost::multiprecision::detail::digits2<number<T, et_on> >::value();
...
void hyp1F0(T& H1F0, const T& a, const T& x)
{
   ...
   typedef typename _Boost::multiprecision::detail::canonical<int, T>::type si_type;
   ...
   T lim;
   eval_ldexp(lim, H1F0, 1 - _Boost::multiprecision::detail::digits2<number<T, et_on> >::value());
   if (eval_get_sign(lim) < 0)
      lim.negate();
...
void eval_exp(T& result, const T& x)
{
   ...
   typedef typename _Boost::multiprecision::detail::canonical<exp_type, T>::type canonical_exp_type;
   ...
   if (xx.compare(si_type(1)) <= 0)
   {
      ...
      {
         result = ui_type(1);
         eval_ldexp(lim, result, 1 - _Boost::multiprecision::detail::digits2<number<T, et_on> >::value());
      }
      unsigned k = 2;
      exp_series = xx;
      result     = si_type(1);
   ...
   typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type ll;
...
const T& get_constant_log10()
{
   ...
   if (!b || (digits != _Boost::multiprecision::detail::digits2<number<T> >::value()))
   {
      b = true;
#else
   if ((digits != _Boost::multiprecision::detail::digits2<number<T> >::value()))
   {
#endif
      typedef typename _Boost::multiprecision::detail::canonical<unsigned, T>::type ui_type;
      T                                                                            ten;
      ten = ui_type(10u);
      eval_log(result, ten);
      digits = _Boost::multiprecision::detail::digits2<number<T> >::value();
   }
...
inline void eval_pow(T& result, const T& x, const T& a)
{
   ...
   case FP_ZERO:
      ...
      case FP_NORMAL:
      {
         ...
         try
         {
            typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type i;
            eval_convert_to(&i, a);
            if (a.compare(i) == 0)
            {
               if (eval_signbit(a))
   ...
   typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type an;
   typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type max_an =
       std::numeric_limits<typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type>::is_specialized ? (std::numeric_limits<typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type>::max)() : static_cast<typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type>(1) << (sizeof(typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type) * CHAR_BIT - 2);
   typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type min_an =
       std::numeric_limits<typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type>::is_specialized ? (std::numeric_limits<typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, T>::type>::min)() : -min_an;
   ...
   catch (const std::exception&)
   {
      ...
      an = (std::numeric_limits<_Boost::intmax_t>::max)();
   }
#endif
   if ((eval_get_sign(x) < 0))
   {
      typename _Boost::multiprecision::detail::canonical<_Boost::uintmax_t, T>::type aun;
#ifndef BOOST_NO_EXCEPTIONS
      try
      {
#endif
...
void eval_exp2(T& result, const T& arg)
{
   ...
   // Check for pure-integer arguments which can be either signed or unsigned.
   typename _Boost::multiprecision::detail::canonical<typename T::exponent_type, T>::type i;
   T                                                                                     temp;
   try
   {
      eval_trunc(temp, arg);
   ...
   catch (const _Boost::math::rounding_error&)
   { /* Fallthrough */
   }
...
void small_sinh_series(T x, T& result)
{
   ...
   T lim(x);
   eval_ldexp(lim, lim, 1 - _Boost::multiprecision::detail::digits2<number<T, et_on> >::value());
