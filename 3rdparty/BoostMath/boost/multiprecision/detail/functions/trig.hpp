...
{
   typedef typename _Boost::multiprecision::detail::canonical<_Boost::int32_t, T>::type  si_type;
   typedef typename _Boost::multiprecision::detail::canonical<_Boost::uint32_t, T>::type ui_type;
   ...
   T tol;
   tol = ui_type(1);
   eval_ldexp(tol, tol, 1 - _Boost::multiprecision::detail::digits2<number<T, et_on> >::value());
...
void eval_sin(T& result, const T& x)
{
   ...
   else
   {
      ...
      for (_Boost::int32_t k = static_cast<_Boost::int32_t>(0); k < n_scale; k++)
      {
...
void eval_asin(T& result, const T& x)
{
   ...
   typedef typename _Boost::multiprecision::detail::canonical<long double, T>::type guess_type;
#else
   ...
   _Boost::intmax_t current_precision = eval_ilogb(result);
   _Boost::intmax_t target_precision  = current_precision - 1 - (std::numeric_limits<number<T> >::digits * 2) / 3;
