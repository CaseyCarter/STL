...
template <class To, class B, ::_Boost::multiprecision::expression_template_option et>
...
#pragma warning(disable : 4127)
   ...
   static const int shift = std::numeric_limits<_Boost::intmax_t>::digits - 1;
   ...
   while (!eval_is_zero(f))
   {
      ...
      typename _Boost::multiprecision::detail::canonical<_Boost::intmax_t, To>::type ll;
...
generic_convert_rational_to_float_imp(To& result, Integer& num, Integer& denom, const mpl::false_&)
{
   ...
   if (shift > 0)
      ...
      denom <<= _Boost::multiprecision::detail::unsigned_abs(shift);
   Integer q, r;
...
void generic_interconvert_float2rational(To& to, const From& from, const mpl::int_<2>& /*radix*/)
{
   ...
   static const int                                               shift = std::numeric_limits<_Boost::long_long_type>::digits;
   typename From::exponent_type                                   e;
   ...
   while (val)
   {
      val = ldexp(val, shift);
      e -= shift;
      _Boost::long_long_type ll = _Boost::math::lltrunc(val);
      val -= ll;
      num <<= shift;
      num += ll;
   }
...
void generic_interconvert_float2int(To& to, const From& from, const mpl::int_<2>& /*radix*/)
{
   ...
   static const exponent_type           shift = std::numeric_limits<_Boost::long_long_type>::digits;
   exponent_type                        e;
   number<To>                           num(0u);
   number<From>                         val(from);
   val      = frexp(val, &e);
...
void generic_interconvert(To& to, const From& from, const mpl::int_<number_kind_floating_point>& /*to_type*/, const mpl::int_<number_kind_complex>& /*from_type*/)
{
   ...
   generic_interconvert_complex_to_scalar(to, from, mpl::bool_<_Boost::is_same<component_backend, To>::value>(), mpl::bool_<_Boost::is_constructible<To, const component_backend&>::value>());
}
template <class To, class From>
