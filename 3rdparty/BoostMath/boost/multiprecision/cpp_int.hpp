...
template <unsigned MinBits = 0, unsigned MaxBits = 0, _Boost::multiprecision::cpp_integer_type SignType = signed_magnitude, cpp_int_check_type Checked = unchecked, class Allocator = typename mpl::if_c<MinBits && (MinBits == MaxBits), void, std::allocator<limb_type> >::type>
...
template <unsigned MinBits, unsigned MaxBits, _Boost::multiprecision::cpp_integer_type SignType, cpp_int_check_type Checked, class Allocator>
struct is_byte_container<backends::cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> > : public _Boost::false_type
{};
...
struct cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>
    : private _Boost::empty_value<typename detail::rebind<limb_type, Allocator>::type>
{
 ...
 private:
   typedef _Boost::empty_value<allocator_type> base_type;
 ...
 private:
   union data_type
   {
      ...
      BOOST_CONSTEXPR data_type(signed_limb_type i) BOOST_NOEXCEPT : first(i < 0 ? static_cast<limb_type>(_Boost::multiprecision::detail::unsigned_abs(i)) : i) {}
#if BOOST_ENDIAN_LITTLE_BYTE
      BOOST_CONSTEXPR data_type(double_limb_type i) BOOST_NOEXCEPT : double_first(i)
      {}
      BOOST_CONSTEXPR data_type(signed_double_limb_type i) BOOST_NOEXCEPT : double_first(i < 0 ? static_cast<double_limb_type>(_Boost::multiprecision::detail::unsigned_abs(i)) : i) {}
#endif
 ...
 public:
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i) BOOST_NOEXCEPT
       : m_data(i),
         m_limbs(i < 0 ? (static_cast<double_limb_type>(_Boost::multiprecision::detail::unsigned_abs(i)) > static_cast<double_limb_type>(max_limb_value) ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
         m_sign(i < 0),
         m_internal(true),
         m_alias(false) {}
#endif
   ...
   //
   struct scoped_shared_storage : private _Boost::empty_value<allocator_type>
   {
    private:
      limb_type*      data;
      unsigned        capacity;
      unsigned        allocated;
      bool            is_alias;
      allocator_type& allocator() BOOST_NOEXCEPT { return _Boost::empty_value<allocator_type>::get(); }

    public:
      scoped_shared_storage(const allocator_type& a, unsigned len)
          : _Boost::empty_value<allocator_type>(_Boost::empty_init_t(), a), capacity(len), allocated(0), is_alias(false)
      {
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>
{
 ...
 public:
   ...
   {}
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i) BOOST_NOEXCEPT
       : m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(_Boost::multiprecision::detail::unsigned_abs(i)) : i)),
         m_limbs(i < 0 ? (static_cast<double_limb_type>(_Boost::multiprecision::detail::unsigned_abs(i)) > max_limb_value ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
         m_sign(i < 0) {}
#endif
#if defined(BOOST_MP_USER_DEFINED_LITERALS)
   template <limb_type... VALUES>
   ...
   {
      m_limbs = static_cast<_Boost::uint16_t>((std::min)(new_size, internal_limb_count));
...
#endif
//
// Traits classes to figure out a native type with N bits, these vary from _Boost::uint_t<N> only
// because some platforms have native integer types longer than _Boost::long_long_type, "really _Boost::long_long_type" anyone??
//
...
{
   typedef typename _Boost::uint_t<N>::least type;
};

template <unsigned N>
struct trivial_limb_type : public trivial_limb_type_imp<N, N <= sizeof(_Boost::long_long_type) * CHAR_BIT>
{};
//
...
protected:
 ...
 protected:
   template <class T>
   BOOST_MP_CXX14_CONSTEXPR typename _Boost::disable_if_c<!_Boost::is_integral<T>::value || (std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits))>::type
   check_in_range(T val, const mpl::int_<checked>&)
   {
      ...
      if (static_cast<common_type>(_Boost::multiprecision::detail::unsigned_abs(val)) > static_cast<common_type>(limb_mask))
 ...
 public:
   ...
   //
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(SI i, typename _Boost::enable_if_c<is_signed<SI>::value && (Checked == unchecked)>::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
       : m_data(i < 0 ? static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(_Boost::multiprecision::detail::unsigned_abs(i)) & limb_mask) : static_cast<local_limb_type>(i & limb_mask)), m_sign(i < 0) {}
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(SI i, typename _Boost::enable_if_c<is_signed<SI>::value && (Checked == checked)>::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
       : m_data(i < 0 ? (static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(_Boost::multiprecision::detail::unsigned_abs(i)) & limb_mask)) : static_cast<local_limb_type>(i & limb_mask)), m_sign(i < 0)
   {
      check_in_range(i);
   }
   template <class UI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(UI i, typename _Boost::enable_if_c<is_unsigned<UI>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT
       : m_data(static_cast<local_limb_type>(i) & limb_mask),
         m_sign(false) {}
   template <class UI>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(UI i, typename _Boost::enable_if_c<is_unsigned<UI>::value && (Checked == checked)>::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<UI>())))
       : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) { check_in_range(i); }
   template <class F>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(F i, typename _Boost::enable_if_c<is_floating_point<F>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT
       : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask),
         m_sign(i < 0) {}
   template <class F>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(F i, typename _Boost::enable_if_c<is_floating_point<F>::value && (Checked == checked)>::type const* = 0)
...
private:
 ...
 protected:
   template <class T>
   BOOST_MP_CXX14_CONSTEXPR typename _Boost::disable_if_c<std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits)>::type
   check_in_range(T val, const mpl::int_<checked>&, const _Boost::false_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
   ...
   template <class T>
   BOOST_MP_CXX14_CONSTEXPR void check_in_range(T val, const mpl::int_<checked>&, const _Boost::true_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
   ...
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void check_in_range(T, const mpl::int_<C>&, const _Boost::integral_constant<bool, B>&) BOOST_NOEXCEPT {}
 ...
 public:
   ...
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(SI i, typename _Boost::enable_if_c<is_signed<SI>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT
...
#endif
   template <class F>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(F i, typename _Boost::enable_if<is_floating_point<F> >::type const* = 0) BOOST_MP_NOEXCEPT_IF((Checked == unchecked))
       : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask)
   {
      check_in_range(i);
      if (i < 0)
          ...
          is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> >::value>
{
 ...
 public:
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend& o) BOOST_MP_NOEXCEPT_IF(_Boost::is_void<Allocator>::value) : base_type(o) {}
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
   ...
   //
   template <class Arg>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(Arg i, typename _Boost::enable_if_c<is_allowed_cpp_int_base_conversion<Arg, base_type>::value>::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(base_type(std::declval<Arg>())))
       : base_type(i) {}
   //
 ...
 private:
   ...
   BOOST_MP_CXX14_CONSTEXPR void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::false_ const&, mpl::true_ const&)
   {
      ...
      *this = static_cast<
          typename _Boost::multiprecision::detail::canonical<
 ...
 public:
   ...
   BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(
       ...
       typename _Boost::enable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
       : base_type()
   {
      do_assign(
          other,
   ...
   explicit BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(
       const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other,
       typename _Boost::disable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
       : base_type()
   {
      do_assign(
          other,
 ...
 private:
   template <class A>
   BOOST_MP_CXX14_CONSTEXPR typename _Boost::enable_if<is_unsigned<A> >::type do_assign_arithmetic(A val, const mpl::true_&)
   ...
   template <class A>
   BOOST_MP_CXX14_CONSTEXPR typename _Boost::disable_if_c<is_unsigned<A>::value || !is_integral<A>::value>::type do_assign_arithmetic(A val, const mpl::true_&)
   ...
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(_Boost::multiprecision::detail::unsigned_abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   template <class A>
   BOOST_MP_CXX14_CONSTEXPR typename _Boost::enable_if_c<!is_integral<A>::value>::type do_assign_arithmetic(A val, const mpl::true_&)
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(_Boost::multiprecision::detail::abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   ...
   {
      this->resize(1, 1);
      *this->limbs() = static_cast<limb_type>(_Boost::multiprecision::detail::unsigned_abs(i));
      this->sign(i < 0);
   }
   ...
   BOOST_MP_CXX14_CONSTEXPR void do_assign_arithmetic(signed_double_limb_type i, const mpl::false_&) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      ...
      if (i < 0)
         s = true;
      double_limb_type                 ui = static_cast<double_limb_type>(_Boost::multiprecision::detail::unsigned_abs(i));
      typename base_type::limb_pointer p  = this->limbs();
#ifdef __MSVC_RUNTIME_CHECKS
   ...
   BOOST_MP_CXX14_CONSTEXPR void do_assign_arithmetic(long double a, const mpl::false_&)
   {
      ...
      if ((_Boost::math::isinf)(a) || (_Boost::math::isnan)(a))
      {
 ...
 public:
   ...
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR typename _Boost::enable_if_c<!_Boost::multiprecision::detail::is_byte_container<Arithmetic>::value, cpp_int_backend&>::type operator=(Arithmetic val) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().do_assign_arithmetic(std::declval<Arithmetic>(), trivial_tag())))
   {
 ...
 private:
   ...
   std::string do_get_string(std::ios_base::fmtflags f, const mpl::false_&) const
   {
      ...
      else
      {
         ...
         if (this->size() == 1)
         {
            result = _Boost::lexical_cast<std::string>(t.limbs()[0]);
         }
         else
         {
 ...
 public:
   template <class Container>
   BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(const Container& c, typename _Boost::enable_if_c<_Boost::multiprecision::detail::is_byte_container<Container>::value>::type const* = 0)
   {
      //
      // We assume that c is a sequence of (unsigned) bytes with the most significant byte first:
      //
   ...
   template <class Arithmetic>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR typename _Boost::enable_if<is_arithmetic<Arithmetic>, int>::type compare(Arithmetic i) const
   {
      // braindead version:
      cpp_int_backend t;
      t = i;
...
using _Boost::multiprecision::backends::cpp_int_backend;
