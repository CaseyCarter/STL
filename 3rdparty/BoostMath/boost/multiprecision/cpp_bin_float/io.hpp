...
namespace _Boost { namespace multiprecision {
...
//
template <class I>
inline I restricted_multiply(cpp_int& result, const cpp_int& a, const cpp_int& b, I max_bits, _Boost::int64_t& error)
{
   result   = a * b;
   I gb     = msb(result);
   I rshift = 0;
...
//
template <class I>
inline I restricted_pow(cpp_int& result, const cpp_int& a, I e, I max_bits, _Boost::int64_t& error)
{
...
inline int get_round_mode(const cpp_int& what, _Boost::int64_t location, _Boost::int64_t error)
{
   //
   ...
   _Boost::int64_t error_radius = error & 1 ? (1 + error) / 2 : error / 2;
...
inline int get_round_mode(cpp_int& r, cpp_int& d, _Boost::int64_t error, const cpp_int& q)
{
   //
...
{
   cpp_int                      n;
   _Boost::intmax_t              decimal_exp     = 0;
   _Boost::intmax_t              digits_seen     = 0;
   static const _Boost::intmax_t max_digits_seen = 4 + (cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count * 301L) / 1000;
   bool                         ss              = false;
   //
   // Extract the sign:
   //
   ...
   _Boost::intmax_t max_bits = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + 32;
#else
   _Boost::intmax_t max_bits = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + ((cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count % limb_bits) ? (limb_bits - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count % limb_bits) : 0) + limb_bits;
#endif
   _Boost::int64_t  error          = 0;
   _Boost::intmax_t calc_exp       = 0;
   _Boost::intmax_t final_exponent = 0;
   ...
   if (decimal_exp >= 0)
   {
      ...
      do
      {
         ...
         if (decimal_exp)
         {
            calc_exp = _Boost::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), decimal_exp, max_bits, error);
            calc_exp += _Boost::multiprecision::cpp_bf_io_detail::restricted_multiply(t, t, n, max_bits, error);
         }
         else
            t = n;
         final_exponent = (_Boost::int64_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count - 1 + decimal_exp + calc_exp;
         int rshift     = msb(t) - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + 1;
         if (rshift > 0)
         {
            final_exponent += rshift;
            int roundup = _Boost::multiprecision::cpp_bf_io_detail::get_round_mode(t, rshift - 1, error);
            t >>= rshift;
   ...
   else
   {
      ...
      {
         cpp_int d;
         calc_exp       = _Boost::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -decimal_exp, max_bits, error);
         ...
         if (gb == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count - 1)
         {
            ...
            roundup = _Boost::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, q);
         }
...
std::string cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::str(std::streamsize dig, std::ios_base::fmtflags f) const
{
   ...
   if (exponent() <= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent)
   {
      ...
      _Boost::intmax_t shift         = (_Boost::intmax_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count - (_Boost::intmax_t)exponent() - 1;
      _Boost::intmax_t digits_wanted = static_cast<int>(dig);
      _Boost::intmax_t base10_exp    = exponent() >= 0 ? static_cast<_Boost::intmax_t>(std::floor(0.30103 * exponent())) : static_cast<_Boost::intmax_t>(std::ceil(0.30103 * exponent()));
      //
      ...
      if (digits_wanted < -1)
      {
         ...
         _Boost::multiprecision::detail::format_float_string(s, base10_exp, dig, f, true);
         return s;
      }
      //
      ...
      //
      _Boost::intmax_t power10 = digits_wanted - base10_exp - 1;
      //
      ...
      _Boost::intmax_t max_bits = cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count + ((cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count % limb_bits) ? (limb_bits - cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count % limb_bits) : 0) + limb_bits;
      if (power10)
         max_bits += (msb(_Boost::multiprecision::detail::abs(power10)) / 8) * limb_bits;
#endif
      do
      {
         _Boost::int64_t  error    = 0;
         ...
         if (shift < 0)
         {
            if (power10 >= 0)
            {
               ...
               BOOST_ASSERT(power10 <= (_Boost::intmax_t)INT_MAX);
               i <<= -shift;
               if (power10)
            ...
            {
               cpp_int d;
               calc_exp = _Boost::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -power10, max_bits, error);
               shift += calc_exp;
               BOOST_ASSERT(shift < 0); // Must still be true!
               i <<= -shift;
               cpp_int r;
               divide_qr(i, d, i, r);
               roundup = _Boost::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, i);
               if (roundup < 0)
               {
#ifdef BOOST_MP_STRESS_IO
                  max_bits += 32;
                  ...
                  max_bits *= 2;
#endif
                  shift = (_Boost::intmax_t)cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count - exponent() - 1 - power10;
                  continue;
               }
            }
         }
         else
         {
            ...
            if (power10 > 0)
            {
               ...
               {
                  cpp_int t;
                  calc_exp = _Boost::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), power10, max_bits, error);
                  calc_exp += _Boost::multiprecision::cpp_bf_io_detail::restricted_multiply(i, i, t, max_bits, error);
                  shift -= calc_exp;
               }
               ...
               if (shift)
               {
                  roundup = _Boost::multiprecision::cpp_bf_io_detail::get_round_mode(i, shift - 1, error);
                  if (roundup < 0)
                  {
#ifdef BOOST_MP_STRESS_IO
                     max_bits += 32;
         ...
         //
         _Boost::intmax_t digits_got = i ? static_cast<_Boost::intmax_t>(s.size()) : 0;
      ...
      {
         _Boost::multiprecision::detail::round_string_up_at(s, static_cast<int>(s.size() - 1), base10_exp);
      }

      if (sign())
         s.insert(static_cast<std::string::size_type>(0), 1, '-');

      _Boost::multiprecision::detail::format_float_string(s, base10_exp, dig, f, false);
   }
   else
   {
      switch (exponent())
      ...
      case exponent_zero:
         ...
         _Boost::multiprecision::detail::format_float_string(s, 0, dig, f, true);
         break;
      case exponent_nan:
         s = "nan";
         break;
