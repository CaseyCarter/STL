...
{
   typedef typename _Boost::multiprecision::detail::canonical<R, CppInt>::type cast_type;
   if (val.sign())
   {
      if (_Boost::is_signed<R>::value == false)
...
eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& backend)
{
   ...
   {
      if ((backend.sign() && _Boost::is_signed<R>::value) && (1 + static_cast<_Boost::multiprecision::limb_type>((std::numeric_limits<R>::max)()) <= backend.limbs()[0]))
      {
         *result = (std::numeric_limits<R>::min)();
         return;
      }
      else if (_Boost::is_signed<R>::value && !backend.sign() && static_cast<_Boost::multiprecision::limb_type>((std::numeric_limits<R>::max)()) <= backend.limbs()[0])
      {
      ...
      if (i < backend.size())
      {
         ...
         *result += (static_cast<R>(backend.limbs()[i]) & mask) << shift;
         if ((static_cast<R>(backend.limbs()[i]) & static_cast<limb_type>(~mask)) || (i + 1 < backend.size()))
         {
            ...
            if (backend.sign())
            {
               check_is_negative(_Boost::is_signed<R>());
               *result = (std::numeric_limits<R>::min)();
            }
            else if (_Boost::is_signed<R>::value)
               *result = (std::numeric_limits<R>::max)();
            return;
         }
      }
   ...
   if (backend.sign())
   {
      check_is_negative(mpl::bool_<_Boost::is_signed<R>::value>());
      *result = negate_integer(*result, mpl::bool_<_Boost::is_signed<R>::value>());
   }
}
...
eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   unsigned result = _Boost::multiprecision::detail::find_lsb(a.limbs()[index]);
...
eval_msb_imp(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   return (a.size() - 1) * cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::limb_bits + _Boost::multiprecision::detail::find_msb(a.limbs()[a.size() - 1]);
}
...
inline BOOST_MP_CXX14_CONSTEXPR limb_type integer_gcd_reduce(limb_type u, limb_type v)
{
   do
   {
      ...
      if (u == v)
         break;
      v -= u;
      v >>= _Boost::multiprecision::detail::find_lsb(v);
   } while (true);
   return u;
}
    ...
    limb_type                                                                   v)
{
   ...
   unsigned vs = _Boost::multiprecision::detail::find_lsb(v);
...
{
   *result.limbs() = _Boost::integer::gcd(*a.limbs(), *b.limbs());
}
...
{
   *result.limbs() = _Boost::integer::lcm(*a.limbs(), *b.limbs());
...
inline BOOST_MP_CXX14_CONSTEXPR typename enable_if_c<
    is_trivial_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value && is_signed_number<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value && _Boost::is_convertible<typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::local_limb_type, R>::value>::type
eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val)
{
   ...
   if (std::numeric_limits<R>::is_specialized && (static_cast<common_type>(*val.limbs()) > static_cast<common_type>((std::numeric_limits<R>::max)())))
   {
      if (val.isneg())
      {
         check_is_negative(mpl::bool_ < _Boost::is_signed<R>::value || (number_category<R>::value == number_kind_floating_point) > ());
      ...
      {
         conversion_overflow(typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::checked_type());
         *result = _Boost::is_signed<R>::value ? (std::numeric_limits<R>::max)() : static_cast<R>(*val.limbs());
      }
   }
   else
   {
...
template <class R, unsigned MinBits1, unsigned MaxBits1, cpp_integer_type SignType1, cpp_int_check_type Checked1, class Allocator1>
inline BOOST_MP_CXX14_CONSTEXPR typename enable_if_c<
    is_trivial_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value && is_unsigned_number<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value && _Boost::is_convertible<typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::local_limb_type, R>::value>::type
eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val)
{
   typedef typename common_type<R, typename cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::local_limb_type>::type common_type;
   if (std::numeric_limits<R>::is_specialized && (static_cast<common_type>(*val.limbs()) > static_cast<common_type>((std::numeric_limits<R>::max)())))
...
eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return _Boost::multiprecision::detail::find_lsb(*a.limbs());
}
...
eval_msb_imp(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return _Boost::multiprecision::detail::find_msb(*a.limbs());
}
...
inline BOOST_MP_CXX14_CONSTEXPR std::size_t hash_value(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val) BOOST_NOEXCEPT
{
   ...
   {
      _Boost::hash_combine(result, val.limbs()[i]);
   }
...
}}} // namespace _Boost::multiprecision::backends
