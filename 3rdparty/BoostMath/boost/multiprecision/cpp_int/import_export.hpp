    ...
    number<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>, ExpressionTemplates>& val, Iterator i, Iterator j, unsigned chunk_size = 0, bool msv_first = true)
{
   ...
   typedef typename _Boost::make_unsigned<value_type>::type                                       unsigned_value_type;
   typedef typename std::iterator_traits<Iterator>::difference_type                              difference_type;
   typedef typename _Boost::make_unsigned<difference_type>::type                                  size_type;
...
inline typename _Boost::disable_if_c<_Boost::multiprecision::backends::is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> >::value, number<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>, ExpressionTemplates>&>::type
import_bits_fast(
...
template <unsigned MinBits, unsigned MaxBits, cpp_integer_type SignType, cpp_int_check_type Checked, class Allocator, expression_template_option ExpressionTemplates, class T>
inline typename _Boost::enable_if_c<_Boost::multiprecision::backends::is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> >::value, number<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>, ExpressionTemplates>&>::type
import_bits_fast(
    number<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>, ExpressionTemplates>& val, T* i, T* j, unsigned chunk_size = 0)
{
   cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator>& result   = val.backend();
...
_Boost::uintmax_t extract_bits(const Backend& val, unsigned location, unsigned count, const mpl::false_& tag)
{
   unsigned         limb   = location / (sizeof(limb_type) * CHAR_BIT);
   unsigned         shift  = location % (sizeof(limb_type) * CHAR_BIT);
   _Boost::uintmax_t result = 0;
   _Boost::uintmax_t mask   = count == std::numeric_limits<_Boost::uintmax_t>::digits ? ~static_cast<_Boost::uintmax_t>(0) : (static_cast<_Boost::uintmax_t>(1u) << count) - 1;
...
template <class Backend>
inline _Boost::uintmax_t extract_bits(const Backend& val, unsigned location, unsigned count, const mpl::true_&)
{
...
#pragma warning(disable : 4244)
   ...
   unsigned bitcount = _Boost::multiprecision::backends::eval_msb_imp(val.backend()) + 1;
