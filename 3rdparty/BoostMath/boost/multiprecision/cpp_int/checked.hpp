...
namespace _Boost { namespace multiprecision { namespace backends { namespace detail {
...
{
   return checked_add_imp(a, b, mpl::bool_<_Boost::is_signed<A>::value>());
}
template <class A>
...
{
   return checked_subtract_imp(a, b, mpl::bool_<_Boost::is_signed<A>::value>());
}
template <class A>
...
template <class A>
inline BOOST_MP_CXX14_CONSTEXPR A checked_left_shift(A a, _Boost::ulong_long_type shift, const mpl::int_<checked>&)
{
   if (a && shift)
   {
...
template <class A>
inline BOOST_MP_CXX14_CONSTEXPR A checked_left_shift(A a, _Boost::ulong_long_type shift, const mpl::int_<unchecked>&)
{
   return (shift >= sizeof(A) * CHAR_BIT) ? 0 : a << shift;
}

}}}} // namespace _Boost::multiprecision::backends::detail
