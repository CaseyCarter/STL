...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename _Boost::type_of::encode_type<\
            typename _Boost::type_of::push_back<\
            V\
            , _Boost::type_of::constant<std::size_t,ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<_Boost::type_of::constant<std::size_t,ID>, Iter>\
    {\
        typedef _Boost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , _Boost::type_of::constant<std::size_t,ID> >::type\
            , _Boost::type_of::constant<std::size_t,N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<_Boost::type_of::constant<std::size_t,ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef _Boost::type_of::decode_type<typename Iter::next> d;\
