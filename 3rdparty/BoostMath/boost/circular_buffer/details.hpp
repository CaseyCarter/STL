...
struct assign_range {
    ...
    template <class Pointer>
    void operator () (Pointer p) const {
        _Boost::cb_details::uninitialized_copy(m_first, m_last, p, m_alloc);
    }
};
...
inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    ...
    BOOST_TRY {
        for (; first != last; ++first, ++dest)
            allocator_traits<Alloc>::construct(a, _Boost::to_address(dest), *first);
    } BOOST_CATCH(...) {
        for (; next != dest; ++next)
            allocator_traits<Alloc>::destroy(a, _Boost::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
    return dest;
...
ForwardIterator uninitialized_move_if_noexcept_impl(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a,
    true_type) {
    for (; first != last; ++first, ++dest)
        allocator_traits<Alloc>::construct(a, _Boost::to_address(dest), _Boost::move(*first));
    return dest;
}
...
template<class InputIterator, class ForwardIterator, class Alloc>
ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    typedef typename _Boost::is_nothrow_move_constructible<typename allocator_traits<Alloc>::value_type>::type tag_t;
...
inline void uninitialized_fill_n_with_alloc(ForwardIterator first, Diff n, const T& item, Alloc& alloc) {
    ...
    BOOST_TRY {
        for (; n > 0; ++first, --n)
            allocator_traits<Alloc>::construct(alloc, _Boost::to_address(first), item);
    } BOOST_CATCH(...) {
        for (; next != first; ++next)
            allocator_traits<Alloc>::destroy(alloc, _Boost::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
}
