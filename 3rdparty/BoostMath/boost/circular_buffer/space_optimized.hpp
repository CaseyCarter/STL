    ...
    */
    ...
    /*! \brief Change the capacity (and the minimal guaranteed amount of allocated memory) of the
        ...
        \note To explicitly clear the extra allocated memory use the <b>shrink-to-fit</b> technique:<br><br>
              <code>%_Boost::%circular_buffer_space_optimized\<int\> cb(1000);<br>
              ...<br>
              %_Boost::%circular_buffer_space_optimized\<int\>(cb).swap(cb);</code><br><br>
...
// Declared only for documentation purpose.
    ...
    */
    void push_back(rvalue_type item) {
        check_low_capacity();
        circular_buffer<T, Alloc>::push_back(_Boost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        check_low_capacity();
        circular_buffer<T, Alloc>::push_front(_Boost::move(item));
    }
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        size_type index = pos - begin();
        check_low_capacity();
        return circular_buffer<T, Alloc>::insert(begin() + index, _Boost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        size_type index = pos - begin();
        check_low_capacity();
        return circular_buffer<T, Alloc>::rinsert(begin() + index, _Boost::move(item));
    }
