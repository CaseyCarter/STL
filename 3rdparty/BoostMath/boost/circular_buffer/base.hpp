...
private empty_value<Alloc>
{
  ...
  // Requirements
    //BOOST_CLASS_REQUIRE(T, _Boost, SGIAssignableConcept);
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(dest), _Boost::move_if_noexcept(*src));
                        ++constructed;
                    } else {
                        value_type tmp = _Boost::move_if_noexcept(*src); 
                        replace(src, _Boost::move_if_noexcept(*dest));
                        replace(dest, _Boost::move(tmp));
                    }
                }
            }
        } BOOST_CATCH(...) {
    ...
    */
    void rotate(const_iterator new_begin) {
        ...
        if (full()) {
            ...
            if (m < n) {
                for (; m > 0; --m) {
                    push_front(_Boost::move_if_noexcept(back()));
                    pop_back();
                }
            } else {
                for (; n > 0; --n) {
                    push_back(_Boost::move_if_noexcept(front()));
                    pop_front();
                }
            }
        }
...
// Construction/Destruction
    ...
    */
    ...
    : base(_Boost::empty_init_t(), alloc), m_buff(0), m_end(0), m_first(0), m_last(0), m_size(0) {}
    ...
    */
    ...
    : base(_Boost::empty_init_t(), alloc), m_size(0) {
    ...
    */
    ...
    : base(_Boost::empty_init_t(), alloc), m_size(n) {
...
#endif
    base(_Boost::empty_init_t(), cb.get_allocator()),
    m_size(cb.size()) {
    ...
    */
    ...
    : base(_Boost::empty_init_t(), cb.get_allocator()), m_buff(0), m_end(0), m_first(0), m_last(0), m_size(0) {
        cb.swap(*this);
    }
    ...
    */
    ...
    : base(_Boost::empty_init_t(), alloc) {
...
private:
    ...
    void push_back_impl(ValT item) {
        if (full()) {
            ...
            cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(m_last), static_cast<ValT>(item));
            increment(m_last);
            ++m_size;
        }        
    }
    ...
    void push_front_impl(ValT item) {
        BOOST_TRY {
            if (full()) {
                ...
                decrement(m_first);
                cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(m_first), static_cast<ValT>(item));
                ++m_size;
            }
        } BOOST_CATCH(...) {
            increment(m_first);
...
public:
    ...
    */
    void push_back(rvalue_type item) {
        push_back_impl<rvalue_type>(_Boost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        push_front_impl<rvalue_type>(_Boost::move(item));
    }
...
// Insert
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        return insert_impl<rvalue_type>(pos, _Boost::move(item));
    }
    ...
    */
    iterator insert(iterator pos) {
        value_type temp;
        return insert(pos, _Boost::move(temp));
    }
...
private:
    ...
    iterator rinsert_impl(iterator pos, ValT item) {
        ...
        if (pos == begin()) {
            ...
            BOOST_TRY {
                while (src != pos.m_it) {
                    construct_or_replace(construct, dest, _Boost::move_if_noexcept(*src));
                    increment(src);
                    increment(dest);
                    construct = false;
                }
...
public:
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        return rinsert_impl<rvalue_type>(pos, _Boost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos) {
        value_type temp;
        return rinsert(pos, _Boost::move(temp));
    }
...
// Erase
    ...
    */
    iterator erase(iterator pos) {
        ...
        for (pointer p = pos.m_it; next != m_last; p = next, increment(next))
            replace(p, _Boost::move_if_noexcept(*next));
        decrement(m_last);
        destroy_item(m_last);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator erase(iterator first, iterator last) {
        ...
        while (last.m_it != 0)
            replace((first++).m_it, _Boost::move_if_noexcept(*last++));
        do {
            decrement(m_last);
            destroy_item(m_last);
            --m_size;
    ...
    */
    iterator rerase(iterator pos) {
        ...
        for (decrement(prev); p != m_first; p = prev, decrement(prev))
            replace(p, _Boost::move_if_noexcept(*prev));
        destroy_item(m_first);
        increment(m_first);
        --m_size;
#if BOOST_CB_ENABLE_DEBUG
    ...
    */
    iterator rerase(iterator first, iterator last) {
        ...
        while (first.m_it != m_first) {
            ...
            decrement(p);
            replace(p, _Boost::move_if_noexcept(*first.m_it));
        }
        do {
            destroy_item(m_first);
            increment(m_first);
...
// Helper methods
    ...
    void construct_or_replace(bool construct, pointer pos, param_value_type item) {
        if (construct)
            cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(pos), item);
        else
            replace(pos, item);
    }
    ...
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(pos), _Boost::move(item));
        else
            replace(pos, _Boost::move(item));
    }

    /*! INTERNAL ONLY */
    void destroy_item(pointer p) {
        cb_details::allocator_traits<Alloc>::destroy(alloc(), _Boost::to_address(p));
#if BOOST_CB_ENABLE_DEBUG
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        initialize(distance, _Boost::make_move_iterator(tmp.begin()), _Boost::make_move_iterator(tmp.end()), distance);
    }
    ...
    void initialize(capacity_type buffer_capacity,
        ...
        while (first != last && !full()) {
            cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(m_last), *first++);
            increment(m_last);
            ++m_size;
        }
        while (first != last) {
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range
                (_Boost::make_move_iterator(tmp.begin()), _Boost::make_move_iterator(tmp.end()), alloc()));
    }
        ...
        if (pos.m_it == 0) {
            ...
            BOOST_TRY {
                for (; ii < construct; ++ii, increment(p))
                    cb_details::allocator_traits<Alloc>::construct(alloc(), _Boost::to_address(p), *wrapper());
