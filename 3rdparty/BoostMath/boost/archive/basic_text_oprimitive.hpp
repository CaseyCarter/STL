...
protected:
    ...
    _Boost::archive::codecvt_null<typename OStream::char_type> codecvt_null_facet;
    ...
    template<class T>
    void save_impl(const T &t, _Boost::mpl::bool_<false> &){
        if(os.fail())
            _Boost::serialization::throw_exception(
                archive_exception(archive_exception::output_stream_error)
            );
    ...
    template<class T>
    void save_impl(const T &t, _Boost::mpl::bool_<true> &){
