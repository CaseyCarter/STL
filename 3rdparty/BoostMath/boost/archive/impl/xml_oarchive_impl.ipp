...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef _Boost::archive::iterators::mb_from_wchar<
        _Boost::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(begin),
        translator(end),
        _Boost::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef _Boost::archive::iterators::xml_escape<
        const char * 
