...
-> decltype(_Boost::pointer_traits<T>::to_address(v))
{
    return _Boost::pointer_traits<T>::to_address(v);
}
...
{
    return _Boost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return _Boost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return _Boost::to_address(v.operator->());
}
#endif
