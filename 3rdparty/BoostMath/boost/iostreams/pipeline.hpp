...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::_Boost::iostreams::pipeline< \
        ::_Boost::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::_Boost::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::_Boost::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/
