...
#include <boost/detail/iterator.hpp>             // _Boost::iterator_traits.
...
// Description: Device based on an instance of _Boost::iterator_range.
// Template parameters:
//     Mode - A mode tag.
...
private:
    ...
    typedef _Boost::detail::iterator_traits<iterator>  iter_traits;
...
struct range_adapter_impl<std::forward_iterator_tag> {
        ...
        (Iter& cur, Iter& last, const Ch* s, std::streamsize n)
    {
        ...
        if (cur == last && n > 0)
            _Boost::throw_exception(write_area_exhausted());
        return n;
    }
};
