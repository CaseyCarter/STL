 ...
 * _Boost::iostreams::detail::execute_all() and the function template 
 * _Boost::iostreams::detail::execute_foreach().
 *
...
namespace _Boost { namespace iostreams { namespace detail {
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
                  ...
                  typename _Boost::result_of<Op()>::type
              #else
...
#define BOOST_PP_LOCAL_MACRO(n) \
   ...
   { \
       ...
       try { \
           r = _Boost::iostreams::detail::execute_all( \
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            _Boost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return _Boost::iostreams::detail::execute_foreach(first, last, op);
}
