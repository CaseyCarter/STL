...
namespace _Boost { namespace iostreams { 
...
protected:
    ...
    const _Boost::core::typeinfo& component_type() const { return BOOST_CORE_TYPEID(T); }
...
{
    if (!ibeg_) 
        _Boost::throw_exception(cant_read());
    if (!gptr()) 
        init_get_area();
    return gptr() != iend_ ? 
...
direct_streambuf<T, Tr>::pbackfail(int_type c)
{
    ...
    _Boost::throw_exception(bad_putback());
}
...
direct_streambuf<T, Tr>::overflow(int_type c)
{
    using namespace std;
    if (!obeg_)
        _Boost::throw_exception(BOOST_IOSTREAMS_FAILURE("no write access"));
...
void direct_streambuf<T, Tr>::close_impl(BOOST_IOS::openmode which)
{
    ...
    _Boost::iostreams::close(*storage_, which);
}
    ...
    (stream_offset off, BOOST_IOS::seekdir way, BOOST_IOS::openmode which)
{
    ...
    if (two_head() && (which & both) == both)
        _Boost::throw_exception(bad_seek());
