...
namespace _Boost
{
//typedef _Boost::int_least64_t time_max_t;
typedef _Boost::intmax_t time_max_t;
...
inline FP init_steady_clock(kern_return_t & err)
{
  ...
  namespace detail
  {
    ...
    inline timespec ns_to_timespec(_Boost::time_max_t const& ns)
    {
      _Boost::time_max_t s = ns / 1000000000l;
      timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline _Boost::time_max_t timespec_to_ns(timespec const& ts)
    {
      return static_cast<_Boost::time_max_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
#endif
    ...
    struct platform_duration
    {
      ...
      explicit platform_duration(_Boost::time_max_t const& ns = 0) : ts_val(ns_to_timespec(ns)) {}
      _Boost::time_max_t getNs() const { return timespec_to_ns(ts_val); }
#else
      explicit platform_duration(_Boost::time_max_t const& ns = 0) : ns_val(ns) {}
      _Boost::time_max_t getNs() const { return ns_val; }
#endif

#if defined BOOST_THREAD_USES_DATETIME
      platform_duration(_Boost::posix_time::time_duration const& rel_time)
      {
#if defined BOOST_THREAD_CHRONO_POSIX_API || defined BOOST_THREAD_CHRONO_MAC_API
...
#else
        ns_val = static_cast<_Boost::time_max_t>(rel_time.total_seconds()) * 1000000000l;
      ...
      _Boost::time_max_t getMs() const
      {
        const _Boost::time_max_t ns = getNs();
    ...
    struct real_platform_timepoint
    {
      ...
      explicit real_platform_timepoint(_Boost::time_max_t const& ns) : dur(ns) {}
      _Boost::time_max_t getNs() const { return dur.getNs(); }

#if defined BOOST_THREAD_USES_DATETIME
      real_platform_timepoint(_Boost::system_time const& abs_time)
        : dur(abs_time - _Boost::posix_time::from_time_t(0)) {}
#endif
    ...
    struct real_platform_clock
    {
      ...
      {
#if defined(BOOST_THREAD_CHRONO_WINDOWS_API)
        _Boost::winapi::FILETIME_ ft;
        _Boost::winapi::GetSystemTimeAsFileTime(&ft);  // never fails
        _Boost::time_max_t ns = ((((static_cast<_Boost::time_max_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime) - 116444736000000000LL) * 100LL);
  ...
  struct mono_platform_timepoint
  {
    ...
    explicit mono_platform_timepoint(_Boost::time_max_t const& ns) : dur(ns) {}
    _Boost::time_max_t getNs() const { return dur.getNs(); }
...
#else
      _Boost::time_max_t ns = (std::numeric_limits<_Boost::time_max_t>::max)();
  ...
  struct mono_platform_clock
  {
    static mono_platform_timepoint now()
    {
      ...
      _Boost::winapi::LARGE_INTEGER_ freq;
      if ( !_Boost::winapi::QueryPerformanceFrequency( &freq ) )
      {
      ...
      _Boost::winapi::LARGE_INTEGER_ pcount;
      unsigned times=0;
      while ( ! _Boost::winapi::QueryPerformanceCounter( &pcount ) )
      {
        if ( ++times > 3 )
        {
      ...
      return mono_platform_timepoint(static_cast<_Boost::time_max_t>(ns));
#else
