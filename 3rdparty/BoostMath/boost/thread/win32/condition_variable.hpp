...
namespace _Boost
{
    namespace detail
    {
        ...
        public:
            ...
            static bool no_waiters(_Boost::intrusive_ptr<basic_cv_list_entry> const& entry)
            {
        ...
        {
            _Boost::mutex internal_mutex;
            long total_count;
            unsigned active_generation_count;

            typedef basic_cv_list_entry list_entry;

            typedef _Boost::intrusive_ptr<list_entry> entry_ptr;
            ...
            {
                _Boost::lock_guard<_Boost::mutex> lk(internal_mutex);
                if(!wake_sem)
                {
            ...
            struct entry_manager
            {
                ...
                entry_manager(entry_ptr&& entry_, _Boost::mutex& mutex_):
                    entry(static_cast< entry_ptr&& >(entry_)), internal_mutex(mutex_)
                {}
#else
                entry_manager(entry_ptr const& entry_, _Boost::mutex& mutex_):
                ...
                {
                  if (entry) {
                    _Boost::lock_guard<_Boost::mutex> internal_lock(internal_mutex);
                    entry->remove_waiter();
                    entry.reset();
                  }
                }
    ...
    public:
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_wait(unique_lock<mutex>& m,_Boost::system_time const& abs_time)
        {
        ...
        bool timed_wait(unique_lock<mutex>& m,_Boost::xtime const& abs_time)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,_Boost::system_time const& abs_time,predicate_type pred)
        {
        ...
        template<typename predicate_type>
        bool timed_wait(unique_lock<mutex>& m,_Boost::xtime const& abs_time,predicate_type pred)
        {
