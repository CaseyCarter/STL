...
namespace _Boost
{
    ...
    inline void condition_variable::wait(unique_lock<mutex>& m)
    {
        ...
        if(! m.owns_lock())
        {
            _Boost::throw_exception(condition_error(-1, "_Boost::condition_variable::wait() failed precondition mutex not owned"));
        }
#endif
        int res=0;
        {
        ...
        if(res)
        {
            _Boost::throw_exception(condition_error(res, "_Boost::condition_variable::wait failed in pthread_cond_wait"));
        }
    }
                ...
                detail::internal_platform_timepoint const &timeout)
    {
        ...
        if (!m.owns_lock())
        {
            _Boost::throw_exception(condition_error(EPERM, "_Boost::condition_variable::do_wait_until() failed precondition mutex not owned"));
        }
#endif
        int cond_res;
        {
        ...
        if(cond_res)
        {
            _Boost::throw_exception(condition_error(cond_res, "_Boost::condition_variable::do_wait_until failed in pthread_cond_timedwait"));
        }
        return true;
    }
    ...
    {
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        _Boost::pthread::pthread_mutex_scoped_lock internal_lock(&internal_mutex);
#endif
    ...
    public:
        ...
        condition_variable_any()
        {
            ...
            if(res)
            {
                _Boost::throw_exception(thread_resource_error(res, "_Boost::condition_variable_any::condition_variable_any() failed in pthread_mutex_init"));
            }
            int const res2 = posix::pthread_cond_init(&cond);
            if(res2)
            {
                BOOST_VERIFY(!posix::pthread_mutex_destroy(&internal_mutex));
                _Boost::throw_exception(thread_resource_error(res2, "_Boost::condition_variable_any::condition_variable_any() failed in pthread_cond_init"));
            }
        }
        ~condition_variable_any()
        {
        ...
        void wait(lock_type& m)
        {
            int res=0;
            {
                ...
                detail::interruption_checker check_for_interruption(&internal_mutex,&cond);
#else
                _Boost::pthread::pthread_mutex_scoped_lock check_for_interruption(&internal_mutex);
#endif
                guard.activate(m);
            ...
            if(res)
            {
                _Boost::throw_exception(condition_error(res, "_Boost::condition_variable_any::wait() failed in pthread_cond_wait"));
            }
        }
        ...
        template<typename lock_type>
        bool timed_wait(lock_type& m,_Boost::system_time const& abs_time)
        {
        ...
        template<typename lock_type>
        bool timed_wait(lock_type& m,::_Boost::xtime const& abs_time)
        {
        ...
        template<typename lock_type,typename predicate_type>
        bool timed_wait(lock_type& m,_Boost::system_time const& abs_time, predicate_type pred)
        {
#if defined BOOST_THREAD_WAIT_BUG
            const detail::real_platform_timepoint ts(abs_time + BOOST_THREAD_WAIT_BUG);
#else
        ...
        template<typename lock_type,typename predicate_type>
        bool timed_wait(lock_type& m,::_Boost::xtime const& abs_time, predicate_type pred)
        {
        ...
        {
            const _Boost::detail::internal_platform_timepoint ts(t);
        ...
        wait_for(
                ...
                Predicate pred)
        {
            return wait_until(lock, chrono::steady_clock::now() + d, _Boost::move(pred));
        }
#endif
    ...
    private:
          ...
          detail::internal_platform_timepoint const &timeout)
        {
          ...
          if(res)
          {
              _Boost::throw_exception(condition_error(res, "_Boost::condition_variable_any::do_wait_until() failed in pthread_cond_timedwait"));
          }
          return true;
        }
    };
