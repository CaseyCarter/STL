...
#define BOOST_THREAD_DEFINE_HAS_MEMBER_CALLED(member_name)                     \
        template<typename T, bool=_Boost::is_class<T>::value>            \
...
;      BOOST_THREAD_DEFINE_HAS_MEMBER_CALLED(unlock);
      ...
      struct has_member_lock<T,
          decltype(void(_Boost::declval<T&>().lock()))
      > : true_type {};
      ...
      struct has_member_unlock<T,
          decltype(void(_Boost::declval<T&>().unlock()))
      > : true_type {};
      ...
      struct has_member_try_lock<T,
          decltype(bool(_Boost::declval<T&>().try_lock()))
      > : true_type {};
#endif
