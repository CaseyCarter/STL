...
// _Boost::thread::future requires exception handling
// due to _Boost::exception::exception_ptr dependency
...
namespace _Boost
{
  ...
  shared_ptr<T const> static_shared_from_this(T const* that)
  {
    ...
    namespace detail
    {
        ...
        struct relocker
        {
            _Boost::unique_lock<_Boost::mutex>& lock_;

            relocker(_Boost::unique_lock<_Boost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
            }
        ...
        {
            typedef std::list<_Boost::condition_variable_any*> waiter_list;
            ...
            _Boost::exception_ptr exception;
            bool done;
            bool is_valid_;
            bool is_deferred_;
            bool is_constructed;
            launch policy_;
            mutable _Boost::mutex mutex;
            _Boost::condition_variable waiters;
            waiter_list external_waiters;
            _Boost::function<void()> callback;
            ...
            void set_executor_policy(executor_ptr_type aex, _Boost::lock_guard<_Boost::mutex>&)
            {
              set_executor();
              ex_ = aex;
            }
            void set_executor_policy(executor_ptr_type aex, _Boost::unique_lock<_Boost::mutex>&)
            {
              set_executor();
              ex_ = aex;
            }

            bool valid(_Boost::unique_lock<_Boost::mutex>&) { return is_valid_; }
            bool valid() {
              _Boost::unique_lock<_Boost::mutex> lk(this->mutex);
              return valid(lk);
            }
            void invalidate(_Boost::unique_lock<_Boost::mutex>&) { is_valid_ = false; }
            void invalidate() {
              _Boost::unique_lock<_Boost::mutex> lk(this->mutex);
              invalidate(lk);
            }
            void validate(_Boost::unique_lock<_Boost::mutex>&) { is_valid_ = true; }
            void validate() {
              _Boost::unique_lock<_Boost::mutex> lk(this->mutex);
              validate(lk);
            }
...
#endif
            notify_when_ready_handle notify_when_ready(_Boost::condition_variable_any& cv)
            {
                _Boost::unique_lock<_Boost::mutex> lock(this->mutex);
                do_callback(lock);
            ...
            {
                _Boost::lock_guard<_Boost::mutex> lock(this->mutex);
...
#define BOOST_THREAD_DO_CONTINUATION
            ...
            void do_continuation(_Boost::unique_lock<_Boost::mutex>& lock)
            {
...
#else
            void do_continuation(_Boost::unique_lock<_Boost::mutex>&)
            {
            }
#endif
...
#define BOOST_THREAD_DO_CONTINUATION \
            ...
            virtual void do_continuation(_Boost::unique_lock<_Boost::mutex>&) = 0;
#endif

#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            virtual void set_continuation_ptr(continuation_ptr_type continuation, _Boost::unique_lock<_Boost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void do_callback(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                if(callback && !done)
                {
                    _Boost::function<void()> local_callback=callback;
            ...
            void wait_internal(_Boost::unique_lock<_Boost::mutex> &lk, bool rethrow=true)
            {
              do_callback(lk);
              if (is_deferred_)
              {
              ...
              waiters.wait(lk, _Boost::bind(&shared_state_base::is_done, _Boost::ref(*this)));
              if(rethrow && exception)
              {
                  _Boost::rethrow_exception(exception);
              }
            }

            virtual void wait(_Boost::unique_lock<_Boost::mutex>& lock, bool rethrow=true)
            {
            ...
            bool timed_wait(Duration const& rel_time)
            {
                ...
                do_callback(lock);
                return waiters.timed_wait(lock, rel_time, _Boost::bind(&shared_state_base::is_done, _Boost::ref(*this)));
            }

            bool timed_wait_until(_Boost::system_time const& target_time)
            {
                _Boost::unique_lock<_Boost::mutex> lock(this->mutex);
                if (is_deferred_)
                    return false;
                ...
                do_callback(lock);
                return waiters.timed_wait(lock, target_time, _Boost::bind(&shared_state_base::is_done, _Boost::ref(*this)));
            }
#endif
#ifdef BOOST_THREAD_USES_CHRONO
            ...
            wait_until(const chrono::time_point<Clock, Duration>& abs_time)
            {
              ...
              do_callback(lock);
              if(!waiters.wait_until(lock, abs_time, _Boost::bind(&shared_state_base::is_done, _Boost::ref(*this))))
              {
...
#endif
            void mark_exceptional_finish_internal(_Boost::exception_ptr const& e, _Boost::unique_lock<_Boost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                _Boost::unique_lock<_Boost::mutex> lock(this->mutex);
                mark_exceptional_finish_internal(_Boost::current_exception(), lock);
            }

            void set_exception_deferred(exception_ptr e)
            {
              unique_lock<_Boost::mutex> lk(this->mutex);
              if (has_value(lk))
              {
...
//              this->is_constructed = true;
            ...
            bool has_value(unique_lock<_Boost::mutex>& )  const
            {
                return done && ! exception;
            }
            ...
            launch launch_policy(_Boost::unique_lock<_Boost::mutex>&) const
            {
                return policy_;
            }

            future_state::state get_state(_Boost::unique_lock<_Boost::mutex>&) const
            {
                if(!done)
                {
            ...
            {
                _Boost::lock_guard<_Boost::mutex> guard(this->mutex);
                if(!done)
                {
                    return future_state::waiting;
                }
            ...
            {
                _Boost::lock_guard<_Boost::mutex> lock(this->mutex);
                callback=_Boost::bind(f,_Boost::ref(*u));
            }

            virtual void execute(_Boost::unique_lock<_Boost::mutex>&) {}
        ...
        {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
              typedef _Boost::optional<T> storage_type;
#else
              typedef _Boost::csbl::unique_ptr<T> storage_type;
#endif
            ...
            typedef typename conditional<_Boost::is_fundamental<T>::value,T,T const&>::type source_reference_type;
            typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
            typedef T move_dest_type;
#else
            typedef T& source_reference_type;
            typedef typename conditional<_Boost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename conditional<_Boost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            void mark_finished_with_result_internal(source_reference_type result_, _Boost::unique_lock<_Boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = result_;
#else
            ...
            void mark_finished_with_result_internal(rvalue_source_type result_, _Boost::unique_lock<_Boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = _Boost::move(result_);
#elif ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                result.reset(new T(_Boost::move(result_)));
#else
            ...
            template <class ...Args>
            void mark_finished_with_result_internal(_Boost::unique_lock<_Boost::mutex>& lock, BOOST_THREAD_FWD_REF(Args)... args)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result.emplace(_Boost::forward<Args>(args)...);
#else
                result.reset(new T(_Boost::forward<Args>(args)...));
#endif
                this->mark_finished_internal(lock);
            }
#endif
            ...
            void mark_finished_with_result(rvalue_source_type result_)
            {
                ...
                mark_finished_with_result_internal(_Boost::move(result_), lock);
#else
                mark_finished_with_result_internal(static_cast<rvalue_source_type>(result_), lock);
#endif
            }

            storage_type& get_storage(_Boost::unique_lock<_Boost::mutex>& lk)
            {
                wait_internal(lk);
                return result;
            }
            virtual move_dest_type get(_Boost::unique_lock<_Boost::mutex>& lk)
            {
                return _Boost::move(*get_storage(lk));
            }
            move_dest_type get()
            {
                _Boost::unique_lock<_Boost::mutex> lk(this->mutex);
            ...
            virtual shared_future_get_result_type get_sh(_Boost::unique_lock<_Boost::mutex>& lk)
            {
...
//              this->is_constructed = true;
            ...
            {
                set_value_deferred(_Boost::move(result_));
//              unique_lock<_Boost::mutex> lk(this->mutex);
//              if (this->has_value(lk))
//                  throw_exception(promise_already_satisfied());
//
...
//              this->is_constructed = true;
            ...
            detail::shared_state_base
        {
            ...
            virtual T& get(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            virtual T& get_sh(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
...
//              this->is_constructed = true;
            ...
            detail::shared_state_base
        {
            ...
            void mark_finished_with_result_internal(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            virtual void get(_Boost::unique_lock<_Boost::mutex>& lock)
            {
            ...
            virtual void get_sh(_Boost::unique_lock<_Boost::mutex>& lock)
            {
                this->wait_internal(lock);
            }
            void get_sh()
...
//              this->is_constructed = true;
        ...
        public:
          ...
          ~future_async_shared_state_base()
          {
            ...
            unique_lock<_Boost::mutex> lk(this->mutex);
            this->waiters.wait(lk, _Boost::bind(&shared_state_base::is_done, _Boost::ref(*this)));
#endif
          }

          virtual void wait(_Boost::unique_lock<_Boost::mutex>& lk, bool rethrow)
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
              {
                relocker rlk(lk);
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = _Boost::thread(&future_async_shared_state::run, static_shared_from_this(this), _Boost::forward<Fp>(f));
#else
            _Boost::thread(&future_async_shared_state::run, static_shared_from_this(this), _Boost::forward<Fp>(f)).detach();
#endif
          }
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = _Boost::thread(&future_async_shared_state::run, static_shared_from_this(this), _Boost::move(f));
#else
            _Boost::thread(&future_async_shared_state::run, static_shared_from_this(this), _Boost::move(f)).detach();
#endif
          }
        ...
        struct future_deferred_shared_state: shared_state<Rp>
        {
          ...
          virtual void execute(_Boost::unique_lock<_Boost::mutex>& lck) {
            try
            {
              Fp local_fuct=_Boost::move(func_);
              relocker relock(lck);
              Rp res = local_fuct();
              relock.lock();
              this->mark_finished_with_result_internal(_Boost::move(res), lck);
            }
            catch (...)
            {
        ...
        private:
            ...
            {
                _Boost::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::notify_when_ready_handle handle;
                count_type index;

                registered_waiter(_Boost::shared_ptr<detail::shared_state_base> const& a_future,
            ...
            struct all_futures_lock
            {
                   ...
                   _Boost::scoped_array<_Boost::unique_lock<_Boost::mutex> > locks;

                all_futures_lock(std::vector<registered_waiter>& futures):
                    count(futures.size()),locks(new _Boost::unique_lock<_Boost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(_Boost::unique_lock<_Boost::mutex>(futures[i].future_->mutex));
                    }
                }

                void lock()
                {
                    _Boost::lock(locks.get(),locks.get()+count);
                }
            ...
            _Boost::condition_variable_any cv;
...
//    typename _Boost::disable_if<is_future_type<Iterator>,Iterator>::type wait_for_any(Iterator begin,Iterator end)
//    {
//        if(begin==end)
//            return end;
//
...
//            waiter.add(*current);
//        }
//        return _Boost::next(begin,waiter.wait());
//    }

#ifdef BOOST_NO_CXX11_VARIADIC_TEMPLATES
    template<typename F1,typename F2>
    typename _Boost::enable_if<is_future_type<F1>,typename detail::future_waiter::count_type>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
...
#else
    template<typename F1, typename... Fs>
    typename _Boost::enable_if<is_future_type<F1>, typename detail::future_waiter::count_type>::type
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef _Boost::shared_ptr<detail::shared_state<R> > future_ptr;
      ...
      public:
        ...
        state get_state(_Boost::unique_lock<_Boost::mutex>& lk) const
        {
            if(!future_)
            {
        ...
        bool is_ready(_Boost::unique_lock<_Boost::mutex>& lk) const
        {
        ...
        launch launch_policy(_Boost::unique_lock<_Boost::mutex>& lk) const
        {
        ...
        {
          if ( future_ ) {
            _Boost::unique_lock<_Boost::mutex> lk(this->future_->mutex);
        ...
        void wait() const
        {
            if(!future_)
            {
                _Boost::throw_exception(future_uninitialized());
            }
            future_->wait(false);
        }
        ...
        bool timed_wait_until(_Boost::system_time const& abs_time) const
        {
            if(!future_)
            {
                _Boost::throw_exception(future_uninitialized());
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_sync_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_deferred_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_async_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_sync_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);


  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(Ex& ex, _Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(Ex& ex, _Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        make_future_unwrap_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    }
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      typename _Boost::disable_if<is_future_type<InputIterator>,
    ...
    private:
        ...
        detail::make_future_async_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_sync_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_sync_continuation_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(Ex& ex, _Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(Ex& ex, _Boost::unique_lock<_Boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template <class F, class Rp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_unwrap_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      friend typename _Boost::disable_if<is_future_type<InputIterator>,
        BOOST_THREAD_FUTURE<csbl::vector<typename InputIterator::value_type>  >
      >::type
      when_all(InputIterator first, InputIterator last);
    ...
    public:
        ...
        base_type(_Boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }
#if defined BOOST_THREAD_PROVIDES_FUTURE_UNWRAP
        ...
        {
            this->base_type::operator=(_Boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }

        shared_future<R> share()
        {
          return shared_future<R>(::_Boost::move(*this));
        }
        ...
        move_dest_type get()
        {
            ...
            unique_lock<_Boost::mutex> lk(this->future_->mutex);
        ...
        typename _Boost::disable_if< is_void<R2>, move_dest_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename _Boost::result_of<F(BOOST_THREAD_FUTURE)>::type>
        ...
        template <typename R2>
        inline typename _Boost::disable_if< is_void<R2>, BOOST_THREAD_FUTURE<R> >::type
    ...
    public:
        ...
        {
            base_type::operator=(_Boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        template <typename R2>
        typename _Boost::disable_if< is_void<R2>, typename detail::shared_state<R>::shared_future_get_result_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename _Boost::result_of<F(shared_future)>::type>
...
#include <boost/thread/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        promise(_Boost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                _Boost::unique_lock<_Boost::mutex> lock(future_->mutex);

                if(!future_->done && !future_->is_constructed)
                {
                    future_->mark_exceptional_finish_internal(_Boost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          if (future_.get()==0)
          {
              _Boost::throw_exception(promise_moved());
          }
          _Boost::lock_guard<_Boost::mutex> lk(future_->mutex);
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_obtained)
            {
                _Boost::throw_exception(future_already_retrieved());
            }
...
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename _Boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type
            set_value(TR const &  r)
        {
            ...
            if(future_->done)
            {
                _Boost::throw_exception(promise_already_satisfied());
            }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(_Boost::move(r), lock);
#else
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->set_value_deferred(_Boost::move(r));
#else
        ...
        void emplace(BOOST_THREAD_FWD_REF(Args) ...args)
        {
            ...
            future_->mark_finished_with_result_internal(lock, _Boost::forward<Args>(args)...);
        }

#endif

        void set_exception(_Boost::exception_ptr p)
        {
            lazy_init();
            _Boost::unique_lock<_Boost::mutex> lock(future_->mutex);
            if(future_->done)
        ...
        {
          set_exception(_Boost::copy_exception(ex));
        }
        void set_exception_deferred(_Boost::exception_ptr p)
        {
            lazy_init();
            if (future_.get()==0)
            {
        ...
        {
          set_exception_deferred(_Boost::copy_exception(ex));
        }

        // setting the result with deferred notification
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename _Boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value_at_thread_exit(TR const& r)
        {
          if (future_.get()==0)
          {
              _Boost::throw_exception(promise_moved());
        ...
        void set_value_at_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          ...
          future_->set_value_at_thread_exit(_Boost::move(r));
        }
        void set_exception_at_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
        ...
        {
          set_exception_at_thread_exit(_Boost::copy_exception(ex));
        }
...
#include <boost/thread/detail/atomic_redef_macros.hpp>
    ...
    class promise<void>
    {
        typedef _Boost::shared_ptr<detail::shared_state<void> > future_ptr;
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CTOR_ALLOCATORS
namespace _Boost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::_Boost::promise<R> , Alloc> : true_type
    {
    };
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
...
namespace _Boost
{
    ...
    namespace detail
    {
      ...
      struct task_base_shared_state:
            ...
            detail::shared_state<R>
        {
            ...
            {
                {
                    _Boost::lock_guard<_Boost::mutex> lk(this->mutex);
                    if(started)
                    {
                        _Boost::throw_exception(task_already_started());
                    }
                    started=true;
                }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(_Boost::move(args)...);
#else
                do_run();
#endif
            }
            ...
            void owner_destroyed()
            {
                ...
                {
                    started=true;
                    this->mark_exceptional_finish_internal(_Boost::copy_exception(_Boost::broken_promise()), lk);
                }
            }
        };
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_at_thread_exit(f(_Boost::move(args)...));
                }
#else
            void do_apply()
            {
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(_Boost::move(args)...));
                }
#else
            void do_run()
            {
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                  R res((f()));
                  this->mark_finished_with_result(_Boost::move(res));
#else
           ...
           task_base_shared_state<R>
            ...
            public:
                ...
                void do_apply()
                {
                    ...
                    {
                        R r((f()));
                        this->set_value_at_thread_exit(_Boost::move(r));
                    }
#endif
                    catch(...)
                    {
    ...
    {
      typedef _Boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      _Boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef _Boost::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      _Boost::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, _Boost::move(args)...));
            future_obtained=false;
        }
  #else
        ...
        explicit packaged_task(BOOST_THREAD_FWD_REF(F) f
            , typename _Boost::disable_if<is_same<typename decay<F>::type, packaged_task>, dummy* >::type=0
            )
        {
          typedef typename decay<F>::type FR;
            ...
            typedef detail::task_shared_state<FR,R> task_shared_state_type;
#endif
            task = task_ptr(new task_shared_state_type(_Boost::forward<F>(f)));
            future_obtained = false;
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
            typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(_Boost::move(f)));
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(_Boost::move(f)));
#endif
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNCTION_PTR)
        template <class Allocator>
        packaged_task(_Boost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(_Boost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          typedef typename decay<F>::type FR;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(_Boost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(_Boost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
        ...
        template <class F, class Allocator>
        packaged_task(_Boost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(_Boost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            packaged_task temp(_Boost::move(other));
#else
        ...
        {
          if (!valid())
              _Boost::throw_exception(task_moved());
          _Boost::lock_guard<_Boost::mutex> lk(task->mutex);
          task->set_executor_policy(aex, lk);
        }
#endif
        void reset() {
            if (!valid())
              _Boost::throw_exception(future_error(system::make_error_code(future_errc::no_state)));
        ...
        void make_ready_at_thread_exit() {
          ...
          if (task->has_value()) _Boost::throw_exception(promise_already_satisfied());
          task->apply();
        }
#endif
        template<typename F>
...
namespace _Boost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::_Boost::packaged_task<R> , Alloc> : true_type
    {};
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
namespace std {
...
namespace detail
{
  ...
  make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    shared_ptr<future_deferred_shared_state<Rp, Fp> >
        h(new future_deferred_shared_state<Rp, Fp>(_Boost::forward<Fp>(f)));
  ...
  async(launch policy, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(_Boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_deferred_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(_Boost::forward<ArgTypes>(args))...
              )
  ...
  async(launch policy, R(*f)()) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      ret.set_async();
      _Boost::thread( _Boost::move(pt) ).detach();
      return ::_Boost::move(ret);
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      std::terminate();
      //BOOST_THREAD_FUTURE<R> ret;
  ...
  template <class F, class ...ArgTypes>
  BOOST_THREAD_FUTURE<typename _Boost::result_of<typename decay<F>::type(
  ...
  async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(_Boost::forward<F>(f))
                , thread_detail::decay_copy(_Boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
  ...
  template <class F>
  BOOST_THREAD_FUTURE<typename _Boost::result_of<typename decay<F>::type()>::type>
  async(launch policy, BOOST_THREAD_FWD_REF(F) f) {
    typedef typename _Boost::result_of<typename decay<F>::type()>::type R;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
    typedef packaged_task<R()> packaged_task_type;
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      packaged_task_type pt( _Boost::forward<F>(f) );
      ...
      //BOOST_THREAD_FUTURE<R> ret;
      //return ::_Boost::move(ret);
      //          return _Boost::detail::make_future_deferred_shared_state<Rp>(
      //              BF(
      //                  thread_detail::decay_copy(_Boost::forward<F>(f))
      //              )
      //          );
...
namespace detail {
    ...
    public:
      ...
      : that(x.that), f_(_Boost::move(x.f_))
      {
        x.that.reset();
      }
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        shared_state_nullary_task<Rp,Cont> t(this->shared_from_this(), _Boost::forward<Fp>(f));
        ex.submit(_Boost::move(t));
      }
...
//#if ! defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(_Boost::forward<ArgTypes>(args))...
        )
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(A1)), BOOST_THREAD_FWD_REF(A1) a1) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(_Boost::forward<A1>(a1))
        )
    ));
  }
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNCTION_PTR
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)  {
    ...
    return _Boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(_Boost::forward<F>(f))
        )
    );
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(_Boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(_Boost::forward<F>(f))
          , thread_detail::decay_copy(_Boost::forward<A1>(a1))
          , thread_detail::decay_copy(_Boost::forward<A2>(a2))
        )
    ));
  }
  ...
  async(R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), f, _Boost::forward<ArgTypes>(args)...));
  }
  #else
  template <class R>
  BOOST_THREAD_FUTURE<R>
  ...
  async(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), _Boost::forward<F>(f), _Boost::forward<ArgTypes>(args)...));
  }
#else
  template <class F>
  BOOST_THREAD_FUTURE<typename _Boost::result_of<F()>::type>
  async(BOOST_THREAD_FWD_REF(F) f) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), _Boost::forward<F>(f)));
  }
#endif
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    p.set_value(_Boost::forward<future_value_type>(value));
  ...
  BOOST_THREAD_FUTURE<typename detail::deduced_type<T>::type> make_ready_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    promise<future_value_type> p;
    p.set_value(_Boost::forward<T>(value));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future(E ex) {
    promise<T> p;
    p.set_exception(_Boost::copy_exception(ex));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future() {
    promise<T> p;
    p.set_exception(_Boost::current_exception());
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  template <typename T>
...
namespace detail
{
  ...
  public:
    ...
    : parent(_Boost::move(f)),
      continuation(_Boost::move(c))
    {
    }

    void init(_Boost::unique_lock<_Boost::mutex> &lock)
    {
    ...
    void call() {
      try {
        this->mark_finished_with_result(this->continuation(_Boost::move(this->parent)));
      } catch(...) {
        this->mark_exceptional_finish();
      }
    ...
    static void run(shared_ptr<_Boost::detail::shared_state_base> that_)
    {
  ...
  public:
    ...
    {
      try {
        this->continuation(_Boost::move(this->parent));
        this->mark_finished_with_result();
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    : base_type(_Boost::move(f), _Boost::forward<Fp>(c))
    {    }

    void launch_continuation() {
#if defined BOOST_THREAD_FUTURE_BLOCKING
      _Boost::lock_guard<_Boost::mutex> lk(this->mutex);
      this->thr_ = _Boost::thread(&future_async_continuation_shared_state::run, static_shared_from_this(this));
#else
      _Boost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  };
...
namespace detail {
  ...
  public:
    ...
    template <class Ex>
    void init(_Boost::unique_lock<_Boost::mutex> &lk, Ex& ex)
    {
    ...
    void launch_continuation() {
      ...
      this->get_executor()->submit(_Boost::move(fct));
    }
...
#if defined BOOST_THREAD_FUTURE_BLOCKING
      _Boost::lock_guard<_Boost::mutex> lk(this->mutex);
      this->thr_ = _Boost::thread(&base_type::run, static_shared_from_this(this));
#else
      _Boost::thread(&base_type::run, static_shared_from_this(this)).detach();
#endif
    }
  ...
  public:
    ...
    virtual void execute(_Boost::unique_lock<_Boost::mutex>& lk) {
      this->parent.wait();
      this->call(lk);
    }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_deferred_continuation_shared_state(
      _Boost::unique_lock<_Boost::mutex> &lock,
      BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new future_deferred_continuation_shared_state<F, Rp, Cont>(_Boost::move(f), _Boost::forward<Fp>(c)));
    h->init(lock);
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_continuation_shared_state(
      _Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
        h(new future_async_continuation_shared_state<F,Rp, Cont>(_Boost::move(f), _Boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_sync_continuation_shared_state(
    ...
    shared_ptr<future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new future_sync_continuation_shared_state<F,Rp, Cont>(_Boost::move(f), _Boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
        h(new future_executor_continuation_shared_state<F,Rp, Cont>(_Boost::move(f), _Boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Cont>(f, _Boost::forward<Fp>(c)));
    h->init(lock);
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_async_continuation_shared_state(
      _Boost::unique_lock<_Boost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Cont>(f, _Boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_sync_continuation_shared_state(
    ...
    shared_ptr<shared_future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_sync_continuation_shared_state<F,Rp, Cont>(f, _Boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(f, _Boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename _Boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    typedef typename _Boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    ...
    _Boost::unique_lock<_Boost::mutex> lock(sentinel->mutex);

    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, _Boost::move(*this), _Boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, _Boost::move(*this), _Boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_future_sync_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, _Boost::move(*this), _Boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
      ...
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, _Boost::move(*this), _Boost::forward<F>(func)
                )));
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    return this->then(this->launch_policy(), _Boost::forward<F>(func));
#else
    typedef typename _Boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get()!=0, future_uninitialized());
  ...
  template <typename R2>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename _Boost::result_of<F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >)>::type>
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename _Boost::result_of<F(shared_future<R>)>::type>
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    typedef typename _Boost::result_of<F(shared_future<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_.get()!=0, future_uninitialized());

    _Boost::unique_lock<_Boost::mutex> lock(this->future_->mutex);
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, _Boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, _Boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_shared_future_sync_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, _Boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
      typedef executor Ex;
      Ex& ex = *(this->future_->get_executor());
      return BOOST_THREAD_MAKE_RV_REF((_Boost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type>(ex,
                    lock, *this, _Boost::forward<F>(func)
                )));
#endif
    } else if (underlying_cast<int>(policy) & int(launch::inherit)) {
...
namespace detail
{
  ...
  struct mfallbacker_to
  {
    ...
    : value_(_Boost::move(v))
    {}

    T operator()(BOOST_THREAD_FUTURE<T> fut) {
      return fut.get_or(_Boost::move(value_));
    }
  };
  template <typename T>
  ...
  BOOST_THREAD_FUTURE<R>::fallback_to(BOOST_THREAD_RV_REF(R2) v) {
    return then(detail::mfallbacker_to<R>(_Boost::move(v)));
  }
...
namespace detail
{
    ...
    void launch_continuation()
    {
      ...
      if (! unwrapped.valid() )
      {
        if (wrapped.has_exception()) {
          ...
          {
            lk.unlock();
            _Boost::unique_lock<_Boost::mutex> lk2(unwrapped.future_->mutex);
            unwrapped.future_->set_continuation_ptr(this->shared_from_this(), lk2);
          } else {
            this->mark_exceptional_finish_internal(_Boost::copy_exception(future_uninitialized()), lk);
          }
        }
      } else {
  ...
  make_future_unwrap_shared_state(_Boost::unique_lock<_Boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f) {
    shared_ptr<future_unwrap_shared_state<F, Rp> >
        h(new future_unwrap_shared_state<F, Rp>(_Boost::move(f)));
  ...
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::unwrap()
  {
    ...
    return _Boost::detail::make_future_unwrap_shared_state<BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >, R2>(lock, _Boost::move(*this));
  }
#endif
...
namespace detail
{
  ...
  struct future_when_all_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<_Boost::detail::shared_state_base> that_) {
      ...
      try {
        _Boost::wait_for_all(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(_Boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = _Boost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this());
#else
      _Boost::thread(&future_when_all_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
      vec_.push_back(_Boost::forward<T0>(f));
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(_Boost::forward<T>(futures)),'0'
          )..., '0'
  ...
  struct future_when_any_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<_Boost::detail::shared_state_base> that_)
    {
      ...
      try {
        _Boost::wait_for_any(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(_Boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = _Boost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this());
#else
      _Boost::thread(&future_when_any_vector_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  public:
    ...
    future_when_any_vector_shared_state(values_tag,
      ...
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(_Boost::forward<T>(futures))
          ,'0'
          )...,
          '0'
      }; //second part of magic unpacker
  ...
  struct wait_for_all_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      _Boost::wait_for_all(_Boost::forward<T>(v)...);
    }
  };
  ...
  struct wait_for_any_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      _Boost::wait_for_any(_Boost::forward<T>(v)...);
    }
  };
  ...
  struct future_when_all_tuple_shared_state: future_async_shared_state_base<Tuple>
  {
    ...
    static void run(shared_ptr<_Boost::detail::shared_state_base> that_) {
      ...
      try {
        // TODO make use of apply(that->tup_, _Boost::detail::wait_for_all_fctor());
        that->wait_for_all(Index());

        that->mark_finished_with_result(_Boost::move(that->tup_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = _Boost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this());
#else
      _Boost::thread(&future_when_all_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(_Boost::csbl::make_tuple(_Boost::forward<F>(f), _Boost::forward<Fs>(futures)...))
    {
    }
...
#ifdef BOOST_THREAD_FUTURE_BLOCKING
      this->thr_ = _Boost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this());
#else
      _Boost::thread(&future_when_any_tuple_shared_state::run, this->shared_from_this()).detach();
#endif
    }
  ...
  when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
    ...
    shared_ptr<factory_type>
        h(new factory_type(detail::values_tag_value, _Boost::forward<T0>(f), _Boost::forward<T>(futures)...));
    h->init();
    return BOOST_THREAD_FUTURE<container_type>(h);
  }
#endif
