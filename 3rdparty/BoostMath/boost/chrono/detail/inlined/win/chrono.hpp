...
namespace chrono_detail
{
  ...
  steady_clock::time_point steady_clock::now( system::error_code & ec )
  {
    ...
    _Boost::winapi::LARGE_INTEGER_ pcount;
    if ( (nanosecs_per_tic <= 0.0L)
            || (!_Boost::winapi::QueryPerformanceCounter( &pcount )) )
    {
        _Boost::winapi::DWORD_ cause =
            ((nanosecs_per_tic <= 0.0L)
                    ? _Boost::winapi::ERROR_NOT_SUPPORTED_
                    : _Boost::winapi::GetLastError());
        if (::_Boost::chrono::is_throws(ec)) {
            _Boost::throw_exception(
                    system::system_error(
                            cause,
                            ::_Boost::system::system_category(),
                            "chrono::steady_clock" ));
        }
        else
        {
            ec.assign( cause, ::_Boost::system::system_category() );
